...
1. **Registration succeeds.** `engine.registerSubgraph(crud('post'))` returns a handler id; invariants 1/2/3/5/6/9/10/12 all pass; no `E_INV_*` thrown.
2. **Three creates + list reflects all three.** `engine.call('post:create', …)` × 3, then `engine.call('post:list')` returns exactly those three posts in `createdAt` order — exercises the 8-primitive evaluator + transaction primitive + View 3 (content listing) incremental maintenance + deterministic `createdAt` injection.
3. **Capability denial routes to `ON_DENIED`.** Revoke the write capability, call `post:create` again, assert the response came through the subgraph's `ON_DENIED` edge with error code `E_CAP_DENIED` — exercises the capability hook, `requires` property recognition, and typed error edges.
4. **Trace has non-zero per-step timing.** `engine.trace(handlerId, input)` returns a `TraceStep[]` where every step has `durationUs > 0` and the step sequence matches the subgraph's topological order — exercises `engine.trace()` DX path and confirms the evaluator isn't shortcut-executing.
5. **Mermaid output parses.** `handler.toMermaid()` returns a string that parses as valid Mermaid `flowchart` syntax, verified via the official `@mermaid-js/parser` npm package as a dev-dep of the scaffolder's test environment (no actual browser rendering needed in CI; parser-only is sufficient to catch malformed output). Exercises `subgraph.toMermaid()` DX path without relying on a hand-rolled Mermaid-subset parser that might accept outputs real Mermaid rejects.
6. **TS ↔ Rust CID round-trip.** Create a Node with the canonical test properties from the spike fixture; assert the returned CID string equals `bafyr4iflzldgzjrtknevsib24ewiqgtj65pm2ituow3yxfpq57nfmwduda` — exercises the full bindings stack + content-addressing.
