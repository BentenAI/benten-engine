...
5. **Mermaid output parses.** `handler.toMermaid()` returns a string that parses as valid Mermaid `flowchart` syntax (verified by a minimal Mermaid grammar check; no actual browser rendering needed in CI) — exercises `subgraph.toMermaid()` DX path.
