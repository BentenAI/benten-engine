...
4. **Fork C RESOLVED by Ben (2026-07-26) — and reshaped: there is NO "primary modality".** Ben's ruling: visual composer + DSL as co-primary **with the DSL compiling to native graph structure**, and graph-canvas **not precluded** — kept as a full editor for people who prefer it for some editing/creation. All three, as peers.

   **The reframe this forces (stronger than the R0's own framing).** "Primary authoring modality" was a false constraint imported from tools where the *text is the source* and round-tripping is lossy (Figma-vs-code). Here the **content-addressed UI graph IS the canonical representation** — the DSL compiles to it, the visual composer emits it, the graph-canvas edits it directly. So they are not competing primaries; they are **N lossless projections of one source of truth.** That is the project's own thesis applied to its own tooling: no source-code-vs-data distinction, therefore no privileged editor.

   **NEW LOAD-BEARING R1 INVARIANT — round-trip losslessness.** Every surface must be able to open anything any other surface produced. This is tractable here *precisely because* the graph is canonical (there is no source formatting to preserve), but it must be stated as an invariant or it will erode.

   **The sharp sub-question R1 MUST answer explicitly:** is `graph → DSL` a deterministic pretty-print (a true bidirectional projection), or is the DSL one-way (`DSL → graph` only)? If one-way, a visual edit **orphans** someone's DSL, and "all three" silently degrades into one real surface plus two toys. *R0 recommendation: require bidirectionality.* It is achievable because the graph is canonical; the cost is a canonical-formatting decision, not a semantic one.

   **Investment hierarchy (NOT an authority hierarchy).** Visual + DSL carry the adoption risk, so the R1 worked-example go/no-go (a real settings screen, built and timed by a designer) is built in those. Graph-canvas ships first-class but later; its genuine strengths are structural/topology work, wiring `ON`/`BINDS` behavior edges, and inspecting what a composition actually *is* — and its genuine weakness is dense form layout. The critique's density warning applies to graph-canvas-as-the-ONLY-surface, which is no longer what anyone is proposing.

5. **Forks E and G ratified by Ben (2026-07-26).** **E** = accept the bespoke-IR onboarding cost; the durable asset is the toolkit-neutral IR, learned once. React-with-Radix is not revisited. **G** = the admin-v0 rebuild is accepted in principle, but Ben signs off at R1 on an **honest LOC/time estimate** before it is committed; frame the new IR as the successor generalization of the shipped schema-rendering vocabulary (the shipped materializer is the seed, not waste).

6. **Forks A / B / D / F remain ORCHESTRATOR RECOMMENDATION, not yet explicitly ratified** — carry into Composing-Admin R1 and re-surface: **A** Svelte-primary with Solid as sanctioned fallback · **B** Leptos/Rust-native as a small parallel research track behind the Renderer trait, off the critical path · **D** constrained typed tokens first, sanitized escape hatch only if the worked example proves it insufficient · **F** optimistic-write reconciliation as a real R1 design task, model not pre-committed.

7. **Forks A / D / F RESOLVED by Ben (2026-07-26).**

   - **A = SVELTE, AUTHORIZED OUTRIGHT — no pre-sanctioned fallback.** Ben: *"I'd just authorize Svelte, and if you feel the need to fall back it should be a discussion."* **This overrides the R0's "Solid is the named, pre-vetted fallback" wording** (§2.3). Solid remains the *technically* pre-vetted alternative and §2.2's ranking stands as analysis, but it is **NOT a standing authorization** — a pivot off Svelte is a Ben conversation, not an R3/R5 implementer's call. Rationale (Ben's, and it is the better posture): a pre-authorized fallback quietly lowers the bar for taking it, and the pick being cheap to reverse (one `Renderer` impl over a portable IR) is an argument for *deciding once*, not for hedging. **Standing guard for R1+: no wave may switch the renderer framework without an explicit Ben decision.**

   - **D = CONSTRAINED TYPED TOKENS (delegated to orchestrator: "proceed however makes the most sense toward the ideal permanent shape").** Decision: **start constrained.** A UI node's appearance is an edge (`STYLES`) to a typed token node drawn from a closed vocabulary (a palette entry, a spacing step, a type-ramp rung) — never an arbitrary CSS string. **The decisive argument is directional asymmetry, and it is the same argument as the crypto-agility posture (CLAUDE.md #5): widening later is ADDITIVE (mint new token kinds, exactly as new codepoints are minted), narrowing later is a BREAKING CHANGE against content people have already authored and shared.** Two concrete hazards constrained-first avoids: (i) arbitrary CSS inside a graph that is *editable and shared between people* is an injection/exfiltration/UI-spoof surface; (ii) CSS **cascades**, so a node's rendered meaning would depend on its ancestors and document order — which breaks content-addressing, because the same CID could render differently in two places. A named, sanitized escape hatch is added ONLY if the R1 worked example proves the closed vocabulary insufficient — and it lands as a new *typed* token kind, not as a raw-CSS hole.

   - **F = R1 DESIGN TASK, model not pre-committed (delegated).** Decision: keep it a real design task; do not default it. **Recorded orchestrator lean for R1 to start from: the engine is AUTHORITATIVE and the UI holds a SPECULATIVE local view — which is deliberately the same shape as IVM's materialized views over the source graph.** Making the UI's local-vs-authoritative relationship isomorphic to a relationship the engine already implements is the elegant permanent shape; it means one mental model and one set of reconciliation semantics instead of two. The design work R1 owes: what happens when the engine *disagrees*. Under shape (b) a write crosses a network hop to a full peer and can be **denied** (capability check), **conflict** (concurrent CRDT edit), or **fail validation** — so disagreement is ordinary, not exotic. **Denial must be a first-class, visible outcome, never a silent revert** (a UI that quietly rolls back an action the user believes succeeded is both a UX failure and a security-legibility failure).
