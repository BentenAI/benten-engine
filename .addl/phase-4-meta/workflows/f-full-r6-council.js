...
const CFG = (args && args.cfg) || {
  ...
  enumerate: 'git ls-tree -r --name-only a3a1ef03 -- crates/benten-crypto-suite crates/benten-membership-set crates/benten-drop crates/benten-engine/src/layer_d crates/benten-engine/tests docs/V1-FROZEN-INTERFACE.md docs/SECURITY-POSTURE.md docs/CRYPTO-CODEPOINTS.md docs/THREAT-MODEL.md docs/SECURITY-PROOFS.md docs/INVARIANT-COVERAGE.md docs/V1-WIRE-FORMAT-INVENTORY.md',
  specRef: 'a3a1ef03',
  specPath: 'docs/V1-FROZEN-INTERFACE.md',
  framing: 'PHASE-CLOSE convergence council - ROUND 11 (re-reviewing the phase at r9-base a3a1ef03 after the ROUND-10 fix wave landed). ROUND 10 = full convergence council against a3a1ef03, a STRONG converging result: every ROUND-9 fix re-CONFIRMED correct (nonce-cache prose now seam-honest, F-07 DUAL-CID delete left no dangling reference, F-06 test model-only, doc cluster + OBS accurate) and every positive freeze-confirmation re-CONFIRMED (X-Wing combiner label APPENDED, LAMPS M-prime/3373/1984/both-verify vs the real IETF test vector, codepoint 0x6600/0x6610 split, blinded 11-field AAD, libcrux constant-time, Inv-15 SUF closure, GAP-1 genuinely closed with NO other public-derivable secret anywhere in the corpus). ROUND 10 was NOT-CONVERGED on exactly 2 DOC-ONLY MAJORs (NO wire change) plus a freeze-hygiene tail, ALL now FIXED on r9-base a3a1ef03: MAJOR (1) F-01 body_cid confirmation-oracle honesty - SECURITY-PROOFS §4.2 claimed no-confirmation-oracle / no-equality-test too broadly; the plaintext body_cid = self_describing_cid(BLAKE3(plaintext)) is an UNSALTED deterministic hash of the body, so a low-entropy or guessable plaintext is confirmable by recomputing the hash and two identical plaintexts link; RETENSED to scope the oracle-free claim to the CIPHERTEXT and honestly disclose the body_cid low-entropy confirmation + equality-linkability across SECURITY-PROOFS §4.2 + THREAT-MODEL §1 Tier-1 row + SECURITY-POSTURE Compromise #43. MAJOR (2) F-05 device-link MITM prose - device_link.rs §10.2-HIGH claimed a MITM-substitution defense that is actually human-fingerprint-comparison-dependent and NOT enforced in code (device_b_fingerprint + fingerprint_recipient have ZERO production consumers); RETENSED to human-fingerprint-dependent + enforcement DEFERRED to G-COMP-1 Row D-30, the two fields KEPT as a reserved seam, and the §10.2 test corrected to pin the property it truly exercises (recipient-confidentiality: a wrong secret rejects K_principal exfil). FREEZE-HYGIENE tail: F-02/F-11 = #[non_exhaustive] added to the 5 Swap* enums + DischargeDisposition + benten_drop::EncryptedEnvelope (BindingContext registered as the exhaustive-by-design carve-out) + §11 doc-blocks + a NEW crypto-suite audit arm-coverage test + V1-FROZEN §11 + D-17 registration + public-api baselines regenerated; F-03 = cfg-gate WriteSpec::test_inject_failure behind test-helpers + widen the no-regression guard is_for_test_pattern (test_ / _test_ / mock_ / inject_) + gate-or-exempt the newly-flagged symbols + baselines regen; F-07 = retense the Drop per_node_attestation docstrings to completed-freeze framing + register Row D-66 (reserved per-Node-signature surface). DOC-PARITY + named-carry: F-04/F-06/F-08/F-09/F-10/F-12/F-13/F-14/F-15/F-16/F-17/F-18/F-19/F-21/GAP-A honest as-built retenses (Loro-faithful-local-fold, declared-tamper-return-shape, recipient-confidentiality, wasm32-wasip1-not-unknown-unknown CI scope, libcrux provenance, BE-migration-landed, gossip flag-to-cross-ref, mlkem-768-pub 0x120c, no-hpke-crate reconcile) + ledger rows (D-30 amend, D-66, D-1/D-64 accept_grant caller-contract, CI-FU-1 wasm32-uu drift-gate); dispatch_codepoint_at_v1_beta renamed federation_reserve_gate_*. RE-VERIFY the ROUND-10 fixes CORRECT + COMPLETE + NO new defect: (a) F-01 §4.2 now scopes the oracle-free property to the CIPHERTEXT and the body_cid low-entropy confirmation + equality-linkability disclosure is ACCURATE and present in all three docs (SECURITY-PROOFS §4.2 + THREAT-MODEL Tier-1 + Compromise #43) with NO residual over-broad no-equality claim anywhere; (b) F-05 §10.2 prose matches as-built (device_b_fingerprint + fingerprint_recipient genuinely have zero production callers; the MITM defense is honestly human-fingerprint-dependent + DEFERRED; the reserved seam is labeled; the renamed test truly pins recipient-confidentiality); (c) F-02/F-11 non_exhaustive is applied on exactly the named enums + the BindingContext carve-out is genuinely exhaustive-by-design + the audit arm-coverage test would FAIL on a missing arm + the regenerated public-api baselines exact-match the source; (d) F-03 test_inject_failure is cfg-gated OFF the frozen surface + the widened guard catches the named patterns; (e) F-07 per_node_attestation docstrings + D-66 match as-built; (f) the doc-parity cluster is accurate against the code. RE-VERIFY THE WHOLE PHASE FRESH every round per feedback_full_phase_review_every_round (re-check EVERY freeze byte / golden / codepoint / invariant versus spec, NOT just the round-10-fix delta). Keep the gap1-closure-and-public-derivable-secret-class-sweep lens (hunt ANY secret derivable from public material anywhere) + crypto-error-oracle/failure-mode-uniformity + at-rest-format-migration/forever-decode + sender-origin-authentication-spoofing + as-built-doc-reconciliation. NOTE a KNOWN pre-tag Ben-gated scope question - do NOT re-flag it as a convergence defect: F-22 = whether to add #[non_exhaustive] to the ~40 ADDITIONAL D-17 public structs is SemVer-breaking to add AFTER the tag, so it is a deliberate pre-tag scope decision reserved for Ben, not a blocker. CONVERGENCE ARC (deeper lenses kept surfacing real defects - do NOT short-circuit): R1 to R2(online-sealed-sender BLOCKER + 4 MAJ) to R3(7 MAJ) to R4(C-01) to R5(2 MAJ) to R6(content-splice BLOCKER) to R7(CONF-1 nonce-reuse BLOCKER) to R8(GAP-1 placeholder-recipient-key BLOCKER) to R9(nonce-durability MAJOR + F-07 dead-surface + doc cluster) to R10(2 doc-only MAJORs: body_cid confirmation-oracle honesty + device-fingerprint MITM prose + freeze-hygiene tail) to R11(this re-review of the round-10 fixes). The LAST review before tag phase-4-meta-core-close freezes the v1 public interface. Q5 cadence: FULL council every round; iterate ACROSS INVOCATIONS until 0 confirmed BLOCKER/MAJOR; pattern-induction findings additive. Review the LANDED implementation + tracked docs + the frozen-interface contract (NOT a red-phase test-pin review). Hunt SUBSTANTIVE defects (crypto correctness, wire-freeze byte-correctness, fail-closed gaps, error-oracle uniformity, at-rest forever-decode, doc-versus-code drift, invariant-coverage at HEAD, deferred-honesty, SemVer/public-surface), not phantom-destinations.',
  ...
  targetLensCount: 23,
  priorMissedLenses: 'ROUND-8 completeness-critic found GAP-1 (the frozen recipient-keying used a public-derivable placeholder secret) as a lens-gap NO panel lens owned. Round 9 MUST include a dedicated lens that (1) VERIFIES the GAP-1 fix genuinely closed it (open takes a real RecipientSecret unrecoverable from public; the [u8;32] placeholder + wrapping_sub(0x80) + deterministic-expansion FULLY deleted with no production caller; a public-only party cannot open on all 3 codepoints; would-FAIL-on-revert) AND (2) SWEEPS FOR THE CLASS - is there ANY OTHER secret/key anywhere in the corpus that is derivable from public material, or a placeholder/deterministic-from-public key, or a secret carrying less entropy than it appears (layer_d device-link + multi-device key-wrap, swap_matrix, hpke, the vault DAK, K_principal, K_Set, any KDF whose secret input is actually public, any _for_test/deterministic keygen reachable in production)? The GAP-1 class - a secret that is not actually secret - is the deepest confidentiality failure mode; hunt it everywhere. ALSO keep the two round-8-added deep lenses (now standard): crypto-error-oracle/failure-mode-uniformity + at-rest-format-migration/forever-decode-of-prior-schema. ALSO keep the sender-origin-authentication-spoofing lens (round-2 origin missed-lens) and the as-built-doc-reconciliation lens (round-1 missed-lens) - re-verify every tracked doc narrative of what-is-built-AND-enforced versus the artifact at r9-base a3a1ef03, especially the NEW recipient-secrecy claims in SECURITY-PROOFS §4.1/4.2 (are they genuinely true as-built) and the DEFERRED D-64 honesty (is the engine-e2r-wiring deferral accurately NOT-a-Compromise).',
  canon: `## PROJECT CANON (inline — review agents CANNOT read gitignored .addl/CLAUDE.md; tracked docs ARE readable via: git show a3a1ef03:docs/...)
