...
const CFG = (args && args.cfg) || {
  ...
  enumerate: 'git ls-tree -r --name-only 5d31406c -- crates/benten-crypto-suite crates/benten-membership-set crates/benten-drop crates/benten-id crates/benten-graph crates/benten-engine/src/layer_d crates/benten-engine/tests docs/V1-FROZEN-INTERFACE.md docs/SECURITY-POSTURE.md docs/CRYPTO-CODEPOINTS.md docs/THREAT-MODEL.md docs/SECURITY-PROOFS.md docs/INVARIANT-COVERAGE.md docs/V1-WIRE-FORMAT-INVENTORY.md',
  specRef: '5d31406c',
  specPath: 'docs/V1-FROZEN-INTERFACE.md',
  framing: 'PHASE-CLOSE convergence council - ROUND 15 (FINAL confirming review at r9-base 5d31406c after the PRE-TAG SWEEP landed on the CONVERGED R14 state). ROUND 14 (at 5d31406c) = CONVERGED: 0 confirmed BLOCKER, 0 confirmed MAJOR, panel-complete; the completeness-critic independently re-derived every frozen crypto byte (X-Wing combiner, LAMPS M-prime, K(V) full-36-byte-CIDv1, 0x6510/0x6610 goldens, Inv-15..22, the 15 SemVer baselines) BYTE-IDENTICAL; F-01 REFUTED (AtriumConfig::for_test is a documented public loopback constructor, not a leak) + F-02/GAP-1 correctly MINOR; benten-id resolve_hybrid parser verified SOUND (length-checks/saturating/typed-reject/no-panic — the B2 non-forgeability soundness dependency). The R14 residual (all MINOR/OBS/named, 0 BLK/MAJ, ZERO frozen-wire-byte/codepoint/crypto-construction change) was CLOSED by the pre-tag sweep now on 5d31406c + ORCH-ground-truthed + full re-gate GREEN (2048 crate tests + 953 engine, 0 fail, no timeout; cargo-doc -D warnings / cite-drift / §3.5g clean; disk-OOM env held). THE PRE-TAG SWEEP (two parts): (A) F-22 — applied #[non_exhaustive] to the Row D-17 EXTENSION pub-type set (~35 types across benten-core [Mode/VersionError/VersionDagError/Anchor/DagVersionChain/VersionDag/Subgraph/SubgraphBuilder/NodeHandle] + benten-ivm [SubgraphSpec/KernelInput/ViewState/ViewBudget/ViewQuery/ViewResult/ViewDefinition/LabelPattern/Subscriber/CanonicalViewEntry/AlgorithmBView/Projection/EffectiveRules + 5 view-instance structs] + benten-platform-foundation [VocabLabel/VocabEdge/Scalar/RenderError/MaterializerError/MaterializerDenialFrame/MaterializerWalkInputs/MaterializerOutput/SubscribeAttachToken] + benten-engine [UserViewInputPattern/TraceStep/StreamCursor/SubscribeCursor/EngineViewsHandle/AtriumConfig/SyncStatus + 12 outcome.rs structs]); ZERO carve-outs (all took it cleanly); 4 minted constructors (Subgraph::from_parts, NodeHandle::new, ViewDefinition::new, MaterializerWalkInputs::new); the cross-crate cascade fixed (32 test/bench sites: enums→wildcard arms, structs→constructors, in-crate exhaustive matches preserved); 4 cargo-public-api baselines regenerated exact-match; §11 non_exhaustive tally corrected to 183 of 638. TxAborted §4.43 + the already-CLOSED CapWriteContext/ReadContext/SuspensionOutcome untouched. (B) DOC SWEEP — the 7 R14 FIX-NOW: F-03 (THREAT-MODEL §6 stanza_count reframed as TRUNCATION-INTEGRITY not alloc-ceiling) + F-05 (V1-WIRE:8 drift-lane tense→deferred) + F-06 (network_observer_can_link_stanzas privacy.rs:42 replaced the inert const-false with a real INPUT-SENSITIVE predicate scanning both wire slices for an exposed cleartext recipient marker, true-only-if-both-leak-same-id; f_nat_2 gained a would-FAIL negative-control) + F-07 (0x6610 membership_set_id_commitment comment corrected §3.9-vs-§3.10 preimage; live construction unaffected) + F-08 (Cfg::Nf1PqPq→NonPqEncryption test-local rename) + F-09 (INVARIANT-COVERAGE Inv-22 2 phantom cites deleted) + F-10 (membership-set lib 6th zero-caller disclosure bullet); NAMED-CARRIES landed (F-04 banner-checklist / F-11 Row-D-1-seam retense / F-12 f_mst_3 honesty / F-13 4th-0x6610-literal SSOT arm / F-14 EncryptedEnvelope-not-AAD-preserving note / F-15 §6-item-6 band cross-ref / F-16 f_nat_2+f_aad_2+Inv-21-crdt cites / F-17 tf2/tf3a RED-PHASE-header retense / F-18 bundle-size reconcile-note); F-02→Row D-74 (is_for_test_pattern bare-for_test widen, v1-GM) + GAP-1→Compromise #66 SGD (UnwrappedKey #[derive(Debug)] renders key bytes + no ZeroizeOnDrop; ZERO v1-beta {:?}/tracing sinks) wired into f_disc_1 catch-net (top→66) + Rows D-75/D-76 (UnwrappedKey + derive_member_key Zeroizing hardening); §16 §1.A.FROZEN FLAG HEAD-re-pin discharged (frozen-surface table re-verified vs lib.rs pub-use + baseline; tag-coupled sign-off residual→checklist); cargo-vet exemption budget reconciled pending→Ben-RATIFIED cap 18 (13-of-18 used) across config/exemptions/SECURITY-POSTURE. RE-VERIFY the PRE-TAG SWEEP correct + complete + NO new defect: (a) F-22 - #[non_exhaustive] applied on the enumerated set, the cascade compiles clean workspace-wide + all-crate nextest green, the attribute is SemVer-ADDITIVE-safe (in-crate exhaustive matches/construction preserved; cross-crate use constructors/wildcards), the 4 minted constructors are sound + do not weaken any invariant, baselines exact-match, NO frozen WIRE byte/golden/codepoint changed (non_exhaustive is a type-attribute not a wire change); (b) the doc sweep is HONEST + accurate - F-06 predicate genuinely input-sensitive + would-FAIL-if-linkable, Compromise #66 accurately scopes the UnwrappedKey limit + zero prod sinks, the F-03/F-07 retenses match the code, the phantom-cite deletions are correct; (c) the vet-budget-18 + §16 FLAG discharge are accurate vs HEAD. RE-VERIFY THE WHOLE PHASE FRESH per feedback_full_phase_review_every_round (re-check EVERY freeze byte / golden / codepoint / invariant / public-api baseline vs spec — this is the LAST review before the interface freezes; do NOT assume the sweep-delta is the only surface). NEW LENS THIS ROUND (R14 missed-lens): secret-lifetime-and-memory-hygiene - systematically walk EVERY secret-bearing type in the corpus (signing keys, KEM RecipientSecret, recovered/UnwrappedKey, DAK, K_principal, K_Set, derived K(N)/K(V), structural_kdf keys, vault seeds) and verify: no #[derive(Debug)] renders raw secret bytes to a production sink, ZeroizeOnDrop/Zeroizing coverage is consistent (flag any secret lacking it that a sibling has — the GAP-1 asymmetry class), no secret is logged/traced/serialized-to-non-encrypted-sink; classify each gap MINOR-hardening (zero prod sink) vs MAJOR (live leak). Keep freeze-safety-of-zero-production-caller-surfaces (GAP-D standing) + gap1-closure-and-public-derivable-secret-class-sweep + crypto-error-oracle/failure-mode-uniformity + at-rest-format-migration/forever-decode + sender-origin-authentication-spoofing + as-built-doc-reconciliation + bounded-decode/unbounded-allocation-and-offset-overflow + public-surface-semver-freeze (VERIFY the F-22 non_exhaustive additions are SemVer-safe + the 15 baselines still real). CONVERGENCE ARC: R11(2 crypto MAJ)->R12(1 DoS)->R13(3 MAJ freeze-leak+overflow+threat-honesty)->R14(CONVERGED 0 BLK/MAJ)->R15(FINAL confirming after the pre-tag sweep). This IS the last review before tag phase-4-meta-core-close freezes the v1 public interface. Q5 cadence: FULL council; if 0 confirmed BLOCKER/MAJOR -> CONVERGED and tag-ready; any MINOR/OBS -> named for the pre-tag banner/v1-GM. Review the LANDED implementation + tracked docs + the frozen-interface contract. Hunt SUBSTANTIVE defects (crypto correctness, wire-freeze byte-correctness, secret-memory-hygiene, non_exhaustive-SemVer-safety, fail-closed gaps, bounded-decode/overflow, error-oracle uniformity, at-rest forever-decode, doc-versus-code drift, invariant-coverage at HEAD, deferred-honesty), not phantom-destinations.',
  mustInclude: ['crypto-construction-correctness','wire-freeze-byte-correctness','codepoint-registry-integrity','invariant-semantics-inv15-22','membershipset-primitive-fidelity','sender-origin-authentication-spoofing','capability-authority-ucan-layerd','confidentiality-threat-model-bounded-decode','distributed-sync-crdt-hlc-gossip','supply-chain-libcrux-cryspen-vet','public-surface-semver-freeze','spec-to-code-ruling-fidelity-pim13-r7','deferred-items-hardrule-honesty','cross-target-wasm-deployment-shapes','as-built-doc-reconciliation-prose-currency','crypto-error-oracle-failure-mode-uniformity','at-rest-format-migration-forever-decode','gap1-closure-and-public-derivable-secret-class-sweep','freeze-safety-of-zero-production-caller-surfaces','secret-lifetime-and-memory-hygiene'],
  targetLensCount: 23,
  priorMissedLenses: 'ROUND-8 completeness-critic found GAP-1 (the frozen recipient-keying used a public-derivable placeholder secret) as a lens-gap NO panel lens owned. Round 9 MUST include a dedicated lens that (1) VERIFIES the GAP-1 fix genuinely closed it (open takes a real RecipientSecret unrecoverable from public; the [u8;32] placeholder + wrapping_sub(0x80) + deterministic-expansion FULLY deleted with no production caller; a public-only party cannot open on all 3 codepoints; would-FAIL-on-revert) AND (2) SWEEPS FOR THE CLASS - is there ANY OTHER secret/key anywhere in the corpus that is derivable from public material, or a placeholder/deterministic-from-public key, or a secret carrying less entropy than it appears (layer_d device-link + multi-device key-wrap, swap_matrix, hpke, the vault DAK, K_principal, K_Set, any KDF whose secret input is actually public, any _for_test/deterministic keygen reachable in production)? The GAP-1 class - a secret that is not actually secret - is the deepest confidentiality failure mode; hunt it everywhere. ALSO keep the two round-8-added deep lenses (now standard): crypto-error-oracle/failure-mode-uniformity + at-rest-format-migration/forever-decode-of-prior-schema. ALSO keep the sender-origin-authentication-spoofing lens (round-2 origin missed-lens) and the as-built-doc-reconciliation lens (round-1 missed-lens) - re-verify every tracked doc narrative of what-is-built-AND-enforced versus the artifact at r9-base 5d31406c, especially the NEW recipient-secrecy claims in SECURITY-PROOFS §4.1/4.2 (are they genuinely true as-built) and the DEFERRED D-64 honesty (is the engine-e2r-wiring deferral accurately NOT-a-Compromise).',
  canon: `## PROJECT CANON (inline — review agents CANNOT read gitignored .addl/CLAUDE.md; tracked docs ARE readable via: git show 5d31406c:docs/...)
