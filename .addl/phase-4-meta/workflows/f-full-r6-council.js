...
const CFG = (args && args.cfg) || {
  ...
  enumerate: 'git ls-tree -r --name-only bbdf4b91 -- crates/benten-crypto-suite crates/benten-membership-set crates/benten-drop crates/benten-engine/src/layer_d crates/benten-engine/tests docs/V1-FROZEN-INTERFACE.md docs/SECURITY-POSTURE.md docs/CRYPTO-CODEPOINTS.md docs/THREAT-MODEL.md docs/SECURITY-PROOFS.md docs/INVARIANT-COVERAGE.md docs/V1-WIRE-FORMAT-INVENTORY.md',
  specRef: 'bbdf4b91',
  specPath: 'docs/V1-FROZEN-INTERFACE.md',
  framing: 'PHASE-CLOSE convergence council - ROUND 10 (re-reviewing the phase at r9-base bbdf4b91 after the ROUND-9 fix wave landed). ROUND 9 = full 14-lens convergence council against bbdf4b91, a STRONG result: every positive freeze-confirmation CORRECT (X-Wing combiner label APPENDED, LAMPS M-prime/3373/1984/both-verify vs the real IETF test vector, codepoint 0x6600/0x6610 split, blinded 11-field AAD, libcrux constant-time, Inv-15 SUF closure); GAP-1 (the round-8 placeholder-recipient-key BLOCKER) CONFIRMED genuinely closed - the dedicated class-sweep lens found NO other public-derivable secret across K_principal/K_Set/K(V)/DAK/device-link/multi-device-wrap/structural_kdf/deterministic-keygen; all 3 governance BLOCKERs (audit-oracle / promote_tier / RBAC-authz) correctly REFUTED to OBS (data-half modeling, ZERO production callers, real enforcement at the engine: Engine::audit_sequence + graph-Inv-13-dedup + Node::load_verified tamper). ROUND 9 was NOT-CONVERGED on 1 MAJOR + a doc/test tail, ALL now FIXED on r9-base bbdf4b91: (1) MAJOR nonce-cache durability OVERCLAIM - THREAT-MODEL §NQ-T4 + SECURITY-POSTURE #64 claimed the jti-nonce-cache is durable-persisted-GUARANTEED, but JtiNonceCache.durable_store (benten-sync/handshake.rs) is an in-RAM HashSet + from_durable hydration SEAM (the type docstring is honest); RETENSED to honest seam-level disclosure (durable-CAS-marker + hydration seam; full disk-persistence is a caller-contract, deferred; the genuinely graph-durable FrameReplayMarker distinguished). (2) F-06 device-link fake-test - open_provisioning_payload takes no nonce so it cannot emit SessionIdReplayed; the false "exercised by the test suite" Row D-30 claim retracted + the test renamed model-only. (3) F-07 the DUAL-CID + generation-staleness surface (two_cid_store Surface-2: DualCidStore/reseal/blind_set_cid/verify_stanza_generation/GenError/k_principal_generation) DROPPED-by-DELETE (Ben-ratified) - it was dead AND misleading (verify_stanza_generation an unsigned < strictly WEAKER than the live signed B2 M_auth generation-recompute; reseal a BLAKE3 digest that does NOT encrypt despite its name; blind_set_cid a byte-identical redundant copy of the live membership_set_id_commitment); every axis ships live in layer_c.rs equal-or-stronger; the wave-3e TwoCidStore (Surface-1) KEPT + tf3e intact; SECURITY-POSTURE + V1-FROZEN §15.g reconciled (B2 M_auth is the generation-staleness defense of record; U20/k_principal_generation subsumed by construction - K_principal is the at-rest/vault encrypt-to-self key, ABSENT from the recipient Layer-C path); the stale GenError §3.5g baseline entries removed. (4) doc cluster - CodepointLifecycle got #[non_exhaustive]; MembershipSetKind cite {Atrium,Garden,Grove} corrected to {Atrium,DeviceMesh,SingleDevice}; vault-frame drift; Inv-19 register-then-enforce parity; keyring Row D-65; 0x6600 LIVE-table + 0x6320 symbol row; wasm32/privacy caveats; F-25 sender-lp-width doc; 6 rustdoc private-intra-doc-links to backticks-only. (5) OBS honestly NAMED: the membership-set governance/audit surface (F-01..F-05) disclosed as data-half models with zero prod callers + engine-layer enforcement; GAP-3 ct_mlkem spec-faithful freeze-record. RE-VERIFY the ROUND-9 fixes CORRECT + COMPLETE + NO new defect: (a) the nonce-cache prose now matches the code seam (NO residual persisted-GUARANTEED overclaim anywhere); (b) F-07 the DELETE left NO dangling reference in src/tests/docs/public-api + the SECURITY-POSTURE/V1-FROZEN §15.g reconciliation is ACCURATE + the KEPT TwoCidStore + tf3e are intact + NO live generation/replay defense was lost (B2 M_auth still binds + recomputes recipient_key_generation on 0x6510/0x6520 and the 3 group generations on 0x6610, fail-closed); (c) F-06 test is honestly model-only + the Row D-30 claim retracted; (d) CodepointLifecycle non_exhaustive + all doc-cluster fixes match as-built; (e) the OBS disclosures are ACCURATE (the governance/audit structs are genuinely zero-caller data-half + the real enforcement is engine-layer). RE-VERIFY THE WHOLE PHASE FRESH every round per feedback_full_phase_review_every_round (re-check EVERY freeze byte / golden / codepoint / invariant versus spec, NOT just the round-9-fix delta). Keep the gap1-closure-and-public-derivable-secret-class-sweep lens (hunt ANY secret derivable from public material anywhere) + crypto-error-oracle/failure-mode-uniformity + at-rest-format-migration/forever-decode + sender-origin-authentication-spoofing + as-built-doc-reconciliation. CONVERGENCE ARC (deeper lenses kept surfacing real defects - do NOT short-circuit): R1 to R2(online-sealed-sender BLOCKER + 4 MAJ) to R3(7 MAJ) to R4(C-01) to R5(2 MAJ) to R6(content-splice BLOCKER) to R7(CONF-1 nonce-reuse BLOCKER) to R8(GAP-1 placeholder-recipient-key BLOCKER) to R9(nonce-durability MAJOR + F-07 dead-surface + doc cluster) to R10(this re-review of the round-9 fixes). The LAST review before tag phase-4-meta-core-close freezes the v1 public interface. Q5 cadence: FULL council every round; iterate ACROSS INVOCATIONS until 0 confirmed BLOCKER/MAJOR; pattern-induction findings additive. Review the LANDED implementation + tracked docs + the frozen-interface contract (NOT a red-phase test-pin review). Hunt SUBSTANTIVE defects (crypto correctness, wire-freeze byte-correctness, fail-closed gaps, error-oracle uniformity, at-rest forever-decode, doc-versus-code drift, invariant-coverage at HEAD, deferred-honesty, SemVer/public-surface), not phantom-destinations.',
  ...
  targetLensCount: 23,
  priorMissedLenses: 'ROUND-8 completeness-critic found GAP-1 (the frozen recipient-keying used a public-derivable placeholder secret) as a lens-gap NO panel lens owned. Round 9 MUST include a dedicated lens that (1) VERIFIES the GAP-1 fix genuinely closed it (open takes a real RecipientSecret unrecoverable from public; the [u8;32] placeholder + wrapping_sub(0x80) + deterministic-expansion FULLY deleted with no production caller; a public-only party cannot open on all 3 codepoints; would-FAIL-on-revert) AND (2) SWEEPS FOR THE CLASS - is there ANY OTHER secret/key anywhere in the corpus that is derivable from public material, or a placeholder/deterministic-from-public key, or a secret carrying less entropy than it appears (layer_d device-link + multi-device key-wrap, swap_matrix, hpke, the vault DAK, K_principal, K_Set, any KDF whose secret input is actually public, any _for_test/deterministic keygen reachable in production)? The GAP-1 class - a secret that is not actually secret - is the deepest confidentiality failure mode; hunt it everywhere. ALSO keep the two round-8-added deep lenses (now standard): crypto-error-oracle/failure-mode-uniformity + at-rest-format-migration/forever-decode-of-prior-schema. ALSO keep the sender-origin-authentication-spoofing lens (round-2 origin missed-lens) and the as-built-doc-reconciliation lens (round-1 missed-lens) - re-verify every tracked doc narrative of what-is-built-AND-enforced versus the artifact at r9-base bbdf4b91, especially the NEW recipient-secrecy claims in SECURITY-PROOFS §4.1/4.2 (are they genuinely true as-built) and the DEFERRED D-64 honesty (is the engine-e2r-wiring deferral accurately NOT-a-Compromise).',
  canon: `## PROJECT CANON (inline — review agents CANNOT read gitignored .addl/CLAUDE.md; tracked docs ARE readable via: git show bbdf4b91:docs/...)
