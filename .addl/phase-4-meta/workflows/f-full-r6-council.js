...
const CFG = (args && args.cfg) || {
  ...
  enumerate: 'git ls-tree -r --name-only 05298fda -- crates/benten-crypto-suite crates/benten-membership-set crates/benten-drop crates/benten-engine/src/layer_d crates/benten-engine/tests docs/V1-FROZEN-INTERFACE.md docs/SECURITY-POSTURE.md docs/CRYPTO-CODEPOINTS.md docs/THREAT-MODEL.md docs/SECURITY-PROOFS.md docs/INVARIANT-COVERAGE.md docs/V1-WIRE-FORMAT-INVENTORY.md',
  specRef: '05298fda',
  specPath: 'docs/V1-FROZEN-INTERFACE.md',
  framing: 'PHASE-CLOSE convergence council - ROUND 12 (re-reviewing the phase at r9-base 05298fda after the ROUND-11 fix wave + adversarial-review polish landed). ROUND 11 = full 22-lens convergence council against 05298fda; the round-10 fixes ALL re-confirmed, but the whole-phase-fresh + gap1-class-sweep lenses caught 2 REAL MAJORs latent in SETTLED crypto territory (NOT round-10 regressions - the crypto was byte-identical R9->R11): MC-1 (the 0x6520 group bulk-CEK was BLAKE3 of PUBLIC wire inputs body_cid ‖ sender_did ‖ generation, NO secret - a relay guessing the sender public DID recomputed the CEK and read the group body; the per-recipient HPKE-wrap was pointless; SECURITY-PROOFS §4.1/§4.2 falsely claimed confidentiality) + MC-6 (the frozen vault on-disk frame persisted NEITHER the 16-byte Argon2id salt NOR params, so vault.cbor could not re-derive the DAK across a restart). ALL ROUND-11 findings now FIXED on 05298fda + ADVERSARIALLY VERIFIED (2 independent skeptic refuters returned refuted=false on EVERY claim) + full re-gate GREEN (1427 crypto-crate tests + 940 engine; cargo-doc -D warnings / cite-drift / §3.5g scanner all clean). MC-1 FIX (crypto, WIRE-TRANSPARENT): seal_group_impl (layer_c.rs:1229-1230) now draws a FRESH RANDOM 32-byte CEK from rand_core::OsRng.fill_bytes, delivered ONLY via the existing per-stanza HPKE-wrap; the old public BLAKE3 derivation is DELETED from production (LAYER_C_GROUP_CEK_CONTEXT retained as a RESERVED label + drift-mirror only); open_group_stanza is UNCHANGED (already unwraps the CEK from stanza.wrapped_cek, never recomputes) so the 0x6520 wire fields/sizes are byte-identical and no golden was invalidated. Tests SUBSTANTIVE + would-FAIL-on-revert: mc_1_non_recipient_cannot_recover_group_cek (recomputes the OLD public-derived candidate and asserts it != the real random CEK AND cannot AEAD-open the body) + f_lc_3_content_splice_rejected_layer_c_group (refactored to the RESIDUAL co-recipient splice threat - a co-recipient unwraps the real CEK from its own stanza, keeps A sig + original body_cid, substitutes a body; the F-01 CID-recompute guard still fail-closes SenderOriginAuthFailed). The SEPARATE 0x6610 MembershipSet model (derive_group_cek keyed by the SECRET K_Set, never on the wire) was correctly left UNCHANGED (genuinely NOT public-derivable). MC-6 FIX (crypto, Ben-ratified frame extension, pre-freeze so NO post-tag wire-break): the vault frame now = magic 0xae ‖ V2 ‖ codepoint(BE) ‖ salt(16) ‖ m_cost(u32 BE) ‖ t_cost(u32 BE) ‖ p_cost(u32 BE) ‖ nonce_len ‖ nonce ‖ ct (32-byte fixed header); serialize_vault threads salt+params, open_vault re-derives the DAK from the FRAME header alone so vault.cbor bytes + password ALONE decrypt across a restart; parse_vault_frame is BOUNDS-SAFE (len-guards before every index; typed errors; no panic/OOB); a tampered in-header salt/param FAILS CLOSED (wrong DAK -> AEAD fail, never a weak-key accept - standard PBKDF-header posture); f_va_1 golden regenerated (frame-shape change only, pre-freeze, no surviving V1/V2 golden) + new self-containment + header-persist tests; benten-crypto-suite.txt public-api regen exact (serialize_vault sig + open_vault + DecodedVault.salt/.params + MC-13 UnlockedKeyMaterial Drop). R11 POLISH (doc/comment-only, from the adversarial reviews): CEK raw-stack-local not-zeroized benign-gap note (benten-drop has no zeroize dep; the load-bearing AeadKeyMaterial copy IS zeroized on drop; future hygiene sweep can adopt zeroize) + vault salt-origination caller-contract (production vault-CREATION must seed salt from OsRng unique-per-vault; creation wiring deferred -> Row D-69) + header-salt/params-not-AEAD-AAD-covered rationale (self-authenticating via the derivation). MINOR TAIL FIXED: MC-7 (CRATES-DEEP-DIVE 14->15 + benten-membership-set section + new crate-count drift-guard test) MC-8 (drop/INTERNALS #31->#62) MC-10 (THREAT-MODEL §6 membership-counts-never-wire-decoded) MC-13 (Drop-zeroize user_did_signing_key) MC-14 (V1-FROZEN:20 14->15) MC-15 (2 phantom TS cites -> real paths). NAMED-CARRY: MC-11 (Row D-67 parse_cbor_bytes v1-GM decode-cap) MC-12 (validator header, Row D-1 CLOSED) MC-16 (Row D-68 f_hlc_2 precision). RE-VERIFY the ROUND-11 fixes CORRECT + COMPLETE + NO new defect: (a) MC-1 - the 0x6520 CEK is genuinely fresh-random from a real OS CSPRNG, NO residual public-derivable/deterministic-in-production path, wire fields/sizes unchanged, open path intact, F-01 guard fail-closed, the 0x6610 K_Set model genuinely secret-keyed; (b) MC-6 - vault.cbor + password decrypt from bytes alone, param-tamper fail-closed, parse bounds-safe, no salt/param error-oracle, freeze/golden/public-api reconciliation honest; (c) the polish notes are accurate; (d) the MINOR tail + named rows are correct. RE-VERIFY THE WHOLE PHASE FRESH every round per feedback_full_phase_review_every_round (re-check EVERY freeze byte / golden / codepoint / invariant vs spec, NOT just the round-11-fix delta). NEW LENS THIS ROUND (round-11 completeness-critic GAP-D, HIGHEST priority): freeze-safety-of-zero-production-caller-surfaces - for EVERY frozen public surface with ZERO production callers at HEAD (derive_kv/K(V) + fork_total_order_key/Inv-21 comparator + accept_grant + device-link replay-store + nonce-cache durability seam + verify_stanza + the membership-set governance/audit data-half + any _for_test-adjacent frozen shape), assert its frozen signature is FORWARD-COMPATIBLE with its eventual post-freeze (Composing) caller: can the deferred wiring be added additively WITHOUT a SemVer break or a wire change? A frozen zero-caller shape that would FORCE a breaking change when its first real caller lands is a FREEZE-RISK to surface BEFORE the tag. Keep the standing deep lenses: gap1-closure-and-public-derivable-secret-class-sweep (hunt ANY secret derivable from public material - re-sweep now that 0x6520 is fixed, incl K(V)/K(N)/K_Set/DAK/device-link/multi-device-wrap) + crypto-error-oracle/failure-mode-uniformity + at-rest-format-migration/forever-decode (ESPECIALLY the NEW vault V2 frame) + sender-origin-authentication-spoofing + as-built-doc-reconciliation. CONVERGENCE ARC (deeper lenses kept surfacing real defects - do NOT short-circuit): R1->R2(online-sealed-sender BLK)->R3(7 MAJ)->R4(C-01)->R5(2 MAJ)->R6(content-splice BLK)->R7(CONF-1 nonce-reuse BLK)->R8(GAP-1 placeholder-key BLK)->R9(nonce-durability MAJ + F-07 dead-surface)->R10(2 doc MAJ)->R11(2 REAL MAJ: 0x6520 public-CEK confidentiality break + vault-salt freeze-fork)->R12(this re-review of the round-11 fixes). The LAST review before tag phase-4-meta-core-close freezes the v1 public interface. Q5 cadence: FULL council every round; iterate ACROSS INVOCATIONS until 0 confirmed BLOCKER/MAJOR; pattern-induction findings additive. Review the LANDED implementation + tracked docs + the frozen-interface contract (NOT a red-phase test-pin review). Hunt SUBSTANTIVE defects (crypto correctness, wire-freeze byte-correctness, fail-closed gaps, error-oracle uniformity, at-rest forever-decode, freeze-safety of zero-caller surfaces, doc-versus-code drift, invariant-coverage at HEAD, deferred-honesty, SemVer/public-surface), not phantom-destinations.',
  ...
  targetLensCount: 23,
  priorMissedLenses: 'ROUND-8 completeness-critic found GAP-1 (the frozen recipient-keying used a public-derivable placeholder secret) as a lens-gap NO panel lens owned. Round 9 MUST include a dedicated lens that (1) VERIFIES the GAP-1 fix genuinely closed it (open takes a real RecipientSecret unrecoverable from public; the [u8;32] placeholder + wrapping_sub(0x80) + deterministic-expansion FULLY deleted with no production caller; a public-only party cannot open on all 3 codepoints; would-FAIL-on-revert) AND (2) SWEEPS FOR THE CLASS - is there ANY OTHER secret/key anywhere in the corpus that is derivable from public material, or a placeholder/deterministic-from-public key, or a secret carrying less entropy than it appears (layer_d device-link + multi-device key-wrap, swap_matrix, hpke, the vault DAK, K_principal, K_Set, any KDF whose secret input is actually public, any _for_test/deterministic keygen reachable in production)? The GAP-1 class - a secret that is not actually secret - is the deepest confidentiality failure mode; hunt it everywhere. ALSO keep the two round-8-added deep lenses (now standard): crypto-error-oracle/failure-mode-uniformity + at-rest-format-migration/forever-decode-of-prior-schema. ALSO keep the sender-origin-authentication-spoofing lens (round-2 origin missed-lens) and the as-built-doc-reconciliation lens (round-1 missed-lens) - re-verify every tracked doc narrative of what-is-built-AND-enforced versus the artifact at r9-base 05298fda, especially the NEW recipient-secrecy claims in SECURITY-PROOFS §4.1/4.2 (are they genuinely true as-built) and the DEFERRED D-64 honesty (is the engine-e2r-wiring deferral accurately NOT-a-Compromise).',
  canon: `## PROJECT CANON (inline — review agents CANNOT read gitignored .addl/CLAUDE.md; tracked docs ARE readable via: git show 05298fda:docs/...)
