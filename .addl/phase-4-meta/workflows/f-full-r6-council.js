...
const CFG = (args && args.cfg) || {
  ...
  enumerate: 'git ls-tree -r --name-only b7dbb681 -- crates/benten-crypto-suite crates/benten-membership-set crates/benten-drop crates/benten-engine/src/layer_d crates/benten-engine/tests docs/V1-FROZEN-INTERFACE.md docs/SECURITY-POSTURE.md docs/CRYPTO-CODEPOINTS.md docs/THREAT-MODEL.md docs/SECURITY-PROOFS.md docs/INVARIANT-COVERAGE.md docs/V1-WIRE-FORMAT-INVENTORY.md',
  specRef: 'b7dbb681',
  specPath: 'docs/V1-FROZEN-INTERFACE.md',
  framing: 'PHASE-CLOSE convergence council - ROUND 9 (re-reviewing the phase at r9-base b7dbb681 after the ROUND-8 council findings were FIXED and landed on the r9-base branch, now on merge PR #1382 - a PRE-MERGE convergence review of the round-8 fix wave). ROUND 8 = full 22-lens council, NOT-CONVERGED: 1 BLOCKER (GAP-1) + 2 MAJOR freeze-forks (F-06, F-07) + a doc cluster + MINOR/OBS. ALL FIXED on r9-base. THE BLOCKER (GAP-1, found by the round-8 completeness-critic as a lens-gap NO panel lens owned): the FROZEN Layer-C encrypt-to-recipient surface used a PLACEHOLDER recipient-keying with ZERO secret entropy - RecipientPubKey/RecipientSecKey were [u8;32] fingerprints and the production open paths derived the recipient secret from the PUBLIC fingerprint (sk = pk minus 0x80 per byte, then deterministic-expand), so ANY holder of the public recipient key could decrypt (total confidentiality break on the frozen surface). FIXED (Ben-ratified elegant permanent shape): layer_c seal/open are now PURE crypto over the REAL hybrid key types RecipientPublic/RecipientSecret (which already existed in benten-crypto-suite); the [u8;32] aliases + the wrapping_sub(0x80) derivation + the deterministic-expansion are DELETED; a canary added real-entropy generate_recipient_keypair (OS RNG) + RecipientPublic/Secret to_bytes/from_bytes (codepoint-dispatched, fail-closed) + zeroize-on-drop; the negative test f_lc_gap1_real_recipient_keying pins closure across all 3 codepoints (single 0x6510 / group 0x6520 / membership-set-group 0x6610) - a public-only party cannot open, would-FAIL-on-revert. WIRE byte-UNCHANGED: only the ONE F_02_LIVE_SEAL_STANZA0_AAD_HEX fixture-golden 32-byte audience_set_commitment component was regenerated (recipient-key representation placeholder to real hybrid pubkey); the AAD SHAPE + field-set + blinding + version + the cross-engine byte-equality arm (f_02_local_assembler_matches_canonical_membership_set) + the round-trip arm are INTACT. The engine-level USE (mint the recipient keypair as Principal identity + vault-store the secret + benten-id DID encryption-key resolution + seal-side pub sourcing) is a named DEFERRED-to-Phase-4-Meta-Composing row (V1-FROZEN-INTERFACE-DEFERRED Row D-64) - nothing in the engine calls encrypt-to-recipient yet, so the frozen primitive is safe + exercisable and the wiring is honestly deferred (NOT a Compromise - keying is REAL now). GAP-1 was verified at THREE levels before this council: orchestrator mini-review + cargo check --workspace (zero stray refs) with nextest 1423 + 939 + a DEDICATED adversarial-verification (6/6 PASS: no placeholder residue, retype correct all 3 codepoints, CONF-1 derive_group_cek confirmed SEAL-ONLY never in open, secret unrecoverable from public, wire intact, no missed sibling surface). THE 2 MAJOR freeze-forks (both landed): F-06 - three _for_test SubgraphExt TRAIT methods were on the frozen benten-eval surface (the no-regression guard was blind to trait-method form); cfg-gated behind cfg(testing) + the guard extended to catch trait-method form. F-07 - six semantic pub enums in benten-membership-set (AdminOp / AuditAccessGradation / AuditReadDecision / RequesterRole / FederationModel / GovernanceTier) lacked #[non_exhaustive] (crate added post-G-CORE-9 sweep); attribute applied + a semantic-pub-enum completeness census guard added; same-crate matches kept EXHAUSTIVE (future variant = compile error). THE DOC CLUSTER (landed): SECURITY-PROOFS §4.1/4.2 recipient-secrecy now affirmatively REAL, THREAT-MODEL recipient-compromise rung, V1-FROZEN §6 retyped signatures + §11 the 6 F-07 enums, V1-WIRE-INVENTORY the F_02 fixture note, DEFERRED Row D-64, and the round-8 doc-cluster (F-01 byte-faithful LAMPS INTERNALS with the phantom hybrid_combine stripped / F-02 libcrux-production RustCrypto-dev-KAT / F-03 21-file inventory / F-04 drop layer_c section / F-05 domain_registry EVERY-to-scoped carve-out / F-10 F-11 LE-to-BE doc slips / F-12 SENDER_AUTH_SIG_CODEPOINT from the SSOT const); 2 rustdoc private-intra-doc-links fixed to backticks-only; the 2 new AeadError decode-time variants (MalformedRecipientPublic/Secret) baselined into the §3.5g item-6 drain wave (AeadError is not ErrorCode/napi-mirrored by design - all 8 siblings baselined). RE-VERIFY the round-9 fixes CORRECT + COMPLETE + NO new defect: (a) GAP-1 - the recipient secret is REAL hybrid key material throughout, the [u8;32] placeholder + wrapping_sub(0x80) + deterministic-expansion are FULLY gone with NO production caller, open takes a real RecipientSecret (not a forgeable [u8;32]), a public-only party cannot open on ANY of the 3 codepoints, the round-7 CONF-1 derive_group_cek stays SEAL-ONLY (never in any open path), and NO OTHER encrypt-to-recipient/key-wrap surface (layer_d device-link, multi-device-wrap, swap_matrix, hpke, vault) uses a public-derivable or placeholder secret; (b) the wire/goldens are truly byte-unchanged except the one intended F_02 fixture component; (c) F-06/F-07 are complete + their guards genuinely catch the class; (d) the docs match the as-built code (no new doc-versus-code drift). TWO STANDING DEEP LENSES this round (now standard, added round 8, returned clean there): CRYPTO-ERROR-ORACLE / FAILURE-MODE-UNIFORMITY (distinguishable typed rejects across seal-open + composite-verify + KEM-decap - any adaptive-query/timing distinguisher; napi/wire boundary uniform) and AT-REST-FORMAT-MIGRATION / FOREVER-DECODE-OF-PRIOR-SCHEMA (forever-decode older vault/DropBundle/SnapshotBlob schema versions per never-strand-content; downgrade-by-schema-byte-rewrite fail-closed). Keep the sender-origin-authentication-spoofing lens + the as-built-doc-reconciliation lens. CONVERGENCE ARC (deeper crypto lenses keep surfacing real defects - do NOT short-circuit): R1 to R2 (1 BLOCKER online-sealed-sender + 4 MAJOR) to R3 (7 MAJOR) to R4 (C-01) to R5 (2 MAJOR) to R6 (1 BLOCKER content-splice) to R7 (1 BLOCKER CONF-1 nonce-reuse) to R8 (1 BLOCKER GAP-1 placeholder-recipient-key) to R9 (this re-review). RE-VERIFY THE WHOLE PHASE FRESH every round per feedback_full_phase_review_every_round (re-check every freeze byte / golden / codepoint / invariant versus spec, NOT just the round-9 delta; ratified DESIGN decisions off-limits to re-debate but the IMPLEMENTATION re-verified every round - the R4.5b precedent where a converged loop had frozen the wrong codepoint in territory its canon told it not to re-examine). The LAST review before tag phase-4-meta-core-close freezes the v1 public interface. Q5 cadence: FULL council every round; iterate ACROSS INVOCATIONS until 0 confirmed BLOCKER/MAJOR; pattern-induction findings are additive. Review the LANDED implementation + tracked docs + the frozen-interface contract (NOT a red-phase test-pin review). Hunt SUBSTANTIVE defects (crypto correctness, wire-freeze byte-correctness, fail-closed gaps, error-oracle uniformity, at-rest forever-decode, doc-versus-code drift, invariant-coverage at HEAD, deferred-honesty, SemVer/public-surface), not phantom-destinations.',
  mustInclude: ['crypto-construction-correctness','wire-freeze-byte-correctness','codepoint-registry-integrity','invariant-semantics-inv15-22','membershipset-primitive-fidelity','sender-origin-authentication-spoofing','capability-authority-ucan-layerd','confidentiality-threat-model-bounded-decode','distributed-sync-crdt-hlc-gossip','supply-chain-libcrux-cryspen-vet','public-surface-semver-freeze','spec-to-code-ruling-fidelity-pim13-r7','deferred-items-hardrule-honesty','cross-target-wasm-deployment-shapes','as-built-doc-reconciliation-prose-currency','crypto-error-oracle-failure-mode-uniformity','at-rest-format-migration-forever-decode','gap1-closure-and-public-derivable-secret-class-sweep'],
  targetLensCount: 23,
  priorMissedLenses: 'ROUND-8 completeness-critic found GAP-1 (the frozen recipient-keying used a public-derivable placeholder secret) as a lens-gap NO panel lens owned. Round 9 MUST include a dedicated lens that (1) VERIFIES the GAP-1 fix genuinely closed it (open takes a real RecipientSecret unrecoverable from public; the [u8;32] placeholder + wrapping_sub(0x80) + deterministic-expansion FULLY deleted with no production caller; a public-only party cannot open on all 3 codepoints; would-FAIL-on-revert) AND (2) SWEEPS FOR THE CLASS - is there ANY OTHER secret/key anywhere in the corpus that is derivable from public material, or a placeholder/deterministic-from-public key, or a secret carrying less entropy than it appears (layer_d device-link + multi-device key-wrap, swap_matrix, hpke, the vault DAK, K_principal, K_Set, any KDF whose secret input is actually public, any _for_test/deterministic keygen reachable in production)? The GAP-1 class - a secret that is not actually secret - is the deepest confidentiality failure mode; hunt it everywhere. ALSO keep the two round-8-added deep lenses (now standard): crypto-error-oracle/failure-mode-uniformity + at-rest-format-migration/forever-decode-of-prior-schema. ALSO keep the sender-origin-authentication-spoofing lens (round-2 origin missed-lens) and the as-built-doc-reconciliation lens (round-1 missed-lens) - re-verify every tracked doc narrative of what-is-built-AND-enforced versus the artifact at r9-base b7dbb681, especially the NEW recipient-secrecy claims in SECURITY-PROOFS §4.1/4.2 (are they genuinely true as-built) and the DEFERRED D-64 honesty (is the engine-e2r-wiring deferral accurately NOT-a-Compromise).',
  canon: `## PROJECT CANON (inline — review agents CANNOT read gitignored .addl/CLAUDE.md; tracked docs ARE readable via: git show b7dbb681:docs/...)
