...
const CFG = (args && args.cfg) || {
  ...
  enumerate: 'git ls-tree -r --name-only e7a610cf -- crates/benten-crypto-suite crates/benten-membership-set crates/benten-drop crates/benten-id crates/benten-graph crates/benten-engine/src/layer_d crates/benten-engine/tests docs/V1-FROZEN-INTERFACE.md docs/SECURITY-POSTURE.md docs/CRYPTO-CODEPOINTS.md docs/THREAT-MODEL.md docs/SECURITY-PROOFS.md docs/INVARIANT-COVERAGE.md docs/V1-WIRE-FORMAT-INVENTORY.md',
  specRef: 'e7a610cf',
  specPath: 'docs/V1-FROZEN-INTERFACE.md',
  framing: 'PHASE-CLOSE convergence council - ROUND 14 (re-reviewing the phase at r9-base e7a610cf after the ROUND-13 fix wave landed, AND with two previously-under-enumerated crates NOW in the review scope: crates/benten-id + crates/benten-graph). ROUND 13 = full 20-lens convergence council against e7a610cf; the R12+F-11 fixes ALL re-confirmed, but the deeper byte-level lens set (X-Wing/LAMPS byte-fidelity + wasm32-overflow + doc-coherence) surfaced 3 CONFIRMED MAJORs (all instances of KNOWN classes, 0 new BLOCKER, 0 new failure-mode), ALL now FIXED on e7a610cf + ORCH-ground-truthed + full re-gate GREEN (1668 crypto-crate tests + 941 engine; cargo-doc -D warnings / cite-drift / §3.5g clean; the sole nextest timeout is the pre-existing in_memory_observationally_equivalent_to_redb slow-under-parallel-load proptest, CONFIRMED PASS isolated 93s, unrelated). F-01 FIX (freeze-hygiene leak): generate_recipient_keypair_deterministic (a forgeable deterministic-from-seed keypair, zero prod callers) was pub on the FROZEN crypto-suite default-feature API — RENAMED generate_recipient_keypair_deterministic_for_test + cfg-gated #[cfg(any(test,feature=testing))] (now GONE from benten-crypto-suite.txt frozen baseline; 7 callers updated; the _for_test no-regression guard now catches it). F-04 FIX (bounded-decode integer-overflow DoS on wasm32): aead_wrap.rs decode_encrypted_node variant 0x01 added cursor+len (attacker u32-as-usize) BEFORE checking against bytes.len() — on 32-bit usize (wasm32 thin-client) it overflow-wrapped below the bound + panicked (pre-auth via redb_backend get_encrypted_node). FIX: a new checked_range_end(off,len,total)->Option helper (mirrors layer_c lp_range_end) routes BOTH cursor+4 and cursor+len through checked arithmetic -> None on overflow-OR-OOB -> typed CiphertextTooShort; neighbor sweep clean (aead.rs from_wire_bytes nonce_len u8-bounded, layer_c open already lp_range_end, membership-set aad encoder-only); tests checked_range_end_returns_none_on_overflow (target-agnostic would-FAIL-on-revert) + per_chunk_len_overflow_is_typed_reject_not_panic. F-06/F-07 FIX (threat-model honesty defect): THREAT-MODEL §1 Untrusted-host row claimed sees-plaintext=NO(per-Node AEAD seals) but K_principal=BLAKE3-keyed-hash(domain_tag[PUBLIC registered const], namespace_did[PUBLIC]) is PUBLICLY-DERIVABLE so per-Node AEAD is a STAND-IN NOT real untrusted-host confidentiality at v1-beta (confidentiality half DEFERRED #1301/D-64 per baked-in #18; authority half = capability/namespace isolation binds only a cooperating engine). RETENSED the untrusted-host row to NOT-YET-at-v1-beta honest framing (RELAY/network-observer NO untouched — Layer-C is live+real) + minted Compromise #65 (SGD) in SECURITY-POSTURE + extended f_disc_1 catch-net to #30..=#65. MINOR/named tail all dispositioned: F-03/08/09/10/11/17 fix-now + Row D-72 vault-version-policy (f_va_12 escalation pin) + Row D-73 Layer-D-u32-sweep + F-13/F-16 + Row D-37 extension. RE-VERIFY the ROUND-13 fixes CORRECT + COMPLETE + NO new defect: (a) F-01 - _for_test cfg-gated off the frozen API, no forgeable deterministic constructor on the production surface, guard catches the class; (b) F-04 - EVERY attacker-controlled length added to an offset in aead_wrap/layer_c/membership-set/drop decode paths is checked-arithmetic (no residual raw offset+wire_len that overflow-wraps on 32-bit); (c) F-06 - the threat-model now HONESTLY scopes v1-beta confidentiality (untrusted-host NOT protected, confidentiality half deferred) matching SECURITY-POSTURE + #18, and Compromise #65 accurately captures the limit; (d) the MINOR/named landings are accurate. NEW-COVERAGE MANDATE (MISSED-LENS-1, HIGHEST priority): crates/benten-id + crates/benten-graph are NOW enumerated - SYSTEMATICALLY review them fresh: benten-id (the hybrid did:key resolve_hybrid parser over two registered-component multikeys — the SOUNDNESS DEPENDENCY of the B2 inter-member non-forgeability claim: verify the parser length-checks before every slice, validates both component multicodecs, uses checked/saturating arithmetic, typed-rejects fail-closed with NO panic path, and that a malformed/over-length did:key cannot forge a verifying key or bypass RotationLog) + benten-graph (aead_wrap.rs per-Node AEAD encrypt/decrypt + redb_backend storage decode + the ChunkedCiphertext path: verify NO remaining unbounded-alloc / offset-overflow beyond the F-04 fix, the caller-supplied key contract is honest per F-06, and the storage read-back is bounded-decode-safe pre-auth). RE-VERIFY THE WHOLE PHASE FRESH every round per feedback_full_phase_review_every_round (re-check EVERY freeze byte / golden / codepoint / invariant vs spec, NOT just the round-13-fix delta). Keep freeze-safety-of-zero-production-caller-surfaces (GAP-D standing) + gap1-closure-and-public-derivable-secret-class-sweep + crypto-error-oracle/failure-mode-uniformity + at-rest-format-migration/forever-decode + sender-origin-authentication-spoofing + as-built-doc-reconciliation + bounded-decode/unbounded-allocation-and-offset-overflow (the F-01-R12/F-04-R13 class — sweep EVERY attacker-controlled length prefix AND every offset+len add across ALL enumerated crates incl the newly-added benten-id/benten-graph). CONVERGENCE ARC (deeper lenses + new coverage kept surfacing real defects - do NOT short-circuit): R8(GAP-1 BLK)->R9(nonce-dur MAJ)->R10(2 doc MAJ)->R11(2 crypto MAJ)->R12(1 DoS MAJ)->R13(3 MAJ freeze-leak+wasm32-overflow+threat-honesty)->R14(re-verify R13 fixes + benten-id/benten-graph new coverage). The LAST review before tag phase-4-meta-core-close freezes the v1 public interface. Q5 cadence: FULL council every round; iterate ACROSS INVOCATIONS until 0 confirmed BLOCKER/MAJOR; pattern-induction findings additive. Review the LANDED implementation + tracked docs + the frozen-interface contract (NOT a red-phase test-pin review). Hunt SUBSTANTIVE defects (crypto correctness, wire-freeze byte-correctness, fail-closed gaps, bounded-decode/unbounded-alloc/offset-overflow, error-oracle uniformity, at-rest forever-decode, freeze-safety of zero-caller surfaces, doc-versus-code drift, invariant-coverage at HEAD, deferred-honesty, SemVer/public-surface, the benten-id did:key parser soundness), not phantom-destinations.',
  ...
  targetLensCount: 23,
  priorMissedLenses: 'ROUND-8 completeness-critic found GAP-1 (the frozen recipient-keying used a public-derivable placeholder secret) as a lens-gap NO panel lens owned. Round 9 MUST include a dedicated lens that (1) VERIFIES the GAP-1 fix genuinely closed it (open takes a real RecipientSecret unrecoverable from public; the [u8;32] placeholder + wrapping_sub(0x80) + deterministic-expansion FULLY deleted with no production caller; a public-only party cannot open on all 3 codepoints; would-FAIL-on-revert) AND (2) SWEEPS FOR THE CLASS - is there ANY OTHER secret/key anywhere in the corpus that is derivable from public material, or a placeholder/deterministic-from-public key, or a secret carrying less entropy than it appears (layer_d device-link + multi-device key-wrap, swap_matrix, hpke, the vault DAK, K_principal, K_Set, any KDF whose secret input is actually public, any _for_test/deterministic keygen reachable in production)? The GAP-1 class - a secret that is not actually secret - is the deepest confidentiality failure mode; hunt it everywhere. ALSO keep the two round-8-added deep lenses (now standard): crypto-error-oracle/failure-mode-uniformity + at-rest-format-migration/forever-decode-of-prior-schema. ALSO keep the sender-origin-authentication-spoofing lens (round-2 origin missed-lens) and the as-built-doc-reconciliation lens (round-1 missed-lens) - re-verify every tracked doc narrative of what-is-built-AND-enforced versus the artifact at r9-base e7a610cf, especially the NEW recipient-secrecy claims in SECURITY-PROOFS §4.1/4.2 (are they genuinely true as-built) and the DEFERRED D-64 honesty (is the engine-e2r-wiring deferral accurately NOT-a-Compromise).',
  canon: `## PROJECT CANON (inline — review agents CANNOT read gitignored .addl/CLAUDE.md; tracked docs ARE readable via: git show e7a610cf:docs/...)
