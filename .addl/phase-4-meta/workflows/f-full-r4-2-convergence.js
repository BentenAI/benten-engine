export const meta = {
  name: 'f-full-r4-2-convergence',
  description: 'F-full R4.2: 15-lens convergence re-review of the R4-FIXED corpus (verify R4.1 findings CLOSED + hunt regressions) → structure → adversarial-verify → completeness-critic → converge',
  phases: [
...
const CORPUS = 'phase-4-meta-core/f-full-r4-fix'
const MAIN = '2172cb6d'
const R03 = 'phase-4-meta-core/f-full-r0-plan-r04'
const ORCH = 'phase-4-meta-core/orchestration-2026-05-26'

const COMMON = `
You are ONE lens on the **F-full R4.2 CONVERGENCE re-review council**. This is the iterate-to-convergence re-review (rule 9 / Q5) of the **R4-FIXED red-phase corpus** that gates R5 implementation.

**WHAT HAPPENED:** R4.1 found 8 BLOCKER + ~26 MAJOR in the corpus (read the triage: \`git show ${'phase-4-meta-core/orchestration-2026-05-26'}:.addl/phase-4-meta/r4-triage.md\`). An R4-fix pass then FIXED them (the corpus you review now = \`phase-4-meta-core/f-full-r4-fix\`, off the original \`f-full-r3-consolidated\`; the spec was revised to R0.4 applying Ben's rulings). **YOUR JOB (3 parts):** (a) **VERIFY** each R4.1 finding in your lens-area is genuinely CLOSED — the fix is correct, complete, and meets the substantive-pin bar (real entry + observable consequence + would-FAIL-on-no-op); a fix that merely *looks* closed but is still weak/wrong is a finding. (b) **HUNT REGRESSIONS** — did any fix introduce a NEW defect, break a sibling pin, or mis-freeze a byte? (c) confirm the corpus is **freeze-safe** from your lens. **Convergence = 0 new BLOCKER/MAJOR.**

**Known-correct (do NOT re-flag):** the AAD-wrapper-is-canonical-TLV reading (vs DAG-CBOR for the members_table snapshot) was applied per R0.4 §4.1 — confirm it's right, don't treat it as a defect. The golden-hex literals are frozen against the *stub* encoders (M-20: R5 confirms-or-deliberately-updates vs the real encoder) — that is the CORRECT red-phase shape, not a defect. The \`_nq_*_gated\` Layer-D arms are RATIFIED (NQ-T2/T3/T4/C5 SPEC-NOW) — leave them.

Find everything still WRONG, WEAK, MISSING, MIS-FROZEN, or NEWLY-BROKEN *from your lens*.
...
- **R0.4 canonical plan** (the spec the tests must faithfully pin — R0.4 applied Ben's R4 rulings: 0x6520+0x6610 Sealed-Sender, 0x6101 assigned, Admin=Mod+governance, M-19 BE site-list, NQ-T2/T3/T4/C5 RATIFIED): \`git show ${R03}:.addl/phase-4-meta/f-full-r0-plan.md\` (~1567 lines)
