...
const CFG = {
  ...
  artifactDesc: 'the F-full red-phase test corpus migrating the Sealed-Sender AAD to spec R0.6',
  mustInclude: ['ss-aad-migration-fidelity', 'commitment-construction-correctness', 'golden-hex-byte-correctness'],
  crates: [
...
- THE MIGRATION (what R0.6 NOW mandates vs what the corpus has):
  ...
  CRITICAL: reuse the EXACT commitment constructions the corpus ALREADY uses for the §3.9 gossip topic (read the gossip / topic-blinding test, e.g. f_gossip / f_d6, and reuse its HMAC primitive + truncation + its BLAKE3 helper) — do NOT invent a different HMAC or hash. If the corpus has a topic-blinding helper, the membership_set_id_commitment is the SAME function applied to the set-id.
  2. body_cid -> self-describing CIDv1 on BOTH bands: bytes 0x01 0x71 0x1e 0x20 || 32-byte BLAKE3 digest (CIDv1 = version 0x01, dag-cbor codec 0x71, blake3 multihash 0x1e, length 0x20). NOT a bare 32-byte digest. Applies to 0x6510 (benten-drop f_lc_hpke DropSealedSender + f_lc_abuse SealedSenderAad + benten-crypto-suite f_inv16_1 Recipient) AND 0x6610 (f_aad_2). Regenerate every affected golden (incl. F_INV18_1_SEALED_AAD_HEX).
  ...
  4. token-binding AAD (f_lc_abuse TokenBindingAad): NO coarse_epoch (already removed); confirm + aad_version byte == 0x01 (NOT 0x02).
- ALREADY-CORRECT — do NOT re-flag/re-litigate: every R4.1-R4.4 fix is verified-correct (X-Wing SHA3-256 APPEND + 6-byte XWING_LABEL pin; MemberRef+RoleId int-discriminant; AAD_VERSION 0x01; DeviceAuthBackend sealed open=6; bounded-decode flagship two-bound; audience reconciliation). SETTLED: coarse_epoch NOT on Drop wire; u16/u32 per-band widths NOT unified (separately-frozen bands — do NOT touch widths); identity-hiding-not-unlinkability is accepted (full unlinkability = U25 v1-GM-reserve — do NOT add salt/nonce now).
- HONEST SCOPE: the group-AAD blinding is identity-HIDING (the commitment recurs for a static group). Recipients hold K_Set + the member list -> recompute + verify both commitments -> ALL bindings (cross-stanza substitution U17; inter-member non-forgeability) PRESERVED; relay sees only opaque 32-byte tags.
- Substantive-pin bar (pim-2): every migrated test stays #[ignore]'d + self-contained stub-shim + asserts the NEW frozen bytes + would-FAIL-on-no-op (revert the blinding -> golden mismatches). Golden-hex = M-20 (frozen vs stub; R5 confirms vs real encoder). Beware Rust-2024 reserved keyword gen.
