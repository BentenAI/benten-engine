...
> ## ★★★ AS-BUILT CORRECTIONS (2026-06-06; SUPERSEDE the R0.2 body where they conflict)
>
> The B2 construction is **IMPLEMENTED + GREEN** on `phase-4-meta-core/sealed-sender-auth-impl`
> (`crates/benten-drop/src/layer_c.rs`). The R0.2 body below is preserved for design rationale,
> but **these as-built corrections WIN** where they disagree (the R0.2 draft carried three
> API/wire errors the implementation corrected):
>
> 1. **F-1 — NO `with_context` API.** The R0.2 §1.3 step 3 / §1.4 step 6 say
>    `SignatureSuite::resolve_codepoint(sig_codepoint)?.sign_with_context(kp, M_auth, ctx = SENDER_AUTH_DOMAIN)` /
>    `…verify_with_context(vk, M_auth, ctx, sig)`. **This is WRONG.** There is no such
>    sealed-sender API call: the implementation uses the plain
>    `benten_crypto_suite::sig::SignatureSuite::v1_default().sign(kp, M_auth)` /
>    `.verify(vk, M_auth, &sig)` (empty LAMPS `ctx`), EXACTLY like `envelope_sig.rs`.
>    **Domain separation is the `SENDER_AUTH_DOMAIN` PREFIX baked into the `M_auth` BYTES**
>    (`build_m_auth` leads with it), NOT a LAMPS `ctx` argument. The `sig_codepoint` is bound
>    inside `M_auth` + the sealed body region; the verify fail-closes a non-`0x0001` codepoint.
> 2. **`Did::resolve_hybrid` ALREADY EXISTS — no "wiring wave" to build.** R0.2 §Q3 / FLAG-2
>    say `did:key` hybrid resolution must be wired (`Did::resolve` is "Ed25519-only";
>    `HYBRID_SIG_MULTICODEC` "does not exist"). On the impl base branch
>    (`phase-4-meta-core/benten-id-hybrid-key`) `benten_id::did::Did::resolve_hybrid` +
>    `Did::from_hybrid_public_key` (the registered ML-DSA-first two-component multikey,
>    `MLDSA65_PUB_MULTICODEC 0x1211 ‖ ED25519_MULTICODEC 0xed`) ARE built. The impl ADDS only a
>    production-safe `Did::parse_validated_hybrid` (validate-on-construct) so the drop verifier
>    never uses the test-only string constructor.
> 3. **ML-DSA-FIRST wire ordering** throughout (consistent with `from_lamps_composite_bytes` /
>    `HybridSignature::to_wire_bytes`) — the `sender_sig` wire is `mldsaSig ‖ tradSig`.
>
> **As-built body-region wire (matches the inventory §26):**
> - `0x6510` single: `inner_v2 = lp_u32(sender_did) ‖ sig_codepoint(u16 BE) ‖ lp_u32(sender_sig) ‖ body` (once-sealed).
> - `0x6520`/`0x6610` group: `body_v2 = sig_codepoint(u16 BE) ‖ lp_u32(sender_sig) ‖ body` (PREPENDED into the once-bulk-sealed body; per-stanza `sealed_inner` UNCHANGED).
>
> **As-built `M_auth` (`build_m_auth`; injective, big-endian):**
> `SENDER_AUTH_DOMAIN ‖ sig_codepoint(u16) ‖ envelope_codepoint(u16) ‖ lp_u32(sender_did) ‖ body_cid(36) ‖ lp_u32(audience_commitment) ‖ generation_count(u32) ‖ generations(u32 each) ‖ stanza_count(u32) ‖ body_aad_digest(32)`.
> - `audience_commitment`: `0x6510` = the recipient's OWN audience DID; `0x6520` = `audience_set_commitment` over the recipient's INDEPENDENT roster; `0x6610` = `audience_set_commitment` over the member roster derived from held set-state.
> - `generations` (F-3): `0x6510`/`0x6520` = `[recipient_key_generation]`; `0x6610` = `[member_key_generation, membership_set_generation, role_assignments_generation]` (explicit — the `body_aad_digest` does NOT cover them, closing the revoked-member cross-generation replay).
>
> **F-2 (SOUNDNESS-CRITICAL, as-built):** every open path re-derives the `audience_commitment` +
> the generations from the recipient's INDEPENDENTLY-held set-state (its own audience DID / roster /
> `K_Set` / held generations), NEVER the attacker-controllable wire value. `open_group_stanza`
> takes an `independent_roster` arg; `open_membership_set_group` takes a `GroupVerifyContext`
> (held roster + the three generations); `open_single` takes the recipient's own audience DID.
> **0x6520 independent-roster note (FLAG-FOR-BEN, see §6 below):** the 0x6520 recipient holds the
> roster independently (it knows the group composition); the impl passes it explicitly. No 0x6520
> flow was found that LACKS independent roster knowledge.
>
> Typed errors: `LayerCError::SenderOriginAuthFailed` + `group_posture::GroupError::SenderOriginAuthFailed`
> (layer-internal; §3.5g-baselined via `// drift-detect: internal-only`, NOT catalog-minted — no
> napi/wire ErrorCode boundary, verified). Goldens: wire-SHAPE + sign→verify round-trip, NEVER a
> fixed signature hex (the ML-DSA half is hedged/randomized — FLAG-6).
>
> ---
...
> **AS-BUILT branch:** `phase-4-meta-core/sealed-sender-auth-impl` (off
> `phase-4-meta-core/benten-id-hybrid-key @ 7db83398`).
...
3. **[AS-BUILT CORRECTION — F-1, supersedes the strikethrough below]** `sender_sig =
   SignatureSuite::v1_default().sign(sender_kp, M_auth).to_wire_bytes()` — the PLAIN `sign`
   (empty LAMPS ctx); domain separation is the `SENDER_AUTH_DOMAIN` PREFIX already inside
   `M_auth`. ~~`SignatureSuite::resolve_codepoint(sig_codepoint)?.sign_with_context(sender_kp,
   M_auth, ctx = SENDER_AUTH_DOMAIN)`~~ (the `with_context` form is WRONG — there is no such
   call). The default suite is `0x0001` (LAMPS hybrid); the call produces a real ML-DSA-65 half +
   a real Ed25519 half over the shared `M'`, no wire change to `sig.rs`. (Routing through
   `benten_crypto_suite::sig` per baked-in #5 — that module stays concat/framing glue only.)
...
4. **[AS-BUILT CORRECTION]** **Resolve `sender_did` → hybrid verifying key** via
   `benten_id::did::Did::parse_validated_hybrid(did_str)?.resolve_hybrid()?` (the hybrid resolver
...
5. Re-build `M_auth` from the recovered `sender_did` + the recipient's **INDEPENDENTLY-held**
   ...
   `K_Set` / held generations (NEVER the wire value), and reads `stanza_count` from the envelope.
6. **[AS-BUILT CORRECTION — F-1]** `SignatureSuite::v1_default().verify(vk, M_auth, &sig)` →
   **fail-closed** (typed error `SenderOriginAuthFailed`) on ANY mismatch. ~~`…verify_with_context(vk,
   M_auth, ctx, &sender_sig)`~~ (the `with_context` form is WRONG). `sig` is reconstructed from the
   ML-DSA-first wire via `HybridSignature::from_lamps_composite_wire(&sender_sig)`. For the hybrid
   suite this requires BOTH halves to verify (the `sig.rs` hybrid arm never returns `Ok` after a
   single half).
