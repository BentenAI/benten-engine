...
- **Wave B `w-gov-audit` (`aab091d3cb742f3e2`)** — 8 families: f_audit_1/2 (REAL Engine::audit_sequence() substrate arms — VERIFY green on pristine tier1 first, risk#2), f_audit_3/4 (RestrictedScope/2-arm-Scope + 12-primitive-compose), f_gov_1 (signed Node), f_inv19_1 (derive_kv reject-mutable-Anchor), f_nat_1 (zero-nature-field Inv-22), f_nat_2 (**doc arm STAYS #[ignore]'d** → TIER-3). MINT membership-set {audit,governance,privacy,keying_kv}[NEW] + member(+MemberNature); engine [dev-dependencies] line; un-ignore the 8 engine tests. **GREP-DEFENSE: audit/gov/nature are GRAPH data-half — NO codepoint consts, NO member_type/MemberKind** (else the grep-defenses false-trip). Branch `f-full-r5-gov-audit-b`.
