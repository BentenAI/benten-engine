...
- **Wave A `w-ms-sync` (`aaffa0670ff96c97b`)** — 7 families: f_crdt (proptest 10k), f_hlc_1_2, f_inv21 (**kani arm STAYS #[ignore]'d**, proptest floor), f_mst (real-drain F4-032/016/022), **f_gossip (MINT benten-sync/src/gossip_transport.rs; M-20 F4-024 ABSOLUTE topic golden-hex = truncate_32(blake3::keyed_hash(K_Set,"benten:setid:v1"||set_id||BE(gen))) + BE/LE differentiator)**, f_fed_1_2 (SubsetRef typed-reject 0x6620), f_ld_5 (nonce-cache wire + **#64 disable-cache negative-control**). Targets: membership-set {aad,keying[MAC-only],set} + benten-sync {gossip_transport[NEW],handshake,mst}. Branch `f-full-r5-ms-sync-b`.
