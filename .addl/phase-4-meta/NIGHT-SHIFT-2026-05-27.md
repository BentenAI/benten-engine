...
- **w-engine-layer-d: SUBSTANTIVELY STRONG, FIX-NEEDED (mechanical)** — 8 Layer-D families (f_ld_1/2/3/4/6/7/8 + f_va_3) / 42 tests GREEN + substantive (real Ed25519/ChaCha20-Poly1305/HPKE-X-Wing, 6-class pipeline, falsifiable coverage); **M-20 goldens independently byte-identical (no FLAG)**. BLOCKER = §3.5j clippy (format_collect in f_ld_2, struct_field_names in f_va_3) + **a PRODUCTION TEST-FEATURE LEAK** (Cargo.toml line-292 added `benten-crypto-suite{features=["testing"]}` to [dependencies] → cargo-unions → `testing` ON in the release cdylib). **Fix agent `a1225e2b109918b2f` in flight** (2 clippy + fold crypto-suite/testing into the `test-helpers` feature per convention).
