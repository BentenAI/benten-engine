...
- **Use the full CRUD surface.** `crud("post")` exposes five actions — `create`, `get`, `list`, `update`, `delete` — dispatched via `engine.call(handler.id, "post:<action>", input)`. The engine assigns `handler.id` as `crud:<label>` for `crud()`-registered handlers (see `Engine::register_crud` at `crates/benten-engine/src/engine.rs:1178-1188`). Capturing the returned handle and passing `handler.id` (as `src/index.ts` does) is the resilient pattern; future label-format changes won't break call sites. The action is the second argument.
