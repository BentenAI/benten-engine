...
- **IVM Algorithm B production registration** at `crates/benten-engine/src/engine_views.rs` was previously a `ContentListingView` fallback for every `Strategy::B`-declared user view (the view ran on the Phase-1 ContentListingView shape regardless of the spec's declared Strategy::B). Wave-8h wired the dispatch to construct `AlgorithmBView::for_id(spec.id())` for the 5 canonical view IDs that `AlgorithmBView` supports natively. **Phase 3 G15-A** generalized the kernel to handle arbitrary `(label_pattern, projection)` triples — non-canonical user-defined view IDs now run under `Strategy::B` with their actual label patterns rather than degrading to the Phase-1 view shape; the 5 hand-written views remain as inner kernels of `Strategy::B`. Documented in `docs/INVARIANT-COVERAGE.md` Algorithm B note. **Drift-detector landed at G15-B** as a proptest harness comparing Algorithm B incremental updates vs from-scratch full computation; verification pins live at `crates/benten-ivm/tests/algorithm_b_drift_detector.rs` (covering arbitrary label patterns, budget-trip state propagation, post-trip recovery, label-pattern extension observability, and the BudgetExceeded-vs-rebuild-succeeds asymmetric path).
