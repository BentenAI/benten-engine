...
- ESC defense matrix (16 named escape vectors per `pre-r1-security-deliverables.md` §1). The canonical numbering in the inventory + the test corpus at `crates/benten-eval/tests/sandbox_escape_attempts_denied.rs` is authoritative; this matrix uses the same numbering. The 16 vectors split into four buckets post-wave-8b/8h:
  ...
  | ESC-7 | Fuel-refill via host-fn re-entry | **Phase-3 G17-A1 wave-5b: defense surface fully landed.** Per-call `Store` lifecycle (D3-RESOLVED no-pool) + per-call `EscDefenseState` carrying `re_entry_count` + `guest_active` flag. `crates/benten-eval/src/sandbox/escape_defenses.rs::run_esc7_check` fires `SandboxError::EscapeAttempt(Esc7FuelRefillViaReEntry)` from the trampoline BEFORE the inner `add_fuel` takes effect. Routes through dedicated `E_SANDBOX_ESCAPE_ATTEMPT` catalog code per phase-3-backlog §6.1 + r1-wsa-1 BLOCKER closure + D-E (R1-revision triage). | Wired-defense + simulation pin green; full fixture-driven adversarial integration test un-ignores at G20-A1 wave-8a | `sandbox_esc_7.rs::esc_7_fuel_refill_via_host_fn_re_entry_blocked` + `..._traps_typed_error` — green at G17-A1 wave-5b |
  ...
  | ESC-9 | Cap-revoke mid-call (TOCTOU between cap-grant and cap-use) | **Phase-3 G17-A1 wave-5b: Fully wired** at the SURFACE level (helper SURFACE per §7.3.A.7). D18 PerCall live-recheck via `HostFnContext::live_cap_check` callback fires BEFORE EVERY host-fn invocation per r1-wsa-3 MAJOR (no caching window). The cadence is once-per-host-fn-entry (cadence (a) per r1-wsa-3 disposition + r4-r1-wsa-4 — within a single host-fn call, the recheck does NOT re-fire per loop iteration). G17-A2 wave-5b pairs the engine-side override that threads a real engine-backed callable through `SandboxStoreData`. Closes phase-3-backlog §6.3. | Wired-defense + simulation pin green at the §7.3.A.7 helper SURFACE level (`testing_revoke_cap_mid_call`); full fixture-driven adversarial integration test un-ignores at G20-A1 wave-8a | `sandbox_capability_check_per_call_after_revoke.rs::sandbox_host_fn_capability_revoked_mid_execution_denies_subsequent` + `sandbox_esc_9.rs::esc_9_live_cap_check_fires_at_every_host_fn_boundary_no_caching_window` + `..._within_kv_read_loop_consults_once_per_call_not_per_iteration` — green at G17-A1 wave-5b |
  ...
  | ESC-13 | Trap during fuel-meter callback / Store-state corruption | **Phase-3 G17-A1 wave-5b: defense surface fully landed.** Per-call `EscDefenseState.fuel_meter_callback_trapped` flag set by panic-catcher around the fuel-meter callback; `crates/benten-eval/src/sandbox/escape_defenses.rs::run_esc13_check` fires `SandboxError::EscapeAttempt(Esc13StorePoison)`. Pairs with D3-RESOLVED per-call `Store` lifecycle: a Store flagged poisoned is dropped + the next SANDBOX call gets a fresh Store (poison-recovery validated by `..._does_not_poison_store` recovery-path pin). Closes r1-wsa-1 BLOCKER + D-E. | Wired-defense + simulation pin green; full fixture-driven adversarial integration test un-ignores at G20-A1 wave-8a | `sandbox_esc_13.rs::esc_13_trap_during_fuel_meter_callback_store_poison_observable` + `..._does_not_poison_store` — green at G17-A1 wave-5b |
  ...
  | ESC-15 | Named-manifest spoofing (typo / non-existent name) | `manifest_ref.resolve(&registry)` returns `Unknown`; no permissive fallback → `SandboxManifestUnknown`. Phase-3 G17-C wave-5b adds the registration-time validation walk in `Engine::register_subgraph::validate_sandbox_manifest_names` so misspelled names trip at register time (operator-actionable) rather than at dispatch time as a confusing wallclock-after-zero-progress shape | Fully wired (registration-time + dispatch-time double defense) | `sandbox_escape_attempts_denied.rs:403` (`sandbox_escape_named_manifest_spoofing_rejected`) + `crates/benten-engine/tests/manifest_unknown.rs::register_subgraph_rejects_unresolved_sandbox_manifest_name_with_e_sandbox_manifest_unknown` |
  | ESC-16 | Wall-clock leak via `time` host-fn fingerprinting | **Phase-3 G17-A1 wave-5b: defense surface fully landed.** `time` host-fn returns monotonic-coarsened values (100 ms granularity) for the host-fn-level defense; the engine-side memory-read helper at `crates/benten-eval/src/sandbox/fingerprint.rs` (NEW per r1-wsa-4) tracks guest reads of host-written wallclock-correlated cells. `crates/benten-eval/src/sandbox/escape_defenses.rs::run_esc16_check` fires `SandboxError::EscapeAttempt(Esc16FingerprintCollapse)` at the next host-fn boundary when the read counter exceeds `FINGERPRINT_COLLAPSE_THRESHOLD` (3 reads-within-one-call) BEFORE the side-channel becomes guest-observable. Closes r1-wsa-4 + phase-3-backlog §6.1. | Wired-defense + simulation pin green; full fixture-driven adversarial integration test un-ignores at G20-A1 wave-8a | `sandbox_esc_16.rs::sandbox_esc_16_fingerprint_collapse_fires_via_committed_wat_fixture` + `..._below_threshold_silent` + `sandbox_host_fn_time.rs::sandbox_host_fn_time_returns_monotonic_coarsened_100ms` — green at G17-A1 wave-5b |

  **Bucket totals (16 vectors, each in exactly one bucket; updated at Phase-3 G17-A1 wave-5b close):** **Fully wired (8):** ESC-1, -2, -3, -4, -5, -6, -8, -15 — production runtime defense + integration test passing. *(Note: ESC-5 now routes through the dedicated `SandboxStackOverflow` typed variant + `E_SANDBOX_STACK_OVERFLOW` catalog code per phase-3-backlog §6.4 + r1-wsa-7 — the prior r6-wsa-8 BELONGS-NAMED-NOW Phase-3 deferral on the `SandboxStackExhausted`-rename catalog churn is retired here; the cascade through ~20 sites was delivered at G17-A1 wave-5b.)* **Wired-defense + simulation pin green (helper SURFACE) (4):** ESC-7, -9, -13, -16 — Phase-3 G17-A1 wave-5b lands the engine-side defense (`escape_defenses.rs::run_esc{7,13,16}_check` + `live_cap_check`-fires-at-every-host-fn-boundary cadence per r1-wsa-3) and the §7.3.A.7 helper SURFACE for simulation-level pins; the full fixture-driven adversarial integration tests un-ignore at G20-A1 wave-8a (each helper is gated behind `cfg(any(test, feature = "test-helpers", feature = "testing"))` per r1-wsa-6). **Partial (1):** ESC-14 (production manifest-authoritative defense structurally correct — embedded WASM custom sections silently ignored for cap purposes; integration test `#[ignore]`'d pending `testing_inject_forged_cap_claim_section` helper full body — G17-A1 ships the helper SURFACE; G20-A1 fills the body). **Wired-defense / test-paper-only (1):** ESC-10 — `AttributionFrame.sandbox_depth` runtime threading wired in `crates/benten-engine/src/primitive_host.rs::execute_sandbox` (R6FP-G1 / PR #62); the eval-side runtime arm in `crates/benten-eval/src/primitives/sandbox.rs::execute` fires `SandboxError::NestedDispatchDepthExceeded` once `attribution.sandbox_depth > config.max_nest_depth`; the adversarial integration test stays `#[ignore]`'d pending the `testing_call_engine_dispatch` helper SURFACE — G17-A1 ships the helper SURFACE; G20-A1 fills the body. **Component-model gated (2):** ESC-11, -12 — `#[cfg(feature = "component-model")]` + `#[ignore]`; the wasmtime workspace dep at `Cargo.toml:299` explicitly omits the feature. **Total:** 8 + 4 + 1 + 1 + 2 = 16 (no double-counting). The honest headline: **12 of 16 vectors fire typed-error defense end-to-end against the production executor at Phase-3 G17-A1 wave-5b close** (8 with full integration tests + 4 with simulation pins green at the helper-SURFACE level), with ESC-10 also defended at runtime but its adversarial integration test pending un-ignore at G20-A1, and ESC-14 partially covered. The remaining 2 (ESC-11, -12) are component-model feature-cut. Promotion from "8 fully wired + 3 paper-only ignored" (Phase-2b close) to "12 with simulation green + 1 fixture-driven helper-pending" (Phase-3 G17-A1 wave-5b) closes r1-wsa-1 BLOCKER (ESC-7+ESC-13) + r1-wsa-3 (ESC-9 cadence) + r1-wsa-4 (ESC-16) + r1-wsa-7 BLOCKER (ESC-5/stack-overflow catalog).
