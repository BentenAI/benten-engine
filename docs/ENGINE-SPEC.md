...
**Dependency note (history):** In Phase 1 the `core2` 0.4.0 crate was archived upstream on 2026-04-14 and yanked from crates.io; it was pulled in transitively via `cid` and `multihash`. Benten maintained a temporary `BentenAI/rust-cid` fork + a technocreatives `core2` pin as a spike-era workaround. Both upstream PRs merged and released as [`cid` 0.11.2](https://github.com/multiformats/rust-cid/pull/185) and [`multihash` 0.19.4](https://github.com/multiformats/rust-multihash/pull/407), swapping `core2` for [`no_std_io2`](https://crates.io/crates/no_std_io2). The workspace now consumes upstream crates.io versions directly — the `[patch.crates-io]` pins are removed as of Phase 2a pre-R5.
