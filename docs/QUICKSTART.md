...
- Inv-4 runtime depth-threading is fully active at Phase 2b close — both registration and runtime arms wired (R6FP-G1 / PR #62). `AttributionFrame.sandbox_depth` threads transitively through `ActiveCall` so SANDBOX → CALL → SANDBOX nesting fires `E_SANDBOX_NESTED_DISPATCH_DEPTH_EXCEEDED` at runtime. The ESC-10 adversarial integration test stays `#[ignore]`'d pending the `testing_call_engine_dispatch` host-fn helper per `docs/future/phase-3-backlog.md` §7.3.A.7 — the runtime defense is wired; only the adversarial-test driver is paper-only. See `docs/INVARIANT-COVERAGE.md` "Inv-4 + Inv-7 runtime arm status" for the wiring trace.
