...
| AT-4 | View-result publication forge (peer publishes a view result for a view they were not authorized to compute) | UCAN-gated `host:atrium:publish_view_result` cap (per Phase-3 D2 ratification; see CLAUDE.md "Phase 3 in flight" + D-PHASE-3-21). | `crates/benten-sync/tests/host_atrium_publish_view_result_caps.rs` |
