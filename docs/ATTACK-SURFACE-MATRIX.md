...
| ESC-10 | Re-entrancy via host-fn (cap-context confusion) | Wired-defense; adversarial pin LIVE but simulation-driven (un-ignored at Phase-3 G20-A1 wave-8a, widened at G21-T3; drives the `testing_call_engine_dispatch` helper's `EscDefenseState` rather than a real host-fn re-entry) | Compromise #4 ESC-10 row |
...
**Cross-reference.** `crates/benten-id/src/ucan.rs::validate_chain_inner` (pure crypto/structural chain-walk primitive — stays benten-id). Post-COLLAPSE-P2 CONSOLIDATE the policy-bearing seam composes with device-DID/envelope attestation (§2.4) at `benten_caps::chain_authority::validate_chain_with_envelope_ceiling` and DID rotation (§2.4) at `benten_caps::chain_authority::validate_chain_with_rotation_log` (moved from benten-id; the CONSOLIDATE line is *pure validation = benten-id; policy-bearing authority consultation = benten-caps*).
...
| # | Vector | Defense | Test pin |
|---|--------|---------|----------|
| SR-1 | Loro op-log violating Inv-13 immutability (re-write of a previously-committed op) | Inv-13 enforcement at dispatch (not just at CID-divergence): the inbound op is checked against the registered-subgraph/version-chain immutability rule at application layer. | `crates/benten-sync/tests/attack_loro_op_log_inv_13.rs::loro_doc_all_writes_surfaces_system_zone_prefixed_keys_for_dispatch_rejection` |
...
> **Post-COLLAPSE note (Compromise #23 SUPERSEDED-BY-COLLAPSE; #1238, 2026-05-16; DECISION-RECORD-trust-model-reframe.md §4, RATIFIED).** The standalone accept-time `Acceptor`/`DeviceRevocation` pipe these vectors were defended at was a *redundant parallel authority pipe* and is deleted. Defense is **not** removed — it is unified: provenance binding is retained as a signature on `DeviceAttestationEnvelope::verify`; the capability-envelope ceiling is AND-ed into the inbound writer's effective caps at the single unified chain-validation seam; the freshness-window anti-replay half is retained on that seam; the durable replay-marker re-home is tracked as the P2/P5 unified-ceiling deliverable (DECISION-RECORD §4b F3). Rows below are retensed to the post-COLLAPSE defense seam + surviving test pins; the deleted parallel-pipe behaviors are annotated rather than dropped (security posture is **strengthened by collapse** — fewer parallel pipes, no un-anchored revocation DoS).

**Concrete attack vectors:**

| # | Vector | Defense | Test pin |
|---|--------|---------|----------|
| DA-1 | Envelope-downgrade attack (re-issuing the attestation with a wider capability envelope without parent re-signing) | Runtime recheck against parent-chain at every UCAN delegation that consumes the attestation; widening fails at the parent-chain comparison. | `crates/benten-caps/tests/collapse_p2_consolidate_chain_authority.rs::device_attestation_capability_envelope_downgrade_attack_blocked_by_runtime_recheck_against_parent_chain` (moved from benten-id at COLLAPSE P2 CONSOLIDATE) |
| DA-2 | Parent-chain forgery (claiming a parent DID that did not actually issue the attestation) | Post-COLLAPSE: provenance-binding signature retained on the envelope; a forged parent-DID fails `DeviceAttestationEnvelope::verify` before the writer's caps reach the unified spine. | `tests/integration/atrium_two_device.rs::forged_device_did_rejected_at_envelope_verify` |
| DA-3 | Freshness-window replay (presenting an old valid attestation to bypass post-revocation state) | Post-COLLAPSE: the freshness-window anti-replay half is retained on the unified chain-validation seam; stale envelopes outside the window rejected. | `tests/integration/atrium_two_device.rs::replayed_stale_envelope_rejected_by_freshness_window` |
| DA-4 | Nonce-replay within the freshness window (presenting the same attestation twice within the window) | Post-COLLAPSE: the accept-time `Acceptor` nonce-store was deleted with the parallel pipe; the durable replay-marker re-home onto the unified seam is **LANDED at COLLAPSE P5** (`benten_caps::chain_authority::FrameReplayMarker` — the same durable-KV grammar `UCANBackend::revoke`/`is_revoked` use; consumed once-per-merge at `Engine::apply_atrium_merge`, keyed by the verified frame's per-envelope `session_nonce`). A frame whose session-nonce was already observed is rejected even *within* the freshness window — the gap the ephemeral `DeviceAttestationEnvelope::verify` step-(3) freshness gate (DA-3) could not close. | `crates/benten-caps/tests/collapse_p5_unified_ceiling.rs::collapse_p5_f3_durable_replay_marker_rejects_replayed_frame` (COLLAPSE P5; DECISION-RECORD §4b F3 closed) |
| DA-5 | Revoked device signing new UCAN delegation | Post-COLLAPSE: there is no separate device-revocation pipe to forge into (the #1230 perpetual-DoS is dissolved by deletion); a device that exceeds its envelope authority fails the unified user-root-anchored UCAN chain-walk at the ceiling-AND. | `crates/benten-id/tests/device_attestation.rs::device_attestation_widening_parent_authority_is_rejected` |
...
| DA-8 | Device-signed re-attestation widening parent's capability envelope | Post-COLLAPSE: re-attestation cannot widen — the parent envelope is the upper bound, AND-ed at the single unified chain-validation seam. **COLLAPSE P5 LANDED the plugin-manifest unification**: `benten_caps::chain_authority::validate_chain_with_manifest_ceiling` is a *second caller* of the SAME `envelope_ceiling_rejects_cap` predicate the device caller uses (build-constraint iii — ONE code path, two callers, NOT a parallel pipe), so a plugin whose signed manifest never declared `host:sandbox:*` is enforced through the identical core as a `runs_sandbox=false` device. A device-signed re-attestation that widens against the parent envelope fails the runtime recheck against the parent chain. | `crates/benten-caps/tests/collapse_p2_consolidate_chain_authority.rs::device_attestation_capability_envelope_downgrade_attack_blocked_by_runtime_recheck_against_parent_chain` (moved from benten-id at COLLAPSE P2) + `crates/benten-caps/tests/collapse_p5_unified_ceiling.rs::collapse_p5_manifest_ceiling_rejects_sandbox_via_shared_predicate` (COLLAPSE P5 #669 unification) |
| DA-9 | DID rotation widening authority post-rotation | Rotation log at chain-walk: the post-rotation DID inherits only the pre-rotation authority envelope; widening rejected. | `crates/benten-caps/tests/collapse_p2_consolidate_chain_authority.rs::superseded_did_cannot_sign_new_ucan_delegations` (moved from benten-id at COLLAPSE P2 CONSOLIDATE) |
| DA-10 | Thin (`runs_sandbox=false`) principal self-delegating `host:sandbox:*` then writing to an ordinary data zone (the F2 shape-not-substance gap: the engine-side per-row ceiling-AND discriminated on the synthetic `{zone}:write` scope, which can never trip the `host:sandbox:` prefix) | **COLLAPSE P5 #1241 cap-predicate completion (DECISION-RECORD §4b):** the cap-predicate is completed so the ONE shared `envelope_ceiling_rejects_cap` core is fed the writer's **actual delegated `cap.resource`** (`benten_caps::envelope_ceiling_first_rejected_resource` + the engine sibling `manifest_envelope_recheck::envelope_ceiling_admits_cap_resources`). Per §4b the inbound-merge surface ships (a) zone-scoped for v1 (gap verified *INERT* at HEAD — `F2-exploitability-investigation.md`: the data-zone write path never dispatches a SANDBOX host-fn); the completion is the predicate (now complete + shared), the wire-chain threading is the §4b-deferred mechanical residual. | `crates/benten-caps/tests/collapse_p5_unified_ceiling.rs::collapse_p5_f2_cap_predicate_rejects_real_sandbox_resource_from_thin_principal` (COLLAPSE P5; #1241 cap-predicate completion) |

**Cross-reference.** `crates/benten-id/src/device_attestation.rs` (attestation/envelope-ceiling *primitive* types — stay benten-id), `crates/benten-id/src/did_rotation.rs` (rotation log primitive — stays benten-id), `benten_caps::chain_authority::validate_chain_with_envelope_ceiling` (post-COLLAPSE-P2 policy-bearing chain-walk seam consuming attestations; was `benten-id::ucan::validate_chain_with_attestations`), `benten_caps::chain_authority::validate_chain_with_rotation_log` (policy-bearing chain-walk seam consuming the rotation log; moved from benten-id). **COLLAPSE P5 (DA-8/DA-10 + DA-4):** `benten_caps::chain_authority::validate_chain_with_manifest_ceiling` + `manifest_to_envelope_ceiling` (the #669 plugin-manifest ceiling, a *second caller* of the ONE `envelope_ceiling_rejects_cap` predicate — build-constraint iii), `benten_caps::chain_authority::envelope_ceiling_first_rejected_resource` (the #1241/F2 cap.resource-complete predicate) + its engine sibling `benten_engine::manifest_envelope_recheck::envelope_ceiling_admits_cap_resources`, and `benten_caps::chain_authority::FrameReplayMarker` (the F3 durable replay-marker re-home, consumed at `Engine::apply_atrium_merge`). The browser-target heterogeneity contract (CLAUDE.md baked-in #17 thin compute surface) feeds DA-6 / DA-7 / DA-8.
...
*Part 3 added at Phase-4-Foundation R6-FP-G pre-tag retense sweep (G26-A); status `FINAL` for Phase-4-Foundation-close. Phase-4-Meta + Phase 5+ phase-close cycles re-issue with new rows for any phase-N-introduced surface.*
