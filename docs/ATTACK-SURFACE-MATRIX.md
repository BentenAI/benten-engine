...
| DA-4 | Nonce-replay within the freshness window (presenting the same attestation twice within the window) | Post-COLLAPSE: the accept-time `Acceptor` nonce-store was deleted with the parallel pipe; the durable replay-marker re-home onto the unified seam is tracked as the P2/P5 unified-ceiling deliverable (DECISION-RECORD §4b F3). Freshness-window (DA-3) bounds the exposure at HEAD. | Tracked P2/P5 — see DECISION-RECORD §4b F3 (no live pin at HEAD; DA-3 freshness-window pin bounds exposure). |
...
| DA-8 | Device-signed re-attestation widening parent's capability envelope | Post-COLLAPSE: re-attestation cannot widen — the parent envelope is the upper bound, AND-ed at the single unified chain-validation seam (structurally unified with the plugin-manifest envelope-ceiling per #669). A device-signed re-attestation that widens against the parent envelope fails the runtime recheck against the parent chain. | `crates/benten-caps/tests/collapse_p2_consolidate_chain_authority.rs::device_attestation_capability_envelope_downgrade_attack_blocked_by_runtime_recheck_against_parent_chain` (moved from benten-id at COLLAPSE P2 CONSOLIDATE) |
