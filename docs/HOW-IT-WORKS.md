...
Operational observability lives next to it: `engine.metrics_snapshot()` returns a `BTreeMap<String, f64>` of flat counter / gauge metrics covering write totals (committed / denied, with per-capability-scope fan-out), change-stream dropped events, IVM view-stale counts, per-handler SANDBOX fuel / output high-water plus last-invocation wall-clock, SUBSCRIBE on-change registration counts, EMIT subscriber counts, the sync-replica per-row cap-recheck call count, and STREAM active-stream counts. The map shape is intentionally flat so it composes cleanly with Prometheus / OpenTelemetry exporters built on top by an operator-side plugin.
...
**Phase 4-Foundation (closed at tag `phase-4-foundation-close`, 2026-05-14)** shipped the substantive Benten Platform engineering — the layer that makes the engine end-to-end usable through a UI rather than only through the napi/TS surface. It added the admin UI v0 (the first plugin, default module, and Foundation entrypoint) on a 4-category navigation IA (Plugins / Workflows / Content Types / Views), the full plugin manifest format with install-time consent + per-plugin DID minting + manifest envelope chain validation + private-namespace caps + DAG-shape versioning, the decentralized self-discovered registry on top of Atriums, the schema-driven rendering pipeline (schemas as subgraphs of typed-field Nodes — 8-label vocabulary `FieldScalar` / `FieldEnum` / `FieldUnion` / `FieldList` / `FieldMap` / `FieldObject` / `FieldRef` / `SchemaRoot`), the materializer pipeline (composed via IVM-subgraph generalization), and the module ecosystem tooling at scale. The workspace grew from ten to twelve crates at Phase-4-Foundation close with `benten-platform-foundation` (11th — schema-rendering compiler + materializer + plugin manifest + admin UI v0 + `Renderer` trait abstraction; the v1 platform-shippable surface) and `benten-renderer-tauri` (12th — Tauri 2.x renderer engine extension per CLAUDE.md baked-in #19; embedded-webview deployment shape (c) on top of the same wasm32 bundle as the browser tab), then grew to fifteen during Phase-4-Meta-Core with `benten-crypto-suite` (13th — the ONE crypto-primitive integration crate), `benten-drop` (14th — Drop bundle format per RATIFIED-S&C §R6), and `benten-membership-set` (15th — the MembershipSet keying primitive). Three deployment shapes are first-class: (a) **full peer** — native Rust on user hardware, full Atrium sync participation; (b) **thin compute surface** — wasm32 in a browser tab or edge worker, stateless reads against snapshot data + writes via fetch to a full peer; (c) **embedded webview** — native shell wraps the same wasm32 bundle, talks to its embedded full peer via in-process IPC. **Phase 4-Meta** then layers self-composing admin meta-circular work + Phase-3-deferred items + the v1-assessment-window on top; the `v1` tag follows. Phase 4-Meta is split into Phase 4-Meta-Core (the v1-gate crypto/identity/multi-tenant substrate — PQ-hybrid Ed25519⊕ML-DSA-65 signature default, codepoint-dispatched crypto-agility seam, encryption-as-confidentiality, storage-partition seam — and the v1 public-interface freeze; **adds the 13th + 14th + 15th workspace crates `benten-crypto-suite` + `benten-drop` + `benten-membership-set`** — the ONE thin Benten-owned crypto-primitive integration crate (per CLAUDE.md baked-in #5; the only crypto-primitive call site), the offline Drop bundle format per RATIFIED-S&C §R6, and the MembershipSet keying primitive respectively; ships the full G-CORE-3 family Sharing & Confidentiality surface — the SubgraphSpec primitive + RestrictedScope + structured UCAN Scope + AuthorizationGrant envelope + two-CID iroh-blobs seam + Drop bundle format + walker-as-Subgraph fractal pin) and Phase 4-Meta-Composing (the self-composing admin layered on the frozen v1 surface).

**Phase 4-Meta-Core (at phase-close)** lifted the v1-gate substrate that the Principal primitive's confidentiality half depends on (per CLAUDE.md baked-in #18). The **storage-partition seam** (issue #989) landed as the wave canary: `WriteContext::namespace_did: Option<Cid>` carries a per-DID storage scope across the `GraphBackend::put_node_with_context` surface, and `RedbBackend::scoped(did)` returns a per-DID view that structurally confines reads to that partition's keyspace. The C1 invariant — **a write under DID-A is not visible under DID-B** — holds end-to-end across the storage layer and across the post-commit change-subscriber fan-out. `BrowserBackend` (the thin-client cache for deployment shapes (b)/(c) per baked-in #17) does not implement an in-RAM partition; it fails CLOSED on `Some(namespace_did)` with `GraphError::NamespacedWriteUnsupported` rather than silently dropping the scope. The shape landed in Phase 4-Meta-Core (before the v1-API freeze) precisely because it is a public `WriteContext` change — adding it post-`v1` would be SemVer-breaking.

On that seam Phase 4-Meta-Core built the whole **Sharing & Confidentiality (S&C)** encryption substrate. **Layer-C encrypt-to-recipient** ships the Benten-canonical X25519⊕ML-KEM-768 X-Wing KEM-DEM at codepoint `0x647a` (the seal/open SURFACE lives in `benten-drop`'s `layer_c` module; the crypto PRIMITIVE — the strip-resistant X-Wing combiner (both shared secrets are mixed into the KEK, so neither KEM half can be stripped; full AEAD key-commitment in the robustness sense is OUT OF SCOPE at v1-beta per Inv-17 / Compromise #30) — lives in `benten-crypto-suite`, the 13th crate, the ONE thin Benten-owned crypto-primitive integration crate per baked-in #5; RFC-9180-cross-stack-interop HPKE framing is a future-additive NQ-C1 direction, not what ships) with **Sealed-Sender** framing (the sender DID is sealed inside each stanza, never plaintext on the wire) and multi-stanza group sends (the `0x6520` fresh-random-CEK band + the `0x6610` MembershipSet K_Set-derived band). The offline **Drop bundle format** (`benten-drop`, the 14th crate — the full S&C composition in CBOR-on-disk) and the **MembershipSet keying primitive** (`benten-membership-set`, the 15th crate — engine `[dev-dependencies]`-only at v1-beta; production consumption is Phase-4-Meta-Composing) round out the surface, and the **layer_d** remote-permission substrate carries the multi-device-key-wrap / DAK path. Per-Node AEAD wrap seals each partition's Nodes.

**Confidentiality-half honesty (Compromise #65).** The **confidentiality half** of the Principal primitive — per-principal encryption of the storage partition against a malicious *storage host* — is **honestly DEFERRED, NOT built at v1-beta**. At this wave the per-Node AEAD key derives from a *publicly-known* domain constant + the *publicly-known* `namespace_did`, so `K_principal` is **publicly derivable**: it is a **stand-in** that keeps the substrate shape stable for the production `K_principal`-store swap-in, not real untrusted-host confidentiality. The LIVE protection is the **AUTHORITY half** (capability + namespace isolation) which binds only a *cooperating* engine, plus the local device's Argon2id-DAK-sealed vault. Real per-DID confidentiality against an untrusted host is the **#1301 / D-64** encryption substrate — it swaps only the `K_principal` synthesis step (the AEAD-wrap layer + per-chunk size are stable) and **closes Compromise #65 when it lands**. Capability-gating binds only a cooperating engine, so a decentralized personal-data platform whose vision (Phases 7-8 untrusted-host / peers-hold-ciphertext / Kith selective-disclosure) requires per-principal confidentiality on hardware *other* principals control needs that encryption sealing each per-DID partition. Storage-partition (authority half, here) + per-partition encryption (confidentiality half, #1301/D-64) together compose the full multi-tenant substrate.
...
- **Understand the architecture.** [`ARCHITECTURE.md`](ARCHITECTURE.md) walks the fifteen crates (eight foundational + `benten-id` + `benten-sync` + `benten-platform-foundation` + `benten-renderer-tauri` + `benten-crypto-suite` + `benten-drop` + `benten-membership-set`), the invariant set, the storage layer, and the evaluator's request flow.
