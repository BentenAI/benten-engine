...
- **`KVBackend` async-shape fork** — Path A (lock sync at v1 + `AsyncKVBackend` post-v1) vs. Path B (convert to RPITIT pre-v1). See §4.62. Couples to `NetworkFetchStubBackend` retirement (hyg-1 #305 Ben-call).
- **`GraphBackend::snapshot()` + `register_subscriber()` Result-shape forks** — see §4.60 (SemVer-commitment docstring already landed; flip-vs-freeze decision pending). Couples safe-1 #501.
- **`BlobBackend` additive-default vs. sub-trait-split** — see §4.61 (additive defaults already landed; lock-vs-split decision pending).
...
### §4.59 `GraphBackend::Transaction` `run<F, R>` composability surface (Phase-4-Meta — Surf-1 #836)
...
### §4.60 `GraphBackend` umbrella SemVer-locking shape forks (Phase-4-Meta — Fwd-2 #1022 + safe-1 #501)
...
### §4.62 `KVBackend` sync-only vs. RPITIT-async v1-stabilization fork (Phase-4-Meta — Surf-1 #855 + Fwd-2 #986)
...
### §4.63 Light-client mode-(b) range-query-proof + mode-(c) signed-checkpoint trait-destination decision (Phase-4-Meta — Fwd-2 #1004)
