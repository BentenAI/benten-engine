...
1. **`Engine::get_node`** at `crates/benten-engine/src/engine_crud.rs::get_node` — `pub`; primary un-attributed read pathway (was originally intended `pub(crate)` per #18). <!-- cite-drift-exempt: pre-G-CORE-9-§8-A historical record; symbol renamed to `read_node` + tightened to `pub(crate)` per RATIFIED Path (a) per STATUS line below -->
2. **`Engine::put_node`** at `crates/benten-engine/src/engine_wait.rs::put_node` — `pub`; doc-comment says 'plugin authors never call this directly'; bypasses `CapabilityPolicy::check_write` so an external caller using this directly escapes the Inv-13 firing matrix. <!-- cite-drift-exempt: pre-G-CORE-9-§8-A historical record; tightened to `pub(crate)` per RATIFIED Path (a) per STATUS line below -->
3. **`Engine::get_node_label_only`** at `crates/benten-engine/src/engine_wait.rs::get_node_label_only` — `pub`; un-attributed read of label-only (lower-bandwidth than full Node but same attribution-bypass shape). <!-- cite-drift-exempt: pre-G-CORE-9-§8-A historical record; tightened to `pub(crate)` per RATIFIED Path (a) per STATUS line below -->
4. **`Engine::resolve_subgraph_cid_for_test`** at `crates/benten-engine/src/engine_wait.rs::resolve_subgraph_cid_for_test` — `pub`; `_for_test` suffix but production-consumed (DX hazard; same shape as `SubgraphBuilder::set_property_for_test` per R6-R4 cag-r6-r4-2). <!-- cite-drift-exempt: pre-G-CORE-9-§8-A historical record; `_for_test` suffix dropped + tightened to `pub(crate)` + relocated to testing module per RATIFIED Path (a) per STATUS line below -->
