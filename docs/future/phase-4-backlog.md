...
- **G-CORE-3e — sync + iroh-blobs UCAN-gating** (depends on 3a + 3b + 3w + 3d; ~1000-1200 LOC across `benten-sync` + `benten-caps`). **Includes the K(N) production-source swap-in** (per HARD RULE 12 clause-(b) BELONGS-NAMED-NOW): the G-CORE-3d landing uses a `Node::derive_key_for_test`-style BLAKE3-of-plaintext-CID test seam as the K(N) source (mirrored at `crates/benten-graph/src/redb_backend.rs::derive_test_seam_key_from_cid`). The production K(N) (per Spike-E Interpretation-B path-tagged derivation, rooted at `K_principal`) requires the `K_principal`-per-DID seam that this wave introduces; the swap-in is a 1-call-site replacement at the put-path + 1-call-site replacement at the read-path — both already point at the same helper function. The AAD-binds-plaintext-CID rebinding-attack defense + the two-CID mapping + per-chunk-AEAD + chunk-size = `IROH_BLOCK_SIZE` are ALL ALREADY LIVE at G-CORE-3d; only the K-source upgrade is named-deferred. NOT a fail-OPEN — namespaced writes ARE AEAD-wrapped post-G-CORE-3d landing; the production K-source upgrade is a swap-in at the same boundary.
