...
1. **`Engine::get_node`** (was at `crates/benten-engine/src/engine_crud.rs`, since renamed) — `pub`; primary un-attributed read pathway (was originally intended `pub(crate)` per #18). _Historical name; per the STATUS line below the symbol was renamed to `read_node` + tightened to `pub(crate)` per RATIFIED Path (a)._
2. **`Engine::put_node`** (was at `crates/benten-engine/src/engine_wait.rs`, since tightened) — `pub`; doc-comment says 'plugin authors never call this directly'; bypasses `CapabilityPolicy::check_write` so an external caller using this directly escapes the Inv-13 firing matrix. _Historical name; per STATUS line below, tightened to `pub(crate)` per RATIFIED Path (a)._
3. **`Engine::get_node_label_only`** (was at `crates/benten-engine/src/engine_wait.rs`, since tightened) — `pub`; un-attributed read of label-only (lower-bandwidth than full Node but same attribution-bypass shape). _Historical name; per STATUS line below, tightened to `pub(crate)` per RATIFIED Path (a)._
4. **`Engine::resolve_subgraph_cid_for_test`** (was at `crates/benten-engine/src/engine_wait.rs`, since refactored) — `pub`; `_for_test` suffix but production-consumed (DX hazard; same shape as `SubgraphBuilder::set_property_for_test` per R6-R4 cag-r6-r4-2). _Historical name; per STATUS line below, `_for_test` suffix dropped + tightened to `pub(crate)` + relocated to testing module per RATIFIED Path (a)._
