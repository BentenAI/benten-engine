...
### §8-B-merkle-range-proof-consumer-wire-up — Post-G-CORE-6b carry (BELONGS-NAMED-NOW destination)

**Carry from G-CORE-6b R5 (Phase-4-Meta-Core, 2026-05-23).** G-CORE-6b landed the `SnapshotBlob.schema_version 1→2` P-III bump autonomously (Ben-authorized per no-users-yet) + added `merkle_root: Option<Cid>` as the §8-B mode-(b) `MerkleRangeProof` hook. The CONSUMER side — the `MerkleRangeProofBackend` trait + the engine `export_snapshot_blob` path that populates `merkle_root` from a source-side MST root + the light-client mode-(b) range-proof verifier — is NOT yet shipped (the brief explicitly authorized a SCAFFOLDING decision: add the v2 field with the `Option<Cid>` sentinel + leave the consumer integration as this named carry). HARD RULE 12 clause-(b): the destination receives the row NOW (not "I'll add it later"). Scope: define `MerkleRangeProofBackend` trait in `crates/benten-sync/src/light_client.rs` (joins the existing `LightClient` + MST machinery); thread MST root population through `crates/benten-engine/src/engine_snapshot.rs::export_snapshot_blob` (writes `Some(mst_root)` instead of `None` when the source has an MST); add light-client mode-(b) verifier consumes `SnapshotBlob::merkle_root` for per-key inclusion proofs. Likely also mints `E_SNAPSHOT_BLOB_MERKLE_ROOT_MISMATCH` ErrorCode at that wave (range-proof verification failure surface). Wave home: §8-B benten-sync work or G-CORE-9 (the v1-interface freeze gate — at the latest the consumer must land before freezing so the field is meaningful in the v1 wire format). Source-comments at `crates/benten-graph/src/backends/snapshot_blob.rs` (`SnapshotBlob::merkle_root` doc-comment) + `crates/benten-engine/src/engine_snapshot.rs:161-170` (the `merkle_root: None` site) cite this row.
...
- **G-CORE-3e — sync + iroh-blobs UCAN-gating** (depends on 3a + 3b + 3w + 3d; ~600-800 LOC across `benten-sync` + `benten-caps` + `benten-graph` + `benten-errors` + `benten-id` test helpers). **SHIPPED 2026-05-23 (Flavor B per-request UCAN check; six-arm validation pipeline; Spike A2 zero-conversion plumbing; HKDF-SHA256 K(N) production swap landed).** Named carry-overs for future waves (HARD RULE 12 clause-(b) BELONGS-NAMED-NOW):
  - **K_principal-per-DID secret-material backend** (carry from this wave). The G-CORE-3e production HKDF-SHA256 K(N) derivation at `crates/benten-graph/src/redb_backend.rs::derive_test_seam_key_from_cid_with_namespace` synthesizes K_principal deterministically from the namespace_did via BLAKE3 keyed-hash over a stable domain tag. The real per-deployment K_principal secret material (#989 / #1301 substrate; per-DID secret store) is a 1-line swap at the helper's K_principal synthesis step — same `(namespace_did, plaintext_cid)` signature. NOT a fail-OPEN: AEAD-wrap is in place at this wave; only the K_principal *source* is wave-deferred to the K_principal-store landing. See the "Confidentiality limit at this wave" callout in `docs/SECURITY-POSTURE.md` (G-CORE-3e key-derivation section) for the explicit wave-state limit ((`namespace_did`, ciphertext_blob) is sufficient to decrypt at this wave).
  - **iroh-blobs `FsStore` bytes-serving wire-up** (carry from this wave). The wave-3e `TwoCidStore::ciphertext_bytes` in-memory `BTreeMap` is the named swap-point for the real `iroh_blobs::FsStore`; the wave-3e `UcanBlobsHandler::serve_request_for_test` test seam is the named swap-point for the real `iroh_blobs::provider::handle_connection` call. Landing iroh-blobs as a workspace dependency is OUT-OF-SCOPE for G-CORE-3e because the brief explicitly named the bytes-serving plumbing as "reuses upstream" and conflating the per-request-UCAN-check arm with the upstream bytes-serving plumbing would have grown wave scope past the ~600-800 LOC budget. The per-request UCAN validation + scope check + identity-cast plumbing are ALL LIVE end-to-end against the test seam at this wave; the iroh-blobs integration is a clean swap at named boundaries.
  - **IVM-cache-invalidation seam at the per-request validation pipeline** (carry from this wave; mini-rev MR3). The wave-3e handler re-runs the full 6-arm validation pipeline on EVERY request (eval-every-time per R5 live-per-request resolver — acceptable for v1-beta scale). A future performance wave wires this through G-CORE-4 CanonicalViews subscription so capability-graph state changes invalidate any cached per-grant validation cache. Name + boundary: `validate_request_for_connection` in `crates/benten-sync/src/ucan_blobs_protocol.rs`. Per `.addl/phase-4-meta/RATIFIED-sharing-and-confidentiality-2026-05-21.md` §R5 ("eval happens at request time against current graph state; IVM-cache-invalidation seam reusing G-CORE-4 CanonicalViews subscription").
  - **G-CORE-3e end-to-end composition pin** (carry from this wave; mini-rev MR4). The wave-3e R3 corpus has 11 per-arm pins but NO end-to-end pin driving "Alice puts ciphertext → grants Bob → Bob's request → handler validates + resolves via TwoCidStore + returns bytes". The production wire-up (G-CORE-3e-followon iroh-blobs `serve_request` swap to `iroh_blobs::provider::handle_connection` per Spike A2 pattern) lands the real composition; this end-to-end pin lands with that swap. Name + boundary: new `tf3e_end_to_end_share_path` test colocated with the iroh-blobs swap wave; until then, the wave-3e thin pin (`tf3e_dispatch_to_iroh_blobs_only_after_positive_validation`) covers the validation-then-dispatch SHAPE.
  - **`UcanEnvelope::unresolved_peer` field removal P-III decision** (carry from this wave; mini-rev MR6). Post-G-CORE-3e production wire-up where `RotationLog` replaces the in-grant sentinel, the field has no production purpose. Either remove from the `UcanEnvelope` wire shape before G-CORE-9 v1-public-interface freeze OR keep + document why it stays. Both options are valid: keep-it preserves a typed sentinel for explicit unresolved-peer testing; remove-it shrinks the v1-beta wire shape. **P-III Ben decision-point at G-CORE-9 freeze time.**
  - **`AuthorizationGrant::issue_with_unresolved_peer_for_test` re-sign discipline** (carry from this wave; mini-rev MR7, OBS-severity). The wave-3e fixture mutates `ucan.unresolved_peer` post-sign so the returned grant has an invalid `binding_sig`. Per current ARM ordering (ARM 1 unresolved-peer fires before ARM 5 binding-sig), the test passes — but is brittle to future ARM-reorders. Two hardening options: (a) re-sign post-mutation so the fixture is `binding_sig`-valid (proper fix; the fixture would then differ from a production grant ONLY by the `unresolved_peer` flag); (b) the tests are already discriminator-asserting (`matches!(..., UnresolvedDeny)`) — landed safety net. Land (a) opportunistically; record (b) as the safety net.
...
> 🏠 **PLAN HOME (2026-05-19, R1-triage recon-r1-1 — this row was DROPPED from the R0.2 plan and is now homed):** `.addl/phase-4-meta/00-implementation-plan.md` §0 provenance row + **G-CORE-4** (the vocabulary-edge / materializer-recursive-walk surface; single home, NOT OR-shaped). The still-`#[ignore]`d proptest un-ignore is a §3.6e RED-PHASE staged-pin obligation in the G-CORE-4 brief (reviewer verifies landing-status). The 8-`LabelType` vocab-fixture coverage addendum below lands with this row.
...
> 🏠 **PLAN HOME (2026-05-19, R1-triage recon-r1-2 — §4.19(a) was DROPPED from R0.2 and is now homed):** `.addl/phase-4-meta/00-implementation-plan.md` §0 provenance rows + **§4.19(a) cross-peer `accept_atrium_share` install seam → G-CORE-8** (plugin-trust Layer-2/3 runtime; the sync-layer entry re-verifies `bytes_cid == announced_cid` AND `peer_did_signature_valid_for_bytes`; the 3 stranded ignored cross-peer pins un-ignored per §3.6e; extends exit criterion C8). **§4.19(b) schema-author trust-list user-prompt → G-CORE-7** (install/lifecycle hardening; extends C7). §4.25 atrium-share-at-sync does NOT subsume the cross-peer install pipeline — they land together at G-CORE-8.
...
1. **`Engine::get_node`** — originally `pub` at `engine_crud.rs::get_node`; primary un-attributed read pathway (was originally intended `pub(crate)` per #18). **POST-§8-A:** renamed + tightened to `pub(crate) fn read_node` at `crates/benten-engine/src/engine_crud.rs::read_node`; historical `get_node` spelling preserved as test-helper re-export at `crates/benten-engine/src/testing.rs::get_node` behind `cfg(any(test, feature = "test-helpers"))`.
2. **`Engine::put_node`** — originally `pub`; doc-comment said 'plugin authors never call this directly'; bypasses `CapabilityPolicy::check_write` so an external caller using this directly escapes the Inv-13 firing matrix. **POST-§8-A:** renamed + tightened to `pub(crate) fn put_node_inner` at `crates/benten-engine/src/engine_wait.rs::put_node_inner`; historical `put_node` spelling preserved as test-helper re-export at `crates/benten-engine/src/testing.rs::put_node`.
3. **`Engine::get_node_label_only`** — originally `pub`; un-attributed read of label-only (lower-bandwidth than full Node but same attribution-bypass shape). **POST-§8-A:** renamed + tightened to `pub(crate) fn read_node_label_only`; historical `get_node_label_only` spelling preserved as test-helper re-export at `crates/benten-engine/src/testing.rs::get_node_label_only`.
4. **`Engine::resolve_subgraph_cid_for_test`** — originally `pub`; `_for_test` suffix but production-consumed (DX hazard; same shape as `SubgraphBuilder::set_property_for_test` per R6-R4 cag-r6-r4-2). **POST-§8-A:** renamed + tightened to `pub(crate) fn resolve_subgraph_cid_inner` at `crates/benten-engine/src/engine_wait.rs::resolve_subgraph_cid_inner`; `_for_test` suffix DELETED from public surface; the historical name is available behind the `test-helpers` feature gate.
...
Path (a) is the bake-in-intent path; path (b) preserves the shipped surface. v1-API-stabilization decision. **~50-200 LOC depending on path** (widened scope from R6-R3's ~20-100 LOC single-function estimate). `[Sibling carry to §4.19 (orchestrator's earlier brief mis-cited that destination) — MIS-CITE NOTED; the correct sibling is §4.50 _for_test cleanup + the §4.43 v1-API-stabilization sweep itself, NOT §4.19 (which is the cross-peer accept_atrium_share install seam, an unrelated surface).]`

> ✅ **STATUS — §8-A DECIDED (a) TIGHTEN (Ben 2026-05-18, `RATIFIED-prework-forks-2026-05-18.md`).** Path (a) is the ratified decision: `Engine::get_node`/`put_node`/`get_node_label_only`/`resolve_subgraph_cid_for_test` → `pub(crate)` + rename + drop `_for_test`, applied atomically at the G-CORE-9 v1-API freeze (napi verified non-broken). Path (b) is NOT taken. Plan cross-ref: `.addl/phase-4-meta/00-implementation-plan.md` §1.A.FROZEN item 1 + the G-CORE-9 group def + §8-A. The COUPLED §4.69 cap-mutation surface organization is a DISTINCT row, RESOLVED at the code level (option (a) already shipped — see the §4.69 STATUS UPDATE); it is not the §4.43 visibility cluster.
...
> 🏠 **PLAN HOME (2026-05-19, R1-triage recon-r1-3 — this row was §0-bucket-mapped in R0.2 but FLOATED with no §3 group home; now homed):** `.addl/phase-4-meta/00-implementation-plan.md` §0 provenance row + **G-CORE-8** (same `benten-sync` attack-fixture surface as §4.25; added to the G-CORE-8 group body explicitly). Exit criterion C8 extended: the 15-of-18 missing sync-attack vectors land OR carry a HARD-RULE-12 disposition.
...
**G-CORE-9 V1-FROZEN-INTERFACE row 5 verify-or-defer outcome (2026-05-23):** Confirmed `MerkleRangeProofBackend` trait does NOT exist at HEAD (only narrative references in `crates/benten-graph/src/backends/snapshot_blob.rs:17,24,29,154` + `crates/benten-graph/tests/tf11_snapshot_blob_schema_version_p3_migration_path.rs:13,39,166` + `crates/benten-engine/src/engine_snapshot.rs:169` placeholder). **Decision: Option A (DEFER to G-COMP-1 §8-B-in-`benten-sync`)** per Planner-B's reasoning + the orchestrator triage default (per HARD RULE 12 clause-(b) BELONGS-NAMED-NOW: this §4.64 row IS the named destination + V1-FROZEN-INTERFACE.md item 3 §4.64 narrative explicitly states "BELONGS-NAMED-NOW G-COMP-1 §8-B-in-`benten-sync`"). Rationale: freezing a phantom shape is overcommit; the trait shape requires R0 design pre-work that should go through its own mini-pipeline at G-COMP-1 (preferred Option (b) above lands the trait in `benten-sync`). V1-FROZEN-INTERFACE.md item 3 row for `MerkleRangeProofBackend` is updated to "(DEFERRED to G-COMP-1; see [phase-4-backlog.md §4.64](docs/future/phase-4-backlog.md))" at this wave's V1-FROZEN-INTERFACE.md refresh.
...
> ⚠️ **STATUS UPDATE — CLOSE-AS-ALREADY-SHIPPED (orchestrator §3.5n ground-truth, origin/main `ed03729a`, 2026-05-19).** The asymmetry this row describes ("8 methods on `Engine` directly … the decision rule is unstable") is **STALE — the (a) `EngineCapsHandle`-canonical migration ALREADY SHIPPED**. Ground-verified: `git show origin/main:crates/benten-engine/src/engine.rs` has **ZERO** cap-mutation methods on `Engine`; all 9 (`install_proof`/`revoke`/`create_principal`/`grant_capability`/`grant_capability_with_proof`/`revoke_capability`/`revoke_capability_by_grant_cid`/`install_ucan_proof`/`delegate_capability`) live on `EngineCapsHandle` (`crates/benten-engine/src/engine_caps.rs:94..429`); `Engine::caps()` (`engine.rs:1628`) returns the handle. The "forced architectural choice (a)-vs-(b)" below is therefore **NOT an open Ben architectural fork** — the codebase converged to (a). This row is **NOT a Phase-4-Meta migration wave**. Its sole Phase-4-Meta footprint = **ONE G-CORE-9 frozen-interface invariant**: *assert no `Engine`-direct cap-mutation method regresses pre-freeze — all 9 stay on `EngineCapsHandle`* (a no-regression freeze-lock, not a relocation). The sibling napi-side U05 (#834 — `bindings-napi` Surf-1 50-method cohesion umbrella) is a DISTINCT surface (point 3 below), in-scope for the G-CORE-9 atomic wave's napi-side cascade; it is NOT this row and NOT a "whichever lands first" fork (the engine-side direction is already shipped (a)). The historical asymmetry framing below is preserved for context only.

Per HARD RULE rule-12 DISAGREE-WITH-EXPLANATION + BELONGS-NAMED-NOW (refinement-audit-2026-05 Qual-2 #820, umbrella #1195). `[HISTORICAL — superseded by the STATUS UPDATE above:` The capability-grant mutation surface was split: 2 pass-through methods (`install_proof` / `revoke`) on `EngineCapsHandle` (returned by `Engine::caps()`, rustdoc'd as "the production-equivalent grant-mutation handle") vs 8 methods on `Engine` directly — including the most-recently-landed `revoke_capability_by_grant_cid` (PR #199) and `delegate_capability` (Phase-4-Foundation, which IS on the handle at `engine_caps.rs:498` — the decision rule was unstable).`]`
...
3. **U05 (#834) is a `bindings-napi` Surf-1 umbrella (napi_surface::Engine 50-method cohesion failure), NOT a benten-engine facet-handle split.** **RECONCILED (2026-05-19):** the napi-side facet split (#834/U05) and the engine-side handle organization are sibling-but-distinct surfaces — and the engine-side direction is NO LONGER a "whichever lands first sets the canonical principle" question: the engine-side already converged to (a) `EngineCapsHandle`-canonical (STATUS UPDATE above, ground-verified). #834/U05 (the napi 50-method cohesion umbrella) is its own surface, in-scope for the G-CORE-9 atomic v1-API wave's napi-side cascade (the napi bindings rebind through `.caps()` consistent with the already-shipped engine-side (a)); it does not re-open the engine-side organizing principle and is not gated on this row. Plan cross-ref: `.addl/phase-4-meta/00-implementation-plan.md` §1.A.FROZEN item 1 + the G-CORE-9 group def + §8-A `[CORRECTION]`.

**The forced architectural choice `[RESOLVED — option (a) already shipped; preserved for context]`:** either (a) `EngineCapsHandle` IS the canonical production grant-mutation surface → migrate `revoke_capability_by_grant_cid` + `install_ucan_proof` + `grant_capability*` + `create_principal` onto it (8→0 on Engine direct; napi rebinds through `.caps()`); OR (b) `Engine`-direct IS canonical → `install_proof` / `revoke` on `EngineCapsHandle` are redundant CLAUDE.md-#5-class aliases and the handle is deleted. **Outcome: (a) is the shipped state at origin/main `ed03729a` (8→0 on `Engine` direct ALREADY done; all 9 on `EngineCapsHandle`).** The Phase-4-Meta footprint is therefore NOT "choose + migrate ~150-250 LOC" — it is the ONE G-CORE-9 no-regression freeze-invariant (assert all 9 stay on the handle; no `Engine`-direct cap-mutation method reappears) + the coupled napi-lane rebind in the G-CORE-9 atomic wave. ~0 LOC migration; the freeze-invariant + napi rebind are the only remaining work.
...
> ⚠️ **STATUS — §4.78 SPLIT across sub-issues (orchestrator §3.5n ground-truth + R1-triage, 2026-05-19; plan cross-ref `.addl/phase-4-meta/00-implementation-plan.md` §0 per-fork row + G-CORE-6/G-CORE-9).** **#807 (sub-1) is SUPERSEDED + ALREADY SHIPPED:** `RATIFIED-decisions-2026-05-17.md:24` ("Standardize on `to_canonical_bytes`") settled the which-name fork — sub-1's "BEN-DECISION / do NOT decide unilaterally" prose below is STALE — AND PR #1295 (`a15d6af4`, an ancestor of plan baseline `ed03729a`) already landed the workspace-wide `to_canonical_bytes` standardization (`git grep legacy_minimal origin/main` = EMPTY, re-confirmed 2026-05-19). #807 is therefore a G-CORE-6 **verify-pass** (assert names-only, zero canonical-byte/CID/on-disk delta, P-III intact), NOT a re-run (re-running = a P-III hazard). **#802 (sub-2, PrimitiveHost READ-verb cross-crate rename ~27 files) + #1042 (sub-3, typed `ErrorCode::EvalBackend` catalog mutation 168→169 + §3.5g TS-mirror) are NOT in the P-II ratified set** — they → G-CORE-9 as brief-decided residual forks (NOT planner-decided, NOT silently swept).
...
1. **#807 canonical-encoding method-name fork — RESOLVED (RATIFIED 2026-05-17, landed in the P-II workspace sweep via PR #1295).** Workspace-wide there were FOUR conventions for the same operation: `to_canonical_bytes`, `canonical_bytes`, `to_dagcbor`, `to_dag_cbor`. RATIFIED disposition (`.addl/refinement-audit-2026-05/RATIFIED-decisions-2026-05-17.md`, #807/#658/#734/#824/#1087): **standardize on `to_canonical_bytes`** (serializer) + `from_canonical_bytes` (deserializer), single P-II post-COLLAPSE workspace sweep. Done: the `canonical_bytes` (×6 benten-core/benten-id), `to_dagcbor`/`to_dag_cbor` (×5 Subgraph/SnapshotBlob/ExecutionStateEnvelope) methods were renamed to `to_canonical_bytes`/`from_canonical_bytes`, the redundant Subgraph encode-aliases DELETED (CLAUDE.md #5), all callsites migrated workspace-wide. The envelope/bytes-specific `Keypair::*_dag_cbor_envelope` + `PeerId::*_dag_cbor_bytes` (domain-specific names; `PeerId::to_dag_cbor_bytes` is wire-load-bearing) were intentionally NOT renamed (out of the named scope; P-III-adjacent). No canonical-byte/CID value change (P-III preserved — names only). **[2026-05-19: settled which-name fork — no agent re-opens it; #807 = the G-CORE-6 verify-pass per the §4.78 STATUS box above + `.addl/phase-4-meta/00-implementation-plan.md`.]**
...
**Acceptance criteria (UPDATED 2026-05-19).** #807 = G-CORE-6 **verify-pass** (PR #1295 already shipped the `to_canonical_bytes` standardization; verify names-only / zero-byte-delta / P-III intact — NOT a re-run). #802 + #1042 resolved at the G-CORE-9 v1-API-stabilization freeze as brief-decided residual forks (the G-CORE-9 sweep brief dedups intra-row sub-issue splits per §8-D-extended before deciding). Fork (1)/#807 is NO LONGER surfaced for Ben (settled by `RATIFIED-decisions-2026-05-17:24`); fork (3)/#1042 (typed-variant-vs-deliberate-Unknown SemVer) surfaced at the G-CORE-9 brief. ~verify-only for #807; ~27 files for #802; ~catalog-bump + TS-mirror for #1042 at G-CORE-9.

### §4.79 benten-core Anchor-shape unification — ONE `VersionDag` with a strict/linear mode (Phase-4-Meta-Core G-CORE-5 — refinement-audit #849, umbrella #1158/#1142) — SHIPPED

Per HARD RULE rule-12 BELONGS-NAMED-NOW (refinement-audit-2026-05 Surf-1 #849; umbrella **#1158** v1-API-stabilization cluster, coupled to **#1142** Subgraph/Anchor canonicalization). This is the named Phase-4-Meta-Core destination for the **#849** "three coexisting Anchor shapes" finding. The third (legacy crate-root `u64`-id) shape was DELETED for **#1003** in the #1142 lane (RATIFIED 2026-05-17 — zero non-test callers; CLAUDE.md rule #5). The unified `version_dag::VersionDag` composability surface — one nominal type with an opt-in `Mode::{Strict, Dag}` selector + one shared `VersionChain` trait + one CURRENT semantic — SHIPPED at G-CORE-5 (closes #849). The TWO underlying per-pattern implementations (`benten_core::version::Anchor` Cid-head-threaded linear; `benten_core::version_chain::DagVersionChain` explicit DAG, branches/merges native) remain alongside as the per-pattern implementations existing engine + platform-foundation callers depend on directly.

**Ratified design (RATIFIED-decisions-2026-05-17.md D3, Phase-4-Meta-Core-windowed, SHIPPED at G-CORE-5).** There is ONE version data structure (the DAG). "Linear" is NOT a subtype or degenerate child — it is an opt-in **strict mode/policy** on the one structure (single-parent invariant + fork-rejected-as-typed-error). `version.rs`'s contribution (its `VersionError::Branched`/`UnknownPrior` + "re-read & re-attempt" optimistic-concurrency contract) is carried into the unified DAG as opt-in strict mode; the DAG is structurally strictly more capable (rejects only cycles). Error surfaces align (`VersionError::code()` ≡ `version_chain::VersionDagError::code()` ≡ `version_dag::VersionDagError::code()` → `ErrorCode::VersionBranched` / `VersionUnknownPrior`). Decision: one `VersionDag` type; strict/linear is a mode on it (not two types, not a subtype); one shared trait; one CURRENT semantic. — Decision LIVE in code as of G-CORE-5.

**Acceptance criteria (Phase-4-Meta-Core G-CORE-5 — SHIPPED).** The unified `VersionDag` type + opt-in `Mode::{Strict, Dag}` + shared `VersionChain` trait + unified `VersionDagError` enum + `.code()` mapping all SHIPPED at G-CORE-5 in `crates/benten-core/src/version_dag.rs`. The Anchor/u64-delete half of #1142 was already CLOSED via #1003 (PR #1290). The strict-mode fork-rejection pin (`tests/version_branched.rs`) carries forward unchanged; new TF-6 pins (`tf6_d3_version_dag_unification_red.rs` + `tf6_d3_dead_anchor_1003_and_995_verify_stays_regression.rs`) cover the unification + the post-#1003 stay-deleted regression. Caller migration (`benten-platform-foundation` plugin-library, `benten-engine` handler-versions/diagnostics, etc.) onto the unified surface remains a downstream-wave deliverable (G-CORE-6 verify-pass / G-CORE-9 v1-API stabilization) — the unified surface is the **post-G-CORE-5 canonical composability surface** new code threads through; existing pre-G-CORE-5 callers keep working unchanged on the two per-pattern implementations.
...
### §4.186 benten-graph hygiene + stale-cite cluster (W20-P5 close-as-named — refinement-audit-2026-05 Hyg/Surf-2)

Per HARD RULE rule-12 BELONGS-NAMED-NOW. Refinement-audit-2026-05 W20 close-as-named (partition P5). `benten-graph` dead-code + stale-cite + doc-completeness/accuracy findings, all `phase-4-meta-deferred` `priority:p2`, none deployed-exploitable: **#305** (`NetworkFetchStubBackend` dead-code), **#424** (`blob_backend.rs` stale rustdoc cite), **#430** (`store.rs:37` "every backend" stale cite), **#437** (`NetworkFetchStubError` stale cite), **#893** (4 rustdoc surfaces doc-completeness), **#898** (`Snapshot` doc-accuracy), **#901** (3 sites doc-completeness), **#906** (`BlobBackend` doc-accuracy), **#909** (registry doc-accuracy). **Why Phase-4-Meta:** documentation/dead-code hygiene on a stable crate surface; correct prose lands with the §4.204 perf-pass + the workspace-wide hyg-3 sweep (§4.207) so cite-drift is fixed once against a settled API. Destination: `crates/benten-graph/src/{blob_backend,store,redb_backend}.rs` rustdoc + dead-code removal, co-routed with §4.204.

### §4.187 benten-graph Safe-1 panic/error-path completeness (W20-P5 — refinement-audit-2026-05 Safe-1)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). `benten-graph` panic + error-path completeness: **#493** (`transaction::scan_edge_index` silently `continue`s on on-disk index corruption in the cascade-delete path — asymmetric with `edges_from`/`edges_to` which propagate; corruption-path symmetry hardening), **#505** (`benchmark_helper_crud_post_create_dispatch_impl` uses 12 `.expect()` on a `pub(crate)` bench-built helper; needs `#[cfg(...)]` gate or `Result` plumbing). **Why Phase-4-Meta:** these are corruption-path / bench-helper robustness refinements, not deployed user-input panics — the steady-state read paths already propagate; the asymmetric helper is bench-only. Destination: `crates/benten-graph/src/transaction.rs:706-717` + `redb_backend.rs:1417-1478`, folded into the §4.204 graph perf-pass wave (same files touched).

### §4.188 benten-graph Surf-1 composability seams (W20-P5 — refinement-audit-2026-05 Surf-1)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). `benten-graph` API-layering / composability-feature surfaces: **#836** (`GraphBackend::Transaction` `run<F,R>` closure-composability surface — sibling/duplicate of the already-tracked §4.60), **#855** (`KVBackend` sync-only vs RPITIT-async — sibling of the already-tracked §4.63). **Why Phase-4-Meta:** v1-API-stabilization architectural forks owed before the v1 tag; both already have canonical decision rows (§4.60, §4.63). This row records the W20 close-as-named binding of the raw issues to those existing decision destinations; no new decision created here. Destination: §4.60 (#836) + §4.63 (#855) — issues homed to the already-ratified-as-pending fork rows.

### §4.189 benten-graph Fwd-2 forward-readiness (W20-P5 — refinement-audit-2026-05 Fwd-2)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). **#1004** (light-client mode-(b)/(c) graph-backend surface — sibling of the already-tracked §4.64 light-client trait-destination decision), **#1022** (`GraphBackend` umbrella SemVer-locking shape forks — sibling of §4.61). **Why Phase-4-Meta:** forward-architecture decisions already carried at §4.61 + §4.64; this records the W20 binding of the raw Fwd-2 issues to those destinations. Destination: §4.61 (#1022) + §4.64 (#1004).

### §4.190 benten-core Safe-1 saturating-conversion + Subgraph robustness (W20-P5 — refinement-audit-2026-05 Safe-1)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). **#506** — 5 `*::try_from(...).unwrap_or(<MAX>)` saturating numeric conversions in `crates/benten-core/src/subgraph.rs` (lines 727/788/835/849/897: handle-slot `u32::MAX`, ITERATE `max` / WAIT `timeout_ms` / `duration_ms` / `iterate_parallel` `i64::MAX`). **Why Phase-4-Meta:** the inputs are author-time DSL builder values bounded well below the saturation point in every shipped path; making the clip an explicit typed error (or documented saturating contract) is a hardening refinement that co-routes with the §4.205 benten-core perf-pass (same `subgraph.rs` file). Destination: `crates/benten-core/src/subgraph.rs` — typed-error or documented-contract pass folded into §4.205.

### §4.191 benten-core Qual-1/Qual-2 simplicity + naming (W20-P5 — refinement-audit-2026-05 Qual-1/2)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). benten-core simplicity + naming refactors: **#666** (`Subgraph` encode/decode simplification), **#678** (`SubgraphBuilder` exposure simplification), **#741** (`SubgraphBuilder` naming), **#775** (`Subgraph::primitive*` naming), **#786** (`canonical_subgraph*` naming). **Why Phase-4-Meta:** public-symbol renames on `benten-core` cascade cross-crate (engine/napi/eval consumers) — they belong in the §4.43 v1-API-stabilization coordinated rename wave, not a disjoint single-crate lane (cross-crate-cascade P-II shape, same class as §4.69/§4.78). Destination: folded into the §4.43 v1-API-stabilization sweep (benten-core symbol slice) co-routed with §4.205.

### §4.192 benten-core Surf-1 composability + Fwd-2 forward-readiness (W20-P5 — refinement-audit-2026-05 Surf-1/Fwd-2)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). **#837 / #842 / #845 / #849** (benten-core API-layering/composability-feature surfaces; #849 is the Anchor-shape input already tracked at §4.79 `VersionDag` unification), **#1024** (`OperationNode` Fwd-2 forward-readiness). **Why Phase-4-Meta:** composability-feature + Anchor-shape decisions are v1-API-stabilization forks; #849 already homed at §4.79. Destination: §4.79 (#849) + §4.43 v1-API-stabilization slice (#837/#842/#845) + §4.205 Fwd-2 carry (#1024).

### §4.193 benten-renderer-tauri hygiene + Safe-2/3/4 IPC-boundary hardening (W20-P5 — refinement-audit-2026-05 Hyg/Safe-2/3/4)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). The Tauri shape-(c) embedded-webview IPC boundary (CLAUDE.md baked-in #17): **#285** (`_assert_renderer*` dead-code), **#550** (untrusted-webview `method` field has no length/charset cap before it reflects into `IpcError` Display — log-injection/log-DoS surface; rung-1 protections exist, this is rung-2 input-narrowing), **#607** (no structural pin asserts production wiring attaches a bridge — bridge-less `TauriRenderer` silently accepts session-less dispatches; test-pin strengthening), **#616** (rung-3 `read_node_as` grep satisfied by allowlist string literals not the module-doc trust-posture narrative — load-bearing-grep-target sharpening), **#625** (compile-time `Send+Sync` assertion has no runtime concurrent-`dispatch_ipc` test pin). **Why Phase-4-Meta:** all are defense-in-depth refinements on a boundary that already has rung-1 protections shipped; the proper fix is one cohesive IPC-boundary-hardening pass (length cap + structural bridge-attach pin + grep-target sharpening + concurrency pin) that co-routes with the §4.206 renderer-tauri perf/bench pass (same `lib.rs` + `tests/` files). Destination: `crates/benten-renderer-tauri/src/lib.rs` (211-270, 502-554, 589-593) + `tests/three_rung_baked_in_17_defense_extension_pin.rs`, one Phase-4-Meta hardening wave.

### §4.194 benten-renderer-tauri Qual-2/Surf-1 naming + composability (W20-P5 — refinement-audit-2026-05 Qual-2/Surf-1)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). **#740** (`new_with_manifest` non-idiomatic constructor name — single constructor over-promising a family), **#827** (Surf-1 composability — the RATIFIED #3 `benten-renderer-webview-shared` extraction sub-issue; see §4.206 umbrella #1200), **#833** (Surf-1 composability-feature surface). **Why Phase-4-Meta:** the constructor rename rides the §4.206 crate-extraction (Ben-RATIFIED 2026-05-15 per refinement-audit §15.3 #3) so the public renderer surface is renamed once at extraction time, not twice. Destination: §4.206 (umbrella #1200) carries #827 + #833 + #740 as the cohesive renderer-surface-stabilization wave.

### §4.195 bindings-napi hygiene + stale-cite cluster (W20-P5 — refinement-audit-2026-05 Hyg-1/3)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). napi binding (`bindings/napi/`) hygiene: **#316** (`error::to_napi` dead-code), **#319** (8.1 MB stale prebuilt binary), **#325** (stale wave-ID handle in dead-code), **#456** (`budget_exhaust*` hyg-3 stale cite). **Why Phase-4-Meta:** binding-layer dead-code + a stale build artifact — zero runtime surface; the binary purge + dead-code removal lands with the §4.196/§4.197 napi cleanup so the binding directory is swept once. Destination: `bindings/napi/src/error.rs` + repo artifact purge + the §4.198 napi-naming wave; INTERNALS.md (gitignored, local-only) is NOT the canonical destination — this tracked row is.

### §4.196 bindings-napi Safe-1/Safe-2 boundary + panic-path hardening (W20-P5 — refinement-audit-2026-05 Safe-1/2)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). napi untrusted-JS-boundary input-validation + panic-path: **#531** (`describeSandbox*` panic path), **#532** (`trace.rs` `.expect()`), **#575** (`declare_device_*` boundary), **#579** (`wait.rs::json_t*` boundary), **#582** (`user_view_drain*` boundary), **#586** (`install_module` + `computeManifestCid` call `serde_json::from_value` with no bounded recursion / size cap — manifest-envelope DoS surface; sibling of the workspace-wide META #629 unbounded-decode pattern), **#588** (`parse_actor_cid_or_derive` silently maps malformed CID strings to deterministic synthetic CIDs — invalid input never surfaces a typed error at `call_as`/`on_change_as`/`callStream_as`). **Why Phase-4-Meta:** the napi boundary is a thin shim over the engine which performs the load-bearing validation; these are defense-in-depth narrowings at the binding seam. #586 explicitly couples to the META #629 / §4.80 `from_canonical_bytes_bounded` workspace untrusted-decode sweep — it lands as the napi callsite of that one cohesive bounded-decode pass. Destination: `bindings/napi/src/{lib.rs,node.rs,wait.rs,trace.rs}` — #586 → §4.80 napi callsite; #588 + the panic-path items → one napi boundary-hardening wave co-routed with §4.198.

### §4.197 bindings-napi Qual-1/Surf-1 simplicity + surface cleanup (W20-P5 — refinement-audit-2026-05 Qual-1/Surf-1)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). **#724** (`subgraph.rs` duplication), **#728** (4 sibling JSON-shape helpers — consolidatable), **#731** (6+ instances of a repeated pattern), **#853** (`browser-target` feature surface), **#861** (`createUserView` deprecation surface), **#865** (`AtriumConfig` napi-derive surface). **Why Phase-4-Meta:** binding-layer simplicity refactors + surface cleanups that ride the §4.43 v1-API-stabilization sweep (the napi public surface stabilizes once before v1 tag). Destination: §4.43 v1-API-stabilization (napi binding slice) co-routed with the §4.198 naming wave.

### §4.198 bindings-napi Qual-2 naming-refactor + Fwd-1/2 forward-readiness (W20-P5 — refinement-audit-2026-05 Qual-2/Fwd-1/2)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). naming: **#780** (`view.rs` naming), **#789** (21+ redundant naming sites), **#804** (`trace.rs` naming), **#808** (`describe_sandbox_node_napi` spurious `_napi` suffix). Fwd-1/2 forward-readiness: **#705** (`BrowserManifestStore` safe-4-async), **#972** (zero bench coverage on napi surface), **#1026** (no AsyncTask migration story), **#1032** (no CSP-nonce surface), **#1036** (`Outcome.error_code` ordinal vs string), **#1043** (`typedCall` dispatch-only — no batching shape for Phase-5+ AI-agent `Promise.all([...])` workloads per CLAUDE.md #15/#18), **#1055** (`Cid::to_base32` perf), **#1057** (`parse_actor_cid` perf), **#1058** (no `.d.ts` type-guard exports). **Why Phase-4-Meta:** the `_napi`-suffix + redundant-naming sweep is a single coordinated public-surface rename owed before v1 tag (§4.43 class); the Fwd-1/2 items are Phase-5+ AI-agent-workload + bench-baseline forward-readiness carries that join the §4.43 / META #1062 bench-harness baseline pass. Destination: §4.43 v1-API-stabilization (napi naming slice) + META #1062 bench-baseline (#972/#1055/#1057) + the Phase-4-Meta forward-readiness decision agenda (#1043/#1026/#1032/#1036/#1058 → META #1094 v1-API decision cluster).

### §4.199 cross-crate X1 dependency-graph hygiene (W20-P5 — refinement-audit-2026-05 X1)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). Workspace dependency-graph hygiene (uncrated, `cross-crate`): **#252** (4 `workspace.dependencies` cleanups), **#254** (tokio + iroh version pinning), **#258** (6 dev-dep cycles), **#260** (`tests/integration` dep usage), **#263** (`napi-build` version-pin), **#265** (`workspace.dependencies` cleanup). **Why Phase-4-Meta:** workspace-Cargo.toml hygiene with zero runtime/API surface; lands as one coordinated workspace-deps sweep at Phase-4-Meta (touches root `Cargo.toml` + every member — best done once at a quiescent point, not piecemeal). Destination: root `Cargo.toml` + member manifests — one Phase-4-Meta workspace-deps-hygiene PR.

### §4.200 cross-crate X2/X7 dead-code + feature-flag consistency (W20-P5 — refinement-audit-2026-05 X2/X7)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). **#276** (`serde_json` dev-dep dead), **#277** (4 pub modules unused in a binding), **#278** (~15 pub mods unreachable), **#259** (`envelope-cache-*` feature flag), **#262** (feature-flag naming inconsistency). **Why Phase-4-Meta:** pub-surface dead-module pruning is v1-API-stabilization-coupled (removing `pub` mods is a public-API change owed before the v1 tag, §4.43 class); feature-flag naming consistency lands with the §4.71 features-table sweep pattern. Destination: §4.43 v1-API-stabilization (dead-pub-module slice) + §4.71-pattern feature-flag-consistency pass, one Phase-4-Meta workspace surface sweep.

### §4.201 cross-crate X4 architectural-invariant boundary (W20-P5 — refinement-audit-2026-05 X4)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). **#264** — engine reaches into `benten_ivm::algorithm_b::*` internals; CLAUDE.md baked-in #2 boundary is softer than declared (the `Strategy`-enum-only seam was sharpened at R6-R3 r6-r3-arch-8 but a residual reach remains). **Why Phase-4-Meta:** this is the same IVM↔engine boundary-seam decision already carried at §4.82 (`CanonicalViews` boundary, RATIFIED 2026-05-17, Phase-4-Meta-windowed non-blocking). #264 is the cross-crate-lens restatement of that seam. Destination: §4.82 — #264 homed to the already-RATIFIED IVM↔engine boundary-tightening row.

### §4.202 cross-crate X9/X11/X12 + x8-wasm32 end-to-end + threat + CI + thin-client coverage (W20-P5 — refinement-audit-2026-05 X9/X11/X12/x8)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). **#1123** (4 canonical cross-crate happy-path workflows — crud DSL, UCAN grant/use/revoke, Atrium join-sync-leave, subscribe/unsubscribe — have no workspace-level integration pin), **#1129** (workspace end-to-end coverage gap), **#272** + **#275** (vitest workflow + branch-protection CI-coverage), **#1134** (CLAUDE.md baked-in #19 engine-level-extension trust model has zero T-class threat coverage — intentional hands-off but not explicitly ruled-out in CLAUDE.md/SECURITY-POSTURE), **#1132** + **#1135** (x8-wasm32 thin-client posture: napi INTERNALS + 4 trait/type leak surfaces), **#1133** (QUICKSTART.md has zero Phase-4 coverage). **Why Phase-4-Meta:** workspace-level e2e integration pins + the threat-model "explicitly rule out #19" SECURITY-POSTURE note + the wasm32-thin-client posture audit are v1-assessment-window dogfood/threat-model items (CLAUDE.md baked-in #15) — they land in the Phase-4-Meta dogfood + threat-model pass, not a per-crate lane. #1133 QUICKSTART Phase-4 coverage couples to the §6 doc-retense pass. Destination: Phase-4-Meta v1-assessment-window e2e/threat/wasm32 sweep + SECURITY-POSTURE.md #19 explicit-rule-out note + §6 (QUICKSTART Phase-4 retense for #1133).

### §4.203 benten-renderer-tauri Fwd-1 perf + bench-coverage (W20-P5 — refinement-audit-2026-05 Fwd-1)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). **#942** (`IpcRequest` allocation hot-path), **#927** (no benchmark coverage on the renderer-tauri IPC surface). **Why Phase-4-Meta:** TIER-2 perf-pass + bench-baseline on a shape-(c) surface that is not on a v1-blocking hot path; lands with the §4.206 umbrella #1200 webview-shared extraction + META #1062 bench-harness baseline. Destination: §4.206 (umbrella #1200) + META #1062 bench-baseline.

### §4.204 benten-graph Fwd-1 perf pass — umbrella #1214 tracker (W20-P5 — refinement-audit-2026-05 Fwd-1)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). **Umbrella #1214** (deferred bundle tracker) + members **#955** (zero bench coverage of edge-fanout/index/blob/subscriber/txn-throughput), **#959** (`value_index_bytes` per-write Vec realloc — 3-12 allocs per `put_node`), **#962** (`BrowserBackend::snapshot()` unbounded BTreeMap clone, no telemetry/cap), **#965** (`subscribers` Vec clone per commit — O(N_subs) Arc-bumps, could be `Arc<Vec>` snapshot), **#969** (no CI bench-regression guard). (#952 txn-reuse N+1 noted in the umbrella body remains under the umbrella's task list.) **Why Phase-4-Meta:** TIER-2 allocation/perf-pass + bench-coverage; not v1-blocking (steady-state correctness unaffected). Destination: this row IS the canonical Phase-4-Meta tracker for umbrella #1214 — substantive PRs fan out at Phase-4-Meta R1 planning; co-routes with the §4.186/§4.187 graph hygiene + Safe-1 items (same `redb_backend.rs`/`transaction.rs` files) and META #1062 bench-harness baseline.

### §4.205 benten-core Fwd-1 TIER-2 perf pass — umbrella #1162 tracker (W20-P5 — refinement-audit-2026-05 Fwd-1)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). **Umbrella #1162** (deferred bundle tracker) + members **#937** (`version::append_version` O(N)/call → O(N²) over N appends), **#943** (`canonical_view` two transient Vec per `Subgraph` encode), **#949** (`walk_versions` eager Vec then `IntoIter` — forces O(N) eager alloc). **Why Phase-4-Meta:** sub-tier-1 allocation hotspots not in the universal-WRITE path; ~80 LOC TIER-2 perf-pass, not v1-blocking. Destination: this row IS the canonical Phase-4-Meta tracker for umbrella #1162 — co-routes with the §4.190 saturating-conversion + §4.191/§4.192 benten-core naming/composability slice (same `subgraph.rs`/`version.rs` files) and META #1062 bench-harness baseline.

### §4.206 benten-renderer-tauri webview-shared crate extraction — umbrella #1200 tracker (W20-P5 — RATIFIED #3 2026-05-15)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). **Umbrella #1200** — Ben-RATIFIED 2026-05-15 per `docs/future/refinement-audit-2026-05.md §15.3` item #3: extract `benten-renderer-webview-shared` crate (~800-1500 LOC across new crate + 2-3 callers). Sub-issue **#827** names the RATIFIED shape; this umbrella also carries the cohesive renderer-surface-stabilization items #833 (Surf-1 composability) + #740 (constructor rename, §4.194) so the public renderer surface is renamed/restructured once at extraction time. **Why Phase-4-Meta:** RATIFIED-as-deferred architectural extraction, explicitly NOT pre-v1. Destination: this row IS the canonical Phase-4-Meta tracker for umbrella #1200 — substantive scope fans out at Phase-4-Meta R1 planning; co-routes with §4.194 + §4.203.

### §4.207 meta-tracker — workspace-wide hygiene (hyg-1/2/3/4) cross-cutting cluster (W20-P5 — refinement-audit-2026-05 META)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). Cross-crate `meta`-labelled hygiene trackers, each consolidating a workspace-wide pattern: **#367** (`#[allow(dead_code)]` + self-documenting-code hyg-1), **#370** (stale `Phase-N` TODO markers post-phase-ship), **#373** (pre-superseded methods retained as shims), **#486** (INTERNALS-documents-deferral but backlog doesn't — doc-coordination gap), **#500** (INTERNALS.md aggregate-count + HEAD-SHA drift), **#513** (rustdoc/docstring forecast-narrative format drift), **#518** (workspace-wide `.addl/phase-N/` → `.addl/_archive/` cite sweep), **#526** (rustdoc + test-file `file.rs:LINE` cite drift), **#530** (phantom-destination docstring cites — 13 sites), **#533** (2026-05-11 phase-rename consequence sweep), **#556** (retrospective docs `docs/history/PHASE-N.md` hyg-4). **Why Phase-4-Meta:** workspace-wide hygiene sweeps best executed once at a quiescent Phase-4-Meta point (cite-drift/phantom-destination/SHA-drift all churn with every merge, so a per-wave fix re-drifts). Destination: one Phase-4-Meta workspace-hygiene sweep PR (couples to the §6 doc-retense pass for the rustdoc/INTERNALS/retrospective slices); these trackers stay open as the canonical sweep scope until that PR lands.

### §4.208 meta-tracker — qual-1/qual-2 + surf-1/2 cross-cutting refactor cluster (W20-P5 — refinement-audit-2026-05 META)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). Cross-crate `meta` refactor/surface trackers: **#761** (back-compat wrappers for prior wave's narrower signatures — workspace-wide), **#772** (hand-rolled `serde_json::Map`+insert+Object JSON construction — repeated pattern), **#823** (test-fixture public-symbol naming + visibility), **#826** (rename-history orphaned runtime payloads), **#828** (verb-rename drift across same-class operations), **#831** (post-R6-Wave-2 file-split-without-dedup), **#925** (`Send + Sync` bounds inconsistency across trait surfaces), **#935** (stale Phase-N-future-tense docstrings + surf-2). **Why Phase-4-Meta:** these consolidate workspace-wide refactor patterns whose fixes are public-symbol/signature changes — they ride the §4.43 v1-API-stabilization coordinated sweep so the workspace surface settles once before the v1 tag. Destination: §4.43 v1-API-stabilization workspace sweep; trackers stay open as the canonical pattern scope until folded.

### §4.209 meta-tracker — safe-4-async + forward-architecture shapes cluster (W20-P5 — refinement-audit-2026-05 META)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). **#746** (forward-architectural shapes shipped at v1 carry a documented-but-unexercised cost — META trajectory tracker), **#764** (documented background-task that doesn't actually spawn — safe-4-async META). **Why Phase-4-Meta:** forward-architecture cost-accounting + the documented-vs-actual async-task reconciliation are v1-assessment-window items (CLAUDE.md baked-in #15) — they fold into the §4.202 / META #1094 v1-API decision agenda + the §4.207 hygiene sweep (doc-vs-code reconciliation slice). Destination: META #1094 decision agenda (#746) + §4.207 hygiene sweep (#764 doc-vs-code reconciliation).

### §4.210 meta-tracker — Fwd-1 bench-harness baseline cluster (W20-P5 — refinement-audit-2026-05 META)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). **#1062** (criterion bench-harness baseline — the workspace bench-harness anchor referenced by §4.76/§4.84/§4.198/§4.203/§4.204/§4.205), **#1067** (N+1 scan-loop-get pattern — workspace-wide), **#1074** (hot-crypto-primitive bench gap). **Why Phase-4-Meta:** these are the workspace bench-baseline + recurring-perf-pattern trackers that the per-crate Fwd-1 perf-pass rows (§4.76/§4.84/§4.203/§4.204/§4.205) all land against — one cohesive Phase-4-Meta bench-harness-baseline pass establishes the informational baselines before any pushdown refactor. Destination: this cluster IS the canonical Phase-4-Meta bench-harness-baseline tracker; the per-crate Fwd-1 rows co-route into it.

### §4.211 meta-tracker — Fwd-2 v1-API-stabilization decision agenda + wasm32 parity cluster (W20-P5 — refinement-audit-2026-05 META)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (W20-P5). **#1094** (v1-API-stabilization decision cluster — 9+ architectural ratifications + 50+ trajectory-hazard surfaces accumulated; surface as a single Ben morning decision agenda before the v1 tag — sibling of META #746), **#1122** (x8-wasm32-thin-client-posture: no Rust→wasm32 parity test). **Why Phase-4-Meta:** #1094 is the consolidated Ben-decision agenda owed at the v1-assessment-window — every §4.x v1-API-stabilization fork row (§4.43/§4.60-§4.79/§4.188/§4.189/§4.192/etc.) rolls up here for one decision pass; #1122 is the wasm32-parity v1-assessment audit. Destination: this row IS the canonical Phase-4-Meta v1-API decision-agenda tracker (consolidates the fork rows for Ben's pre-v1-tag morning); #1122 → the §4.202 wasm32-posture sweep.
...
### §4.86 refinement-audit-2026-05 W19 META closure-pin + §15.5 pim-N catalog tracking destination (Phase-4-Meta — refinement-audit Wave E / W19; META #737 / #1070 / #1140)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (refinement-audit-2026-05 Wave E / W19 META closure-pin sweep; siblings §4.84 / §4.85 precedent — "the campaign summary doc + dispatch-conventions are local-only (gitignored), so this tracked-doc backlog row is the canonical HARD-RULE clause-(b) named destination" for the §15.5 pim-N catalog + the three Pattern-F/retense META trackers). This row makes the campaign-doc `§15.5` and `.addl/dispatch-conventions.md` cites self-contained so they are not phantom destinations in fresh CI clones.

**Reproduce-verified at main `20fb5c78` (Wave E / W19):** all three META trackers STAY OPEN — each still has genuinely-open tracked items (NOT force-closed):

- **META #737** (Pattern-F flagship — Qual-1 simplicity refactors close coupled Safe-1/Safe-2 findings). Bundles 1–3 (the three *confirmed* coupled-bundle instances — #534/#695 engine ReadContext, #511/#667 sync CBOR, #499/#611/#670/#681/#693 renderer-tauri parallel-array) are ALL CLOSED at `20fb5c78`. The four *sibling* bundles tracked in the META body STAY OPEN: #516 + #700 (benten-sync `PeerId` accessor / `to_dag_cbor_bytes`), #527 + #702 (benten-platform-foundation `materialize_format` cap-recheck), #505 + #698 (benten-graph bench-helper production-path), and the #1161 benten-id `CanonicalBytes`/`RevocationSet` trait-extraction bundle (#787/#971/#973/#976 open). #700/#527/#505/#698 carry `phase-4-meta-deferred`; #1161 lands in W20. **#737 stays OPEN as the Phase-4-Meta Pattern-F tracking row.**
- **META #1070** (Fwd-1 3-axis Pattern-F extension). Only the renderer-tauri Bundle-3 expansion (#933/#693/#681/#670/#747/#499/#611/#829) is fully CLOSED. The benten-id (#968/#973/#976/#699/#787/#905/#711), bindings/napi (#975/#1055/#696/#728/#772), platform-foundation cycle-detect (#948/#981/#720), sync-LoroDoc (#1011/#384), and caps-cascade (#936/#946/#954) bundles all retain open tracked items (many `phase-4-meta-deferred` / W20). **#1070 stays OPEN as the Phase-4-Meta 3-axis Pattern-F tracking row.**
- **META #1140** (phase-close-retense-itself-introduces-regressions; §15.4 item 23 / §15.5 Tier-1 #3 `pim-N-phase-close-retense-self-verify-gate`). Origin closure-pins #1098/#1107/#1113/#1116/#1120/#1128/#1133/#1134/#1136 are mostly OPEN at `20fb5c78` (only #1112/#1121/#1125 closed); the `pim-N-phase-close-retense-self-verify-gate` codification itself is NOT landed (lives in local-only `.addl/dispatch-conventions.md` per the §15.5 "do NOT codify unilaterally at W19 — this row is the named tracking destination, not the codification" directive). **#1140 stays OPEN as the Phase-4-Meta retense-gate codification + origin-pin tracking row.**

**§15.5 pim-N catalog (~28 candidates, Tier-1/Tier-2):** the catalog enumeration lives in the local-only campaign summary doc (`docs/future/refinement-audit-2026-05.md §15.5`, gitignored — internal methodology artifact, not publicly shipped per the `.gitignore` "local-only dev infrastructure" partition). The recommended ratification path (single `.addl/dispatch-conventions.md` amendment PR landing Tier-1 + Tier-2 + closure-pins per §3.6h) is an orchestrator/Ben action, NOT codified unilaterally in this disjoint W19 doc lane (consistent with the §4.68 / §4.75 / §4.85 "surfaced for orchestrator/Ben pim-N codification at W19 — do NOT codify unilaterally" directive). This row is the tracked anchor; the catalog body + the dispatch-conventions edits are orchestrator-maintained local-only artifacts.

**Acceptance criteria (Phase-4-Meta).** #737/#1070/#1140 convert to Phase-4-Meta tracking rows alongside the W20 `phase-4-meta-deferred` umbrella reconciliation (close-as-named-destination, NOT force-close) — they close only when their remaining tracked sub-items close at Phase-4-Meta. The §15.5 Tier-1 + Tier-2 pim-N catalog ratifies via the orchestrator/Ben single dispatch-conventions amendment PR (~150-250 LOC of `.addl/dispatch-conventions.md` edits) with closure-pins per §3.6h. ~no committable LOC now (the substantive surfaces are local-only); ~150-250 LOC at the orchestrator-direct dispatch-conventions ratification PR.

### §4.87 benten-id Hyg-1 dead-code / surface-drift cluster (Phase-4-Meta cleanup — refinement-audit Hyg-1, W20 close-as-named)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (refinement-audit-2026-05 Hyg-1, W20 partition P1). Eleven Hyg-1 dead-code / unreferenced-surface findings in `benten-id` that are NOT pre-v1-actionable (zero correctness/security impact; surface-area-bloat hygiene) and only retire cleanly once the coupled G14-B durable-lift / library-subgraph wire-ups land (deleting them now would strand RED-PHASE pins + future-consumer seams). **Member issues:** #301 (`is_did_superseded` free-fn alias never called), #306 (`Keypair::secret()` zero crate-internal callers), #308 (`SecretKey::bytes_ptr_for_test` zero callers), #311 (`Ucan::to_canonical_bytes` unreferenced private wrapper), #313 (`PluginDidHandle::keypair()` getter never called), #318 (`PluginDidStore` `mint_and_store`/`iter`/`len`/`is_empty` unreferenced outside own unit test), #320 (`audience_matches_plugin_did` SHAPE-check helper zero production callers), #329 (`DeviceAttestation::from_canonical_bytes` decoder zero callers), #338 (`TrustDomain::allow` mutator never called outside hypothetical path), #340 (`CredentialBuilder::extra_type` builder method never called), #345 (near-duplicate `to_canonical_bytes` helper across 4 modules — shared-util candidate).

**Acceptance criteria (Phase-4-Meta).** During the §4.81/§4.83-adjacent benten-id Phase-4-Meta cleanup wave: for each surface, either (a) delete it if still zero-consumer and no forward-seam, or (b) wire it to the production consumer that the coupled durable-lift/library-subgraph work introduces, or (c) `#[doc(hidden)]` + `#[cfg(test)]`-gate test-only conveniences. The #345 near-duplicate `to_canonical_bytes` consolidation co-routes with §4.90 (Qual-1 shared-helper extraction). ~80-150 LOC net deletion at Phase-4-Meta.

### §4.88 benten-id Hyg-2 stale G14-B / durable-persistence stub comments (Phase-4-Meta — refinement-audit Hyg-2, W20 close-as-named)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (refinement-audit-2026-05 Hyg-2, W20 P1). **Member issues:** #372 (9+ stale `G14-B replaces...` deferral comments across 5 src files — Phase 3 SHIPPED without G14-B lifting in-RAM primitives to durable backings: `Acceptor::nonce_store`, `Acceptor::revocations`, `RotationLog`, `RevocationRegistry`, `PluginDidStore`), #379 (`PluginDidStore` doc claims redb-persistence at Phase-4-Foundation but HEAD store is in-memory; durable lift unnamed). These are documentation-accuracy stubs whose correct resolution is bound to the actual durable-lift implementation; the comments must be re-tensed (or deleted) at the same time the durable backing lands so they describe shipped reality.

**Acceptance criteria (Phase-4-Meta).** Co-routed with the benten-id durable-substrate decision (couples to §4.26 RotationLog rehydration + the PluginDidStore redb-lift): when the durable backing lands, rewrite the 9+ G14-B comments to describe the shipped substrate; if a given primitive stays in-RAM by design, re-tense the comment to state that intentional decision (not a forward-promise). ~30-50 LOC comment retense.

### §4.89 benten-id Hyg-3 / Surf-2 stale-cite + doc-accuracy cluster (Phase-4-Meta doc retense — refinement-audit Hyg-3 + Surf-2, W20 close-as-named)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (refinement-audit-2026-05 Hyg-3 + Surf-2, W20 P1). Doc-string / comment / Cargo-metadata accuracy drift; zero code-behaviour impact. **Member issues:** #428 (did_rotation.rs module-doc claims chain-walker consults `is_did_superseded` alias — actually calls `RotationLog::is_superseded`), #431 (multi_sig.rs module-doc + `ThresholdMultiSig` docstring stale on Kith/SelfRevocation identity-recovery roadmap), #435 (vc.rs §Cag-r4-2 narrative implies benten-id cannot reach `benten_core::Node` — at HEAD benten-core IS a direct dep), #478 (7 `#[ignore]` rationale strings cite `phase-3-backlog §2.1-followup` whose Phase-3 G16 re-evaluation trigger has passed — post-Phase-3-close stale-framing), #905 (device_attestation.rs:200-203 comment names migrated-away `OsRng::fill_bytes`), #908 (ucan.rs:462-468 'defense-in-depth' comment misframes `let _ = aud_bytes` dead-code workaround), #910 (plugin_did.rs:31-43 `PluginDidHandle` docstring describes Clone semantics for non-Clone type), #915 (Cargo.toml:9 package description omits Phase-4-Foundation scope — plugin-DID/GrantReader/rotation hardening).

**Acceptance criteria (Phase-4-Meta).** One cohesive benten-id doc-retense pass: correct each cited doc-string/comment/Cargo-metadata to describe shipped reality at the Phase-4-Meta HEAD; re-point the 7 `#[ignore]` rationale strings (#478) to the live Phase-4-Meta destination row(s) (§4.26 / this row family). The #908 + #717 (§4.90) `let _ = aud_bytes` framing must be resolved together (comment + code). ~50-80 LOC doc-only retense.

### §4.90 benten-id Qual-1 simplicity / duplication / surface-bloat cluster (Phase-4-Meta refactor — refinement-audit Qual-1, W20 close-as-named)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (refinement-audit-2026-05 Qual-1, W20 P1). Eleven simplicity / duplication / premature-flexibility findings; none are correctness bugs (refactor-quality only) and several are coupled to production-wiring that hasn't landed. **Member issues:** #665 (6× duplicated `canonical_bytes() -> Vec<u8>` + `expect("DAG-CBOR…cannot fail")` boilerplate across 4 modules), #672 (4× duplicated `verify_signature_with(&self, pk)` body), #677 (`DeviceAttestation::issue_at`/`issue_with_nonce` public test-conveniences with zero production callers), #686 (`Keypair::from_dag_cbor_envelope` no-op alias for `from_seed_bytes`), #691 (`validate_chain_no_time_check` self-described-ambiguous complexity smell), #699 (`generate_fresh_nonce` 32 hand-rolled array-index exprs vs loop), #706 (`Acceptor::with_parent_lookup`/`new_with_revocations` test-only constructors — 3 ctors where one builder suffices), #711 (`RotationAttestation` `previous_keypair_did`/`next_keypair_did` re-wrap pub String fields), #717 (`validate_chain_inner` `let _ = aud_bytes;` discard + apologetic comment for a check that never lands), #722 (`AttestationKind` 1-variant enum + constant-returning `kind()` — premature flexibility), #725 (`SeedImportError::InvalidSecret` reserved-but-unreachable variant + self-documented dead-arm).

**Acceptance criteria (Phase-4-Meta).** Phase-4-Meta benten-id simplicity refactor wave: extract the shared `to_canonical_bytes`/`verify_signature_with` helpers (#665 + #672 + Hyg-1 #345 co-routed); delete or `#[cfg(test)]`-gate the test-only constructors/aliases (#677/#686/#706); collapse premature-flexibility surfaces (#722/#725) when no extension materializes by Phase-4-Meta; resolve the #717 `aud_bytes` discard together with the #908 comment + the audience-binding decision it gates. ~150-250 LOC net simplification at Phase-4-Meta.

### §4.91 benten-id Qual-2 substrate-naming + envelope-const disambiguation (Phase-4-Meta — refinement-audit Qual-2, W20 close-as-named)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (refinement-audit-2026-05 Qual-2, W20 P1). **Member issues:** #787 (3 in-RAM revocation/rotation substrates — Registry / Log / nonce_store — with 3 naming patterns + 3 concurrency patterns + same G14-B forward trajectory; opportunity to extract a `RevocationSet` trait), #801 (`ENVELOPE_VERSION`/`ENVELOPE_ALG`/`MIN_ENVELOPE_BYTES` const names don't disambiguate across 4 envelope shapes — seed/capability/UCAN/credential; flagged for composition with cross-crate #738). Both are naming/abstraction refactors gated on the G14-B durable-lift decision (a `RevocationSet` trait extraction is far cheaper to land together with the durable backing than as a standalone churn).

**Acceptance criteria (Phase-4-Meta).** Co-routed with the §4.88 durable-substrate decision: when the durable backing lands for the 3 substrates, extract the unifying `RevocationSet` trait (#787) at the same time; rename the envelope consts to shape-disambiguated names (#801) composed with cross-crate #738 if that lands first. ~100-180 LOC at Phase-4-Meta.

### §4.92 benten-id Safe-1 RevocationRegistry mutex-poison hardening (Phase-4-Meta — refinement-audit Safe-1 #512, W20 close-as-named)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (refinement-audit-2026-05 Safe-1 #512, W20 P1). **Member issue:** #512 (`RevocationRegistry::revoke` + `is_revoked` `.expect("registry mutex poisoned")` panic at vc.rs:373/381). The finding's own disposition is MINOR not MAJOR: VC revocation is a G14-A2 in-RAM helper (production VC verification flows through `verify_in_trust_domain`), `is_revoked` panic is fail-CLOSED relative to "accepted as unrevoked", and the HashSet-of-String operations cannot panic under non-OOM conditions. It is genuinely Phase-4-Meta-deferred because the correct fix (lock-poison-recovery or move to a `parking_lot` / poison-free mutex) co-routes with the §4.88 durable-lift of `RevocationRegistry` — replacing the in-RAM `Mutex<HashSet>` with the durable backing dissolves the poison surface entirely, so a standalone poison-recovery patch would be discarded by the durable-lift wave.

**Acceptance criteria (Phase-4-Meta).** When the §4.88 `RevocationRegistry` durable-lift lands: ensure the new backing has no `.expect`-on-poison panic site (return a typed `VcError` / fail-closed bool with logged poison-recovery); if the durable-lift slips past the §4.92 wave-target, ship the standalone poison-recovery patch (recover the guard via `into_inner()` / `clear_poison()` and continue fail-closed) as the interim. ~20-40 LOC at Phase-4-Meta.

### §4.93 benten-id Safe-4 concurrency-contract documentation (Phase-4-Meta — refinement-audit Safe-4, W20 close-as-named)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (refinement-audit-2026-05 Safe-4, W20 P1). **Member issues:** #639 (OBS, disposition WONTFIX per the finding itself — `Acceptor::accept_at` does Ed25519 sig-verify BEFORE nonce-store insert; alternative ordering opens a worse TOCTOU; carried as audit-trail row), #642 (mixed interior-sync vs caller-sync API shape across 6 public types is undocumented; INTERNALS.md needs a §Concurrency contract + compile-time `Send`+`Sync` pins). #639 is intentional-by-design preserved as an audit row; #642 is a documentation/test-pin gap appropriately landed alongside the broader Phase-4-Meta INTERNALS retense.

**Acceptance criteria (Phase-4-Meta).** Add a §Concurrency section to benten-id INTERNALS.md documenting the interior-sync (`Acceptor`/`RevocationRegistry`) vs caller-sync (`RotationLog`/`PluginDidStore`) split + add compile-time `assert_impl_all!`-style `Send`+`Sync` pins for the public types (#642); record #639's verify-before-insert ordering as an intentional documented decision (WONTFIX rationale: alternative ordering opens worse TOCTOU). ~30-60 LOC doc + test-pin at Phase-4-Meta.

### §4.94 benten-id Surf-1 public-API layering / re-export / feature-surface cluster (Phase-4-Meta — refinement-audit Surf-1, W20 close-as-named)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (refinement-audit-2026-05 Surf-1, W20 P1). API-surface consistency findings deferred to the v1-API-stabilization window so they land atomically with the SemVer freeze rather than churning the surface twice. **Member issues:** #854 (OBS — `ReaderError` missing from lib.rs top-level re-export; 8 of 9 typed-error enums re-exported; G27-C drift), #858 (`RuntimeTarget` enum models a workspace-wide CLAUDE.md #17 deployment-shape concept but lives buried in device_attestation with one consumer + no shape-(c) variant), #862 (`testing` Cargo feature has zero downstream consumers; required-features gate silently skips R6-FP-3 substantive arm at workspace test runs), #867 (secret-hygiene `!Clone` + redacted-`Debug` invariant propagates to `PluginDidHandle`/`Ed25519SingleKey` by accident-not-pin; one docstring drifts from `!Clone` reality).

**Acceptance criteria (Phase-4-Meta v1-API-stabilization).** Add `ReaderError` to the lib.rs re-export (#854); decide `RuntimeTarget` relocation/variant-add against the CLAUDE.md #17 three-shape model — likely promote to a workspace-shared location with the shape-(c) variant (#858, couples to renderer-tauri shape-(c) work); resolve the `testing` feature consumer story (#862, couples to Fwd-2 §4.96 #1071); add explicit `assert_not_impl_all!` `!Clone` pins for `PluginDidHandle`/`Ed25519SingleKey` + correct the drifting docstring (#867). One cohesive pre-v1-tag API-surface pass. ~60-100 LOC at Phase-4-Meta.

### §4.95 benten-id Fwd-1 crypto-hot-path bench-coverage + allocation cluster (Phase-4-Meta perf-pass — refinement-audit Fwd-1, W20 close-as-named)

Per HARD RULE rule-12 BELONGS-NAMED-NOW (refinement-audit-2026-05 Fwd-1, W20 P1; sibling of §4.84 benten-caps + §4.76 benten-eval bench-coverage clusters — one Phase-4-Meta bench-harness baseline pass). **Member issues:** #957 (F-FWD-1-01 absent bench coverage — no `benches/` dir; 6 hot crypto surfaces without regression-gate), #964 (F-FWD-1-02 `Did::resolve` allocates `Vec<u8>` per call via `bs58::decode().into_vec()` — hit per chain-walk link), #968 (F-FWD-1-03 `generate_fresh_nonce` 4× `OsRng.next_u64()` vs `fill_bytes()` — Pattern F bundle with Qual-1 #699 + Surf-2 #905), #971 (F-FWD-1-04 `to_canonical_bytes` recomputed per sig-verify — DISAGREE-WITH-EXPLANATION: intentionally recomputed for adversarial-input safety; audit-trail row), #973 (F-FWD-1-05 `validate_chain_with_device_revocations`/`_with_attestations` nested-loop O(chain×revocations×caps) — HashMap-migration candidate, Pattern F with Qual-2 #787), #976 (F-FWD-1-06 `Acceptor::nonce_store` `(String,[u8;32])` key — `String::clone` per accept; key-narrowing candidate), #977 (OBS F-FWD-1-07 Did-* accessors clone String per call — folds under Qual-1 #711 + Qual-2 #776; audit-trail), #978 (OBS F-FWD-1-08 `RotationLog::accept_rotation_event` two-pass O(N) per accept — bounded at current scale; G14-B durable-lift seam), #980 (OBS F-FWD-1-09 Ed25519 verify ~150µs/link not batched — DISAGREE-WITH-EXPLANATION: early-rejection load-bearing; audit-trail).

**Acceptance criteria (Phase-4-Meta perf-pass).** Add 4-6 informational Criterion benches under `crates/benten-id/benches/` covering the 6 hot surfaces (#957) landed together with the §4.84/§4.76 bench-harness baseline pass + META #1062; land the allocation/loop cleanups (#964/#968/#973/#976) with before/after baselines from those benches; #971/#977/#978/#980 are preserved as audit-trail (DISAGREE / G14-B-seam) rows requiring no code change unless their coupled cluster (Qual-1 #699/#711, Qual-2 #787) lands first. ~150-250 LOC bench + opt at Phase-4-Meta.

### §4.96 benten-id Fwd-2 forward-readiness / v1-stabilization cluster (Phase-4-Meta — refinement-audit Fwd-2 bundle #1178, W20 close-as-named)
...
Per HARD RULE rule-12 BELONGS-NAMED-NOW (refinement-audit-2026-05 #1233, W20 P1). **Member issue:** #1233 ([Part B] Trust-model encryption leg — content-encryption key model + decrypt-capability + peer-agnostic + multi-tenant crypto-isolation). The 2026-05-15 trust-model reframe split into Part A (authority spine — already SHIPPED + settled) and Part B (the genuinely-greenfield encryption leg, ABSENT from all shipped code by design). Part B does NOT gate the current fix-pass; it is correctly Phase-4-Meta / v1-assessment-window scope, design-coupled with the already-named "identity-recovery protocol choice" (CLAUDE.md baked-in #15). Authoritative scope lives at `.addl/refinement-audit-2026-05/DECISION-RECORD-trust-model-reframe.md §5` + `.addl/refinement-audit-2026-05/vision-graph-ownership-encryption-multitenancy.md §4` (≈10 open questions) + the existing §4.58 row; this §4.97 row is the benten-id-crate-side close-as-named anchor so #1233 closes to a tracked benten-id destination.

**Acceptance criteria (Phase-4-Meta / v1-assessment-window).** Dispatch the Part-B design effort alongside the identity-recovery protocol-choice decision: resolve the ~10 open questions (content-encryption key model; decrypt-capability ↔ UCAN relationship; peer-agnostic ciphertext-on-any-peer; multi-tenant crypto-isolation + per-tenant key recovery; hash-over-ciphertext-vs-plaintext fork). Greenfield design effort, no LOC estimate until the design lands; tracked jointly with §4.58.

---

## W20 close-as-named — benten-engine + benten-errors Phase-4-Meta-deferred clusters (§4.113–§4.135)

Per HARD RULE rule-12 BELONGS-NAMED-NOW. These rows are the canonical named destinations for the refinement-audit-2026-05 `phase-4-meta-deferred` / `priority:p2` issues filed against `benten-engine` + `benten-errors` that are genuinely Phase-4-Meta-windowed (cleanup, naming, micro-perf, forward-readiness seams, documented-backward-compat tracking). W20 partition P2. Each cluster lists member issue numbers + scope + why-Phase-4-Meta. These are tracked carries, not silent deferrals; the member issues are closed against these rows by the W20 set-diff consolidation.

### §4.113 benten-engine rule-#5 deprecated-shim + Phase-1-stub deletion sweep (#350, #353, #420)

CLAUDE.md rule-#5 ("no deprecated aliases or backward-compat shims; delete, don't comment") cleanup that touches the public `Engine`/`EngineError` surface, so it is sequenced into the Phase-4-Meta §4.43 v1-API-stabilization window rather than landed piecemeal pre-v1. Scope: delete `NestedTx` + `EngineTransaction::begin_nested` always-Err Phase-1 stub + the `nested_tx.rs` asserts-unreachable test (#350); delete the 3 deprecated-shim public methods `describe_sandbox_node` (always-Err relic of pre-G7-A scaffolding) / `register_crud_with_grants` (Phase-1 forwarding alias) / `EngineError::code_as_str` (legacy `'static str` accessor with zero live callers) + migrate the handful of caller tests to the canonical surfaces (#353); drop the dead `EngineError::NotImplemented` variant + its single self-rationalizing round-trip test (#420). **Why Phase-4-Meta:** public-API-shape deletions belong in the §4.43 v1-API-stabilization sweep so the napi/TS mirror + INTERNALS.md cite-coupling move atomically; no production caller relies on any of these shims today.

### §4.114 benten-engine hyg-2 stub-shape contract gaps (#414, #433)

Schema slots / test-helper signatures whose declared contract has outrun the implementation. Scope: `ModuleManifest.migrations: Vec<MigrationStep>` is accepted at native install time but no migration runner exists and the stated "Phase 3 alongside persistence" destination has SHIPPED — decide ship-runner / reject-on-native-mirror-of-wasm32-path / retire-the-schema-slot, and re-point the `module_manifest.rs` docstring at this row (#414); `fabricate_test_suspend_envelope_with_attribution_cid_bytes` carries an `_attribution_bytes` parameter the body ignores while the docstring promises "spliced in at CID-encode time" — reconcile signature-to-contract (#433). **Why Phase-4-Meta:** #414 couples to the `BlobBackend` durable-lift / manifest-evolution story (§4.62-class); #433 is a test-only contract reconciliation with no production blast radius — both are post-v1-foundation refinement.

### §4.115 benten-engine safe-1 silent-error-swallow + panic-discipline sweep (#537, #538, #539, #541)

Production paths that collapse errors/panics into silence against a stale or falsifying rationale. Scope: `change.rs` + `emit_broadcast.rs` `catch_unwind` discard subscriber-panic payloads where the rustdoc forecast a `tracing` log that never landed (tracing is now a workspace dep) — log the payload via `tracing::error!` (#537); `StreamHandle::drop` `let _ = handle.join()` discards the producer-thread panic payload behind a "panic propagates here" comment that is false for `JoinHandle::join` — log + drop (#538); `handler_router.rs` 6× `.lock().expect("poisoned")` outliers vs the workspace-wide `PoisonError::into_inner` recovery discipline — mechanical 6-site sweep to `lock_recover()` (#539); `termination_reason` / `cache_get` / `suspension_store` `.ok()` patterns silently collapse backend errors into `None`, violating the documented JS/TS observability contract (`E_SUBSCRIBE_REVOKED_MID_STREAM`) and masking redb-corruption as `E_NOT_FOUND` — propagate the `Result` / use `lock_recover()` (#541). **Why Phase-4-Meta:** these are observability-hardening passes (no deployed correctness regression — failures degrade fail-safe today) bundled into one Phase-4-Meta safe-1 sweep; pairs with the §3.5g-class single-module-outlier discipline.

### §4.116 benten-engine entropy-restricted-runtime CSPRNG surface narrowing (#542, #592)

Documented-backward-compat / process-abort surfaces tracked for v1-API stabilization tightening rather than audit-driven fixes. Scope: `DidKeyedSession::new()` `getrandom().expect("OS CSPRNG must be available on full-peer hardware")` is sound for native shape-(a) hardware but process-aborts on entropy-restricted CI/container/embedded targets — add a `# Panics` rustdoc section OR refactor to a fallible constructor minting `EngineError::ThinClientUnavailable` (CATALOG increment) (#542); `DeviceAttestationEnvelope::verify` returns `Ok(())` skipping all signature/replay checks when `attestation = None` (documented V1-wire backward-compat shape) — the v1-stabilization question of whether V1-shape envelopes should still be accepted vs rejected with a typed `LegacyEnvelopeUnsupported` is a Ben/architect ratification (#592). **Why Phase-4-Meta:** both are explicitly v1-API-stabilization ratification forks (the `verify` exemption is documented-intentional with a fail-back to local `device_cid`; the CSPRNG expect is justified for the current native-only shape per CLAUDE.md #17) — architectural calls, not audit fixes.

### §4.117 benten-engine safe-2 boundary defense-in-depth (#590, #591)

Caller-controlled-input defense-in-depth gaps on privileged engine surfaces. Scope: `thin_client` `emit_challenge` / `establish_session` unchecked `u64 + u64` on caller-supplied `SessionConfig.ttl_secs` — debug-panic / release-wrap (fail-safe-by-accident) on degenerate misconfiguration; add `checked_add` with a typed error (#590); `register_module_bytes` in-memory `BTreeMap<Cid, Vec<u8>>` cache accepts arbitrary-size module bytes with no per-entry/aggregate RAM cap — CID-recompute gates content but not size; add a configurable cap (#591). **Why Phase-4-Meta:** both are MINOR defense-in-depth on privileged surfaces (not network-edge; #590 is fail-safe-closed in release, #591 is bounded by the durable-backend disk limit) — folded into the broader META #629 DoS-bounding pass rather than landed in isolation pre-v1.

### §4.118 benten-engine safe-3 Inv-12 aggregate-validation surface (#634)

`Engine::register_subgraph` (canonical user-facing registration) routes to `validate_subgraph(aggregate=false)`, so multi-invariant inputs surface only the first detected violation; the Inv-12 aggregate roll-up declared ACTIVE in INVARIANT-COVERAGE.md row 12 is reachable only via the opt-in `register_subgraph_aggregate` sibling. Decide whether the canonical surface should default to `aggregate=true` (better DX, surfaces all violations at once) or whether the doc/coverage matrix should be amended to reflect the opt-in shape. **Why Phase-4-Meta:** this is an API-default ratification fork that couples to the §4.43 v1-API-stabilization surface review (changing the default alters the error shape callers route on); no invariant is unenforced — Inv-12 fires, just first-only on the canonical path.

### §4.119 benten-engine safe-4 async lock-discipline sweep (#675, #685, #692)

Async/concurrency lock-discipline refinements. Scope: `DidKeyedSession::establish_session` holds the inner `std::sync::Mutex` guard across the user-injectable CPU-bound Ed25519 verifier closure — reentrant-deadlock risk with custom verifiers + serialized handshakes; narrow the critical section (#675); `set_local_device_attestation` writes `local_device_attestation` + `local_device_did` under separate `tokio::Mutex` acquisitions across an await point — concurrent reader observes inconsistent (new attestation, old DID) state; single combined lock or atomic swap (#685); `ACTIVE_STREAMS` is a process-wide static `AtomicUsize` exposed via the per-engine-looking `Engine::active_stream_count` accessor — misleading multi-engine observability; rename/document or scope per-engine (#692). **Why Phase-4-Meta:** #675/#685 are correctness-hardening on the thin-client/sync paths whose practical trigger requires custom verifiers or concurrent multi-writer attestation updates (not deployed today); #692 is observability-naming — bundled into one Phase-4-Meta safe-4 sweep that couples to the multi-engine admin-UI composition work where the static-counter semantics actually bite.

### §4.120 benten-engine qual-1 internal-collapse simplicity sweep (#676, #680, #709, #714)

Internal (non-public) structural simplifications with zero behavior change. Scope: `apply_atrium_merge` calls `atrium.resolve_peer_dids` 4× redundantly per merge — collapse to one call + reuse (#676); `Engine::from_parts` is a dead `pub(crate)` one-line wrapper over `from_parts_with_clocks` with zero workspace callers — delete (#680); 4 near-identical `EngineInner` cap-write tally methods (`record_committed`/`record_denied`/`committed_snapshot`/`denied_snapshot`) collapse to 2 + an enum tag (#709); `Engine::is_actor_revoked_for_zone` is a trivial `pub(crate)` pass-through to the same-named `EngineInner` method — collapse one layer (#714). **Why Phase-4-Meta:** internal-only refactors with no v1-shippable impact; sequenced into the Phase-4-Meta qual sweep alongside §4.121 to land as one cohesive engine-internal simplification pass (the `apply_atrium_merge` #676 collapse couples to META #752 magnet + the §4.121 dispatch-chain collapse touching the same hot path).

### §4.121 benten-engine qual-2 naming + wrapper-chain + dead-fn refactor (#799, #805, #809, #819)

Naming-over-promises + wrapper-chain + dead-public-fn refactors. Scope: `dispatch_call → dispatch_call_with_mode → dispatch_call_with_mode_and_trace → dispatch_call_inner` 4-deep wrapper chain — collapse to a single fn with an optional-args/trace-builder struct (#799); 3 misnamed/dead `pub fn` variants — `call_with_suspension_on_thin_config_for_test` (always-Err, `_for_test` suffix but not cfg-gated, leaks into production), `call_with_revocation_at` (dead args), `code()` (delegates to `error_code()`) (#805); 4 handler-cid lookup sites duplicate the same not-found `EngineError::Other` arm — extract a single `resolve_handler_cid` helper (3 identical + 1 divergent-miss-semantics) (#809); inconsistent `&mut self` vs `&self`-via-Mutex setters with no decision rule across 5 `set_*` methods (#819). **Why Phase-4-Meta:** #799/#809 are internal; #805/#819 touch the public surface (the `_for_test`-leak + setter-signature normalization) so they belong in the §4.43 v1-API-stabilization window; cohesive Phase-4-Meta qual-2 pass with §4.122.

### §4.122 benten-engine surf-1 public-API ergonomics + feature-flag coherence (#874, #879, #880, #881, #882)

Public-API layering / composability / feature-flag coherence. Scope: `Engine::set_manifest_envelope_rechecker` takes `&mut self`, breaking the `Arc<Engine>` napi-wiring pattern + inconsistent with sibling `&self`-via-Mutex setters — interior-mutability migration (#874, sibling of #819); engine does not re-export the load-bearing `benten-core`/`benten-graph`/`benten-caps` types (`Node`/`Cid`/`Value`/`Edge`/…) so consumers need 3 extra direct deps to use the public `Engine` API — add the re-exports (#879); `browser-backend` feature has no target-arch mutual-exclusion guard — a native build with `--features browser-backend` compiles a silently-dysfunctional engine; add a `compile_error!` guard (#880); INTERNALS.md §4n+§3c cite `EngineCapsHandle::delegate_capability` but the method lives on `impl Engine` — doc-drift correction (#881); `Engine::dispatch_typed_call_public` `_public` suffix is unidiomatic (every `pub fn` is public) — rename per trait-disambiguation idiom (#882). **Why Phase-4-Meta:** every item here is a public-API-surface change (re-exports, feature-guard, setter signature, rename) that must move atomically with the napi/TS mirror + cite-coupling in the §4.43 v1-API-stabilization sweep; #881 doc-drift is local-only INTERNALS.md housekeeping co-routed with the same change.

### §4.123 benten-engine fwd-1 allocation-hot-path + atomic-ordering perf-pass (#950, #983, #984, #985, #987, #988, #994, #1090)

Forward-readiness performance refinements on the dispatch hot path — sequenced into the Phase-4-Meta perf-pass (couples to §4.76 benten-eval Fwd-1 cluster + §4.84 benten-caps bench cluster + META #1062 bench-harness baseline). Scope: `Outcome::error_code: Option<String>` forces per-cap-deny heap-alloc of a `&'static str` — `Option<Cow<'static, str>>` coerces the 168 named arms for free (#950, named-in from benten-errors-fwd-1 per clause-(b)); `AstCache::lookup` + `SubgraphCache::get` allocate owned `String` cache-keys per dispatch on the cache-hit path the cache was designed to make zero-alloc — borrowed-key lookup (#983); `check_capability` allocates `WriteContext { label: required.to_string(), ..Default::default() }` per primitive dispatch — borrowed `WriteContextRef` companion (#984); `apply_atrium_merge` per-row recheck couples reject-determinism to `BTreeMap` iteration order — materialize a sorted `Vec` + document the determinism contract (#985); ~12 `AtomicU64` observability counters use `SeqCst` where `Relaxed` is correct — extra `mfence` per RMW on the hot dispatcher path (#987); `Engine::open_atrium` builds a fresh iroh `Endpoint` per call without a `#[must_use]` hint or rustdoc contract — add the hint (~1 LOC) (#988); `dispatch_typed_call_public` eagerly allocates `op.required_cap().to_string()` on the cap-deny arm despite `&'static str` upstream — cold-path widening removed via a `TypedCallCapDenied::required` cross-crate signature change (#1090). `metrics_snapshot` ~10-30 `format!` keys per call is filed **DISAGREE-WITH-EXPLANATION** (operator-dashboard cold path; `BTreeMap` rebalancing dominates; readability beats `Cow` shenanigans) — recorded here for audit-trail completeness, no action (#994). **Why Phase-4-Meta:** no deployed regression; this is the v1-platform perf-claim verifiability + micro-allocation pass folded into the cross-crate Phase-4-Meta perf-harness wave, where the borrowed-companion signature changes (#984/#950/#1090) ripple across crates and want one coordinated landing.

### §4.124 benten-engine fwd-2 Class-B-β write-attribution + generic-cascade-lift seams (#1061, #1068)

Forward-readiness architectural seams for the Phase-4-Meta self-composing admin work. Scope: no public `Engine::write_node_as(principal, node)` sibling to the Class-B-β `read_node_as` — write-attribution asymmetry forces handler-indirection for every direct attributed write; add the public sibling (#1061); the engine impl-block generic-cascade lift is incomplete — ~17 methods on the resolved-alias `impl Engine` block stay coupled to `RedbBackend`, and the §4.43 v1-API-stabilization enumeration row that should list them is missing — enumerate the ~17 methods into the §4.43 row (#1068). **Why Phase-4-Meta:** both are explicitly v1-API-stabilization / v1-assessment-window seams already named (INTERNALS.md cites the generic-cascade lift as Phase-4-Meta-deferred); #1061 is the write-side mirror of the Class-B-β surface whose primary consumer is the Phase-4-Meta self-composing admin — designed to land with that work, not pre-v1.

### §4.125 benten-errors qual + surf + fwd Phase-4-Meta-deferred cluster (#658, #733, #825, #1053)

The full benten-errors `phase-4-meta-deferred`/`priority:p2` set. Scope: `routed_edge_label` returns `Option<&'static str>` for a closed set of 5 edge labels — a typed `EdgeLabel` enum catches consumer typos + removes `.to_string()` allocations across benten-eval primitives (#658, couples to the §4.123-class allocation pass + cross-crate consumer ripple); ~~`ErrorCode::from_str` is an inherent fn shadowing the `core::str::FromStr` idiom and returns `ErrorCode` (not `Result`)~~ — **#733 RESOLVED (RATIFIED P-I 2026-05-17, landed in the P-II workspace sweep): migrated fully to `impl core::str::FromStr for ErrorCode` (fallible — `Err(ParseErrorCodeError)` for unrecognized codes); the infallible inherent `from_str` was DELETED (no alias/shim per CLAUDE.md #5); all workspace callsites migrated; forward-compat boundaries recover `Unknown` explicitly via `.unwrap_or_else(|e| ErrorCode::Unknown(e.into_inner()))`;** empty `[features]` stanza — no `serde` feature flag prepared for the Phase-4-Meta plugin-manifest + Phase-8 decentralized-discovery wire-forms; add a gated `serde` feature preserving the no-std/zero-deps default (#825); Tauri-IPC + webview deployment-shape-(c) catalog coverage gap — 5-8 anticipated `E_TAURI_IPC_*` ErrorCode mints for the Phase-4-Meta admin-UI / renderer-tauri work (#1053, mint-with-the-feature per the established catalog-grows-with-the-surface discipline). **Why Phase-4-Meta:** #733 is a public-API rename (atomic with the §4.43 sweep + the Rust+TS ErrorCode mirror); #825/#1053 are explicitly forward-readiness for Phase-4-Meta plugin-manifest + Tauri-IPC surfaces (catalog/feature grows when that surface lands, per CLAUDE.md catalog discipline); #658 is an enum-ification with a cross-crate consumer ripple folded into the Phase-4-Meta qual + allocation pass.
...
- **Dead-code (Hyg-1):** #315 `MaterializerOutput::delivered_row_cids` zero-caller dead alias for `materialized_row_cids`; #321 `build_admin_ui_v0_subgraph` reimplements `build_category_route_subgraph` per-category instead of composing it; #326 `fixture_manifest_for_test` duplicates canary-mod `fixture_manifest` verbatim; #331 `IngestDialect` single-variant pseudo-extension-point + `VocabEdge::from_str` reserved-but-unfired + dead `E_SCHEMA_VOCAB_EDGE_MISMATCH` (couples §4.52 `VocabEdge::from_str` error-shape row); #339 asymmetric SANDBOX banned-host-fn lists `parse.rs`(6) vs `materializer.rs`(4) (couples §4.55 banned-list consolidation row).
- **Inert helpers (Hyg-2):** #390 `validate_schema_author_within_manifest_envelope` test-only consumers / no v1 install-path call site; #392 `validate_with_rotation_log` rotation-aware variant test-only (production calls `validate_with_clock` only) — both retire/wire at the Phase-4-Meta plugin-install-path hardening (sibling to §4.43.1 + §4.74).
- **Stale-cite (Hyg-3):** #444/#445/#448/#451 cite-drift to nonexistent/relocated tests (`admin_ui_v0_source_never_calls_engine_read_node_*`, `admin_ui_v0_subscribe_paths_only_via_on_change_*`, `admin_ui_v0_engine_adapter.rs` relocated to `tests/common/`, `materializer_pipeline_reactive_update_*` renamed at G23-B per §4.13 mr-5); #454 `workflow_to_plugin.rs:41` phantom `module_ecosystem::publish_plugin`; #463 `BrowserRender::render` stale forward-promise that G24-A would fill the DOM render path (couples §4.56 `Renderer::render()` no-op stub row).
...
- #730 four nearly-identical Value-tree walkers in `materializer.rs` (`render_value_html` + `render_value_plaintext` + `value_to_json` + per-Map inner-loop) reimplement Text/Int/Float/Bool/Null/Bytes/List/Map dispatch 3-4× → single generic visitor.
- #800 4-layer materializer render indirection (`Materializer` trait → free `materialize_*` fn → private `FormatBackend` enum → inherent `render_*` fn) collapses to 2 layers (couples #730).
- #732 `MaterializerError` redundant code-field-per-variant + `code()` accessor (variant identity already determines the typed code; the field is duplicate state — couples dead-code #312).
- #727 `UninstallContext` + `InstallContext` 3-port trait generic-bundle (R/P/S + M/P) is structural cost without isolation benefit (single `InMemoryCascade` implements all ports yet 3 generic slots force test code to instantiate 3 copies) → collapse the generic surface.
...
- #335 `test_fixtures::{fixture_root, fixture_paths}` pub fns zero callers (tests use local copies); #341 `evaluator::budget::{cumulative_budget_for_subgraph, check_per_iteration_budget}` zero non-test callers (INTERNALS.md stale — local-only doc; this tracked row is the canonical clause-(b) destination); #347 `expr::make_map` + `diag::trace::pretty` pub fns zero callers.
- #485 two `sandbox_escape_attempts_denied.rs` Component-Model `#[ignore]` rationales contain "Phase-4 pre-R1 inherits this deferral when Phase 4 opens" — stale post-2026-05-11 phase-rename (retense to Phase-4-Meta per §4.65 wasmtime fork); #487 `subscribe_handler_router.rs:29` `#[ignore]` rationale cites `engine.rs:1918` for `subscribe_with_handler`; actual line is 2003 (line-pin drift).
