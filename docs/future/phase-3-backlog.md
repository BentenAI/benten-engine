...
- **`benten-engine` test bodies — wasm32 bundle audit + napi cdylib erasure** (`wasm_no_redb.rs`, `no_unauthorized_dyn_error.rs`, `prop_no_state_leak.rs`, `component_model_decision.rs`, `bindings/napi/tests/wasm_bundle_content.rs`, `bindings/napi/tests/engine_b_erasure_boundary.rs`, `bindings/napi/tests/register_user_view_view_label_mismatch_post_g15_a.rs`) — destination: **§4.4 Bundle-content audit pins** (existing destination — R4b architecture / wasm-bundle lens carry; G16-B-D sub-item D NAMED-NOW). The `wasm-bundle-content-audit.yml` CI workflow named at §4.4 is the production landing site for these audits; test bodies un-ignore when that workflow lands.

- **`benten-engine` test bodies — anchor-prefix + IVM rebuild equivalence** (`anchor_prefix.rs`, `view1_capability_grants.rs`, `view2_event_dispatch.rs`, `view3_content_listing.rs`, `view5_version_current.rs`) — destination: **§5.1 IVM Algorithm B maturity** (closed at G15-A / G15-B / W9-T1) + **§7.3.A.X view-registry residuals**. The `anchor_prefix.rs` pins are post-G15-B prefix-matching invariant pins; un-ignore at v1-assessment-window when the registry + AnchorPrefix selector lift integration tests get cross-cut maintenance.
