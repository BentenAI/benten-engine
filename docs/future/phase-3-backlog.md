...
2. **`WriteContext::now`-as-real-clock injection.** Today `UcanGroundedPolicy::now_secs` defaults to the `DEFAULT_NOW_SECS = 0` **fail-CLOSED sentinel** (post-G16-B-B-rest inversion): a chain carrying ANY time-bounded delegation (`nbf > 0` OR `exp > 0`) evaluated at the sentinel is REJECTED with `CapError::UcanClockNotInjected`, not accepted — only a chain with NO time bounds walks at the sentinel. Callers inject via the `with_now_secs` builder (`crates/benten-caps/src/ucan_grounded.rs::with_now_secs`; renamed from the misleading `with_now_for_test` per #793/P-II). **Second half of this axis, R6 tail fold-in F45 — the injected clock is a CONSTRUCTION-TIME instant, not a live per-check wallclock:** `now_secs` is a plain struct field (`ucan_grounded.rs:194`) written once at engine build (`crates/benten-engine/src/builder.rs:576-581`) and read at every `check_write` (`ucan_grounded.rs:342`), so once a positive value IS injected, every subsequent write validates `exp` against the engine-OPEN instant — a UCAN expiring after engine-open can still validate on the write path for the lifetime of the process. The un-injected case is safe (fail-closed); it is the *injected* case that goes stale. Production needs `CapWriteContext::now_secs: u64` populated by the engine's `TimeSource` at every write-check entry, which closes both halves at once. This is the same threading axis as Phase-2a G9-A's wall-clock-refresh cadence work but at the policy-hook surface (vs the per-iteration boundary).
...
**Phase 3 target (NAMED at G16-B-B-rest 2026-05-09; tightened at Phase-3 R6 R1 fix-pass Wave E 2026-05-09 per `feedback_no_defer_HARD_RULE.md` 2026-05-08 Project-frame-marker phantom-destination guidance):** **v1-assessment-window** per CLAUDE.md item #15, **co-routed with §10.1 Compromise #1 TOCTOU window bound** as a single composing v1-assessment workstream. The two hooks share the same iterate-batch-boundary cap-recheck cadence mechanism: §10.1 asks "is the 100-iter batch bound still right under v1 multi-tenant context?" and the cap-r4-8 hooks supply the policy-driven knob that makes any answer to that question implementable. The coupling is structural (same evaluator surface, same cap-snapshot-refresh seam), not speculative — both items must be evaluated and shipped together because a policy-driven iterate-batch-boundary without a TOCTOU re-evaluation produces a knob with no operator-facing rationale, and a TOCTOU re-evaluation without policy-driven knobs limits the response options. The shared coupling is the reason for v1-assessment-window destination rather than Phase-3-close: Compromise #1's "is the bound right?" question is itself a v1-assessment input (it asks whether v1 multi-tenant agentic-loop deployments need a tighter bound — a question that requires v1-assessment context to answer).
...
- §10.1 Compromise #1 TOCTOU window bound — the v1-assessment-window co-routed item; both items share the iterate-batch-boundary cap-recheck cadence mechanism + must ship together per the structural coupling above
...
**Origin (G21-T1 fp-mini-review, 2026-05-08):** the security mini-review on PR #145 surfaced 4 MAJORs (closed end-to-end at PR #145 fix-pass) plus 4 minors. The MAJORs are closed; the original four named carries (a)-(d) and one G21-T2 add (e) below were originally written as "land in a follow-up wave" — phantom-destination shape per HARD RULE clause-(b). RE-NAMED at Phase-3 R6 R1 fix-pass Wave E 2026-05-09 (see updated **Phase 3 target** below). Status update: **(a) CLOSED at G21-T1 fix-pass** (`build_seed_envelope` `Zeroizing<Vec<u8>>` wrap landed); **(d) CLOSED at G21-T3 PR #147** (`ReservedHandlerNamespace` variant + register_subgraph guard landed; verifiable at `crates/benten-errors/src/lib.rs::ErrorCode::ReservedHandlerNamespace`). **(b), (c), (e) remain open** — see Phase 3 target below.
...
**Phase 3 target (RE-NAMED at Phase-3 R6 R1 fix-pass Wave E 2026-05-09 per HARD RULE clause-(b); previous "post-G21-T1 follow-on (T2/T3/T4 or sibling cleanup wave)" became phantom when T2/T3/T4 closed at PR #146/#147/#148 without absorbing (b)/(c)/(e)):** explicit per-item destinations per the no-phantom-destination discipline:

- **(b) `did_resolve` op DID-method validation (sec-minor-3)** — **v1-assessment-window** per CLAUDE.md item #15. Concrete coupling: composes with §2.3 (i) `WriteContext` audience + clock threading for arbitrary-scope UCAN proof-chain enforcement (which is also v1-assessment-window per its own destination ratification 2026-05-08) — both items hinge on the v1 question "what DID methods does v1 production target need?" `did:web:` and `did:plc:` are not Phase-3-scope (Phase-3 scope is `did:key:` for Atrium peer authentication; the §2.3 named ssi-spec-interop re-evaluation at G16 covered the Phase-3 Atrium-peer-handshake DID surface). Touch ~20-40 LOC at landing.
- **(c) `cap:typed:*` namespace consumer-side mapping (sec-minor-4)** — **v1-assessment-window** per CLAUDE.md item #15. Concrete coupling: composes with §2.3 (i) `WriteContext` audience + clock threading work (the cap:typed namespace mapping fundamentally requires the arbitrary-scope-string proof-chain enforcement that §2.3 (i) names as v1-assessment-window scope). Under `NoAuthBackend` all typed caps are permitted (canary-scope intent shipped at G21-T1); the UCAN-claim-string → typed-cap mapping is the per-deployment policy choice that v1-assessment determines. Touch ~50-100 LOC at landing alongside §2.3 (i).
- **(e) `Value::SensitiveBytes` discriminant** — **v1-assessment-window** per CLAUDE.md item #15. Concrete coupling: cross-crate `Value` enum touch (~150-250 LOC across 6+ Value-consumer crates) is exactly the surface CLAUDE.md item #15 v1-milestone-gate framing names — large API-shape changes that "marketing/positioning/release-readiness" framing benefits from re-evaluating at v1-assessment rather than landing as a Phase-3-close residual. The G21-T2 fp-mini-review MAJOR-6 option (b) closure (rename to `secret_bytes_unprotected`) is the bounded current-state defense; the option (a) `Value::SensitiveBytes` discriminant is the deeper invariant + can re-evaluate at v1-window where the v1 cryptographic hardening posture is set holistically.

**Touch size:** ~220-390 LOC total across remaining (b)/(c)/(e); all v1-assessment-window scoped per concrete couplings above.
...
**Phase 3 target (RE-NAMED at Phase-3 R6 R1 fix-pass Wave E 2026-05-09 per HARD RULE clause-(b); previous "post-G21-T2-close DX cleanup wave" became phantom when G21-T2 closed at PR #148 commit `7a6c36a` without spawning the named wave):** each item gets an explicit current-state-real destination per the no-phantom-destination discipline:

- **DX-1 (cargo dx items)** — **v1-assessment-window** per CLAUDE.md item #15. Concrete coupling: the orchestrator-direct workspace pre-push verification surface (memory `feedback_orchestrator_workspace_pre_push.md`, codified §3.5h 2026-05-08) prescribes 5 commands run serially. A `cargo dx` aggregation surface (workspace alias OR xtask crate) is the natural DX collapse for this discipline + would compose with the v1-assessment-window evaluation of operator-facing tooling readiness (CLAUDE.md item #15 v1-milestone-gate framing names "marketing/positioning/release-readiness" — workspace tooling DX falls in this band). Touch ≤30 LOC at landing.
- **DX-2 (Display rendering)** — **v1-assessment-window** per CLAUDE.md item #15. Concrete coupling: composes with DX-1 (the orchestrator-direct pre-push verification surface) AND v1-assessment-window operator-facing tooling readiness (CLAUDE.md item #15) — the `format!("{err:?}")` debug rendering across the typed-CALL pin set is operator-DX-facing only when the operator hits a typed-CALL fault; v1-assessment is when the typed-CALL surface gets cross-cut DX evaluation alongside the rest of the operator-error-rendering surface. Touch ≤30 LOC at landing.
- **DX-3 (`ucan_validate_chain` test fragility)** — **Phase-3-close pre-tag fix-pass batch** (sibling to the `orchestrator-direct fix-pass batch` shape demonstrated at §7.18 + §2.4 closure). The work is bounded (~80 LOC `tests/common/ucan_fixtures.rs` shared helper module) + known-needed (the 4 `ucan_validate_chain_returns_*` pins in `crates/benten-engine/tests/typed_call_engine_dispatch.rs` already build chains via duplicated `Ucan::builder` + manual `now`-fixture composition; the helper extraction is straight refactor with zero production-code touch). Per `feedback_no_defer_HARD_RULE.md` 2026-05-08 Project-frame-marker phantom-destination guidance, deferring known-needed bounded work to v1-assessment-window is the same shape as the typed-CALL fork Ben caught — wrong destination. Wave-E rationale-only scope; the helper extraction lands in a sibling Wave-E-followup or Wave-F orchestrator-direct fix-pass batch.
- **DX-4 (multi-Atrium handle dedup)** — **v1-assessment-window** per CLAUDE.md item #15. Concrete coupling: per Ben's D1 ratification, the per-handle `JsAtrium` shape is **intentional** for the handle contract (multiple `engine.atrium({atriumId: "family"})` calls produce distinct handles routing to the same logical atrium). The `declared_attestations` registry being per-handle is a **consequence** of D1, not an oversight — engine-level hoisting requires re-evaluating D1's handle-contract against v1-assessment multi-tenant operator usage patterns (whether attestations declared on handle A SHOULD be visible on handle B for the same atrium is a v1-tenant-isolation question, not a Phase-3-close hardening question). Touch ~50-80 LOC at landing if D1 is revisited; OUT-OF-SCOPE if D1 is reaffirmed at v1-assessment-window.

**Touch size:** ~190-220 LOC total across the 4 minors (~80 LOC pre-tag for DX-3; ~110-140 LOC v1-assessment-window for DX-1+DX-2+DX-4 pending D1 outcome).
...
- `feedback_orchestrator_workspace_pre_push.md` (memory, 2026-05-08 codified) — DX-1 coupling rationale.
- CLAUDE.md item #15 v1-milestone-gate framing — DX-1 + DX-2 + DX-4 destination rationale.
- D1 ratification (Ben, 2026-05-08) — DX-4 multi-handle contract that DX-4 hoisting must re-evaluate.
...
**Sub-mechanism — GC-pressure-timeout polling fallback (§7.1.2.1):** for environments without `--expose-gc` (Node default + most browser-target runtimes), the FinalizationRegistry callback timing is non-deterministic. A bounded-retry polling fallback that fires the leak event on a configurable timeout (default ~5s) is a follow-up. Touch size: ~50-100 LOC TS + 2-3 test scenarios. **Destination (RE-NAMED at Phase-3 R6 R1 fix-pass Wave E 2026-05-09 per HARD RULE clause-(b); previous "Phase-3 narrow-iter cycle (post-G19-C2 close)" became phantom when G19-C2 closed at PR #130 without spawning the named cycle):** **v1-assessment-window** per CLAUDE.md item #15. Concrete coupling: the polling-fallback fires only in production-deployment environments without `--expose-gc` (browser tab + most Node deployments without explicit GC exposure); v1-assessment-window is when production-deployment-shape DX gets cross-cut evaluation alongside the rest of the operator-facing observability surface (§7.1.2 master scenarios + sub-mechanism share the same observable-consequence contract per pim-2 §3.6b). The 4 master scenarios already-landed at G19-C2 wave-7 satisfy the load-bearing observable-consequence contract; the polling-fallback is a DX-widening, not a correctness gap.
...
**pim-stable-rustfmt-pre-push candidate (R6-final NEW) — orchestrator-direct push without `cargo +stable fmt --all --check` pre-flight.**
- Origin: PR #171 fp-6 `d3ac995` self-induced — orchestrator added new code in fp-5 `e6aea5d` to `crates/benten-engine/tests/code_to_ctor.rs` without running stable fmt; CI failed all 4 stable cells on fmt step. 1 instance at HEAD.
- Sub-3-recurrence threshold; folded under broader §3.5h MANDATORY-PRE-MERGE-AFTER-MINI-REVIEW-APPROVE rule (R6-final ratification — see §7.11b status update below) which mechanically covers this surface. Track here as sub-class observation; no separate codification needed.
- **Status 2026-05-09 (R6 R6-final sub-threshold-track-only verdict):** subsumed by §3.5h pre-merge gate ratification; codify separately if/when 3rd recurrence fires NOT covered by §3.5h.

**pim-codegen-coupling-after-doc-edits candidate (R6-final NEW) — agent/orchestrator edits doc surface that sources TS codegen, forgets to regen `errors.generated.ts`.**
- Origin: 2 instances at HEAD — (a) Wave-C2 fp2 (markdown ErrorCode 'Fix' field edit needs `npm run codegen:errors` regen); (b) PR #171 fp-4 `c888144` (E_CAP_NOT_IMPLEMENTED markdown 'Fix' retense in fp-3 `93be3e4` didn't regen `errors.generated.ts`; T7 parity gate caught the drift). 2 instances; sub-3-recurrence threshold.
- §3.5g already covers '4-surface ErrorCode atomic-update' including the markdown ↔ generated TS parity dimension; the gap is mini-reviewer-APPROVE doesn't exercise the codegen step. SAME LANDING as §3.5h ratification — running `node --import tsx scripts/codegen-errors.ts` + `git status` clean is one of the §3.5h-class checks.
- **Status 2026-05-09 (R6 R6-final sub-threshold-track-only verdict):** subsumed by §3.5h pre-merge gate ratification + §3.5g cross-language-rule-mirror extension; codify separately if/when 3rd recurrence fires NOT covered by either.

**pim-compound-stem-rust-mirror-gap candidate (R6-final NEW) — TS-codegen helper table edits miss the parallel Rust-test mirror that implements the same rule.**
- Origin: PR #171 fp-5 `e6aea5d` — Wave-C2 implementer added `COMPOUND_STEM_EXPANSIONS` to `scripts/codegen-errors.ts` (TS-side) but missed the Rust-side mirror at `crates/benten-engine/tests/code_to_ctor.rs:118+` that implements the same lowercase-fallback rule. Rust test panicked on `EValueFloatNonfinite/EValueFloatNonFinite` mismatch. 1 instance at HEAD.
- This is a NEW shape inside the broader cross-language rule-mirror class that §3.5g covers (post R6-final rename + extension — see §7.11b status update). Sub-class observation: when a TS-codegen helper has a Rust-test mirror (defense-in-depth per pim-1 §3.5b HARDENED), edits MUST sweep both sides. Sub-3-recurrence threshold.
- **Status 2026-05-09 (R6 R6-final sub-threshold-track-only verdict):** subsumed by §3.5g cross-language-rule-mirror generalization; codify separately if/when 3rd recurrence fires NOT covered.

**pim-cand-crypto-attestation-transport-reuse candidate (R6-final RE-CONFIRM) — wire-format extensions COMPOSE existing hardened primitives at the wire boundary, NOT introduce parallel unsigned transport.**
- Origin: G16-D wave-6b fp V2 envelope (parent-signed attestation + Acceptor + FreshnessPolicy + payload-hash binding composed at the wire boundary instead of introducing parallel unsigned transport). Named in g16d6b-fp-corr mini-review as WORTH_R6_RATIFICATION; deferred at R6 R1 + R6-final (sub-3-recurrence). 1 instance at HEAD.
- DISPOSITION per HARD RULE clause-(b) at R6-final (3+-recurrence-deep-sweep lens 2026-05-09): DEFER-WITH-CARRY — destination NOW = this §7.11b sub-threshold queue (already there per R6 R1; re-confirmed at R6-final). Pre-emptive ratification not justified at single instance; lighter-weight memory-file landing reserved for next ratification opportunity if a 2nd instance surfaces.
- **Status 2026-05-09 (R6 R6-final sub-threshold-track-only verdict; deferred per HARD RULE clause-(b)):** track at this entry; revisit if 2nd instance fires in Phase-4+ networking work (heterogeneous-cap-envelope per §6.12 item 9, peer-DID rotation envelopes, handler-attestation envelopes).

**napi shim-only-test structural-pattern gap (R6-R2 napi-bindings sub-threshold + R6-final carry; OUT-OF-SCOPE-WITH-EXPLANATION per HARD RULE clause-(a)) — JsAtrium surface methods have TS-side shim coverage but lack rust-side integration test pins.**
- Origin: R6 R2 napi-bindings lens deep-verify (`r6-r2-napi-info-1`) found Wave A's `rejoin` / `is_active` / DeviceAttestationEnvelope setters have TS-side `atrium.test.ts` pins but NO matching rust-side integration tests in `bindings/napi/tests/`. Grep across 23 *.rs files in `bindings/napi/tests/` returns 0 matches for `rejoin|is_active|set_local_device|set_acceptor`. Structural pattern: NO JsAtrium method has rust-side integration tests at HEAD — pre-existing across all of `bindings/napi/`, NOT a Wave A regression. R6 final napi-bindings re-confirmed unchanged from R2.
- DISPOSITION per HARD RULE clause-(a) at R6-final 2026-05-09: **OUT-OF-SCOPE-WITH-EXPLANATION** — phase-3-close goal is the new Phase-3 surface coverage (criteria 1/15/16 + Atrium handshake + cap-policy + iroh sync + cryptographic attestation). The structural napi-test-shape gap is (i) pre-existing across the entire JsAtrium surface (not introduced by any Phase-3 wave), (ii) covered at the SHAPE level by TS-side vitest pins that exercise the napi binding through the production cdylib path, and (iii) the rust-side integration-test gap mirrors the wider `feedback_engine_primitives_vs_application_layer` framing (most JsAtrium-via-napi behavior is application-layer composition that the TS test surface is the natural exercise point for). A targeted phase-3-close-blocking close would touch ALL ~30+ JsAtrium method surfaces in a single sweep — order-of-magnitude scope inflation for a structural shape that doesn't gate Phase-3 exit.
- **Future-Phase consideration (NOT a Phase-3 deferral):** if Phase-4+ surfaces a new contract that benefits from rust-side integration coverage of napi paths (e.g. Phase-4 contention-detection / pre-replay state-rebuild that exercises the napi boundary distinctly from TS), open a fresh §6.12 item naming the specific shape needing coverage at that time. Bulk-rewriting all 23 napi tests at Phase-3 close is the wrong tradeoff.
- **Status 2026-05-10 (R6 R6-final OUT-OF-SCOPE-WITH-EXPLANATION verdict; closes the floating disposition options surfaced at R6 R2 napi-bindings):** the disposition options "OUT-OF-SCOPE per structural-pattern framing OR BELONGS-NAMED-NOW at new phase-3-backlog §6.12 item" surfaced at R6 R2 napi-bindings JSON are now resolved to (a) OUT-OF-SCOPE-WITH-EXPLANATION per the rationale above. Tracked here for retrospective audit + future-Phase consideration trigger.

**pim-N-test-escape-valve-naming-convention candidate (R6-final RATIFY-NOW pre-emptive per 3+-recurrence lens recommendation) — `_for_test` suffix vs `with_X_for_test` prefix discipline for test-only escape valves.**
- Origin: R6 R1 hlc-r6-r1-5 surfaced 3 instances (`ucan_grounded_now_for_test`, `Hlc::with_skew_tolerance`, `Acceptor::with_parent_lookup`); R6 R1 3+-recurrence deep-sweep flagged at 2 instances + recommended pre-emptive ratification at R6-final round. R6 R2 hlc-r6-r2-4 re-affirmed BELONGS-NAMED-NOW. 2 instances of the explicit `_for_test` suffix discipline at HEAD.
- DISPOSITION per HARD RULE clause-(b) + DISAGREE-WITH-EXPLANATION combined: tracked at this §7.11b sub-threshold queue with the explicit promotion convention "promotes to ratify-inline at 3rd instance per pattern_induction lens convention." Defensible per HARD RULE clause-(c) DISAGREE-WITH-EXPLANATION — the R6 R1 3+-recurrence sweep recommended pre-emptive ratification at 2 instances, but at R6-final the orchestrator-direct codification batch is already absorbing 3 inline ratifications (§3.5h MANDATORY-PRE-MERGE + §3.5b HARDENED point 4 extension + §3.5g cross-language-rule-mirror rename). Adding a 4th inline ratification at this round risks scope-bloat for a 2-instance pattern; tracking here keeps the named destination real per HARD RULE while honoring the 3+-recurrence threshold for inline codification. The naming convention itself: test-only escape valves on a builder use the `X_for_test` suffix; test-only setter methods on a runtime type use the `with_X_for_test` prefix; both forms MUST include the `_for_test` marker so cargo-grep finds them at production-cleanup audits.
- Touch size when ratified at 3rd instance: ~5-10 LOC `dispatch-conventions.md §3.13` codification + ~30 LOC memory file.
- **Status 2026-05-09 (R6 R6-final sub-threshold-track-only verdict; closes phantom-destination per HARD RULE clause-(b) + clause-(c)):** track at this entry; auto-promote to ratify-inline at 3rd instance.
...
**Status 2026-05-09 (R6 R6-final ratification batch CLOSED for the 3 ratify-inline candidates + 5 NEW sub-threshold-track-only candidates filed):** R6-final-ratification-batch ratified inline (orchestrator-direct gitignored dispatch-conventions edits per pim-17 §3.5f handshake-discipline): (1) **§3.5h MANDATORY-PRE-MERGE-AFTER-MINI-REVIEW-APPROVE clause** — extends §3.5h pre-push gate from orchestrator-direct-only to ALSO MANDATORY before merging any fp1-PASS wave that has APPROVED mini-review; addresses the 4-instance pim-cite-drift-fp1-recurrence pattern (Wave-D fp2 + Wave-C2 fp + Wave-C1 fp2 + PR #171 fp-4/5/6 cluster); (2) **§3.5b HARDENED point 4 extension** — `#[ignore = "..."]` directives + `unimplemented!("...")` + TODO comment + module-level `//!` rationale strings subject to same post-fix doc-coupling pre-flight as path:NNN line cites; closes the 3-round-carry codification gap from R6 R1+R2+R6-final after 122+ closed substance instances across the cluster; (3) **§3.5g rename + extension** — from "ErrorCode catalog 4-surface atomic-update discipline" to "Cross-language rule-mirror atomic-update discipline" — covers ErrorCode + COMPOUND_STEM_EXPANSIONS + future cross-language-rule cases (Rust + TS lockstep). Memory files added: `feedback_pim_cite_drift_fp1_recurrence`, `feedback_pim_test_isolation_process_scoped_shared_state`, `feedback_pim_cross_language_rule_mirror`. MEMORY.md index updated. The 5 NEW sub-threshold-track-only candidates (pim-stable-rustfmt-pre-push + pim-codegen-coupling-after-doc-edits + pim-compound-stem-rust-mirror-gap + pim-cand-crypto-attestation-transport-reuse re-confirm + pim-N-test-escape-valve-naming-convention) filed above. Phantom-destination resolved for `_for_test` suffix discipline (R6-R2-hlc-4 + R6-final-hlc-1) per HARD RULE clause-(b)+(c) combined disposition.
...
**Origin (preserved for retrospective):** the cite-drift detector's `numeric_claims_source_of_truth()` table previously hardcoded the crate count `10` (bumped from 8 to 10 at orchestrator-direct cleanup 2026-05-05 closing pim-12 NEW shape iii at the immediate-fix arm). The hardcode bump closed the false-positive flood (28 findings against correctly-stated Phase-3 docs); the recurrence-resistant arm — parsing `Cargo.toml` `members =` at runtime + counting `crates/*` entries dynamically — was deferred here and is now closed. (The dynamic derivation also automatically picked up the Phase-4-Foundation 10 → 12 bump with no source-of-truth edits.)
...
**Phase-3 state update (2026-05-05):** R3-A + R3-C landed `benten-id` + `benten-sync` canary stubs taking the workspace from eight to ten crates (Phase-3-close shape; Phase-4-Foundation R6-FP-G G26-A subsequently bumped to twelve crates with `benten-platform-foundation` + `benten-renderer-tauri`); the prior eight-crate detector file was renamed + retensed to the then-current ten-crate shape at the orchestrator-direct cite-drift detector source-of-truth bump. Test stays `#[ignore]`'d; body-lift to executable still lands at G20-B per `tests/phase_3_workspace/architecture_md_g20b_final.rs`.

**Files:**
- `crates/benten-engine/tests/architecture_md_12_crate_count_post_phase_4_foundation_canaries.rs` — 2 tests (12-crate assertion enumerating benten-id + benten-sync + benten-platform-foundation + benten-renderer-tauri + dsl-compiler + native-only flag for benten-sync, + canary-crate-dir presence pin). Renamed-history: `architecture_md_8_crate_count_after_dsl_compiler.rs` → `architecture_md_10_crate_count_post_phase_3_canaries.rs` (2026-05-05 orchestrator-direct) → current Phase-4-Foundation 12-crate retense at R6-FP-G G26-A pre-tag sweep.
...
### 7.3.D — STALE-RATIONALE-NO-DESTINATION sweep #2 (Phase-3 R6 R1 fix-pass Wave E 2026-05-09; corr-r6-r1-1 BLOCKER closure)

**Origin (Phase-3 R6 R1 correctness lens, 2026-05-09):** the Phase-3 R6 R1 correctness lens (`.addl/phase-3/r6-r1-correctness.json` finding `r6-r1-corr-1`) flagged ≥70 `#[ignore]` rationales whose named gate has SHIPPED on main but whose test body still requires implementation work or remains intentionally deferred. Per §7.3.C precedent (the Phase-2b STALE-RATIONALE-NO-DESTINATION sweep closed at G20-A3 wave-8a), the discipline regressed across G14..G20 landings + was never re-swept post-G16-D / post-G17-A / post-G18-A. This is the same shape, re-applied at the Phase-3 close convergence council.

**Phase 2b precedent recap (§7.3.C, G20-A3 wave-8a, 2026-05-07):** rewrote 8 stale rationales pointing at closed/removed waves to current-state-real destinations. That sweep was the discipline-establishing close; the regression here is that subsequent Phase-3 R5 waves (G14 / G15 / G16 / G17 / G18 / G19 / G20) shipped production code but did NOT systematically retense the matching `#[ignore]` rationales when the named gate landed.

**Sweep scope (this destination):** Phase-3 R6 R1 fix-pass Wave E rewrites the rationale strings for ≥70 stale `#[ignore]` rationales in `crates/` + `tests/` + `bindings/napi/tests/` to point at this destination (or a more specific §X.Y subsection where applicable). Bodies remain `#[ignore]`'d because Wave E scope is **rationale-only** (per the brief: "NO new ErrorCode variants. NO Rust code changes (rationale strings only)."); test-body un-ignores at landing waves are out-of-scope for Wave E.

**Body landing destinations per file cluster (named here per HARD RULE clause-(b); test bodies un-ignore at the landing waves named below):**

- **`benten-engine` test bodies — durable-grant-store-threaded subscribe/wait/cap-recheck cluster** (`subscribe_cap_recheck.rs`, `subscribe_cap_recheck_concurrency.rs`, `subscribe_device_revoke.rs`, `wait_resume_cross_process.rs`, `wait_resume_policy.rs`, `ucan_replay_audience.rs`, `cap_recheck_in_flight.rs`, `cap_recheck_helper_no_refactor_on_g14d_or_g17a1_landing.rs`, `emit_broadcast_replicas.rs`, `hlc_attribution_frame.rs`, `manifest_temporal_binding.rs`) — destination: **§2.3 (i) `WriteContext` audience + clock threading for arbitrary-scope UCAN proof-chain enforcement** (v1-assessment-window). The §2.3 (i) workstream un-blocks the chain-walker accessor + audience + clock plumbing that these test bodies pin against; un-ignore lands when §2.3 (i) lands.

- **`benten-engine` test bodies — Loro merge + IVM cross-replica + CID-rebake cohort** (`inv_13_dispatch.rs`, `algorithm_b_cross_replica.rs` in `benten-ivm`; `cid_pin_loro_merged_versions.rs` + `engine_ucan_b_alignment.rs` DELETED — **CLOSED at Phase-4-Meta-Core R6 round #1 (F-073), by deletion.** The `benten-engine` test bodies named in this bullet were 100 % `unimplemented!()` placeholders whose un-ignore destination (`§4.29` / Phase-4-Foundation pre-tag) shipped 2026-05-14 without firing. The obligations they described have real green coverage elsewhere: Loro merged-Version canonical bytes at `loro_lww` / `loro_rich_type` / `mst_diff` / `node_cid` (all on the required frozen-bytes corpus); `AnchorPrefix` prefix-vs-equality at `benten-ivm/tests/algorithm_b_general.rs:119-147`; the Component-Model decision at `component_model_phase3_decision_lands_per_d_phase_3_6.rs`. `engine_ucan_b_alignment.rs` pinned `EngineGeneric::with_caps`, an API that was never minted.) — destination: **§3.1-followup Substantive end-to-end multi-peer iroh sync** (CLOSED at G16-B-E 2026-05-09 PR #160) + **§5.1 IVM Algorithm B drift-detector**. Pre-conditions for body un-ignore landed; the test bodies pin observable behaviors that the production runtime now exhibits but that need test-driver authoring (production drivers exist; test scaffolding from these specific pin angles does not).

- **`benten-engine` test bodies — Atrium discovery + AtriumHandle factory** (`atriums_no_new_primitives.rs`, `g21_t3_section_d_pins.rs`) — destination: **§6.12 G16-B post-canary residuals** (cross-references existing destination); these tests pin the "Atriums compose via existing primitives" structural invariant + G21-T3 section-D end-to-end pins. The structural invariant pin can be un-ignored at the next Phase-3-close orchestrator-direct fix-pass batch (`benten_core::PrimitiveKind` enum + `benten_eval::PrimitiveOp` variants are unchanged at Phase-3 close per the production code at `crates/benten-core/src/lib.rs` + `crates/benten-eval/src/primitives/`).

- **`benten-engine` test bodies — wasm32 bundle audit + napi cdylib erasure** (`wasm_no_redb.rs`, `no_unauthorized_dyn_error.rs`, `prop_no_state_leak.rs`, `bindings/napi/tests/wasm_bundle_content.rs`, `bindings/napi/tests/engine_b_erasure_boundary.rs`, `bindings/napi/tests/register_user_view_view_label_mismatch_post_g15_a.rs`) — destination: **§4.4 Bundle-content audit pins** (existing destination — R4b architecture / wasm-bundle lens carry; G16-B-D sub-item D NAMED-NOW). The `wasm-bundle-content-audit.yml` CI workflow named at §4.4 is the production landing site for these audits; test bodies un-ignore when that workflow lands.

- **`benten-engine` test bodies — anchor-prefix + IVM rebuild equivalence** (`view1_capability_grants.rs`, `view2_event_dispatch.rs`, `view3_content_listing.rs`, `view5_version_current.rs`) — destination: **§5.1 IVM Algorithm B maturity** (closed at G15-A / G15-B / W9-T1) + **§7.3.A.X view-registry residuals**. The `anchor_prefix.rs` file was DELETED at Phase-4-Meta-Core R6 round #1 (F-073): both its arms were `unimplemented!()` against a `§4.29` destination that shipped 2026-05-14, and the obligation it described — `AnchorPrefix` does true prefix matching, not silently-coerced label equality — has real green coverage at `benten-ivm/tests/algorithm_b_general.rs:119-147`. The remaining view pins un-ignore at v1-assessment-window when the registry + AnchorPrefix selector lift integration tests get cross-cut maintenance.

- **`benten-eval` test bodies — handler-id-router + EMIT routing + per-primitive cap enforcement** (`subscribe_handler_router.rs`, `emit_handler_router.rs`, `requires_enforcement.rs`, `requires_property_call_time_check.rs`, `toctou_iteration.rs` in `benten-caps`) — destination: **§2.3 (i)** (the per-primitive Inv-13 enforcement + handler-id-router cap-recheck threading composes with the §2.3 (i) WriteContext threading). The handler-id-router seam is wired in production at G14-D wave-5a (handler-id-router routing intent recorded via SubscribeOp `properties["handler"]` side-band pin in `crates/benten-eval/src/primitives/subscribe.rs`); the test bodies pin specific routing-behavior contracts that need driver authoring at §2.3 (i) landing.

- **`benten-sync` test bodies — cap-required-for-publish + revocation-order + wire-envelope** (`host_atrium_publish_view_result_caps.rs`, `atrium_revoke_order.rs`, `wire_envelope.rs`, `rate_limit_consumption.rs`) — destination: **§3.1-followup Substantive end-to-end multi-peer iroh sync** (CLOSED at G16-B-E PR #160) + **§6.12 G16-B post-canary residuals** for items 9 (per-device write filter) + 10 (silent-unsigned-emission setter-ordering trap). Production transport landed; test bodies pin specific defensive contracts that compose with §6.12 v1-assessment-window items.

- **`benten-id` test bodies — VC + DID rotation + device attestation + UCAN chain-construction** (`did_rotation.rs`, `device_attestation.rs`, `graph_encoded.rs`, `ucan.rs`) — destination: **§2.1-followup `ssi` external UCAN/VC spec compatibility re-evaluation at G16 Atrium handshake** (existing entry — re-evaluation point at G16; G16-D wave-6b shipped 2026-05-09 PR #163). The `RevocationSet + validate_chain_with_revocations` durable-backend integration shipped at G14-B PR #109; the specific test pins here drive against graph-Node persistence + cross-DID-rotation propagation that compose with the §2.1-followup re-evaluation.

- **`tests/integration/` + `tests/integration_browser_thin_client/` + `tests/phase_3_workspace/` — workspace-level integration drivers** (`atrium_two_process.rs`, `atrium_three_peer.rs`, `atrium_browser_thin_client.rs`, `atrium_browser_thin_client_g18_a_indexeddb.rs`, `thin_client_protocol.rs`, `paper_prototype_revalidation.rs`, `cargo_vet_policy_phase_3.rs`, `security_posture_compromises.rs`, `doc_drift_security_posture.rs`) — destinations split by file:
  - thin-client / browser e2e drivers (`atrium_browser_thin_client*`, `thin_client_protocol.rs`) — **§4.3 G18-A-followup IndexedDB integration + Playwright fixture authoring** (existing destination)
  - cross-process atrium drivers (`atrium_two_process.rs`, `atrium_three_peer.rs`) — **§3.1-followup Substantive end-to-end multi-peer iroh sync** (CLOSED at G16-B-E; the multi-peer Byzantine 3+-peer proptest pins at `atrium_three_peer.rs` un-ignore at Phase-3-close orchestrator-direct fix-pass batch when test infrastructure for 3-peer driver lands)
  - paper-prototype + cargo-vet + security-posture + doc-drift retense pins (`paper_prototype_revalidation.rs`, `cargo_vet_policy_phase_3.rs`, `security_posture_compromises.rs`, `doc_drift_security_posture.rs`) — **already CLOSED at G20-A3 / G20-B per §7.3.A.5 + §7.3.A.9** (production retense work landed); the remaining ignore'd test pins represent specific assertions whose drivers were not re-bodied at G20-A landings — un-ignore at next Phase-3-close orchestrator-direct fix-pass batch (rationale-only Wave E scope skips this LOC).

- **`bindings/napi/tests/` — napi-side parity pins** (`identity_keypair.rs`, `device_attestation.rs`, `emit_event.rs`, `attribution_frame_widening_napi_serializer.rs`) — destination: production landings already shipped (G14-A1/A2 + G19-B + G14-D wave-5a + G16-B wave-6b) for these specific surfaces; test pins remain `#[ignore]`'d because the napi-side test infrastructure for round-trip contract assertion needs the typed attestation + emit-event napi serializer driver authoring (~50-150 LOC each). **Destination: next Phase-3-close orchestrator-direct fix-pass batch OR v1-assessment-window operator-DX-cross-cut wave** (per the same v1-assessment-window framing as §2.5(f) DX-1/DX-2/DX-4); rationale-only Wave E scope skips test-body authoring.

**Touch size (Wave E rationale-only):** ~70+ rationale-string rewrites across ~50 files. Touch size at landing waves (test-body un-ignore work): ~3000-5000 LOC across the test files; landing distributes across the destinations named above.

**Wave-E disposition discipline (per `feedback_no_defer_HARD_RULE.md` 2026-05-08 update):** every rewrite explicitly names a current-state-real destination per the no-phantom-destination discipline. Rewrites that point to v1-assessment-window carry concrete coupling rationale (per the 2026-05-08 Project-frame-marker memo — v1-assessment-window destinations require explicit coupling, not stage-name-alone). Rewrites that point to existing §X.Y subsections cross-reference the current-state-real workstream named at that subsection.

**Cross-references:**
- `.addl/phase-3/r6-r1-correctness.json` — the BLOCKER finding `r6-r1-corr-1` closed by this sweep
- §7.3.C — the Phase-2b STALE-RATIONALE-NO-DESTINATION sweep precedent (G20-A3 wave-8a)
- `feedback_no_defer_HARD_RULE.md` (memory, 2026-05-08 Project-frame-marker phantom-destination guidance)
...
- `bindings/napi/src/lib.rs::testing` (the test-helper symbol catalog — `make_giant_map` / `make_cbor_bomb` / `engine_err_message`, gated at `#[cfg(any(test, feature = "in-process-test"))]`).
...
- **Co-routed with §2.3 (ii)** cap-r4-8 evaluator-delegation runtime-arm instrumentation. The two items share the same iterate-batch-boundary cap-recheck cadence mechanism: §2.3 (ii) supplies the policy-driven knob (`policy.iterate_batch_boundary()` consulted at the evaluator's iterate-batch loop today using a hardcoded constant) + corresponding metrics surface; this entry asks "what bound is correct?" Both must ship together — a policy knob without a TOCTOU re-evaluation produces a knob with no operator rationale, and a TOCTOU re-evaluation without policy-driven knobs limits response options.
- **Touch size if revisit lands:** ~50-150 LOC depending on bound shape (per-iter check vs adaptive vs unchanged-with-doc-rationale) — composes with §2.3 (ii) ~80-150 LOC for policy knob + metrics surface.
...
- `DeviceAttestationEnvelope::new_unsigned` / `new_signed` / `declared_device_did` / `to_canonical_bytes` / `from_canonical_bytes` / `verify` at `crates/benten-engine/src/engine_sync.rs:442-631` — 6-method public surface shipped at PR #163 G16-D wave-6b. **CLOSED 2026-05-10 pre-v1 Cluster 7 PR** at `crates/benten-engine/tests/device_attestation_envelope_direct.rs` (10 direct tests, ~300 LOC): round-trip `new_signed` + `new_unsigned` (preserves `declared_device_did`); `verify` success path; `verify` no-op for legacy `attestation=None`; `verify` failure paths for tampered envelope_signature / swapped payload (frame-pair binding) / replayed attestation nonce (Acceptor nonce-store) all surfacing `E_DEVICE_ATTESTATION_FORGED`; future-version rejection at `from_canonical_bytes`; malformed-bytes rejection at decode; `WIRE_VERSION` / `MAX_WIRE_VERSION` constant pin. Pairs with the existing integration-shape tests at `tests/integration/atrium_two_device.rs` (forged-DID + payload-swap + replay + future-version) that exercise the same failure-mode contracts through the full two-device sync apex. `crates/benten-sync/tests/wire_envelope.rs` remains `#[ignore]`'d (deferred to v1-assessment-window per the §6.12 G16-B post-canary residuals scope).
...
- `MerkleProof::with_tampered_node` (on `MerkleProof`, NOT `Mst`) — three callsites: `crates/benten-sync/src/mst.rs` `mod tests`, `crates/benten-sync/src/light_client.rs` `mod tests`, and the `crates/benten-sync/tests/light_client.rs` integration pin; the tampering itself (does the helper actually corrupt a node?) is not unit-tested. **No longer a public-surface gap:** the helper is `#[cfg(any(test, feature = "testing"))]`-gated OFF the frozen v1-beta surface (R6 tail fold-in F09), so only the un-pinned-tampering half of this row remains open.
...
**Status:** CLOSED 2026-05-10 pre-v1 Cluster 7 PR. Production fix landed at `bindings/napi/src/trace.rs::trace_step_to_json` (~50 LOC added including comments + per-slot skip-on-default discipline) emitting `peerDidSet` / `deviceDid` / `syncHopDepth` camelCase keys mirroring the Rust producer's `AttributionFrame::cid()` canonical-bytes shape. Inline `#[cfg(test)] mod tests` at `bindings/napi/src/trace.rs::tests` carries 3 LOAD-BEARING pins (full-population emit + omit-when-default + per-slot skip). Sibling integration-test grep-witness pins at `bindings/napi/tests/attribution_frame_widening_napi_serializer.rs` (4 tests) defend the production-source emission sites against regression. TS interface at `packages/engine/src/types.ts::AttributionFrame` dropped the phantom `deviceCid?: string` slot (never had a Rust producer) + added the real `syncHopDepth?: number` slot. TS-side schema declaration pins at `packages/engine/test/attribution_frame_widening.test.ts` (5 tests) exercise the optional-field type contract.

**DISAGREE-WITH-EXPLANATION (HARD RULE clause-c) — `device_cid` vs `sync_hop_depth`:** the original §13.9 brief + the pre-fix TS interface declared a phantom `deviceCid` slot as the third Phase-3 widening field. The Rust producer `AttributionFrame` at `crates/benten-eval/src/exec_state.rs` actually carries `sync_hop_depth: u32`, NOT `device_cid`. The `device_cid` slot lives on `Engine` / `WriteContext` / `ReadContext` but is never woven into `AttributionFrame`. The Cluster 7 closure emits the 3 fields that ACTUALLY exist on the producer (`peer_did_set` / `device_did` / `sync_hop_depth`) and drops the phantom `deviceCid` TS slot. The §13.9 entry's narrative below is left intact for retrospective traceability but the on-the-wire reality is the 3 fields named above. The negative-pin test `napi_trace_serializer_does_not_emit_phantom_device_cid_key` is the regression guard against re-introduction of the phantom.

**Note re fix-shape sub-item (d) drift-defense lint:** NOT landed in Cluster 7. The cross-language rule-mirror discipline §3.5g + pim-N-cross-language-rule-mirror is the structural defense for now (every public `AttributionFrame` field is covered by the inline trace.rs tests + the integration grep-witness pins). A workspace-wide automated lint remains a v1-assessment-window candidate per §3.5g + pim-6's CI-infrastructure half (CI-infra item in §7.11).
...
The workspace guard `is_for_test_pattern` in `tests/phase_3_workspace/for_test_symbols_are_feature_gated.rs` (`:351`) currently catches only the `_for_test` / `_for_testing` / bare-`for_test` / `test_` / `_test_` / `mock_` / `inject_` degenerate-constructor / test-seam name patterns (the bare-`for_test` arm landed at the R6 fix-pass wave — see V1-FROZEN-INTERFACE-DEFERRED.md Row D-74). It does NOT catch the `from_raw*` / `_unchecked` constructor-name family — a `pub fn from_raw_bytes` / `from_raw` / `*_unchecked` degenerate constructor that bypasses an invariant would slip past the scanner. Widen the pattern set to also match `from_raw` / `_unchecked` (word-boundary aware so it does not over-match unrelated identifiers), and add explicit `EXEMPT_PUB_ITEMS` allowlist entries for any legitimately-public `from_raw*` / `_unchecked` constructors already on the frozen surface so the widened scanner stays green. Distinct from Row D-74 (which widens for the bare `for_test` form); this row widens for the raw/unchecked constructor family. Cross-ref: `docs/V1-FROZEN-INTERFACE.md` verify-item-1 (backstop roster). Destination: v1-GM backstop-completeness hardening (with Row D-74).

---

### 15.7 widen the frozen-surface leak scanner to catch `*KatVector` (and sibling) test-fixture struct name patterns (R18 FZ-KAT-LEAK, companion to the C4 cfg-gate)

**Disposition:** BELONGS-NAMED-NOW per HARD RULE rule-12 clause-(b). Backstop-completeness hardening; no wire change.

R18 C4 cfg-gated the two test-only KAT-fixture structs (`SignatureKatVector` / `KemKatVector` in `crates/benten-crypto-suite/src/swap_matrix.rs`) off the default-feature public-api surface via `#[cfg(any(test, feature = "testing"))]`, because they had leaked onto the frozen baseline (`docs/public-api/benten-crypto-suite.txt`) even though their loaders were already `testing`-gated. That was a manual catch — no scanner asserts that a `*KatVector` (or similarly-named test-fixture) `pub struct` is gated OFF the default surface. Widen the frozen-surface leak scanner (the `for_test_symbols_are_feature_gated.rs` family) to flag `pub` items whose type name matches a test-fixture pattern set (`*KatVector`, `*TestVector`, `*Fixture`, `*Kat`) unless they are explicitly `#[cfg(any(test, feature = "testing"))]`-gated or on an `EXEMPT_PUB_ITEMS` allowlist — so a future KAT/test-vector fixture cannot silently re-leak onto the frozen baseline. Companion to the C4 cfg-gate (which fixed the two current leaks). Cross-ref: `docs/V1-FROZEN-INTERFACE.md` verify-item-1 (backstop roster); Row D-74 / §15.6 (the sibling scanner-widening rows). Destination: v1-GM backstop-completeness hardening (with §15.6 / Row D-74).
