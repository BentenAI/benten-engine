...
- `wait_signal_shape_optional_typing.rs:98` — RE-IGNORED at G20-A3 with re-targeted rationale (the deferral condition "runtime check addition in `wait::evaluate_op` lands Phase 3" IS satisfied — runtime shape-match wired end-to-end at `crates/benten-eval/src/primitives/wait.rs:345` SUSPEND-time + `resume_with_meta` resume-time + `SuspensionStore` round-trip). The re-ignore reason is a routed_edge_label classification mismatch: `ErrorCode::InvRegistration` is routed to `None` in `routed_edge_label` (lib.rs:1045-1062 group), but the test asserts `Some("ON_ERROR")`. Either the test's expected edge label needs adjustment OR the variant needs reclassification under the runtime-error group. Bundle with the next round of routed_edge_label classification hardening (reassessment cadence: v1-assessment-window per CLAUDE.md baked-in #15).
