...
- `tests/integration/atrium_two_device.rs::atrium_two_device_same_identity_selective_zone_sync` — exit-criterion 16 multi-device-same-identity. **CLOSED 2026-05-09 G16-D wave-6b** — un-ignored + GREEN. Two engines under same actor_cid + distinct device_cids/device-DIDs sync bidirectionally over real iroh transport with on-the-wire `DeviceAttestationEnvelope` carrying originating-device-DID; receiver-side AttributionFrame.device_did reflects the ORIGINATING device per Inv-14 device-grain attribution. Cap-denial halves (heterogeneous-cap-envelope per-device write filter — phone CANNOT write to /zone/notes) remain BELONGS-NAMED-NOW carry to §6.12 item 5 G14-D follow-up (G14-D heterogeneous-cap-envelope wave keys its filter on the device-DID-attestation envelope this wave landed).
...
- `.addl/phase-3/00-implementation-plan.md §1` exit criteria 1 ✅ + 15 ✅ + 16 ✅ (multi-device-same-identity CLOSED at G16-D wave-6b — on-the-wire device-DID-attestation envelope landed; the cap-denial halves of the original test pin steps 3+6 carry to G14-D heterogeneous-cap-envelope wave per §6.12 item 5)
