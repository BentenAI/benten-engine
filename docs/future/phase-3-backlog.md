...
5. **D-PHASE-3-19a pinned-CID rebake — AttributionFrame.device_did convention shift `device-cid:<hex>` → `did:key:<resolved>`.** G16-B-prime's `apply_atrium_merge` populates `AttributionFrame.device_did` with the synthetic-string convention `device-cid:<hex(device_cid)>` pre-G16-D-handshake-protocol-body landing. Once the production trust-store promotes device-CID → resolved `did:key:<…>` form (G16-D wave-6b), the convention shift will mutate the canonical bytes of every minted AttributionFrame at sync-merge boundaries. **NAMED-NOW destination for the D-PHASE-3-19a pinned-CID rebake cohort entry.** Touch site: G16-D wave-6b implementer adds the `device-cid:` → `did:key:` translation at `engine.rs:apply_atrium_merge` + rebakes any pinned-CID fixtures asserting the legacy `device-cid:<hex>` shape (currently: `sync_replica_attribution_carries_device_did_alongside_parent` constructs the expected frame with `device-cid:<hex>`; `sync_replica_explicit_actor_cid_decouples_from_device_cid` does the same). Both tests + their pinned `expected.cid()` fixtures are part of the rebake cohort. **OPEN — to be closed at G16-D wave-6b alongside handshake-protocol-body landing.**

6. **sec-r4r1-2 BLOCKER — sync-replica WRITE per-write cap-recheck-at-delivery (mirrors SUBSCRIBE side per CLR-2).** Two RED-PHASE pins at `crates/benten-engine/tests/sync_replica_attribution.rs`: `sync_replica_write_cap_recheck_at_delivery_against_local_grant_store` + `sync_replica_write_after_local_grant_revoke_post_handshake_rejected_with_e_sync_revoked_during_session`. Both pins assume a `Engine::consume_sync_replica_message` API + a `bob_engine.caps()` / `caps().install_proof()` / `caps().revoke()` API surface + a `metrics().sync_replica_cap_recheck_calls()` metric + a typed `EngineError::SyncRevokedDuringSession` variant — none of which exist at HEAD (G16-B-D filed via `wave-g16-b-d`). Production API today is `Engine::apply_atrium_merge` (not `consume_sync_replica_message`); the existing capability-grant-installation path is via `CapabilityPolicy::pre_write` hooks (not a `caps()` accessor returning a grant-mutation surface). **Substantial wiring (>200 LOC + new ErrorCode + new metrics field + new caps-surface API)** — out of scope for G16-B-D's ~300-600 LOC budget. **NAMED-NOW destination** for both pins per HARD RULE rule-12 clause-(b). Concrete fix-shape:
   - (a) Wire `cap_recheck.rs` G13-pre-C helper into `apply_atrium_merge`'s per-write loop (the merge produces N row-WRITEs from the Loro doc; each must per-row recheck the originating peer's cap-set against the receiving engine's `CapabilityPolicy::pre_write` surface).
   - (b) Add `EngineError::SyncRevokedDuringSession { peer_did, zone, cid }` variant + `ErrorCode::SyncRevokedDuringSession` (4-surface atomic update per §3.5g).
   - (c) Expose a `caps()` accessor returning a grant-mutation surface (or thread the existing `CapabilityPolicy` reference through so tests can install + revoke at the policy layer).
   - (d) Surface a `metrics().sync_replica_cap_recheck_calls()` counter (additive metric; cap-r4-3 / r4b-cap-4 reinforcement).
   - (e) Un-ignore both pins; assert the metric increments + the typed-error-rejection shape end-to-end.
   **Rationale for BELONGS-NAMED-NOW (not in-wave):** the API gaps are non-additive — `consume_sync_replica_message` is a different routing surface than the production `apply_atrium_merge` (the merge callback does NOT today route through a per-message dispatcher — it's a direct Loro-bytes apply). Wiring per-write cap-recheck requires either (i) refactoring `apply_atrium_merge` to thread `CapabilityPolicy` through the per-row apply loop OR (ii) introducing a parallel `consume_sync_replica_message` entry point that wraps `apply_atrium_merge`. Either path is a substantive arch decision (whose call site is the cap-recheck — engine vs the merge callback?) that warrants Ben ratification + a dedicated wave.
   **Convergence target:** Phase-3 close-blocking (the SUBSCRIBE side has 3 cap-recheck pins; the symmetric sync-replica WRITE side currently has ZERO; closure narrative for sec-r4r1-2 BLOCKER requires both halves landed pre-tag). Touch size: ~250-400 LOC across `engine_sync.rs` (per-write recheck loop) + `error.rs` (new variant) + `engine.rs` (caps accessor) + 2 test pins un-ignored. **OPEN — to be closed at G16-B-E iroh-substantive wave OR a dedicated sec-r4r1-2-BLOCKER wave before R6 phase-close convergence council.**
