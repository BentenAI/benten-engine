...
|-----------|----------------|
| 4 — SANDBOX nest depth | **CLOSED-IN-PHASE-2B.** Both arms active: registration-time `validate_registration` + runtime `AttributionFrame.sandbox_depth` threading wired at R6FP-G1 (PR #62). Adversarial integration test (`sandbox_escape_attempts_denied.rs::sandbox_escape_reentrancy_via_host_fn_denied`) is LIVE (`#[test]`) — un-ignored at Phase-3 G20-A1 wave-8a; the [`phase-3-backlog.md`](phase-3-backlog.md) §7.3.A.7 carry is CLOSED. |
...
### 9.2 Subgraph AST cache — **CLOSED-IN-PHASE-3-G19-E**

Full wire-up landed at Phase-3 R5 wave-7b G19-E. The Phase-2a G2-B subgraph-template cache (`SubgraphCache` keyed on `(handler_id, op, subgraph_cid)`) memoised the structural shape but did NOT eliminate per-call parsing of TRANSFORM expression source. G19-E adds a sibling [`crates/benten-engine/src/ast_cache.rs::AstCache`] keyed on `(handler_cid, node_id)` that stores the parsed `benten_eval::expr::Expr` AST for every TRANSFORM node in a registered handler. Population happens at `register_subgraph` / `register_subgraph_replace` time (synchronous parse walk through every `PrimitiveKind::Transform` node); invalidation drops the prior version's entries before the swap. The dispatch path consults the cache via the new [`benten_eval::PrimitiveHost::cached_transform_ast`] hook — the engine's `impl PrimitiveHost for Engine` resolves the active call's `handler_cid` from the `active_call` stack and returns the cached `Arc<Expr>`. Defensive re-parse remains the fallback for `NullHost` + every test host (default trait impl returns `None`).

Pinned by:
- [`crates/benten-engine/tests/ast_cache.rs::subgraph_ast_cache_full_wire_up`] — G19-E core wire-up (drives `Engine::call` 100× and asserts hit_count == 100, miss_count == 0; defends against the "cache wired but never consulted" failure mode per pim-2 §3.6b).
- [`crates/benten-engine/tests/ast_cache.rs::subgraph_ast_cache_correctness_under_handler_re_register`] — load-bearing correctness pin (re-registering with different bytes flips `handler_cid`, drops the prior version's entries, populates fresh entries for the new version).
- [`crates/benten-engine/tests/ast_cache.rs::subgraph_ast_cache_per_call_parse_cost_reduction`] — counter-based perf pin (1000 dispatches; asserts every dispatch served via cache lookup, zero parse fall-throughs, subgraph_cache template_parse ≤ 1).
- [`crates/benten-engine/tests/ast_cache.rs::engine_call_no_residual_todo_marker`] — C-14 architectural pin retiring the in-code TODO marker at `register_subgraph`.
- [`crates/benten-engine/tests/ast_cache.rs::subgraph_ast_cache_preserves_stream_execute_loud_fail_for_engine_call_path`] — stream-r1-3 cross-pin (the cache populate filter rejects non-TRANSFORM kinds; STREAM loud-fail at `Engine::call` survives the wave).
- [`crates/benten-engine/tests/ast_cache.rs::subgraph_ast_cache_preserves_subscribe_execute_loud_fail_for_engine_call_path`] — stream-r4r1-9 cross-pin (asserts populate filter produces zero entries for SUBSCRIBE-bearing handlers; sharper than the original R3-E stub which mis-stated the SUBSCRIBE arm as loud-failing — see test rustdoc for the HARD RULE rule-12 DISAGREE-WITH-EXPLANATION narrative).
