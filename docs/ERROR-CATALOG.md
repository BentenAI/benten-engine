...
>
> The B8 DAG-CBOR checker at `bindings/napi/src/input_limits.rs::limit()` — reached from `scan_dag_cbor` (map / list / bytes / text / depth / declared-length / item-count) and `parse_cid_string_bounded` (CID shape) — **guards the in-process test harness only, and does NOT fire in the shipped cdylib.** It compiles under `napi-export`, but its sole callers are `testing::deserialize_value_from_js_like` / `deserialize_cid_from_js_like` (`lib.rs:2444`, `:2453`) inside `#[cfg(any(test, feature = "in-process-test"))] mod testing`, and `default = ["napi-export"]` does not enable `in-process-test`. **Compiling is not firing.** B8 closed the Phase-1 R3 contract exactly as that contract was written — against `benten_napi::testing::*` — which is a real closure of a real gap, and is not the same claim as production coverage.
>
