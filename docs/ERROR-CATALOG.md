...
**Catalog count narrative (post Phase-4-Meta-Core G-CORE-9 R1 fix-pass + R6-R2-FP G-COMP-1 cohort + F-full MembershipSet mints, HEAD `fdfda621`):** four distinct counts coexist by design — each measures a different surface:

| Count | Source | Value (at HEAD `fdfda621`) | Meaning |
|---|---|---|---|
...
- **Phase-4-Meta-Core F-full MembershipSet mints:** +1 `E_ROLE_STALE_AT_VERIFY` (w-ms-canary; BC-5 role-staleness verify gate) → 198; +1 `E_KV_TARGET_NOT_IMMUTABLE` (w-gov-audit; F-INV19-1 / Inv-19 `K(V)` immutable-target reject) → **199 throwable at HEAD `fdfda621`**. ⚠️ A parallel F-full CODE wave may mint one more (F-01) → **200**.
