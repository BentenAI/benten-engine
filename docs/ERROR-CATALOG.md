...
- **Fix:** The TS → Rust boundary rejects oversized or pathologically-nested inputs to prevent DoS, in a pre-scan over the raw wire bytes that runs BEFORE the canonical decoder is handed anything (for an allocation attack the allocation *is* the attack, so a check that runs after the tree is built has already lost). Enforced limits, DAG-CBOR path (`bindings/napi/src/input_limits.rs`): `Value::Map` 10 000 keys, `Value::List` 10 000 items, `Value::Bytes` 16 MiB, `Value::Text` 1 MiB, total payload 16 MiB, aggregate item count 1 000 000, and any collection header declaring more children than the remaining bytes can hold. Either simplify the input or split the call.
  - **Nesting depth is 64 on the CBOR path, not 128.** `NAPI_MAX_DEPTH` is derived from `benten_core::MAX_VALUE_DECODE_DEPTH` (byte-pinned at 64 by `canonical_bytes_v1_const_values_core.rs`) — a napi cap of 128 could never fire there, because the canonical decoder refuses first. **The JSON path (`bindings/napi/src/node.rs`) still enforces `JSON_MAX_DEPTH = 128`, and that asymmetry is a live defect:** `Value`'s encoder has no depth guard while its decoder stops at 64, so a property bag nested 65–128 deep submitted through `engine.putNode(...)` is accepted, hashed and persisted — and then cannot be decoded on read. Aligning `JSON_MAX_DEPTH` to 64 narrows the accept-set of a shipped API, so it is **surfaced for Ben, not decided here** (R6 round #1, B8).
  - **There is no configurability knob.** A prior version of this entry said "Limits are configurable via the engine builder". No builder method, no config struct, nothing — that sentence described a mechanism that does not exist and is removed rather than retensed (`feedback_compromise_ledger_not_exempt_from_hard_rule`, FALSE-RECORD class).
  - **`subgraph_bytes` / `node_count` / `edge_count` were listed and are NOT implemented anywhere**, and the "subgraph pre-parse bytes 1MB" default had no enforcing code. They are removed from the `limit_kind` union above; if that bound is wanted it needs building, not documenting.
