...
**Phase-4-Meta-Core mint trajectory:** 170 (Phase-4-Foundation close) → 171 (#989 storage-partition seam `E_NAMESPACED_WRITE_UNSUPPORTED`) → ... → 192 (G-CORE-9 R1 fix-pass) → 197 (R6-R2-FP G-COMP-1 cohort 8: `+4` `ON_ERROR`-routed codes) → 198 (`E_ROLE_STALE_AT_VERIFY`, F-full w-ms-canary) → **199** (`E_KV_TARGET_NOT_IMMUTABLE`, F-full w-gov-audit; Inv-19) via the G-CORE-1/2/3/4/5/6/7/8/10/DSL + F-full MembershipSet ErrorCode mints. Per-mint cohort detail in commit messages PR #1304..#1364.
...
- **Phase-4-Meta-Core R6-R2-FP G-COMP-1 cohort 8:** +5 `ON_ERROR`-routed codes (the G-COMP-1 wave cluster). 192 + 5 = **197 throwable**.
