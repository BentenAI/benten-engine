...
**Catalog count narrative (post Phase-4-Meta-Core G-CORE-9 R1 fix-pass + R6-R2-FP G-COMP-1 cohort + F-full MembershipSet mints, HEAD `b93b2efc`):** four distinct counts coexist by design — each measures a different surface:

| Count | Source | Value (at HEAD `b93b2efc`) | Meaning |
|---|---|---|---|
| **Throwable enum variants** | `crates/benten-errors/src/lib.rs::ErrorCode` (minus `Unknown(String)` fallback) | **201** | What the engine can actually emit at runtime. Authoritative source of THROWABLE variants. |
| **Regression-list entries** | `crates/benten-errors/catalog_roster.rs.in` (included by `crates/benten-errors/src/lib.rs::catalog_roster_pin` and by `crates/benten-errors/tests/stable_shape.rs::ALL_CATALOG_VARIANTS` / `CATALOG_VARIANT_COUNT`) | **201** | The round-trip-pinned list. Matches the throwable enum 1:1 because the lib-side expansion is a **wildcard-free** `match` — a variant in the enum and not in the roster is `error[E0004]`. (Corrected 2026-07-27, F-014: the former `catalog_variant_count_matches_enum` "dual tripwire" cited here was a tautology and is deleted.) |
| **Catalog entries (this doc + TS classes)** | `### E_XXX` headings here + `packages/engine/src/errors.generated.ts` CATALOG_CODES | **203** | = 201 throwable + `E_UNKNOWN` (forward-compat sentinel mirroring Rust's `Unknown(String)` fallback) + `E_INV_ITERATE_NEST_DEPTH` (Phase-2a-retired ITERATE-nest-depth stopgap; catalog ID stays reserved across phases per the retention discipline at line ~112). |
| **Rust enum entries** | `ErrorCode` enum (incl `Unknown(String)`) | **202** | = 201 throwable + 1 `Unknown(String)` forward-compat fallback. No `InvIterateNestDepth` variant (removed at Phase-2a-open when `E_INV_ITERATE_BUDGET` multiplicative form superseded it; catalog heading retained at line ~112 for backward-compat string round-trip). |

**Why four counts (201 / 201 / 202 / 203):** the Rust enum is the source of throwable variants (201); plus a forward-compat `Unknown(String)` fallback (= 202 in rust); the TS catalog + this doc additionally retain 1 Phase-2a-retired catalog ID (= 203 in catalog/ts); the test list at stable_shape.rs::ALL_CATALOG_VARIANTS round-trips the throwable subset (201). Single canonical headline number: **201 production-throwable codes** (200 at the F-INJ-2 Drop envelope-issuer anchor + `E_RECIPIENT_KEM_NOT_COMMITTED`, the GAP-KDB Shape-B recipient-binding closure, Inv-23). ⚠️ Parallel GAP-KDB waves (W2 benten-drop flagship uses the crate-local `RecipientBindingError`, NOT a throwable) do not double-mint; the strategy-C integrator reconciles the count at integrate-time per the historical #1319↔#1318 collision pattern.

**Phase-4-Meta-Core mint trajectory:** 170 (Phase-4-Foundation close) → 171 (#989 storage-partition seam `E_NAMESPACED_WRITE_UNSUPPORTED`) → ... → 192 (G-CORE-9 R1 fix-pass) → 194 (R6-R1 FP-F4 §S3a/§S3b: `+2` plugin install-consent / per-delegation codes) → 197 (R6-R2-FP G-COMP-1 cohort 8: `+3` `ON_ERROR`-routed DSL codes) → 198 (`E_ROLE_STALE_AT_VERIFY`, F-full w-ms-canary) → 199 (`E_KV_TARGET_NOT_IMMUTABLE`, F-full w-gov-audit; Inv-19) → 200 (`E_DROP_BUNDLE_ENVELOPE_ISSUER_MISMATCH`, F-INJ-2) → **201** (`E_RECIPIENT_KEM_NOT_COMMITTED`, GAP-KDB Shape-B; Inv-23) via the G-CORE-1/2/3/4/5/6/7/8/10/DSL + F-full MembershipSet ErrorCode mints. Per-mint cohort detail in commit messages PR #1304..#1364.
...
- **Phase-4-Meta-Core G-CORE-3 cluster + DSL chunk-3 + pre-FREEZE bundle + G-CORE-9 build-out wave:** +22 new codes across the S&C primitive + sealed-CapabilityPolicy + crypto-suite-swap-matrix + DSL Backend/Io classes + walker + audit-gated pure-PQ + chain validator + manifest envelope rechecker + plugin install record clusters (`RecipientLacksKeysForSuite` + `ValueOutOfRange` + AEAD/two-CID/per-chunk cluster + `DslBackendRejected` + `DslIoError` + `AuditNotLandedPurePqRejected` + `SubgraphSpecWalkFailed` + chain-narrowing + binding-sig + install/runtime-delegation envelope + manifest envelope rechecker arms + thin-client bridge + write-boundary chain validator + audience-aware caps). 170 + 22 = **192 throwable at the Phase-4-Meta-Core G-CORE-9 FREEZE milestone**.
- **Phase-4-Meta-Core R6-R1 FP-F4 §S3a/§S3b + R6-R2-FP G-COMP-1 cohort 8:** +2 plugin install-consent / per-delegation codes (FP-F4 §S3a/§S3b; Row D-3-a / D-3-b closures — these two are NOT `ON_ERROR`-routed) then +3 `ON_ERROR`-routed codes (the G-COMP-1 wave cluster, Row D-19). 192 + 2 + 3 = **197 throwable**.
- **Phase-4-Meta-Core F-full MembershipSet mints:** +1 `E_ROLE_STALE_AT_VERIFY` (w-ms-canary; BC-5 role-staleness verify gate) → 198; +1 `E_KV_TARGET_NOT_IMMUTABLE` (w-gov-audit; F-INV19-1 / Inv-19 `K(V)` immutable-target reject) → 199; +1 `E_DROP_BUNDLE_ENVELOPE_ISSUER_MISMATCH` (F-INJ-2 Drop envelope-issuer anchor) → 200; +1 `E_RECIPIENT_KEM_NOT_COMMITTED` (GAP-KDB Shape-B recipient-binding closure; Inv-23) → **201 throwable — LANDED**, matching the preamble table above and the `CATALOG_VARIANT_COUNT == 201` pin at `crates/benten-errors/tests/stable_shape.rs`.

Authoritative count assertion lives in `crates/benten-errors/catalog_roster.rs.in`, consumed by two length pins that expand from it: `variant_count_is_pinned` (`tests/stable_shape.rs`, via `CATALOG_VARIANT_COUNT`) and `catalog_roster_length_is_pinned` (`src/lib.rs::catalog_roster_pin`). The lib-side expansion is a wildcard-free `match` over `ErrorCode`, so a variant added to the enum without a roster entry is `error[E0004]` — a compile failure, not a runtime miss. **Corrected 2026-07-27 (F-014):** the previously-cited `catalog_variant_count_matches_enum` "exhaustive-match cross-check" was neither exhaustive (`ErrorCode` is `#[non_exhaustive]`, so a downstream `match` must carry `_`) nor a cross-check (it filtered the same list it counted); the R6 round-#1 falsification sweep left it green under a fully-wired new variant, and it is deleted.
...
> **Status (stated precisely, because the distinction is the whole point).** `E_INPUT_LIMIT` has real production firing sites — `bindings/napi/src/node.rs` (JSON depth / map-key count / string length, on the live `createNode`/`updateNode` path), `bindings/napi/src/view.rs`, and `benten-eval/src/chunk_sink.rs`. Those fire today.
>
> The B8 DAG-CBOR checker at `bindings/napi/src/input_limits.rs::limit()` — reached from `scan_dag_cbor` (map / list / bytes / text / depth / declared-length / item-count) and `parse_cid_string_bounded` (CID shape) — **guards the in-process test harness only, and does NOT fire in the shipped cdylib.** It compiles under `napi-export`, but its sole callers are `testing::deserialize_value_from_js_like` / `deserialize_cid_from_js_like` (`lib.rs`) inside `#[cfg(any(test, feature = "in-process-test"))] mod testing`, and `default = ["napi-export"]` does not enable `in-process-test`. **Compiling is not firing.** B8 closed the Phase-1 R3 contract exactly as that contract was written — against `benten_napi::testing::*` — which is a real closure of a real gap, and is not the same claim as production coverage.
>
> Wiring a production entry point for the DAG-CBOR path is tracked as a named residual; until it lands this entry must not be read as saying the cdylib bounds attacker-supplied DAG-CBOR. The `reachability: ignore` annotation below reflects the drift-detector scanning `crates/*/src/` only (per `phase-3-backlog §10.7`), not an absent firing site — the `node.rs` sites above are genuine.
>
> **Provenance of the depth bound (kept here, out of the user-facing `Fix:` hint, because it is our record rather than the caller's instruction).** Nesting depth is **64 on every path**, and on both paths it is *derived*, not written: `NAPI_MAX_DEPTH` and `JSON_MAX_DEPTH` both read `benten_core::MAX_VALUE_DECODE_DEPTH` (byte-pinned at 64 by `canonical_bytes_v1_const_values_core.rs`). The JSON path previously carried a literal `128`, and that asymmetry was a **live data-loss defect**: `Value`'s encoder has no depth guard while its decoder stops at 64, so a property bag nested 65–128 deep submitted through `engine.putNode(...)` was accepted, hashed and persisted — and then could not be decoded on read. Closed by derivation (W2 residual, `bindings/napi/src/node.rs`), which makes the mismatch unrepresentable rather than merely corrected, and guarded by `crates/benten-core/tests/no_boundary_accepts_deeper_than_the_decoder_returns.rs` — a scan over the CLASS of `*_MAX_DEPTH` declarations with a vacuity floor, proven by mutation.
>
> Two claims were **removed** from the `Fix:` hint rather than retensed, because both described mechanisms that do not exist: *"Limits are configurable via the engine builder"* (no builder method, no config struct — the limits are fixed constants) and the `subgraph_bytes` / `node_count` / `edge_count` limit kinds with a *"subgraph pre-parse bytes 1MB"* default (no enforcing code anywhere; also struck from the `limit_kind` union below). If either bound is wanted it needs building, not documenting.

<!-- reachability: ignore -->
<!-- Rationale: construction site is `bindings/napi/src/input_limits.rs::limit()` (the bounded pre-scan enforcing map / list / bytes / text / depth / declared-length / item-count limits) and `parse_cid_string_bounded` (CID shape). The drift-detector's scan path scopes to `crates/*/src/` — same scanner asymmetry as E_RELOAD_SUBSCRIBER_UNSUBSCRIBED + E_DEVSERVER_STOPPED + E_STORAGE_QUOTA_EXCEEDED below. Remove this annotation if/when (a) a `crates/`-resident construction site is added (e.g., a shared limits module in `benten-core`) OR (b) `phase-3-backlog §10.7` widens the detector's scan path to include `bindings/napi/src/`. -->

- **Message:** "Napi boundary input exceeds {limit_kind} limit: {actual} > {max}"
- **Context:** `{ limit_kind: "map_size"|"list_size"|"bytes_len"|"text_len"|"nesting_depth"|"total_bytes"|"total_items"|"declared_length"|"cid_shape", actual: number, max: number }`
- **Fix:** The TS → Rust boundary rejects oversized or pathologically-nested inputs to prevent DoS, in a pre-scan over the raw wire bytes that runs BEFORE the canonical decoder is handed anything (for an allocation attack the allocation *is* the attack, so a check that runs after the tree is built has already lost). Enforced limits: `Value::Map` 10 000 keys, `Value::List` 10 000 items, `Value::Bytes` 16 MiB, `Value::Text` 1 MiB, total payload 16 MiB, aggregate item count 1 000 000, nesting depth 64, and any collection header declaring more children than the remaining bytes can hold. Either simplify the input or split the call. The limits are fixed constants — there is no configuration knob for them.
...
- **Context:** `{ view_id: string }`
- **Fix:** D8-RESOLVED (Phase 2b). The `Strategy::Reserved` slot (Z-set / DBSP cancellation algorithm; renamed from the prior `Strategy::C` spelling at G23-0a + Row D-19 G-COMP-1 wave Cohort 8 atomic 4-surface rename) is reserved for Phase 3+; refused at registration time. Use `Strategy::B` (or omit the field; user views default to B).
- **Thrown at:** `Engine::create_view` registration (G8-B)
- **Phase:** 2b (mint); renamed at Row D-19 G-COMP-1 wave Cohort 8 (Phase-4-Meta-Core R6 R2 FP integration) — wire string `E_VIEW_STRATEGY_C_RESERVED` → `E_VIEW_STRATEGY_RESERVED`; variant `ViewStrategyCReserved` → `ViewStrategyReserved`; TS class `EViewStrategyCReserved` → `EViewStrategyReserved`.
...
- **Fix:** One of two classes. **(a) Structural validation** — module bytes failed wasmtime structural validation (malformed module, type mismatch, OOB section, OOB linear-memory read, recursion-depth overflow, etc.). Audit the module compiler output. ESC-1 / ESC-3 / ESC-5 / ESC-11 / ESC-12 escape vectors all route here. **(b) SANDBOX-ABI incompatibility** — the bytes are valid wasm, but the module's exported `run` entry does not fit the ABI: there is no `run` export at all, or `run` returns a result type the ABI cannot encode. The return ABI serialises scalar results (`i32` / `i64` / `f32` / `f64`) little-endian; a `v128` (wasm SIMD) result, or any reference type (`funcref` / `externref` / `anyref` / `exnref` / `contref`), is REJECTED with this code rather than encoded — return a scalar, or write the result into the module's linear memory and return its offset. Store/linker setup failures (`set_fuel`, host-fn registration) also route here. Class (b) is long-standing; it is spelled out here because the prior wording named only class (a) and so understated the code's real domain.
- **Thrown at:** SANDBOX executor — `Module::new` / link / instantiation, plus the entry-resolution and return-ABI gate in `crates/benten-eval/src/primitives/sandbox.rs`.
- **Phase:** 2b G7-A
...
- **Fix:** SANDBOX guest module's call stack exceeded the configured `max_wasm_stack` ceiling (default 512 KiB; matches wasmtime's `Config::max_wasm_stack` default). Distinct from `E_SANDBOX_FUEL_EXHAUSTED` (CPU-bound runaway) and `E_SANDBOX_MODULE_INVALID` (structural validation failure) — stack-overflow-via-recursion is its own observable class so operator dashboards can distinguish a benign-but-buggy recursive guest from a generic invalid module. Either reduce module recursion depth or audit for adversarial recursion. **Raising `SandboxConfig::max_wasm_stack` does NOT work and never did**: the ceiling is applied to the process-wide `OnceLock<wasmtime::Engine>` at first SANDBOX use, and wasmtime resolves `max_wasm_stack` at `Engine` construction, so the per-call field is inert (it is retained only because removing a `pub` field would be a breaking narrowing of the frozen v1-beta surface). The enforced ceiling is `benten_eval::sandbox::ENGINE_MAX_WASM_STACK_BYTES`, and the error payload reports that enforced value, not your request. Phase-3 G17-A1 wave-5b mints the dedicated typed variant per phase-3-backlog §6.4 + r1-wsa-7 BLOCKER closure (the prior R6FP-G1 r6-wsa-8 BELONGS-NAMED-NOW deferral is honored here).
...
- **Thrown at:** `crates/benten-engine/src/engine_sync.rs::AtriumHandle::merge_remote_change_with_hop_depth` constructs `Err(AtriumError::SyncHopDepthExceeded)` at the chain-bound checks (lines ~1748 + ~1753); the routing arm at `engine_sync.rs::744` (inside `AtriumError::code`) maps `AtriumError::SyncHopDepthExceeded` to `ErrorCode::SyncHopDepthExceeded`. Originally reserved at G14-D wave-5a; production firing site landed in Phase-3 sync.
...
- **Message:** "UCAN chain-walker invoked with no clock injected (now_secs=0 sentinel) against a chain with time-bounded delegations; inject a real clock via with_now_secs (or wait for CapWriteContext::now threading per phase-3-backlog §2.3 (i))"
- **Context:** `{}` (no structured payload — the violation is at the policy boundary, not at any specific token)
- **Fix:** The `UcanGroundedPolicy` chain-walker observed the `DEFAULT_NOW_SECS = 0` sentinel against a UCAN chain that carries time-bounded delegations (`nbf > 0` OR `exp > 0`). Pre-fail-closed-fix the chain-walker silently fail-OPENed: it walked tokens against `now=0`, so a forged chain with `nbf=0` + `exp > 0` accepted whenever the rest of the chain-walk passed (no operator-visible surface signaling the missing-clock misconfiguration). The inversion at G16-B-B-rest sub-item D fail-CLOSES with this typed code so the caller MUST inject a real wallclock. Production AND test callers both inject via `UcanGroundedPolicy::with_now_secs` today; the `CapWriteContext::now`-threading work named in `docs/future/phase-3-backlog.md §2.3 (i)` will replace that construction-time injection with a per-write wallclock. A chain WITHOUT time bounds (`nbf=0` AND `exp` unset) is safe to walk at the sentinel and does NOT trigger this code.
...
- **Fix:** Per the G-CORE-3 input-constraint F-3 typed-reject rule (the `DropBundle` wire shape is documented in `docs/V1-WIRE-FORMAT-INVENTORY.md`): a future Drop bundle version this reader does not know about MUST yield typed `UnsupportedDropVersion` — NEVER silent skip. Silent skip would let a malicious "future-version" bundle be ignored without warning + invite header-confusion attacks. Fix at the consumer side: upgrade `benten-drop` to a version that knows the on-wire discriminator. The test-only `DropBundleVersion::Synthetic(u16)` arm exists solely to drive this typed-reject pin. No primitive-edge routing (None).
...
- **Message:** "Drop bundle envelope-issuer mismatch: the envelope-sig verifying key is not the trusted grant's authoritative issuer"
- **Context:** `{ detail: String }` (short narrative of the mismatch: the bundle `issuer_verifying_key` did not equal `auth_grant.issuer_verifying_key`)
...
- **Phase:** 4-Meta-Core F-INJ-2 (Drop envelope-issuer anchoring)

<!-- reachability: ignore -->

### E_RECIPIENT_KEM_NOT_COMMITTED

- **Message:** "recipient KEM key is not committed by its audience did:benten — no RecipientBinding could be constructed"
- **Context:** `{ audience_did: String }` (the `did:benten` audience whose key-set document did not hash to the DID's committed CID)
- **Fix:** GAP-KDB Shape-B closes the recipient-side key-substitution seam (design §5, Inv-23): a Layer-C Drop seal's KEM key is committed by its audience DID. `Did::resolve_kem(keyset_doc)` recovers-and-verifies the recipient KEM key FROM the DID — it fail-closed rejects when (a) the received key-set document does not hash to the DID's committed CIDv1 (a BLAKE3-256 2nd-preimage — the flagship active-substitution defense), (b) the document's embedded `sig` multikey disagrees with the DID's embedded signing key (spliced-sig reject), (c) the `kem_cp` disagrees with the wired `{0xec, 0x120c}` components (algorithm-confusion reject), or (d) the committed suite is below the PQ floor (a `0x6400` classical-only commitment is refused). `RecipientBinding::resolve(audience_did, keyset_doc)` is the SOLE constructor; there is NO public constructor pairing an arbitrary `kem_pub` with an `audience_did`, so a substituted KEM key an attacker tries to seal to is unconstructible. A bare `did:key` commits no key-set → `NoKemCommitment` (signing-only principals need no KEM key; Drop recipients MUST be `did:benten`). Fix at the sender side: obtain the audience's key-set document over the same first-contact / address-book / cached-vault channel that verifies against the DID (a changed key-set = a changed DID). This is a construction-side guarantee — no wire byte, CBOR field, or golden vector changes. No primitive-edge routing (None) — the typed-reject IS the defense.
...
- **Phase:** 4-Meta-Core GAP-KDB Shape-B (recipient KEM-commitment / RecipientBinding — Inv-23)
...
> **Status (R6 R1 FP integration PR #1351 2026-05-24):** firing in production via the typed `SwapMatrixError::AuditNotLandedPurePqRejected` constructor at `crates/benten-crypto-suite/src/swap_matrix.rs::try_pure_pq_sole_trust_path` (`:257`; C11b safety pin). The `ErrorCode::AuditNotLandedPurePqRejected` enum variant is the engine-wide catalog mirror, surfaced via an engine-error lift that materializes when a future production caller invokes the pure-PQ constructor through an engine entry point; at G-CORE-3c the only entry is the conformance pin in tests + the typed-arm reservation for downstream waves. Same scanner-asymmetry shape as E_WRITE_CONFLICT (variant exists for forward-compat with an enum-valued surface; production firing routes via the SwapMatrixError surface, which the `crates/*/src/` reachability scanner doesn't catalogue as a `ErrorCode::*` construction). Drift-detector reachability is `ignore` on the enum variant only.
...
- **Phase:** 4-Meta-Core R6 R2 FP integration (Row D-19 G-COMP-1 wave Cohort 8; CATALOG_VARIANT_COUNT 194 → 195). Routes to `ON_ERROR`.

### E_DSL_UNKNOWN_PRIMITIVE

- **Message:** "DSL semantic error: unknown primitive '{name}'" (carries the inner [`Diagnostic`])
- **Context:** `{ message: string, source_position?: Span, primitive_name: string }`
- **Fix:** The DSL referenced a primitive that is not part of the 12 operation primitives (READ / WRITE / TRANSFORM / BRANCH / ITERATE / WAIT / CALL / RESPOND / EMIT / SANDBOX / SUBSCRIBE / STREAM). Per CLAUDE.md commitment #1 (12 primitives irreducible), the DSL compiler refuses unknown keywords. Check spelling; for typed-CALL-style helpers, write `call('engine:typed:<op>', { ... })` directly (the `typedCall` DSL helper lives at the TS layer, not in the Rust dsl-compiler grammar). Phase-4-Meta-Core R6 R2 FP integration (Row D-19 G-COMP-1 wave Cohort 8) mints the first-class catalog mirror for the pre-existing `pub const benten_dsl_compiler::E_DSL_UNKNOWN_PRIMITIVE` wire-string constant + `CompileError::Semantic(_)` variant.
...
- **Phase:** 4-Meta-Core R6 R2 FP integration (Row D-19 G-COMP-1 wave Cohort 8; CATALOG_VARIANT_COUNT 195 → 196). Routes to `ON_ERROR`.
