...
- **Phase-4-Meta-Core F-full MembershipSet mints:** +1 `E_ROLE_STALE_AT_VERIFY` (w-ms-canary; BC-5 role-staleness verify gate) → 198; +1 `E_KV_TARGET_NOT_IMMUTABLE` (w-gov-audit; F-INV19-1 / Inv-19 `K(V)` immutable-target reject) → **199 throwable at HEAD `b93b2efc`**. ⚠️ A parallel F-full CODE wave may mint one more (F-01) → **200**.
