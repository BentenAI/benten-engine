...
**Catalog count narrative (post Phase-4-Meta-Core G-CORE-9 R1 fix-pass + R6-R2-FP G-COMP-1 cohort + F-full MembershipSet mints, HEAD `84280d31`):** four distinct counts coexist by design — each measures a different surface:

| Count | Source | Value (at HEAD `84280d31`) | Meaning |
|---|---|---|---|
| **Throwable enum variants** | `crates/benten-errors/src/lib.rs::ErrorCode` (minus `Unknown(String)` fallback) | **199** | What the engine can actually emit at runtime. Authoritative source of THROWABLE variants. |
| **Regression-list entries** | `crates/benten-errors/tests/stable_shape.rs::ALL_CATALOG_VARIANTS` + `CATALOG_VARIANT_COUNT` | **199** | The round-trip-pinned list. Matches the throwable enum 1:1. The `catalog_variant_count_matches_enum` test asserts exact equality. |
| **Catalog entries (this doc + TS classes)** | `### E_XXX` headings here + `packages/engine/src/errors.generated.ts` CATALOG_CODES | **201** | = 199 throwable + `E_UNKNOWN` (forward-compat sentinel mirroring Rust's `Unknown(String)` fallback) + `E_INV_ITERATE_NEST_DEPTH` (Phase-2a-retired ITERATE-nest-depth stopgap; catalog ID stays reserved across phases per the retention discipline at line ~112). |
| **Rust enum entries** | `ErrorCode` enum (incl `Unknown(String)`) | **200** | = 199 throwable + 1 `Unknown(String)` forward-compat fallback. No `InvIterateNestDepth` variant (removed at Phase-2a-open when `E_INV_ITERATE_BUDGET` multiplicative form superseded it; catalog heading retained at line ~112 for backward-compat string round-trip). |

**Why four counts (199 / 199 / 200 / 201):** the Rust enum is the source of throwable variants (199); plus a forward-compat `Unknown(String)` fallback (= 200 in rust); the TS catalog + this doc additionally retain 1 Phase-2a-retired catalog ID (= 201 in catalog/ts); the test list at stable_shape.rs::ALL_CATALOG_VARIANTS round-trips the throwable subset (199). Single canonical headline number: **199 production-throwable codes** at HEAD `84280d31`. ⚠️ A parallel F-full CODE wave may mint one additional ErrorCode (F-01) bumping the throwable count to **200** (catalog 202) — the strategy-C integrator reconciles the count at integrate-time per the historical #1319↔#1318 collision pattern.
...
- **Phase-4-Meta-Core F-full MembershipSet mints:** +1 `E_ROLE_STALE_AT_VERIFY` (w-ms-canary; BC-5 role-staleness verify gate) → 198; +1 `E_KV_TARGET_NOT_IMMUTABLE` (w-gov-audit; F-INV19-1 / Inv-19 `K(V)` immutable-target reject) → **199 throwable at HEAD `84280d31`**. ⚠️ A parallel F-full CODE wave may mint one more (F-01) → **200**.
