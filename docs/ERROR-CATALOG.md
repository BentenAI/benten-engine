...
<!-- reachability: ignore -->
<!-- Rationale: production-source construction site is `SwapMatrixError::AuditNotLandedPurePqRejected` at `crates/benten-crypto-suite/src/swap_matrix.rs:252` (the C11b safety pin at `try_pure_pq_sole_trust_path`). The `ErrorCode::AuditNotLandedPurePqRejected` enum variant is the engine-wide catalog mirror, surfaced via the engine-error lift (lift impl materializes when a future production caller invokes the pure-PQ constructor through an engine entry point; at G-CORE-3c the only entry is the conformance pin in tests + the typed-arm reservation for downstream waves). Same scanner-asymmetry shape as E_WRITE_CONFLICT (variant exists for forward-compat with an enum-valued surface; production firing routes via a different path that the scanner doesn't catalogue as a `ErrorCode::*` construction). Remove this annotation if/when a production-source engine call invokes `try_pure_pq_sole_trust_path` and lifts the typed SwapMatrixError via `From<SwapMatrixError> for ErrorCode` (the lift impl itself counts as a construction site for the reachability scanner). Annotation added at R6 R1 FP integration PR #1351 fix-up #3 (drift-detect surfaced the gap during consolidation). -->
