...
> **Status:** the production firing site is `bindings/napi/src/input_limits.rs::limit()`, reached from `scan_dag_cbor` (map / list / bytes / text / depth / declared-length / item-count) and from `parse_cid_string_bounded` (CID shape). The module is NOT test-gated — it compiles into the production cdylib under `napi-export`. The drift-detector scans `crates/*/src/` only (per `phase-3-backlog §10.7` — v1-window scope widening), so the `reachability: ignore` annotation below reflects the structural scanner asymmetry, not a missing firing site. (Before 2026-07-29 that was aspirational: the "bounded streaming decoder" this entry described did not exist and `testing::deserialize_value_from_js_like` returned `Err(CoreError::NotFound)` unconditionally. B8 landed it.)
