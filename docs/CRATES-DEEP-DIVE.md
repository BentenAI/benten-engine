...
A plain-English walk across the 15 workspace crates. This doc reads the per-crate `INTERNALS.md` deep-dives as primary sources and synthesizes patterns at the workspace level — what's working well, what's leaking, what's elegant, what could be more so, and what the open architectural questions are.
...
Fifteen crates. Roughly stacked from foundational types at the bottom to the orchestrator at the top, with two sibling layers (capabilities, IVM) feeding into the engine, one Phase-3 surface (sync) sitting alongside, two Phase-4-Foundation additions (`benten-platform-foundation` for the v1 platform-shippable surface — schema-rendering + materializer + plugin manifest + admin UI v0 + `Renderer` trait — and `benten-renderer-tauri` for the embedded-webview engine extension per CLAUDE.md baked-in #19), and three Phase-4-Meta-Core additions (`benten-crypto-suite` as the ONE thin Benten-owned crypto-primitive integration crate per CLAUDE.md baked-in #5; `benten-drop` as the offline Drop bundle format per RATIFIED-S&C; `benten-membership-set` as the MembershipSet keying primitive).

**`benten-errors`** — The single, frozen list of every error the engine is allowed to emit. 192 production-throwable named variants today (193 with the `Unknown(String)` forward-compat fallback; 194 catalog entries with `E_UNKNOWN` + `E_INV_ITERATE_NEST_DEPTH`; see `docs/ERROR-CATALOG.md` preamble for the four-count reconciliation); the strings are the contract that TypeScript classes, operator dashboards, and audit pipelines route on. The crate is `no_std + alloc` with zero workspace deps, no `thiserror`, no `strum` — deliberately the irreducible nucleus that every other crate can depend on without pulling Benten machinery along. It earns its keep by being the engine's "what compositions are illegal" vocabulary; growing the catalog is how the engine teaches itself new ways to say no. (`crates/benten-errors/INTERNALS.md`.)
...
**`benten-crypto-suite`** — The 13th crate (Phase 4-Meta-Core addition; G-CORE-2 + G-CORE-3a/3c). The ONE thin Benten-owned signature / hash / cipher-suite agility integration crate per CLAUDE.md baked-in #5. This is the **only crypto-primitive call site** in the workspace; every other crate that signs, verifies, hashes, derives a key, wraps or unwraps an AEAD ciphertext routes through this crate's typed APIs. Implements: codepoint-dispatched hash + signature + KEM/encryption tables with a typed `UnsupportedAlgorithm` arm (never silent fallback); HKDF-SHA256 two-path structural KDF (`derive_root`/`derive_step`); per-Node + per-chunk AEAD envelopes; the bidirectional swap matrix (PQ-hybrid default × classical/no-enc downgrades + the NF-1 PQ⊕PQ non-default arm) covering Ed25519⊕ML-DSA-65 (sig default), X25519⊕ML-KEM-768 (KEM default) + ChaCha20-Poly1305 bulk. NEVER forks / reimplements primitives — concat / hash / codepoint / envelope GLUE over vetted upstream RustCrypto-class crates only. NEVER hardcodes sizes — codepoint-dispatched dynamic size reporting. Public surface frozen at V1-FROZEN-INTERFACE.md item 6. The PQ-hybrid default is the v1-beta correctness contract per RATIFIED-pq-default-reframe-2026-05-19. (`crates/benten-crypto-suite/INTERNALS.md`.)

**`benten-drop`** — The 14th crate (Phase 4-Meta-Core addition; G-CORE-3f at PR #1340). The self-contained offline Drop bundle format for the Sharing & Confidentiality stack. A `DropBundle` is a CBOR-on-disk artifact carrying four things: an authorized snapshot of a `RestrictedScope`-shaped SubgraphSpec (what-can-be-shared); per-Node AEAD ciphertexts for the subgraph's content (payload, via `benten-graph::aead_wrap`); ONE signed `AuthorizationGrant {ucan, key_material, binding_sig}` (authority per RATIFIED-S&C §R3); and an envelope-level Ed25519 signature over the bundle header (defense-in-depth outer layer per Spike G). Three modes in the contract: Mode 1 OnlinePull (live G-CORE-3e ALPN serve), Mode 2 OfflineDrop (substantive v1-beta shape), Mode 3 InlineTiny (≤16 KiB inlined into share URL; DEFERRED post-v1-beta — fires typed `E_DROP_BUNDLE_MODE3_INLINE_REJECTED`). Revocation reach per Compromise #62: Drop bundles are forever-valid once distributed; mitigated by tight `nbf`/`exp` + key rotation (RATIFIED-S&C §R6). 4 KiB upper bound on 5-Recipe bundles per Spike G measurement. Public surface frozen at V1-FROZEN-INTERFACE.md item 15. (`crates/benten-drop/INTERNALS.md`.)

**`benten-membership-set`** — The 15th crate (Phase 4-Meta-Core addition; F-full). The MembershipSet keying primitive — a **thin keying-glue Rust engine plugin** (mechanism-half per GN-2 / R0 §6.1) whose entire governance / audit / members / economics / federation surface is graph Nodes (data-half); the Rust-mechanism that lives here is the keying-frozen minimum. It owns: the exactly-3 `MembershipSetKind` enum + its `0x6600` / `0x6610` / `0x6620` codepoint family; the multi-stanza-HPKE keying glue (delegates EVERY crypto primitive to `benten-crypto-suite` — the #5 ONLY-call-site; NEVER forks it); the fused per-DID `MemberEntry` / `MembersTable` (one-DID-one-record, ZERO nature field per Inv-22); the `members_table` snapshot canonical-CBOR serialization (the AAD-bound keying minimum); the per-Kind cardinality constructors; the `0x6610` group AAD = the BLINDED 11-field set assembly handed as OPAQUE bytes to the crypto-suite (no reverse dep); the RBAC `RoleId` (5-value ordinal) + per-role UCAN ability-templates; and the role-staleness verify gate (`E_ROLE_STALE_AT_VERIFY`) + ephemeral UCAN grants. Owns Inv-19..22. Data-half models carry ZERO production callers (register-then-enforce disclosure; enforcement is engine-layer). (`crates/benten-membership-set/src/lib.rs`.)
...
```
   benten-renderer-tauri ──┐                                              (15 crates total;
       (prod deps:         │                                               this view omits the
        engine + foundation)│                                              Phase-4-Meta-Core
                            │                                              crypto-suite / drop /
                            │                                              membership-set leaves)
        engine + foundation)│
                            ↓
...
**Orchestrator layer — engine.** `benten-engine` depends on the eight foundational crates below it (`benten-core` + `benten-errors` + `benten-graph` + `benten-eval` + `benten-caps` + `benten-id` + `benten-ivm` + `benten-sync`) plus `benten-napi` as a separately built binding. It deliberately does NOT depend on `benten-dsl-compiler` (which consumes a pre-built `SubgraphSpec` shape, not source) and does NOT depend on `benten-platform-foundation` or `benten-renderer-tauri` (which sit ABOVE it in the dep graph — the platform layer composes engine primitives, not the reverse).

**Platform layer — Phase-4-Foundation additions.** `benten-platform-foundation` (the 11th crate) sits one layer above `benten-engine` and composes engine primitives into the v1 platform shape. Production deps: `benten-core` + `benten-errors` + `benten-id`; `benten-engine` is dev-dep-only (end-to-end test scaffolding). `benten-renderer-tauri` (the 12th crate) sits above platform-foundation: production deps include `benten-engine` + `benten-platform-foundation` (implements the `Renderer` trait for embedded-webview deployment shape (c) per CLAUDE.md baked-in #19). Both are intentionally OUTSIDE the engine orchestrator's dep set — the dep direction goes engine → platform-foundation → renderer-tauri, never the reverse (defended by Cargo.toml-walking arch-N tests).
...
Flat list of the most-impactful unresolved decisions across all 15 crates. Roughly priority-ordered for the pre-Phase-4-Meta + v1-assessment-window contexts.
