...
A plain-English walk across the 14 workspace crates. This doc reads the per-crate `INTERNALS.md` deep-dives as primary sources and synthesizes patterns at the workspace level — what's working well, what's leaking, what's elegant, what could be more so, and what the open architectural questions are.
...
```
   benten-renderer-tauri ──┐                                              (14 crates,
...
Flat list of the most-impactful unresolved decisions across all 14 crates. Roughly priority-ordered for the pre-Phase-4-Meta + v1-assessment-window contexts.
