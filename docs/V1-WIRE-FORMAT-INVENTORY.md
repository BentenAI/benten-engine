...
## 30. Layer-A vault on-disk AEAD frame (`${BENTEN_DATA_DIR}/vault.cbor`)
...
**Wire format:**
- **Outer on-disk frame** is a **hand-rolled magic-prefixed AEAD frame** (NOT a DAG-CBOR-encoded `EncryptedEnvelope`): `magic 0xae | format-version V2 | codepoint(BE) | nonce_len | nonce | ct` (`serialize_vault`, `crates/benten-crypto-suite/src/vault.rs`). It is a **byte-twin** of the `EncryptedEnvelope` symmetric-AEAD wire header (same field layout, same `ENVELOPE_MAGIC` / `ENVELOPE_FORMAT_VERSION_V2` constants); the frame is emitted by hand rather than by re-serializing an `EncryptedEnvelope`. This code duplication is intentional-but-noted (the two frames MUST stay byte-identical if a future refactor unifies them; there is no version-drift risk today because both reference the shared `envelope.rs` magic/version constants).
...
**FREEZE-WAVE status:** ✅ COVERED at v1-beta substrate-level; the hand-rolled `magic 0xae | V2 | codepoint | nonce_len | nonce | ct` frame + the `0x6100` codepoint + 24-byte XNonce + Argon2id `OWASP_DEFAULT` params + canonical CBOR inner-payload field-order are byte-locked (format-version V2 from first commit, M-20).
