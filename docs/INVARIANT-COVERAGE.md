...
| 12 | Aggregate validation catch-all — multi-invariant violations roll up | 1 | `InvariantViolation::Registration` (at `crates/benten-eval/src/lib.rs::InvariantViolation` — fires when two or more invariants violate simultaneously; symbol-form per §3.5b HARDENED point 3 high-churn-surface rule) | `crates/benten-eval/tests/invariants_9_10_12.rs::registration_catch_all_populates_violated_list` |
