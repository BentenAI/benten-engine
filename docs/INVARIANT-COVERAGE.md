...
## Inv-16..22 Phase-4-Meta-Core F-full design-mints (AS-BUILT + ENFORCED at HEAD `84280d31`)
...
**Status semantics:** AS-BUILT + ENFORCED at HEAD. Each of Inv-16..22 has (1) a typed error or typed-reject
path, (2) at least one landed regression test pinning the firing condition, and (3) no observable bypass path —
the table rows above carry the real construction-site + test cites. The R2 test-landscape
...
now **verified construction sites at HEAD `84280d31`** — the crate `benten-membership-set` (the 15th crate)
exists and is green, as do `benten-crypto-suite`'s F-full envelope/swap-matrix surfaces and `benten-engine`'s
`layer_d` module. The `f_disc_2` registration catch-net (`crates/benten-drop/tests/`) asserts the doc-wave
end-state; it needs an enforced-state assertion strengthened to match this AS-BUILT reclassification (FLAGGED —
that test lives under `benten-drop/tests`, owned by the parallel CODE agent / a follow-up, not edited here).
...
**Inv-16..22 (Phase-4-Meta-Core F-full design-mints)** are AS-BUILT + ENFORCED at HEAD `84280d31`. All of (1) (2) (3) hold — the typed errors / typed-reject paths, the landed regression pins, and the no-bypass consumption all SHIPPED across the F-full wave sequence (R3 red-phase corpus → R5 impl-to-green → F-full close). See "Inv-16..22 Phase-4-Meta-Core F-full design-mints" section above + the table rows for the per-invariant construction sites + test cites. The `benten-membership-set` crate (the 15th crate, owner of Inv-19..22) exists and is green at HEAD; its enforcer-path + test-family cites are verified construction sites. The one honestly-disclosed v1-beta floor: Inv-21's `F-INV21-3` kani convergence proof is `#[cfg(kani)]`-gated with the proptest surrogate as the v1-beta floor.
