...
`ProductionManifestEnvelopeRechecker` adapter is deferred to
Phase-4-Meta-Composing (the G-COMP-1 wave — Phase-4-Meta-Core closes
without it) per `docs/future/phase-4-backlog.md §4.36`). Inv-14 doesn't gain a new
...
**Status semantics:** AS-BUILT + ENFORCED at HEAD for Inv-16/17/18/20/22; **Inv-19 and Inv-21 are the two
...
Inv-16/17/18/20/22 has (1) a typed error or typed-reject
