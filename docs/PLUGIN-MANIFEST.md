...
| 3 | **Plugin-DID minted at install** | UCAN audience AND constrained issuer within manifest envelope. NOT an attested sub-identity of user-DID. Per D-4F-16: `did:key:...` shape, fresh Ed25519 keypair **minted by the caller (admin UI / installer)** via OsRng + inserted into `PluginDidStore` BEFORE `install_plugin` runs (caller-mint-first contract — see §3 below). The engine NEVER mints plugin-DIDs internally | `benten_id::plugin_did::mint(rng)` called caller-side; persisted in engine's `PluginDidStore` (in-memory at v1); one keypair per install |
...
6. Install record persisted to `ManifestStore`. Two backends ship at Phase-4-Meta-Core G-CORE-7 (per `docs/future/phase-4-backlog.md §6.4`): (a) the in-memory `HashMap`-backed `ManifestStore` for browser-wasm32 / scoped-test use (`crates/benten-platform-foundation/src/manifest_store.rs`); (b) the durable `RedbManifestStore` (native-only; gated `#[cfg(any(not(target_arch = "wasm32"), target_os = "wasi"))]`) whose persisted records survive a process / handle restart — opening a `RedbManifestStore` at the same path reloads + re-verifies the previously-installed records via the same canonical-bytes DAG-CBOR `InstallRecord` encoding, extending the verify-on-every-load defense across the restart boundary.
...
**Phase-4-Meta-Core G-CORE-7 — atomicity + caps-grew gate (SHIPPED):**

3. **(SHIPPED at G-CORE-7 per `docs/future/phase-4-backlog.md §4.41`)** Cap-change consent rule (ratification #8):
   - Silent within-lineage upgrade if `requires` is a strict subset of installed manifest;
   - Full re-consent if `requires` GREW (any cap added or scope widened) — `install_plugin` consults `module_ecosystem::decide_upgrade_consent` against `params.prior_installed_cid`'s resolved manifest; a within-lineage upgrade whose `requires` GREW relative to the prior manifest is REJECTED with `E_PLUGIN_INSTALL_CONSENT_REQUIRED` unless the install record's `granted_caps_bytes` covers every scope in the new manifest's `requires`. Initial installs (`prior_installed_cid = None`) skip this check — the manifest_cid binding in the user-signed install record is itself the consent for the initial cap set.
   - Cross-fork merge = user-initiated through same consent flow (no separate cross-fork merge surface).
4. **(SHIPPED at G-CORE-7 per `docs/future/phase-4-backlog.md §4.21` + §4.35)** Atomic install: Steps 9 (cap-cascade mint) → 10 (provision namespace) → 11 (library insert + active-ref) are all-or-nothing. Step 9's mint-loop tracks every minted grant CID; a mid-loop mint failure unwinds the already-minted grants via `CapMinter::revoke_root_grant` before propagating the error (§4.35 cap-cascade atomicity). A Step-10 provision failure unwinds Step-9 grants; a Step-11 set-active failure unwinds Step-10 namespace provision AND Step-9 grants AND the just-inserted library entry, leaving zero observable residue (§4.21 Steps 9/10/11 rollback). `CapMinter::revoke_root_grant` + `PrivateNamespaceProvisioner::deprovision_private_namespace` ship as additive trait methods with no-op defaults so existing adapters compile against an additive signature.
