...
- [`ARCHITECTURE.md`](ARCHITECTURE.md) §"14 crates" — `benten-platform-foundation` is the 11th crate (schema-compiler + materializer + plugin-manifest + Renderer trait); the workspace expanded to 14 with `benten-renderer-tauri` (12th), `benten-crypto-suite` (13th), and `benten-drop` (14th) at Phase 4-Meta-Core
