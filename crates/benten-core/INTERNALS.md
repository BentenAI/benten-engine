...
**G-CORE-6a / #506 closure: `.build()` is the canonical single-fallible-point.** The chainable setters (`push`, `iterate`, `iterate_parallel`) are infallible (`-> NodeHandle` / `-> &mut Self`); over-range numeric arguments (a `NodeHandle(u32)` slot-budget exhaustion in `push` at ~4.29B nodes, an iterate-depth overflow, or a parallel-fanout overflow) are recorded into the builder's `build_errors: Vec<CoreError>` field instead of being returned at the setter callsite. `.build()` collects those deferred errors and returns `Err(CoreError::ValueOutOfRange { field, value, bound })` (catalog code `E_VALUE_OUT_OF_RANGE`) if any were recorded, else returns the well-formed `Subgraph`. The setter callsite also `debug_assert!`-loud-fails in debug builds (release builds keep the no_std-compatible saturating fallback for safety AND the deferred error). New callsites that want the typed-error surface should prefer `.build()` over `build_unvalidated_for_test`; the latter stays for invariant edge-case tests that intentionally bypass it. The WAIT primitives (`wait_signal_with_timeout`, `wait_duration`) stay saturating per the #506 benign-leave-as-is disposition — the over-range arm requires a `Duration` past ~292M years and never trips on real wall-clock inputs (DISAGREE-WITH-EXPLANATION on the lens recommendation to make WAIT fallible). The new `ValueOutOfRange` variant takes `CoreError` from eleven to twelve variants and bumps `ALL_CATALOG_VARIANTS` / `CATALOG_VARIANT_COUNT` 170 → 171 in `benten-errors`.
...
**Subgraph.** `SubgraphBuilder::new("handler_id")`, then `.read(...)`, `.write(...)`, `.transform(prev, _)`, `.respond(prev)`, `.declare_deterministic(true)`, etc. Finalise with `.build()` (G-CORE-6a / #506 canonical single-fallible-point — surfaces deferred `CoreError::ValueOutOfRange` recorded by chainable setters), `build_unvalidated_for_test()` for invariant edge-case tests, or `benten_eval::SubgraphBuilderExt::build_validated()` to run the invariants pass. `sg.cid()` gives the content-addressed handler identity.
...
**Errors.** `CoreError` is `#[non_exhaustive]` and twelve-variant (G-CORE-6a / #506 added `ValueOutOfRange { field, value, bound }` mapping to `E_VALUE_OUT_OF_RANGE`; bumps `CATALOG_VARIANT_COUNT` 170 → 171). Every variant has a stable catalog code via `CoreError::code() -> ErrorCode`. Sibling `version::VersionError`, `version_chain::VersionDagError`, and `version_dag::VersionDagError` expose `.code()` symmetrically — all map into the shared `E_VERSION_*` family.
