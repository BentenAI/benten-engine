...
- **Schema → SubgraphSpec types.** Phase 3.5 schema-driven rendering is mentioned as upcoming work. The `Subgraph` + `SubgraphBuilder` pair already lives here, so schema-to-subgraph projections naturally compose at the core boundary. A `SchemaSpec` value type might live here; the projection function (`schema_to_subgraph(spec: &SchemaSpec) -> Subgraph`) probably lives in `benten-engine` or a new `benten-schema` crate, since it's a transformation rather than a primitive shape.
