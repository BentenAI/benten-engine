...
#[test]
#[ignore = "un-ignore at G-CORE-3w: walker fail-closed on missing root CID (F-1)"]
...
#[test]
#[ignore = "un-ignore at G-CORE-3w: walker self-referential cycle typed error (F-2)"]
fn walker_self_referential_spec_cycle_rejected() {
    ...
    assert!(
        matches!(err, SubgraphSpecError::SelfReferentialCycle),
        "expected SelfReferentialCycle; got {:?} (F-2)",
        err
    );
}
...
#[test]
#[ignore = "un-ignore at G-CORE-3w: walker max_depth bound enforcement (F-3)"]
...
#[test]
#[ignore = "un-ignore at G-CORE-3w: spec conflicting label predicates typed error (A-1)"]
