...
// `DEFERRED_HOST_FN_RANDOM_CAP_PREFIX` constant + the validate-time
// deferral guard at the top of `execute()` are RETIRED. `random` now
// lives alongside `time` / `log` / `kv:read` in the codegen-default
// surface (`crates/benten-eval/src/sandbox/host_fns.rs`) and is wired
// through `register_default_host_fns` against `getrandom` direct
// (D-PHASE-3-11 RESOLVED-at-R1). The cap-string is `host:random:read`
// (4-segment shape; mirrors the kv:read precedent). Per-call entropy
// budget defaults to 4096 bytes (per r1-wsa-8); module manifests may
// override via the additive optional `host_fns.random.budget_bytes_per_call`
// field per `docs/MODULE-MANIFEST.md`.
...
impl SandboxStoreData {
    fn new(
        ...
        let log_bytes_remaining = host_fns
            ...
            .unwrap_or(65_536);
        // Phase-3 G17-A2 — `random` per-invocation budget. Resolution
        // order (r1-wsa-8): per-call manifest override (if any) →
        // codegen-default (4096 per the host_fns table) → hardcoded
        // fallback (4096; matches `DEFAULT_RANDOM_BUDGET_BYTES_PER_CALL`).
        let codegen_random_budget = host_fns
            .get("random")
            .and_then(|s| match &s.behavior {
                HostFnBehavior::Random {
                    budget_bytes_per_call,
                } => Some(*budget_bytes_per_call),
                _ => None,
            })
        ...
        let random_budget_bytes_per_call = config
            ...
            .unwrap_or(codegen_random_budget);
        Self {
