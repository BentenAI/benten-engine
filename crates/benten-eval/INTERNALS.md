...
**Last refreshed: 2026-05-24 against main HEAD `a0b75637` (post `phase-4-meta-core/r4b-r1-fix-pass` base `4bbc4cac`).** Substantive Phase 4-Foundation work in this crate was bounded — exactly one source change landed during 4-Foundation (PR #210 / `f3930e1`: SUBSCRIBE per-event cap-recheck enum lift; see §10 below). Phase 4-Foundation platform layer (admin UI v0 + plugin manifest schema + materializer + schema-rendering) lands in the separate `benten-platform-foundation` crate, not here. The 12 primitives + the SANDBOX 4-host-fn floor + the arch-1 dep-break are unchanged at HEAD; all 14 invariants are production-runtime LIVE (Phase 2b + Phase 3 closure).
