...
Total: ~120 integration test files / ~17.4k LOC. Organized into eight clusters by surface.
...
- **The 4 `todo!()` stubs at `crates/benten-engine/src/engine_wait.rs:1011-1026`** are the Class B β `Engine::read_node_as` / `read_node` migration target per CLAUDE.md #18. They are NOT in this crate, but the `PrimitiveHost::read_node` semantics this crate exposes are what the migration plugs into. New primitive executors that perform reads should call `host.read_node` and `host.check_read_capability` and let the engine impl thread the active-principal switching.
