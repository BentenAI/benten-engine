...
#[test]
#[ignore = "Phase 3+ anytime backlog — wait_signal_shape_mismatch routed-edge-label classification. Destination: docs/future/phase-3-backlog.md §7.3.C row 7. End-to-end shape-match correctness IS fully wired: SUSPEND-time populates `WaitMetadata.signal_shape` at `crates/benten-eval/src/primitives/wait.rs:345` (reads `wait_node.property(\"signal_shape\").cloned()`) + stores at line 380; resume-time consults via `shapes_match` in `resume_with_meta`; `SuspensionStore::put_wait`/`get_wait` round-trips the field. The OUTSTANDING gap is purely a routing classification: `ErrorCode::InvRegistration` is routed to `None` in `crates/benten-errors/src/lib.rs::routed_edge_label` (line 1045-1062 group), but this test asserts `Some(\"ON_ERROR\")`. Either the test's expected edge label needs adjustment OR the variant needs reclassification under the runtime-error group. Bundle with the next round of routed_edge_label classification hardening (reassessment cadence: v1-assessment-window per CLAUDE.md baked-in #15)."]
