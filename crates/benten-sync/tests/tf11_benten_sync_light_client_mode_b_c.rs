...
// W5 owns the cross-wave + freeze-conformance + §8-B sub-lane partition
// per R2 §7. The pre-existing R3-B4 mode-(b) and mode-(c) pins cover
// the named-destination per-arm work; the W5 extensions below add:
//   ARM-W5-1: MerkleRangeProofBackend TRAIT SURFACE LOCATION pin —
//             a structural assertion that when the trait lands, it
//             lands in `benten-sync` (per §8-B "keep benten-graph
//             thin = no proof *trait* in storage"), NOT benten-graph.
//   ARM-W5-2: bandwidth-budget bounded-by-range-size cross-arm —
//             pins that mode-(b)'s bandwidth budget DOES scale with
//             the range size (the load-bearing efficiency property
//             that distinguishes mode-(b) from mode-(a)'s per-key
//             proofs).
// =====================================================================

/// **TF-11 / W5 ARM-W5-1 — MerkleRangeProofBackend TRAIT LOCATION.**
///
/// §8-B-(i) structural resolution (Ben-ACKED 2026-05-19): the
/// `MerkleRangeProofBackend` trait + mode-(c) signed-checkpoint logic
/// land in `benten-sync` (the "keep benten-graph thin = no proof
/// *trait* in storage" intent). This pin asserts the trait, when it
/// lands, is in `benten-sync` — NOT `benten-graph`. Structural
/// source-scan pin (§3.6f SHAPE-not-SUBSTANCE waiver: location is a
/// structural fact, not a runtime behavior).
///
/// Would-FAIL if a future agent ships `MerkleRangeProofBackend` in
/// `benten-graph` — the §8-B (b) decision being silently inverted.
#[test]
#[ignore = "RED-PHASE: un-ignore at benten-sync light-client sub-lane wave (TF-11) when the trait lands. r2-test-landscape §7 W5 row TF-11 extension."]
fn merkle_range_proof_backend_trait_lives_in_benten_sync_not_benten_graph() {
    // Body intent at un-ignore:
    //
    //   use std::path::PathBuf;
    //   let benten_sync_src = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
    //       .join("src");
    //   // Scan benten-sync sources for the trait definition.
    //   let sync_has_trait = walk_source(&benten_sync_src,
    //       |s| s.contains("trait MerkleRangeProofBackend"));
    //   assert!(sync_has_trait,
    //       "§8-B-(i): MerkleRangeProofBackend MUST live in benten-sync \
    //        (the 'keep benten-graph thin = no proof *trait* in storage' \
    //        intent); structural assertion at trait-landing time.");
    //
    //   // The structural counter-check: it must NOT live in benten-graph
    //   // (would silently invert the §8-B (b) decision). benten-sync does
    //   // not depend on benten-graph, so a benten-graph-grep needs the
    //   // workspace-relative path:
    //   let workspace = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
    //       .join("..").join("..");
    //   let graph_src = workspace.join("crates/benten-graph/src");
    //   let graph_has_trait = walk_source(&graph_src,
    //       |s| s.contains("trait MerkleRangeProofBackend"));
    //   assert!(!graph_has_trait,
    //       "§8-B-(i): MerkleRangeProofBackend MUST NOT live in \
    //        benten-graph (would invert §8-B (b)).");
    //
    panic!(
        "RED-PHASE (W5 extension): MerkleRangeProofBackend TRAIT \
         LOCATION pin. §8-B-(i) Ben-ACKED 2026-05-19: trait lives in \
         benten-sync (the 'keep benten-graph thin' intent); pure-trait/ \
         non-wire surface is NOT P-III. Un-ignore at trait landing; \
         structural source-scan asserts location."
    );
}

/// **TF-11 / W5 ARM-W5-2 — mode-(b) bandwidth-budget scales with
/// range size (the load-bearing efficiency property).**
///
/// mode-(a) is per-key proofs; mode-(b)'s value is amortizing the
/// proof cost across a sub-range (bandwidth scales with range size
/// rather than per-key constants times N). A regression where the
/// "range proof" is implemented as N independent mode-(a) proofs
/// concatenated would FAIL this property (bandwidth = N × per-key);
/// the genuine range proof scales sub-linearly. This pin asserts
/// the post-implementation bandwidth budget reflects the range-
/// scaling property.
#[test]
#[ignore = "RED-PHASE: un-ignore at TF-11 mode-(b) MerkleRangeProofBackend landing. r2-test-landscape §7 W5 extension."]
fn mode_b_bandwidth_scales_with_range_size_not_n_times_per_key_baseline() {
    // Body intent at un-ignore:
    //
    //   let mut mst = Mst::new();
    //   // Insert N=100 sorted entries.
    //   for i in 0..100 {
    //       mst.insert(MstEntry::from_payload(
    //           &format!("/zone/{i:03}"), vec![i as u8]));
    //   }
    //
    //   // mode-(a) baseline: 10 per-key proofs cost 10×per_key bytes.
    //   let per_key_proof_size: usize = (0..10)
    //       .map(|i| mst.merkle_proof_for(&format!("/zone/{i:03}"))
    //                  .unwrap().byte_size())
    //       .sum();
    //
    //   // mode-(b): a single range proof for entries 0..10.
    //   let range_proof = mst.merkle_range_proof_for_range(0, 10);
    //   let range_proof_size = range_proof.byte_size();
    //
    //   // The load-bearing property: range proof < N × per-key
    //   // (amortization; the actual constant depends on MST shape but
    //   // a genuine range proof MUST achieve sub-linear in range size).
    //   assert!(range_proof_size < per_key_proof_size,
    //       "§4-E mode-(b): range proof bandwidth MUST scale \
    //        sub-linearly in range size (the load-bearing efficiency \
    //        property). N×per_key={per_key_proof_size}, \
    //        range={range_proof_size}. Would-FAIL on a 'range proof' \
    //        = N concatenated mode-(a) proofs (the wrong shape).");
    //
    panic!(
        "RED-PHASE (W5 extension): mode-(b) bandwidth-scales-with- \
         range-size efficiency property. Un-ignore at MerkleRangeProof \
         Backend landing. Would-FAIL on a degenerate impl that just \
         concatenates per-key proofs."
    );
}
