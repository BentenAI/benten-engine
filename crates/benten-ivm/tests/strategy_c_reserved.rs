...
//! `Strategy::Reserved` (Z-set / DBSP cancellation) is reserved-not-implemented in
...
//! complete and stable, but constructing a Strategy::Reserved view returns a typed
...
//! - `benten_ivm::testing::testing_construct_view_with_strategy(Strategy::Reserved)`
//!   returns `Err(ViewError::StrategyNotImplemented { strategy: Strategy::Reserved, deferred_to_phase: "Phase 3+" })`
...
fn algorithm_b_strategy_c_reserved_no_implementation_in_2b() {
    // Strategy::A + Strategy::B are constructable; Strategy::Reserved is not.
    let a = try_construct_view_with_strategy(Strategy::A);
    let b = try_construct_view_with_strategy(Strategy::B);
    let c = try_construct_view_with_strategy(Strategy::Reserved);
    ...
    let err = c.expect_err(
        "Strategy::Reserved is RESERVED in Phase 2b — must surface a typed error, \
    ...
    match &err {
        ViewError::StrategyNotImplemented {
            ...
            assert_eq!(*strategy, Strategy::Reserved);
            assert!(
        ...
        other => panic!(
            "expected ViewError::StrategyNotImplemented, got `{other:?}` — \
             the Strategy::Reserved reserved-not-implemented contract requires a \
