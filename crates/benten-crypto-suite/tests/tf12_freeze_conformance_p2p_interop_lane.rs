...
// W5 owns the freeze-conformance + p2p-interop arms per R2 §7. The pre-
// existing pins cover the §1.A.FROZEN item 14 P2P-interop conformance
// surface (typed-unsupported-error / no-wire-break / old-codepoints-
// forever). W5 extension adds the §1.A.FROZEN item 6 hybrid-default
// codepoint VALUE freeze pin — assert codepoint `0x647a` is the FROZEN
// constant for the X25519⊕ML-KEM-768 hybrid KEM at the freeze, AND that
// classical-only / no-encryption / non-PQ downgrade codepoints are all
// present (the full swap matrix per C11b / G-CORE-3c).
// ===========================================================================

/// R3-W5 EXTENSION — operative hybrid-default codepoint VALUE
/// `0x647a` (X25519⊕ML-KEM-768) frozen at G-CORE-9. The plan body
/// carries the value in multiple places (§1.A.FROZEN item 6,
/// G-CORE-2/3 defs, §8-F P-III package) — this pin asserts the post-
/// freeze codepoint table actually contains `0x647a` AND it is wired
/// to the X-Wing-style combiner (the load-bearing identity).
#[test]
#[ignore = "RED-PHASE: un-ignore at G-CORE-9 FREEZE (when the cipher_suite codepoint table lands the X-Wing-hybrid 0x647a + the full swap-matrix codepoints). r2-test-landscape §7 W5 + §1.A.FROZEN item 6."]
fn hybrid_default_codepoint_0x647a_present_in_cipher_suite_table_post_freeze() {
    // Body intent at un-ignore: scan the cipher_suite source for the
    // `0x647a` constant + assert it is reachable via codepoint dispatch
    // to the X-Wing-hybrid path. Pre-freeze (HEAD) the constant does
    // not yet appear in the canary stub (G-CORE-2 ships the table
    // structure; G-CORE-3a wires `0x647a` to the X-Wing combiner).
    panic!(
        "RED-PHASE (W5 extension): hybrid-default codepoint 0x647a \
         present at G-CORE-9 freeze. Un-ignore when the cipher_suite \
         codepoint table is wired. Would-FAIL on (a) a different \
         encryption-hybrid codepoint silently chosen (drift from \
         §1.A.FROZEN item 6 / §8-F), (b) the X-Wing combiner not \
         vendored into THIS integration crate (a transitive \
         x-wing-crate surprise — explicitly forbidden by the plan \
         body), or (c) the swap-matrix downgrade codepoints absent \
         (the C11b / G-CORE-3c full-swap-matrix property)."
    );
}
