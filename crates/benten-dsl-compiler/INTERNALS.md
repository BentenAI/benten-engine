...
**`src/lib.rs` (~1660 lines post-G-CORE-DSL chunk-3; ~230 LOC of growth = Diagnostic Span + Span struct + 5th `CompileError::Backend` variant + `CompileError::error_code()` boundary + 6 new `validate_shapes`-shape rules' inline span-tracking)** — the entire crate. Logical sections, in order of appearance:
