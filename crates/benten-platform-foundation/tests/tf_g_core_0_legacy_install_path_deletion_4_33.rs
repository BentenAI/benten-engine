...
fn g_core_0_verify_4_test_files_migrated_off_legacy_install_path() {
    ...
    for rel in LEGACY_TEST_FILES {
        ...
        // `module_ecosystem` install path. We scan only NON-DOC-COMMENT
        // source lines (filtering `//!` module-doc + `//` line-comment
        // narrative) so legitimate retrospective references to the
        // deleted symbol in docstrings (which are normal for migration
        // notes citing what was removed) are not mistaken for an
        // actual legacy import. The structural migration marker is the
        // presence/absence of a `use` line bringing the symbol into
        // scope.
        let has_legacy_import = body.lines().any(|line| {
            let trimmed = line.trim_start();
            if trimmed.starts_with("//") {
                // Doc-comment (`///`, `//!`, `//`) — narrative; skip.
                return false;
            }
            // Real source — check for any `use ...module_ecosystem...`
            // pattern bringing the legacy install symbols into scope,
            // or any non-doc-comment reference to the deleted
            // `install_plugin_persisting_did` symbol.
            (trimmed.starts_with("use ")
                && trimmed.contains("module_ecosystem::")
                && (trimmed.contains("install_plugin") || trimmed.contains("InstallerShape")))
                || (trimmed.contains("install_plugin_persisting_did(")
                    || trimmed.contains("install_plugin_persisting_did,"))
        });
        assert!(
            !has_legacy_import,
            "§4.33 verify-pass: test file `{rel}` MUST no longer import \
             ...
             paths with different security envelopes. (Scope: non-doc-\
             comment `use` lines + call-site invocations of the deleted \
             symbols; doc-comment narrative references to the deleted \
             names are intentionally permitted to document the \
             migration history.)"
        );
    }
}
