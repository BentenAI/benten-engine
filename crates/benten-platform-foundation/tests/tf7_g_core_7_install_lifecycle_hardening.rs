...
impl CapMinter for FailAfterNGrants {
    ...
    /// G-CORE-7 §4.21 + §4.35 rollback observable: drain on revoke.
    /// The mint loop minted N grants — the rollback iterates revoke
    /// for each; this impl drops `minted` so the test's
    /// `minter.minted.is_empty()` assertion observes the all-or-
    /// nothing post-rollback shape regardless of which grant-CID is
    /// passed back (the test minter's CIDs are deterministic but the
    /// observable property is zero-residual; emptying the Vec is the
    /// simplest correct shape).
    fn revoke_root_grant(&mut self, _grant_cid: &Cid) -> Result<(), ErrorCode> {
        if let Some(idx) = self.minted.len().checked_sub(1) {
            self.minted.remove(idx);
        }
        Ok(())
    }
}
...
#[test]
// G-CORE-7 §4.21 un-ignored: install_plugin Steps 9/10/11 rollback.
...
#[test]
// G-CORE-7 §4.35 un-ignored: Step-9 cap-cascade atomicity (all-or-nothing).
...
#[test]
// G-CORE-7 §4.41 un-ignored: caps-grew fresh-consent e2e wiring.
...
#[test]
// G-CORE-7 §4.20 un-ignored: validate_with_clock e2e (through install_plugin).
...
#[test]
// G-CORE-7 §4.19(b) + §4.32 un-ignored: schema-author trust-list prompt
// path lands; trust-list is read from the SIGNED manifest envelope.
fn schema_author_not_in_trust_list_returns_user_prompt_outcome_from_signed_envelope() {
    use benten_platform_foundation::{ProvenanceOutcome, verify_schema_provenance_with_trust_list};

    // Default-empty admin UI envelope (Q3 ratification) — the envelope
    // is the SIGNED manifest body whose `peer_signature` is asserted
    // BEFORE `requires_schema_authors` is consulted; the helper
    // (`validate_with_clock` / `validate_with_rotation_log`) verifies
    // the envelope's signature first. The trust-list check here is
    // the §4.32 within-envelope arm — it reads the trust-list FROM the
    // returned `&PluginManifest`, not from a post-install side-channel.
    let admin_ui = common::manifest_fixtures::admin_ui_v0_manifest();
    assert!(
        admin_ui
            .requires_schema_authors
            .as_ref()
            .is_none_or(std::vec::Vec::is_empty),
        "Ben Q3: admin UI v0 default trust-list MUST be EMPTY \
         (None or empty Vec); got {:?}",
        admin_ui.requires_schema_authors
    );

    // §4.19(b): unknown peer-DID → UserPromptRequired (NOT
    // auto-accept, NOT auto-reject).
    let unknown_peer = common::manifest_fixtures::stub_peer_did_alice();
    let outcome = verify_schema_provenance_with_trust_list(&unknown_peer, &admin_ui);
    let ProvenanceOutcome::UserPromptRequired { peer_did } = outcome else {
        panic!(
            "§4.19(b): unknown-author schema MUST surface \
             UserPromptRequired (NOT Trusted); got {outcome:?}"
        );
    };
    assert_eq!(
        peer_did, unknown_peer,
        "the prompt outcome MUST carry the unknown peer-DID so the \
         admin UI can show it to the user"
    );

    // §4.32: a trust-list extension (within-envelope; would re-sign in
    // production) flips the outcome to Trusted for the same peer.
    let admin_ui_with_alice =
        common::manifest_fixtures::admin_ui_v0_manifest_with_trust_list(vec![unknown_peer.clone()]);
    let outcome2 = verify_schema_provenance_with_trust_list(&unknown_peer, &admin_ui_with_alice);
    assert!(
        matches!(outcome2, ProvenanceOutcome::Trusted),
        "§4.32: peer in trust-list (within-envelope) MUST surface \
         Trusted (silent); got {outcome2:?}"
    );

    // §4.32 envelope-bound assertion: an unrelated peer is still
    // prompted even with alice in the trust-list — the trust-list is
    // PER-DID, not a blanket switch.
    let other_peer = common::manifest_fixtures::stub_peer_did_attacker();
    let outcome3 = verify_schema_provenance_with_trust_list(&other_peer, &admin_ui_with_alice);
    assert!(
        matches!(outcome3, ProvenanceOutcome::UserPromptRequired { .. }),
        "§4.32 per-DID granularity: a different peer (not in the \
         trust-list) MUST still surface UserPromptRequired even when \
         alice is in the list; got {outcome3:?}"
    );
}
...
#[test]
// G-CORE-7 §6.4 un-ignored: RedbManifestStore durable round-trip
// survives a store reopen on the same redb path.
#[cfg(any(not(target_arch = "wasm32"), target_os = "wasi"))]
fn redb_manifest_store_install_record_survives_store_reopen() {
    use benten_id::did::Did;
    use benten_platform_foundation::manifest_store::RedbManifestStore;
    use benten_platform_foundation::plugin_manifest::InstallRecord;

    let user_kp = Keypair::generate();
    let plugin_did = Did::from_string_for_test_fixture("did:key:zPluginRedb".to_string());

    // Build an honestly user-signed install record.
    ...
    record.user_signature = user_kp.sign(&record.signing_payload()).to_bytes().to_vec();

    let tempdir = tempfile::tempdir().expect("tempdir");
    let path = tempdir.path().join("manifest-store.redb");

    // Open store-1; install the record; drop store-1.
    {
        ...
        assert!(
            ...
            "post-install: store-1 MUST observably contain the record"
        );
    }

    // Re-open at the SAME path; the previously-persisted record MUST
    // load and re-verify (would-FAIL if the store is in-RAM only /
    // not flushed on drop).
    let mut store2 = RedbManifestStore::open_or_create(&path).expect("reopen at same path");
    let loaded = store2
        .load_verified(&plugin_did)
        .expect("§6.4: install record MUST survive a store reopen");
    assert_eq!(
        loaded.consenting_user_did,
        user_kp.public_key().to_did(),
        "loaded record's consenting user-DID matches the signer"
    );
    assert_eq!(loaded.nonce, record.nonce, "loaded record byte-equality");
    assert!(
        store2.captured_user_notifications().is_empty(),
        "clean round-trip MUST NOT emit drift notifications"
    );
}

#[test]
#[cfg(any(not(target_arch = "wasm32"), target_os = "wasi"))]
// Sibling GREEN-arm for §6.4: the durable store still surfaces
// `PluginManifestInvalid` for a missing plugin-DID at load.
fn redb_manifest_store_load_for_unknown_plugin_did_surfaces_plugin_manifest_invalid() {
    use benten_errors::ErrorCode;
    use benten_id::did::Did;
    use benten_platform_foundation::manifest_store::RedbManifestStore;

    let tempdir = tempfile::tempdir().expect("tempdir");
    let path = tempdir.path().join("manifest-store-unknown.redb");
    let mut store = RedbManifestStore::open_or_create(&path).expect("open_or_create");
    let unknown = Did::from_string_for_test_fixture("did:key:zPluginUnknownRedb".to_string());
    let err = store
        .load_verified(&unknown)
        .expect_err("unknown plugin-DID load MUST surface a typed error");
    assert_eq!(
        err,
        ErrorCode::PluginManifestInvalid,
        "missing-record load MUST surface PluginManifestInvalid"
    );
}
