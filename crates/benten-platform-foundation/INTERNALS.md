...
**G-CORE-4 §4.24 recursive vocabulary walk.** Post-G-CORE-4 the format renderers route through `render_html_json_recursive` / `render_plaintext_recursive` (the flat-walk `render_html_json` / `render_plaintext` shapes are preserved as `#[allow(dead_code)]` for inline canary tests). The new arm threads a `RecursiveVocabWalker` (private — engine seam + walk principal + bounded recursion depth, default `8`) through the walk: for each emitted READ primitive whose `VOCAB_LABEL_PROPERTY_KEY` is `FieldRef` the walker follows the `REF_TARGET` edge to the descriptor Node + performs a secondary `MaterializerEngine::read_node_as` against the walk principal + embeds the resolved body inline via `render_resolved_node_html` (HTML arm) or a `[ref:<field>=resolved]` marker (plaintext arm); for `FieldList` / `FieldMap` the walker iterates `ITEM_TYPE` / `KEY_TYPE` + `VALUE_TYPE` descriptor edges + emits per-edge markers; for `FieldEnum` / `FieldUnion` the walker dispatches `VARIANT` edges. The 4 marker constants `RESOLVED_REF_BODY_MARKER` / `RESOLVED_LIST_MARKER` / `RESOLVED_MAP_MARKER` / `RESOLVED_VARIANT_MARKER` are re-exported from the crate root so tests + downstream consumers can grep for the recursive arm's effect. Recursion-depth-cap emits a `<benten-resolved-ref-depth-cap>` marker (HTML) / `[ref:<field>=depth-cap]` (plaintext) and stops descending — would-FAIL pin against deep-FieldRef-chain stack-blowup attacks. End-to-end pin: `tests/tf5_424_materializer_recursive_walk_into_vocabulary_edges.rs`.
...
- `CapRevoker` / `PrivateNamespaceTeardown` / `SubscriptionRegistry` trait ports + the install-side `CapMinter` / `PrivateNamespaceProvisioner` ports.
- `UninstallPorts` bundle + the symmetric `InstallPorts` / `InstallParams` split (Surf-1 #888).
...
- `module_ecosystem` now hosts only upgrade-time + trust auxiliaries (`verify_install_record`, `decide_upgrade_consent`, `verify_upgrade_author_continuity`, `new_version_available_code`, `check_author_trust`, `UpgradeConsentDecision`); the legacy `install_plugin` / `install_plugin_persisting_did` precursors + their `InstallResult` / `InstallerShape` supports were DELETED at Phase-4-Meta-Core G-CORE-0 (plan §1.A.FROZEN item 7, HARD-RULE-12 clause-(a)) — the canonical install path is `plugin_lifecycle::install_plugin` (the 11-step pipeline with full Layer-1 cap cascade + Layer-2 consent + Layer-3 envelope per CLAUDE.md #18). `InstallerShape` lives at `plugin_lifecycle::InstallerShape`.
...
Six trait ports live in the crate (three uninstall-side + two install-side + the materializer read seam); the uninstall-side three are bundled in `UninstallPorts` and the install-side two in `InstallPorts`, a deliberately symmetric `*Ports` shape (Surf-1 #888):
...
2. `CapRevoker` (in `plugin_lifecycle.rs`, `UninstallPorts`) — engine-side adapter wires to `Engine::revoke_capability_by_grant_cid` (PR #199) iterated over the cap-store's `audience` / `issuer` indexes.
3. `PrivateNamespaceTeardown` (in `plugin_lifecycle.rs`, `UninstallPorts`) — engine-side adapter walks the storage backend for rows scope-prefixed `private:<plugin_did>:` and deletes each.
4. `SubscriptionRegistry` (in `plugin_lifecycle.rs`, `UninstallPorts`) — engine-side adapter walks the subscription registry for `subscriber_did == plugin_did` handles + terminates each.
5. `CapMinter` (in `plugin_lifecycle.rs`, `InstallPorts`) — engine-side adapter mints the Layer-1 root grant `user_did → plugin_did` per manifest `requires` scope.
6. `PrivateNamespaceProvisioner` (in `plugin_lifecycle.rs`, `InstallPorts`) — engine-side adapter provisions the `private:<plugin_did>:*` scope-prefix root.
...
- `module_ecosystem::install_plugin` legacy path was DELETED at Phase-4-Meta-Core G-CORE-0 (plan §1.A.FROZEN item 7, HARD-RULE-12 clause-(a)); all 4 prior consumer integration tests are now routed through the canonical `plugin_lifecycle::install_plugin`.
