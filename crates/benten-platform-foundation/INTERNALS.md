...
2. **`materializer`** (G23-B) — the `Materializer` trait + `HtmlJsonMaterializer` default impl + `PlaintextMaterializer` 2nd impl (arch-r1-10 output-format-pluggability validation per cag-r1-6) + the `Renderer` transport-abstraction trait + `BrowserRender` default impl. TauriRender lives in the sibling crate `benten-renderer-tauri` per G24-E + CLAUDE.md #19 (engine-extension category).
...
3. **`MaterializerError`** — six-variant error enum (`SchemaMismatch`, `SubscribeSeamFailure`, `UcanClockNotInjected`, `Backend`, `Other` + a `code()` accessor). Catches the `E_MATERIALIZER_CAP_DENIED` / `E_MATERIALIZER_SCHEMA_MISMATCH` / `E_MATERIALIZER_SUBSCRIBE_SEAM_FAILURE` G23-B ErrorCodes + the `E_UCAN_CLOCK_NOT_INJECTED` Phase-3 inheritance.
...
- `MaterializerError` (six variants + `code()` accessor).
