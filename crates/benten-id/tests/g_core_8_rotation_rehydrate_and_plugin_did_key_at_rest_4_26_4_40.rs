...
fn rotation_log_rehydrates_persisted_events_at_engine_open() {
    ...
    let some_did = Did::from_string_for_test_fixture("did:key:zRotatedKey".to_string());
    assert!(
...
fn resolve_did_for_cid_returns_owning_did_round_trip() {
    ...
    let absent =
        benten_id::did::Did::from_string_for_test_fixture("did:key:zNotInStore".to_string());
    assert!(
