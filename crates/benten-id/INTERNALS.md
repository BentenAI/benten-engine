...
- `benten-caps` — UCAN backend (`crates/benten-caps/src/backends/ucan.rs`) imports `Ucan`, `Did`, `Capability`, `validate_chain_*` functions. *(COLLAPSE-WITH-RESIDUAL refinement-audit-2026-05 S3 P1: the `DeviceRevocation` import + the durable revocation-set construction `validate_chain_with_device_revocations` were DELETED — the un-anchored device-revocation pipe was the #1230 BLOCKER; see `docs/SECURITY-POSTURE.md` Compromise #23 SUPERSEDED-BY-COLLAPSE.)* The Phase-4-Foundation `manifest_envelope_chain_validation` module (G24-D-FP-2) consumes `validate_chain_for_capability` + `RotationLog::accept_rotation_event` for HLC-monotonic-strict rotation acceptance.
...
- **Chain-walk validators:** `validate_chain_no_time_check` / `validate_chain_at` / `validate_chain_for_audience` / `validate_chain_for_capability` / `validate_chain_with_rotation_log` / `validate_chain_with_attestations`. *(COLLAPSE-WITH-RESIDUAL refinement-audit-2026-05 S3 P1: `validate_chain_with_device_revocations` was DELETED — the standalone un-anchored device-revocation walker was the #1230 perpetual-DoS BLOCKER root site; device-key revocation now flows through user-root UCAN-grant revocation `benten-caps::revoke`. See `docs/SECURITY-POSTURE.md` Compromise #23.)* Composition via the private `validate_chain_inner(chain, now, expected_audience)` (ucan.rs:388).
...
**UCAN issuance + chain validation (G14-A1).** `Ucan::builder` → chained builder. Validation entry points: `(time? × audience? × capability? × revocation? × rotation? × device-attestation?)`. The composed `validate_chain_for_capability(chain, expected_audience, required, now)` is the engine-typed-CALL surface AND the single subsume-relation surface (it wraps the private `caps_match_or_subsume`); narrower entries for the single-gate cases. (Hyg-1 #304: the redundant public `capability_satisfies_requirement` wrapper — zero callers workspace-wide — was removed.)

**Verifiable Credentials (G14-A2).** `Credential::builder` → `sign`. Verify-side: `verify` / `verify_at` / `verify_with_registry` / `verify_in_trust_domain` / `verify_bytes_in_trust_domain`. `TrustDomain::new` / `TrustDomain::empty` + `RevocationRegistry::new` + `RevocationRegistry::revoke` / `is_revoked` (poison-tolerant; Safe-1 #512). *(refinement-audit-2026-05 S-ID: `TrustDomain::allow` + `CredentialBuilder::extra_type` removed — zero-caller Hyg-1.)*

**DID rotation (G14-A2 + G24-D-FP-2).** `rotate_keypair(did, old_kp, new_kp, superseded_at)` → `RotationAttestation`. `RotationLog::from_entries` + `RotationLog::append` + `RotationLog::is_superseded` + `RotationLog::accept_rotation_event` (HLC-monotonic-strict + verbatim-replay defense per phase-4-backlog §4.10). *(S-ID: the `is_did_superseded` free-fn alias is REMOVED — Hyg-1 #301; callers use `RotationLog::is_superseded`.)*

**Device-DID attestation (G14-A2; post-COLLAPSE — Compromise #23 SUPERSEDED-BY-COLLAPSE, see `docs/SECURITY-POSTURE.md`).** Issuance entry points (`issue` / `issue_at` / `issue_with_nonce` / `issue_for_browser_target` / `issue_with_runtime_check` / `issue_with_authority`). Whole-struct canonical-bytes round-trip via the inherent `DeviceAttestation::canonical_bytes` + `from_canonical_bytes` (includes `signature`; #329). Signature-input encoding is the `CanonicalBytes` trait impl (excludes `signature`; UFCS-reached — see the `CanonicalBytes` entry below). `verify_signature_with(parent_pk)`. *(COLLAPSE P0-P3: `Acceptor` (the entire acceptance pipe — `Acceptor::new` / `new_with_revocations` / `with_parent_lookup` / `accept_at` / `accept`), `DeviceRevocation`, `RevocationReason`, `FreshnessPolicy`, `revocation_canonical_bytes`, and the chain-walker gate `validate_chain_with_device_revocations` are ALL DELETED — the device envelope is no longer a distinct trust-root. Stale-frame replay is bounded by a plain freshness window at the engine inbound-sync seam; durable revocation collapses to user-root UCAN revocation `benten_caps::revoke`. The surviving primitive is the signed `DeviceAttestation` struct + `CapabilityEnvelope` ceiling type + `envelope_widens` + `generate_fresh_nonce`.)* Chain-walker hook MOVED OUT of benten-id by COLLAPSE P2 CONSOLIDATE: the validate-side `runs_sandbox` envelope-violation gate is now `benten_caps::chain_authority::validate_chain_with_envelope_ceiling` (formerly `benten_id::ucan::validate_chain_with_attestations`); benten-id retains only the pure `validate_chain*` primitive + `RotationLog`/`DeviceAttestation` types. Broader multi-dimension enforcement extends that same single benten-caps seam — COLLAPSE P5 #1252 territory, NOT benten-id.
...
**Plugin-DID mint + store (G24-D / Phase-4-Foundation).** `plugin_did::mint() -> PluginDidHandle` + `PluginDidHandle::did()` / `keypair()` (Hyg-1 #313: keypair() retained for the CLAUDE.md #18 constrained-issuer path, Phase-4-Meta) + `audience_matches_plugin_did` + `PluginDidStore::{new, mint_and_store, insert (caller-mint-first; Err(PluginDidHandleDuplicate) on dup), get, revoke, len, is_empty}`. Test-only: `handle_with_did_for_test` (gated on `cfg(any(test, feature = "testing"))`). *(S-ID: `PluginDidStore::iter` REMOVED — Hyg-1 #318 zero callers.)*

**Sibling `GrantReader` trait (G27-C / Phase-4-Foundation §4.3) — NEW.** `benten_id::grant_reader::{GrantReader, ReaderError, ReaderError::BackendFailed, ReaderError::RecordMalformed}` (ReaderError re-exported at crate root — Surf-1 #854). Two-method trait: `has_unrevoked_grant_for_scope(&str)` (lift of existing benten-caps shape) + `has_unrevoked_grant_for_grant_cid(&Cid)` (CID-keyed companion closing §13.11 structural gap). Sibling-not-extension shape per arch-r1-10.

**`CanonicalBytes` trait (Qual-2 #759 — NEW; refinement-audit-2026-05 S-ID).** `benten_id::CanonicalBytes` (re-exported at crate root from `canonical_bytes` module). Single `fn canonical_bytes(&self) -> Vec<u8>` consolidating the 6 previously-duplicated private `canonical_bytes` sites + 2 inline `SigInput<'a>` projection structs (Pattern-F Qual-2). Implemented for `RotationAttestation` / `UcanClaims` / `CredentialClaims` / `DeviceAttestation`. **Byte-identical to the prior per-site encodings** — pure structural extraction, zero wire-byte change (§3.5m P-III; byte-equality + sign/verify round-trip pin at `tests/canonical_bytes_trait.rs`). For `RotationAttestation`/`DeviceAttestation` the impl is the *signature-input projection* (excludes the `signature` field); for `DeviceAttestation` this is **distinct from** the inherent `canonical_bytes`/`from_canonical_bytes` whole-struct round-trip pair — the trait impl is reached via UFCS so the two never silently alias (the inherent method wins in method-call position and remains the round-trip encoder; #329 round-trip pin unaffected). *(Pattern-F #1161 reconciliation: the `RevocationSet`-trait half (#787) eroded post-COLLAPSE — `Acceptor::nonce_store` deleted + `validate_chain_with_*` relocated to benten-caps; only `RevocationRegistry` + `RotationLog` remain, an insufficient 2-substrate basis for a forced trait. #971/#973/#976 are now benten-caps-lane (out of ST-ID scope).)*

---

## 4a. Concurrency contract (Safe-4 #642)

Per refinement-audit-2026-05 Safe-4 (META #739 lock-poisoning posture / META #735 lock-duration), the crate's public types fall into two concurrency shapes. Callers MUST NOT add a defensive outer lock around an interior-synchronized type (the engine-side defensive `Mutex<Acceptor>` that motivated this contract is moot — `Acceptor` is COLLAPSE-deleted).

**Interior-synchronized (`&self` mutators; safe to share across threads without an outer lock):**

- **`RevocationRegistry`** — `revoke(&self, …)` / `is_revoked(&self, …)` take `&self` and lock an internal `Mutex<HashSet<String>>`. The lock is **poison-tolerant**: a panic in another thread while the lock is held does NOT brick this security-critical revocation substrate — both methods recover via `PoisonError::into_inner` (Safe-1 #512). The guarded `HashSet` has no cross-call invariant that a poison could leave half-built, so continuing with the recovered guard is fail-CLOSED-preserving.

**Caller-synchronized (`&mut self` mutators; the borrow checker forces external serialization — no interior lock):**

- **`RotationLog`** — `append(&mut self, …)` / `accept_rotation_event(&mut self, …)`; read path `is_superseded(&self)` / `entries(&self)` is shared-borrow-safe. No interior `Mutex`; callers serialize writes via `&mut`.
- **`PluginDidStore`** — `insert(&mut self, …)` / `mint_and_store(&mut self)` / `revoke(&mut self, …)`; reads `get(&self)` / `len(&self)` shared-borrow-safe. Caller-synchronized.
- **`TrustDomain`** — constructed once (`new` / `empty`), then read-only via `contains(&self)`. Effectively immutable post-construction.

**Send + Sync.** All public value types (`Keypair`, `Did`, `Ucan`, `Credential`, `RotationAttestation`, `DeviceAttestation`, `PluginDidHandle`, `RevocationRegistry`, `RotationLog`, `PluginDidStore`, `TrustDomain`) are `Send + Sync` (no `Rc`/`RefCell`/raw-thread-unsafe interior). The `Send + Sync` bound on the `GrantReader` trait is explicit in its definition. A compile-time pin lives at `crates/benten-id/tests/concurrency_contract.rs` (`assert_send_sync` generic-fn instantiation per type — `static_assertions` is intentionally NOT a dep; the generic-fn trick is the stable-Rust equivalent).
...
- **`validate_chain_for_capability` is the single subsume-relation surface.** Engine-side query uses the SAME private `caps_match_or_subsume` relation the chain-walker uses internally — single source of truth. (Hyg-1 #304: the standalone public `capability_satisfies_requirement` wrapper was zero-caller dead code and was removed; the relation is reached through `validate_chain_for_capability`, not a separate exported wrapper.)
...
- ✅ **UCAN revocation observance closure** — Track B PR #109/#199; the durable observance composes against `validate_chain_with_rotation_log` here. *(COLLAPSE-WITH-RESIDUAL refinement-audit-2026-05 S3 P1: the `validate_chain_with_device_revocations` half was DELETED — the un-anchored device-revocation pipe was the #1230 BLOCKER; user-root UCAN-grant revocation `benten-caps::revoke` is now the single revocation seam. See `docs/SECURITY-POSTURE.md` Compromise #23.)*
