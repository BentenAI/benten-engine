...
**Device-DID attestation (G14-A2 + G16-D wave-6b; Compromise #23 SUPERSEDED-BY-COLLAPSE — see `docs/SECURITY-POSTURE.md`).** Issuance entry points (`issue` / `issue_at` / `issue_with_nonce` / `issue_for_browser_target` / `issue_with_runtime_check` / `issue_with_authority`). Canonical-bytes round-trip via `canonical_bytes` + `from_canonical_bytes`. `verify_signature_with(parent_pk)`. `Acceptor::new` + `Acceptor::new_with_revocations` + `Acceptor::with_parent_lookup` + `Acceptor::accept_at` / `accept`. `DeviceRevocation::issue` + `verify_signature_with`. *(COLLAPSE-WITH-RESIDUAL refinement-audit-2026-05 S3: the standalone chain-walker revocation gate `validate_chain_with_device_revocations` was DELETED in P1 — it was the #1230 un-anchored-DoS root site. The `Acceptor::accept_at` revocation step + `DeviceRevocation` type are RETAINED until P3 because deleting `Acceptor` is compile-coupled to the deferred `engine_sync::DeviceAttestationEnvelope::verify` rewire. The retained `Acceptor::accept_at` revocation step still resolves `revocation.parent_did` to a public key + `verify_signature_with` before honoring — forged-revocation fail-CLOSED preserved on that path.)* Chain-walker hooks: `validate_chain_with_attestations` (envelope-violation gate) — the revocation-gate hook is deleted. `issue_with_authority` is wired at the napi boundary (`JsDeviceAttestation::issue_with_authority` factory) — construction-time cap-r4-7 envelope-widening defense, the companion to the chain-walker's consume-time attenuation gate (closes Hyg-1 #333).
