...
#[test]
#[ignore = "SEAM WIRED (Row D-1 CLOSED at R6 R1 FP-F4 §S1 / sharpened R6 R2 FP-B): the WriteBoundaryChainValidator seam is now structurally-always-on at 14 WRITE entry points via Engine::admit_write_chain, with the always-mounted NoopWriteBoundaryChainValidator returning NotApplicable (admit). What remains deferred is installing a PRODUCTION validator that fires the per-WRITE user-DID root-chain check — the G-COMP-1 consumption per docs/V1-FROZEN-INTERFACE-DEFERRED.md Row D-1 (FORENSIC). Named destination: Row D-1 (FORENSIC) / G-COMP-1 (docs/future/phase-4-backlog.md §4.23 tracks the same production-validator wave). Pin source: r4-triage §1 r4-tc-1 + threat-model §T4 step 2 defense-in-depth; substantive shape preserved in body."]
