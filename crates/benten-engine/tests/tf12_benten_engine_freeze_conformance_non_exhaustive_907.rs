...
// W5 owns the freeze-conformance arms per R2 §7. The pins above cover the
// pre-existing #907 ONE-coherent-sweep manifest. W5 adds the §1.A.FROZEN
// item 15 (c) Scope enum + (e) EncryptionClass enum closure arm: these two
// new S&C enums MUST appear in the post-freeze #907 manifest (since they
// are public enums in the workspace's frozen inventory). This is the
// cross-wave coupling between G-CORE-3 (which mints the enums) and G-CORE-9
// (which sweeps #[non_exhaustive] across the inventory).
// ===========================================================================

/// R3-W5 EXTENSION — Scope + EncryptionClass MUST appear in the
/// #907 ONE-coherent-sweep manifest post-freeze. Closes the cross-
/// wave coupling: G-CORE-3 mints the enums; G-CORE-9 enumerates them.
#[test]
#[ignore = "RED-PHASE: un-ignore at G-CORE-9 FREEZE (when the #907 manifest enumerates Scope + EncryptionClass + their #[non_exhaustive] decision). r2-test-landscape §7 W5 + §1.A.FROZEN item 11 + item 15 (c)(e)."]
fn scope_and_encryption_class_enums_appear_in_907_manifest_post_freeze() {
    // The pre-existing 8 pins in this file scan the workspace for the
    // #907 manifest at the FREEZE-committed location. The W5 closure
    // arm asserts the two new S&C enums are PRESENT in that manifest.
    // Body intent at un-ignore (mirrors the pattern of
    // `all_named_anchor_enums_have_recorded_decision_in_manifest`):
    //
    //   let manifest_path = workspace_root()
    //       .join("docs/freeze/non_exhaustive_workspace_enumeration.tsv");
    //   let manifest = std::fs::read_to_string(&manifest_path)
    //       .expect("manifest exists post-FREEZE");
    //   for needed in ["Scope", "EncryptionClass"] {
    //       assert!(manifest.contains(needed),
    //           "#907 manifest MUST enumerate {needed} (§1.A.FROZEN \
    //            item 15 (c)/(e)) with its #[non_exhaustive] decision");
    //   }
    //
    panic!(
        "RED-PHASE (W5 extension): #907 manifest enumerates Scope + \
         EncryptionClass. Un-ignore at G-CORE-9 FREEZE (when both \
         enums land + the #907 sweep applies). Would-FAIL if a future \
         agent ships either enum without recording its #[non_exhaustive] \
         decision in the manifest (the cross-wave coupling between \
         G-CORE-3 enum-mint and G-CORE-9 sweep)."
    );
}
