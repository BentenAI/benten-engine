...
/// count matches the F1 path-(a) ratified value.
///
/// Per-file breakdown at landing:
...
///
/// Total: 13 sites (production consumers; the `pub(crate) fn admit_write_chain`
...
/// existing wiring being silently deleted).
///
...
fn workspace_walker_audit_thirteen_admit_write_chain_call_sites() {
    ...
    assert_eq!(
        total, EXPECTED_WRITE_SITES_PER_F1_PATH_A,
        "F1 path-(a) WRITE-site count drift detected: source scan found {total} \
         ...
         but EXPECTED_WRITE_SITES_PER_F1_PATH_A={EXPECTED_WRITE_SITES_PER_F1_PATH_A}. \
