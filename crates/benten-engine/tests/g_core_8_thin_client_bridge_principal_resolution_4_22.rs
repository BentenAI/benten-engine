...
fn bridge_resolves_principal_from_authenticated_session_not_client_input() {
    ...
    // -----------------------------------------------------------------
    // G-CORE-8 §4.22 LANDED: the bridge is at
    // `benten_engine::thin_client_bridge::ThinClientBridge`. Exercise
    // it end-to-end: an attacker's session resolves to the attacker's
    // DID (not a victim's); the bridge surface has NO client-supplied
    // principal field (structurally impossible to spoof in-band).
    // -----------------------------------------------------------------
    use benten_engine::thin_client_bridge::ThinClientBridge;
    use std::sync::Arc as StdArc;

    let bridge = ThinClientBridge::new(StdArc::new(test_session()));
    let bridge_session = bridge.session();
    let bridge_token = establish_session_as(bridge_session, ATTACKER_DID, origin);
    let resolved_via_bridge = bridge
        .resolve_principal_for_request(&bridge_token, origin)
        .expect("bridge resolves to session-bound attacker DID");
    assert_eq!(
        resolved_via_bridge, ATTACKER_DID,
        "§4.22 G-CORE-8: bridge.resolve_principal_for_request returns \
         the session-bound principal-DID; structurally cannot route a \
         client-supplied principal because the API takes no such field"
    );

    // Cross-origin via the bridge: typed rejection at the bridge boundary
    // surfaces the §4.22 typed code.
    let cross_origin_err = bridge
        .resolve_principal_for_request(&bridge_token, "https://attacker-controlled.example")
        .expect_err("cross-origin presentation through bridge rejects");
    assert_eq!(
        cross_origin_err.code(),
        benten_errors::ErrorCode::ThinClientBridgePrincipalUnresolved,
        "§4.22 G-CORE-8: cross-origin presentation surfaces the typed \
         ThinClientBridgePrincipalUnresolved code at the bridge boundary"
    );
}
...
fn thin_client_boundary_carries_no_cap_tokens_to_the_client() {
    ...
    // -----------------------------------------------------------------
    // G-CORE-8 §4.22 LANDED: the bridge surface IS the no-cap-tokens
    // boundary. The bridge takes only `(SessionToken, &str
    // presented_origin)` and returns ONLY a principal-DID string —
    // structurally no cap-bundle field exists. The structural pin
    // above (compile-time field destructuring) IS the assertion.
    // -----------------------------------------------------------------
    use benten_engine::thin_client_bridge::ThinClientBridge;
    use std::sync::Arc as StdArc;
    ...
    // The bridge return type is `Result<String, EngineError>` — the
    // Ok variant is a `String` (the principal-DID), NOT any cap-
    // bearing artifact. This is the structural anchor for "no cap
    // tokens cross the boundary."
    let principal: String = bridge
        .resolve_principal_for_request(&bridge_token, origin)
        .expect("bridge round-trip");
    assert!(
        principal.starts_with("did:key:"),
        "§4.22 G-CORE-8: bridge return type is a String principal-DID — \
         a regression that added a cap-bundle field to the return type \
