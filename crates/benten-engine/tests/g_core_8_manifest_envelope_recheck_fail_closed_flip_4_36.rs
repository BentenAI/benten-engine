...
// ---------------------------------------------------------------------------
#[test]
#[ignore = "RED-PHASE: un-ignore at G-CORE-8 (§4.25 sync-hydrate path \
            — unresolvable peer-DID at sync hydration MUST fail-closed; \
            parallel defense to §4.36 merge-recheck on the SAME \
            recheck-outcome surface)"]
fn sync_hydrate_unresolvable_peer_did_fails_closed_parallel_to_merge_recheck() {
    // SHIPPED-SURFACE EXERCISE: the recheck-outcome enum + reject
    // primitive ARE the substrate the §4.25 sync-hydrate path will
    // consult. Exercise the SHIPPED OutsideEnvelope reject (the only
    // currently-reject-shaped arm at HEAD) on a sync-hydrate zone label
    ...
    let res = outcome_to_row_reject(outside_at_hydrate, "sync-hydrate-zone", "row-hydrate-1");
    assert!(
        res.is_err(),
        "shipped substrate: the recheck-outcome reject primitive is \
         zone-label-agnostic at HEAD (any zone string composes); the \
         §4.25 sync-hydrate path will pass a hydrate-shaped zone label \
         through the SAME primitive post-G-CORE-8 (would-FAIL if the \
         primitive grew zone-specific behavior that excluded hydrate)"
    );

    // RED-arm: the §4.25 sync-hydrate path's STRUCTURAL CONSULTATION of
    // this recheck primitive on an unresolvable peer-DID is the
    // G-CORE-8 deliverable. At HEAD the sync-hydrate path does NOT
    // consult the recheck (only the §4.36 merge path does, opt-in).
    panic!(
        "§4.25 sync-hydrate unresolvable-peer-DID defense undelivered: \
         at HEAD the §4.25 sync-hydrate path does NOT consult the \
         manifest-envelope-recheck on incoming peer-DIDs. The shipped \
         OutsideEnvelope reject primitive (substrate exercised above) \
         is the rail the post-rename UnresolvedDeny arm rides; G-CORE-8 \
         wires the §4.25 path to consult the same rechecker as the \
         §4.36 merge path so an unresolvable peer-DID at hydrate fails \
         closed (V-3 / F-2 sync-attack-fabric anchor)."
    );
}
