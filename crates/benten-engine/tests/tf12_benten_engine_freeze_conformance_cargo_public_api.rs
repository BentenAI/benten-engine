...
// W5 owns the freeze-conformance + cross-wave arms per R2 §7. The pins above
// (8 tests) cover the §1.A.FROZEN item 9 baseline-regen / deny-on-drift /
// napi-non-broken arms. W5 adds the §1.A.FROZEN item 14 P2P-interop sibling
// arm at the engine boundary: a freeze-time assertion that the
// `cargo-public-api` baseline reflects the §1.A.FROZEN item 15 (g) two-CID
// mapping public surface (the engine-level mirror of the crypto-suite-side
// p2p-interop pin in tf12_freeze_conformance_p2p_interop_lane.rs).
// ===========================================================================

/// R3-W5 EXTENSION — cargo-public-api baseline reflects the
/// §1.A.FROZEN item 15 (g) two-CID-mapping public surface at the
/// engine boundary.
///
/// Mirror at the engine level of the crypto-suite p2p-interop lane.
/// The engine exposes the public `walk_share_scope` entry that consumers
/// use to ride the two-CID mapping; this pin asserts that surface is
/// present in the engine's baseline at FREEZE. Sibling-structural to the
/// `public_api_gate_is_deny_on_delta_post_freeze` test above; both rely
/// on the same `.github/workflows/cargo-public-api.yml` machinery.
#[test]
#[ignore = "RED-PHASE: un-ignore at G-CORE-9 FREEZE (when docs/public-api/benten-engine/benten-engine.txt is regenerated to include the new `walk_share_scope` surface). r2-test-landscape §7 W5 + §1.A.FROZEN item 15 (g)+(h)."]
fn engine_baseline_includes_walk_share_scope_public_surface_post_freeze() {
    let baseline = workspace_root().join("docs/public-api/benten-engine/benten-engine.txt");
    let body = std::fs::read_to_string(&baseline).unwrap_or_else(|e| {
        panic!(
            "§4-F / R3-W5: docs/public-api/benten-engine/benten-engine.txt \
             MUST exist post-FREEZE per §1.A.FROZEN item 9 (per-crate \
             baselines regenerated + committed at G-CORE-9). Missing at \
             {}: {e}",
            baseline.display()
        )
    });
    assert!(
        body.contains("walk_share_scope"),
        "§4-F / R3-W5: the engine's cargo-public-api baseline MUST \
         enumerate the new `walk_share_scope` entry-point (§1.A.FROZEN \
         item 15 (h) — the walker IS a Subgraph shipped once in \
         benten_core; the engine exposes it as a public entry). \
         Would-FAIL if G-CORE-9 forgets to regen the engine baseline \
         after G-CORE-3 lands the entry-point. Sibling pin: \
         tf12_freeze_conformance_p2p_interop_lane.rs at the crypto-suite \
         side."
    );
}
