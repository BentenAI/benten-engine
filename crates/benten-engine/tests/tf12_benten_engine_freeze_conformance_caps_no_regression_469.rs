...
// W5 owns the freeze-conformance arms per R2 §7. The pre-existing 7 pins
// cover the §4.69 cap-mutation no-regression invariant. W5 adds the
// cross-wave coupling: the §8-E SEALED-discipline CapabilityPolicy hook
// shape (new defaulted trait methods) MUST NOT regress onto `Engine`
// directly (the surface-organization invariant extends to the §8-E hooks
// landed by G-CORE-8).
// ===========================================================================

/// R3-W5 EXTENSION — §8-E SEALED hooks (audience-aware check_write,
/// install-time consent, per-delegation runtime) land on
/// `CapabilityPolicy` trait, NOT directly on `Engine`. This pin
/// extends the §4.69 no-regression invariant to the new G-CORE-8
/// hooks. §3.6f waiver: structural source-scan (same pattern as the
/// pre-existing pins in this file).
#[test]
fn r3_w5_extension_sealed_hooks_live_on_capabilitypolicy_not_on_engine_no_regression() {
    // The pre-existing pins assert: zero cap-mutation `pub fn` on
    // `Engine`; all 9 on `EngineCapsHandle`. W5 extension: the §8-E
    // SEALED hooks (audience-aware check_write, install_consent,
    // per_delegation_runtime — the three new Phase-4-Meta hooks per
    // plan §1.A.FROZEN item 8) MUST land on `CapabilityPolicy`
    // (sealed trait), NOT as `pub fn` on `Engine`.
    //
    // This is GREEN at HEAD ONLY in the trivial sense that those
    // hooks are unbuilt — so the pin asserts "no `Engine::check_write`,
    // `Engine::install_consent`, `Engine::per_delegation_runtime`
    // method exists today AND ALSO post-G-CORE-8".  Becomes load-
    // bearing once G-CORE-8 ships.
    let engine_src_dir = engine_src();
    let mut forbidden_hook_sites: Vec<String> = Vec::new();
    // Local helper: walk a directory tree and call `f` for each .rs
    // file (path, body). Same shape as the visit_rs helper in the
    // sibling tf12_benten_engine_freeze_conformance_cargo_public_api.rs
    // (no shared module — tests are compilation units; we keep the
    // helper local to this test for §3.13 per-test-isolation).
    ...
    walk_rs(&engine_src_dir, &mut |path, body| {
        for hook in [
            "pub fn check_write",
            "pub fn install_consent",
            "pub fn per_delegation_runtime",
            "pub async fn check_write",
            "pub async fn install_consent",
            "pub async fn per_delegation_runtime",
        ] {
            if body.contains(hook) {
                forbidden_hook_sites.push(format!(
                    "{}: contains `{}` — §8-E hooks live on \
                     CapabilityPolicy SEALED trait, NOT on Engine",
                    path.display(),
                    hook
                ));
            }
    ...
    assert!(
        forbidden_hook_sites.is_empty(),
        "§4.69 + §8-E no-regression: §8-E SEALED hooks (check_write, \
         install_consent, per_delegation_runtime) MUST live on \
         CapabilityPolicy sealed trait, NOT directly on Engine. \
         Findings: {forbidden_hook_sites:?}. Would-FAIL if a future \
         agent inadvertently shadows the trait method on Engine \
         (forecloses the SEALED extensibility-by-trait-impl path)."
    );
}
