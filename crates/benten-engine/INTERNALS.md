...
**Consumers (out):** `bindings/napi` (the napi-rs cdylib that powers `@benten/engine` JS/TS), `benten-platform-foundation` (Phase-4-Foundation crate — owns admin UI v0, plugin manifest, plugin lifecycle, manifest store, materializer, schema compiler, registry, workflow→plugin promotion; consumes `Engine` through its public surface only), every integration test in `tests/`, every bench in `benches/`, every crate's `dev-dependencies` cross-link that needs `Engine` in a fixture (with `features = ["test-helpers"]`).

**Features:**
- `default = []` (closed-by-default)
- `test-helpers` — implies `iteration-budget-test-grade`; lights up `crate::testing` + the `testing_*` methods sprinkled across `Engine` + `pub` accessors gated on `cfg(any(test, feature = "test-helpers"))`. (The former `envelope-cache-test-grade` feature was retired + fully deleted per #868 — the durable `benten_eval::SuspensionStore` replaced its test-grade gate.)
...
- **`engine.rs`** (~4556 LOC — largest file in the crate) — `EngineGeneric<B: GraphBackend>` (the actual struct), `EngineInner` (Arc'd shared state with ~26 fields including the `manifest_envelope_rechecker: Option<Arc<dyn ManifestEnvelopeRechecker>>` slot at `engine.rs:1002` and the Wave-E HELD `shares_policy_resolver: Option<Arc<dyn SharesPolicyResolver>>` slot — the Layer-3 manifest-`shares` enforcement port consulted by `delegate_capability`), `SubgraphCache`, the `pub type Engine = EngineGeneric<RedbBackend>` alias (or `<BrowserBackend>` under `--features browser-backend`), and the public dispatch methods (`open` / `builder` / `apply_atrium_merge` / `caps()` / `views()` (Wave-E HELD #1190 facet-handle split part 1) / `register_subgraph` / `register_subgraph_replace` / `register_crud` / `call` / `call_as` / `trace` / `handler_to_mermaid` / `handler_predecessors` / `handler_version_chain` / `dispatch_typed_call_public` / `set_manifest_envelope_rechecker` / `set_shares_policy_resolver`). Contains the `subgraph_for_spec` + `subgraph_for_crud` walkers that materialize a runnable `Subgraph` from a registered DSL spec. Contains the `WAIT TTL GC` entry-point glue and the `Drop` impl that fires the final-sweep. The huge file size is intrinsic to the responsibility: this IS the orchestrator.
- **`builder.rs`** (~973 LOC) — `EngineBuilder` (fluent surface), `BackendGrantReader` (the `GrantReader` impl backed by `RedbBackend::get_by_label("system:CapabilityGrant")`), the `:memory:` sentinel routing, the `NOAUTH_STARTUP_LOG` constant + once-fire emission, the assembly pipeline (`build` → `assemble`) that wires the change-broadcast tap, the IVM subscriber, the broadcast→eval-subscribe bridge, the policy resolution (NoAuth / explicit / grant-backed / UCAN-grounded), the clock-source defaults, and the three rehydration passes at engine open (module manifests, module bytes, handler-version chains).

### 3c. CRUD + transactions + diagnostics (resolved-alias `impl Engine` blocks)

- **`engine_crud.rs`** (~310 LOC) — public node/edge CRUD: `create_node` / `get_node` / `update_node` / `delete_node` / `create_edge` / `get_edge` / `delete_edge` / `edges_from` / `edges_to`. Every mutation refuses against a `read_only_snapshot` engine via `E_BACKEND_READ_ONLY`. **Hosts the canonical read seam (refinement-audit-2026-05 D1 #1189, Pattern-F Bundle 1 — Qual-1 #695 + Safe-1 #534 / META #593):** `read_node_inner(cid, principal: Option<Cid>)` is the ONE Node-read body (backend resolve → Inv-11 runtime probe → Option-C cap gate → optional principal threading) that `get_node` (principal `None`), `read_node_as` (principal `Some`), and `read_denied_for_cid` all compose against — replacing 3 prior near-duplicate bodies. `check_read_gate(ctx) -> Result<ReadGate, EngineError>` is the canonical **fail-CLOSED** capability decision: `Ok(())` → `Permitted`, `Err(CapError::DeniedRead)` → `DeniedReadCollapse` (Option-C / named compromise #2 — denied read indistinguishable from a clean miss), **`Err(other)` → `Err(EngineError::Cap(other))`** (Revoked / RevokedMidEval / any future `#[non_exhaustive]` denial variant surfaces a typed error rather than the pre-fix silent permit). `device_cid()` accessor is the single device-CID access pattern (closed the prior `get_node` raw-`MutexExt` vs `read_node_as` accessor drift). Closure-pinned by `tests/read_path_check_read_fails_closed_d1_1189.rs`.
...
- **`engine_caps.rs`** (~621 LOC) — `EngineCapsHandle<'eng>` + `CapProof` (Phase-3 G16-B-F), `install_proof` / `revoke` / `revoke_capability_by_grant_cid` (Phase-3 PR #199 UCAN revocation observance closure — surgical revoke by grant CID; updates BOTH durable `system:CapabilityRevocation` Node AND in-memory `(actor, zone)` revocation pair set consulted by `apply_atrium_merge` per-row recheck), the legacy `create_principal` / `grant_capability` / `grant_capability_with_proof` / `revoke_capability` privileged-write paths, `create_view` (legacy), `install_ucan_proof` for the UCAN-durable policy path, and `delegate_capability` (Phase-4-Foundation; threads through the plugin-runtime delegation envelope from `benten-platform-foundation`).
- **`engine_views.rs`** (~1205 LOC) — view materialization (`read_view` / `read_view_with` / `read_view_strict` / `read_view_allow_stale`), user-view registration (`register_user_view`), user-view drain + snapshot (`user_view_snapshot` / `user_view_on_update` / `user_view_drain_updates_since` / `user_view_change_offset`), the IVM view-strategy accessor (`view_strategy`), `materialize_view_with_gate` (composes the G15-A per-row read gate), the **`EngineViewsHandle<'eng>` facet** (Wave-E HELD #1190 / #834 facet-handle split part 1 — returned by `Engine::views()`; sibling to `EngineCapsHandle`; delegates to the direct `impl Engine` view methods, which STAY — additive, zero call-site cascade per RATIFIED P-II; the `#[deprecated]`-shim migration + napi JS-mirror are the RATIFIED Phase-4-Meta part-2/3), and the change-stream surface (`subscribe_change_events` / `test_subscribe_*` / `change_event_count` / `subscribe_emit_events*` / `emit_subscriber_count`). The `read_view_with` IVM read-gate routes through the shared `engine_crud::Engine::check_read_gate` canonical fail-CLOSED decision (refinement-audit-2026-05 D1 #1189 / Safe-1 #534): `DeniedRead` keeps the Option-C empty-`Outcome` collapse; any other `CapError` denial now surfaces `Err(EngineError::Cap(..))` instead of the pre-fix silent full-row-set leak. (It cannot share `read_node_inner` — no resolved Node, label is the view-registry hint, `target_cid: None`, fall-through is an empty `Outcome` not `Ok(None)` — but it MUST share the fail-CLOSED arm.)
...
- **`engine_wait.rs`** (~1553 LOC) — the WAIT primitive engine-side surface: `call_with_suspension` / `call_as_with_suspension` / `suspend_to_bytes` / `resume_from_bytes_unauthenticated` / `resume_from_bytes_as` / `resume_with_meta` (Phase-3 R6-FP closure — preserves typed `ResumePayload` distinction None-vs-Signal into `resume_from_bytes_inner` so eval-side maps None→`DurationElapsed` and Signal(v)→`Signal{value:v}`; see `engine_wait.rs:545`), `SuspendedHandle`, `ResumePayload`, `SuspensionOutcome`, the `HandlerRef` trait that lets callers identify a handler by either CID or stored handler-id. Also hosts the `put_node` + `read_node_as` Class-B-β surface (the load-bearing plugin-engine seam per CLAUDE.md baked-in #18 — public `_as` for non-trusted principals, `engine_crud.rs::get_node` for the user-facing-default-no-principal path; per `read_node_as` rustdoc the evaluator is the only caller of `_as`). Post-D1 #1189 (refinement-audit-2026-05) `read_node_as` is a thin `self.read_node_inner(cid, Some(*principal))` over the canonical `engine_crud` read seam — the Inv-11 probe + Option-C collapse + the Safe-1 #534 fail-CLOSED `check_read_gate` arm all live there; `read_node_as` was the most load-bearing of the 3 prior near-duplicate sites for the silent-permit auth-bypass class. plus the `get_node_label_only` fast-path probe at `engine_wait.rs:1007` used by the Inv-11 runtime check, `put_node` at `engine_wait.rs:1036` (engine-internal write counterpart routing through the same backend transaction as user-facing `create_node`), the WAIT-resume cap-snapshot-hash recompute, the `fabricate_test_suspend_envelope*` test helpers (both `#[cfg(any(test, feature = "test-helpers"))]`-gated per refinement-audit-2026-05 D1 #1153 — these synthesise an `ExecutionStateEnvelope` attributing writes to a caller-chosen principal, so shipping them UNGATED `pub` in the production cdylib was a credentialed-write bypass for any engine-handle holder / hostile plugin per CLAUDE.md baked-in #18; the default-feature production build compiles them out entirely so they are absent at the napi boundary — closure-pinned by `tests/sec_r6r2_02_test_helpers_gating_preserved_under_g12_c_migration.rs::fabricate_test_suspend_envelope_methods_are_cfg_gated_d1_1153`), `register_wait_reference_handler` (likewise `#[cfg(any(test, feature = "test-helpers"))]`-gated), and the bench helpers + AST-cache testing accessors.
...
- **`engine_subscribe.rs`** (~850 LOC) — SUBSCRIBE primitive engine-side surface: `on_change` / `on_change_as` / `on_change_as_with_cursor` / `on_change_with_cap_recheck` / `on_change_with_cursor`, `Subscription`, `SubscribeCursor` (Latest / Sequence / Persistent), `OnChangeCallback`, the cap-recheck-at-delivery closure construction, and the `testing_subscribe_observable_change_events` helper. The handle's `Drop` impl unsubscribes automatically. D5-RESOLVED semantics: engine-assigned `u64 seq`, exactly-once at the handler API surface via dedup against `max_delivered_seq`. **Phase 4-Foundation R1-FP G22-FP-1 option-D closure (`engine_subscribe.rs:303`):** `on_change_as_with_cursor` cap-recheck closure returns `CapRecheckOutcome` and composes two gates — whole-actor-revoke (`is_actor_active=false`) → `Cancel` (preserves Phase-3 R6-FP Wave-C1 `E_SUBSCRIBE_REVOKED_MID_STREAM` typed termination contract); per-event `CapabilityPolicy::check_read` denial → `Drop` (fail-soft new — this specific event's anchor uncovered, subscription stays active, future events continue). Closes sec-4f-r1-1 BLOCKER + mat-r1-4 materializer-subscribe ordering. Policy now lives behind `Arc<dyn CapabilityPolicy>` (Box→Arc migration landed alongside option-D).
...
- **`change.rs`** — `ChangeBroadcast` + `ChangeCallback` alias. Stdlib-only Arc-callback fan-out behind a Mutex. Implements `benten_graph::ChangeSubscriber` so the backend registers it directly. Two taps land at engine assembly: one records into `EngineInner::observed_events` for `ChangeProbe::drain`; the other bridges to the eval-side SUBSCRIBE registry via `benten_eval::primitives::subscribe::publish_change_event_with_labels`. **Fwd-2 #1038 prefilter (refinement-audit #1194):** the internal `Registry` splits subscribers into an *unfiltered* bucket (`subscribe` / `subscribe_fn` — receive EVERY event, exact Phase-1 semantics; the eval-side SUBSCRIBE bridge uses this path because it multiplexes many narrow patterns through one subscriber + matches downstream) and a *prefix-indexed* `HashMap<String, Vec<_>>` (`subscribe_fn_with_prefix(prefix, f)`). `publish` invokes all unfiltered subscribers, then probes the prefix index with each colon-delimited prefix of each event label (mirroring the `engine.onChange("post:*", ...)` glob hierarchy) and invokes only matching prefix subscribers — non-matching prefix subscribers are skipped *without* invocation. Semantically transparent (a skipped prefix subscriber would have self-filtered to the same accept/reject decision in its own callback); cost scales with *relevant* subscribers, not *total*. The semantic invariant is pinned deterministically by the `change.rs` unit tests (`non_matching_prefix_subscriber_is_skipped` / `unfiltered_subscribers_unaffected_by_prefix_index` / `prefix_subscriber_matches_colon_hierarchy`); the O(1)-vs-O(N) scaling claim is the `change_broadcast_prefilter` criterion bench (N up to 1024).
...
- **`thin_client_subscribe.rs`** (~572 LOC) — thin-client SUBSCRIBE surface for the wasm32-unknown-unknown bundle (browser tabs + Tauri webviews as authenticated views into a full peer per CLAUDE.md baked-in #17 shapes (b)+(c)). `ThinClientConnection` + `ThinClientSubId` + `ThinClientMetrics` + `ThinClientError`. `connect` / `connect_unauthenticated` / `subscribe` / `try_next_event` / `delivered_count` / `revoke_device_did` / `is_device_did_revoked` / `thin_client_metrics`. F6 filter at the full-peer edge supersedes the connection-time cap-check.
- **`thin_client.rs`** (~809 LOC) — **NEW in Phase-4-Foundation G24-F wave (T2 defenses 1-3 + br-r1-1 + sec-4f-r1-5 + Family F1 gap #2).** Full-peer-side session-token protocol for thin compute surfaces. `DidKeyedSession` + `Challenge` + `SessionToken` + `SessionConfig`. Public methods: `emit_challenge(claimed_origin)` returns a per-attempt challenge bytes the thin client signs with its device-DID Ed25519 key; `establish_session(challenge, signature, claimed_origin)` verifies challenge replay-defense + origin-binding and mints a `SessionToken`; `resolve(token)` validates token lifetime + nonce reuse and returns the session principal. 4 new ErrorCodes minted in R5 G24-F (HandshakeInvalid + ChallengeReplay + OriginMismatch + SessionExpired). Native-only per baked-in #17 — full peers MINT/VERIFY tokens; thin compute surfaces consume them as opaque bytes through the transport boundary.
- **`manifest_envelope_recheck.rs`** (~144 LOC) — **NEW in Phase-4-Foundation R4b-FP-1 Seam 3.** `ManifestEnvelopeRechecker` trait + `NoopManifestEnvelopeRechecker` default. Defense-in-depth port at the sync merge boundary for the T8 plugin-manifest-substitution attack class. `Engine::apply_atrium_merge`'s per-row recheck loop consults the installed rechecker; the real implementation lives in `benten-platform-foundation::plugin_lifecycle` and is wired in post-build via `Engine::set_manifest_envelope_rechecker(Arc::new(<real>))` at `engine.rs:1761`. `outcome_to_row_reject` is the conversion helper at `manifest_envelope_recheck.rs:124` that maps a recheck outcome into the row-reject reason consumed by the merge loop.
- **`shares_policy_resolver.rs`** (~180 LOC) — **NEW in refinement-audit-2026-05 Wave-E HELD #1197/#1146/#596.** `SharesPolicyResolver` trait + `DelegationResolution` enum + `NoopSharesPolicyResolver` default. The CLAUDE.md baked-in #18 Layer-3 manifest-`shares`-envelope enforcement port consulted by `EngineCapsHandle::delegate_capability` (which previously used a hard-coded always-true `AllPermit` shim — Layer 3 was paper-only at HEAD per META #669). `delegate_capability` resolves the source grant's `actor` principal and calls `resolve_delegation` **fail-CLOSED**: `Denied` / `NoManifest` reject with `ErrorCode::PluginDelegationOutsideManifestEnvelope`; `NotPluginPrincipal` (user-root, Layer-1-anchored) / `Admitted` proceed. Wired post-build via `Engine::set_shares_policy_resolver(Arc::new(<real>))`. Same composition-root posture as `ManifestEnvelopeRechecker` (the production library adapter wires at the Phase-4-Meta full-composition window — zero production library impls by arch-r1-1 dep-direction design; the default `Some(Noop)` is observably identical to the deleted `AllPermit` until a real adapter is installed, with NO always-true bypass remaining anywhere).
...
- **`engine_sync.rs`** (~1830 LOC — second-largest file) — `AtriumHandle` + `AtriumError` + `AtriumResult`, the `DeviceAttestationEnvelope` (signed on-the-wire device-DID attestation envelope V2 + Ed25519 sig + payload-hash binding + session-nonce — Phase-3 G16-D wave-6b cryptographic closure for criterion 16). **COLLAPSE P3 (DECISION-RECORD §4 RATIFIED):** `DeviceAttestationEnvelope::verify` no longer consumes a `benten_id::Acceptor` — the device-attestation *acceptance* pipe (expected-parent / nonce-store / revocation-list) is DELETED. `verify` keeps the provenance signature check + verifies the embedded attestation against its `parent_did` (user-root delegation link) + a freshness window, and returns the verified `CapabilityEnvelope` ceiling so `Engine::apply_atrium_merge`'s single per-row chain-validation seam ANDs it (J8 ceiling-AND, unified with the #669 manifest ceiling-check — `manifest_envelope_recheck::envelope_ceiling_admits_row`, NOT a parallel pipe). `set_acceptor` is replaced by `set_envelope_freshness_window`. Also: `DeclaredDeviceAttestation`, `DeclaredCapabilityClaim`, `SyncMergeAttribution`. `open` / `open_with_keypair` / `set_local_device_did` / `set_local_device_attestation` / `set_local_device_keypair` / `set_envelope_freshness_window` / `register_device_attestation` / `list_declared_device_attestations` / `register_peer_did` / `resolve_peer_dids` / `peer_id` / `loopback_addr` / `atrium_status` / `sync_subgraph` / `sync_subgraph_over` / `accept_sync_subgraph` / `register_zone` / `with_zone` / `merge_remote_change` / `merge_remote_change_with_hop_depth` / `hlc_node_id` / `local_hlc` / `inbound_hlc_skew_classifier_calls` / `is_active` / `leave` / `rejoin` / `close`. Owns the per-zone Loro CRDT documents, the iroh `Endpoint`, the local `Hlc` clock, the Inv-13 row-4 SPLIT classifier (rejects user-zone op-logs whose keys target `system:*` prefixes with `E_SYNC_DIVERGENT_CID_REJECTED`).
...
- `Engine::builder() -> EngineBuilder` — fluent surface (`.path` / `.backend` / `.capability_policy` / `.capability_policy_grant_backed` / `.capability_policy_ucan_durable` / `.production` / `.without_ivm` / `.without_caps` / `.without_versioning` / `.change_stream_capacity` / `.monotonic_source` / `.time_source` / `.suspension_store` / `.ucan_grounded_now_for_test` / `.with_test_ivm_budget` / `.ivm_max_work_per_update`). `.build()` performs the assembly + the three rehydration passes (module manifests, module bytes, handler-version chains).
...
- `Engine::read_node_as(principal: &Cid, cid: &Cid) -> Result<Option<Node>, EngineError>` at `engine_wait.rs:1082` — the load-bearing read entry point for any read attributed to a non-trusted principal. Plugin authors NEVER call this directly; the evaluator threads the active principal through this surface when dispatching a plugin's read. Mirrors the `Engine::call_as` precedent. Inv-11 runtime probe + Option-C symmetric-None denial both apply. Admin UI v0 in `benten-platform-foundation` uses `read_node_as` exclusively (pinned via the `admin_ui_v0_source_never_calls_engine_read_node_only_engine_read_node_as` grep-assert test).
- `Engine::put_node(node: &Node) -> Result<Cid, EngineError>` at `engine_wait.rs:1036` — the engine-internal write counterpart. Routes through the same backend transaction as user-facing `create_node` (so ChangeEvents + IVM materialization fire on commit) but skips the user-facing Inv-11 system-zone label rejection at the API surface. Storage-layer Inv-11 guard stays as defense-in-depth.

(The user-facing `Engine::get_node` at `engine_crud.rs:95` is the user-facing default-no-principal read pathway with the Inv-11 runtime probe + `ReadContext::actor_cid = None` posture; `read_node_as` is the public `_as` flanking entry point with `actor_cid: Some(*principal)`. The 4 `todo!()` stubs cited in older docs at `engine_wait.rs:1011-1026` are CLOSED — those line numbers now host real `get_node_label_only` / `put_node` / `read_node_as` / `resolve_subgraph_cid_for_test` implementations. The v1-assessment-window question about tightening `Engine::get_node` from `pub` → `pub(crate)` per `docs/future/phase-4-backlog.md §4.43` remains deferred to Phase-4-Meta.)
...
- `Engine::caps() -> EngineCapsHandle<'_>` at `engine.rs:1538` — the production-equivalent grant-mutation handle: `install_proof` / `revoke` / `revoke_capability_by_grant_cid` at `engine_caps.rs:246` (PR #199, UCAN revocation observance closure) / `delegate_capability` at `engine_caps.rs:498` (Phase-4-Foundation plugin-runtime delegation envelope seam). Updates BOTH the durable `system:CapabilityGrant` / `system:CapabilityRevocation` zone-Node mirror AND the in-memory `(actor, zone)` revocation-pair set that `apply_atrium_merge`'s per-row cap-recheck consults.
...
- `Engine::create_view(view_id, opts)` / `register_user_view(spec)` — IVM view registration (legacy id-string form + new UserViewSpec builder form).
...
- `Engine::apply_atrium_merge(atrium, anchor, zone, bytes, incoming_hop_depth).await -> Result<Cid, EngineError>` — the engine-side completion of the row-4a user-data sync path: CRDT merge → peer-DID resolution → per-row HLC skew classifier → Inv-13 row-4b per-key SPLIT defense → per-row cap-recheck (structural-always-on per Ben's RATIFIED Option (a) at sec-r4r1-2 BLOCKER closure) → AttributionFrame mint → Version Node persistence + anchor advance.
  - **apply_atrium_merge per-row defense-in-depth (META #752 / #629 Cluster III / refinement-audit #1192).** The per-row loop (`engine.rs:1245`+) is the workspace's highest-density trust-boundary loop. Wired layers, in order: (1) per-row recheck counter bump (`sync_replica_cap_recheck_count`, observable via `sync_replica_cap_recheck_calls()`); (2) Inv-13 row-4b system-zone-prefix-key reject (`E_SYNC_DIVERGENT_CID_REJECTED`); (3) inbound HLC skew classifier (`E_SYNC_HLC_DRIFT`); (4) in-memory `(actor_cid, zone)` / `(actor_cid, zone:write)` revocation-pair reject (`E_SYNC_REVOKED_DURING_SESSION`); (5) **active `CapabilityPolicy::check_write` consultation** — the configured policy (NOT `NoAuthBackend`) is invoked with a row-shaped `CapWriteContext`; a `Revoked`/`Denied` verdict surfaces the same typed `SyncRevokedDuringSession` rejection so backends routing revocation through durable `system:CapabilityRevocation` Node writes (the G14-B durable UCAN backend) are honored; (6) manifest-envelope rechecker (Layer-3 #669 T8 defense); (7) COLLAPSE P3/P5 unified envelope-ceiling AND; (8) COLLAPSE P5/F3 durable replay-marker. The merge is atomic — a single rejected row vetoes the whole merge before the Version Node is minted. Regression-pinned by `tests/sync_replica_attribution.rs::sync_replica_write_after_local_grant_revoke_post_handshake_rejected_with_e_sync_revoked_during_session` (Bob installs Alice's `<zone>:write` grant → Bob revokes it locally before applying Alice's inbound merge → `apply_atrium_merge` rejects with typed `E_SYNC_REVOKED_DURING_SESSION`, CURRENT pointer unadvanced, `ON_DENIED` routing asserted) + the clean-path counter pin `sync_replica_write_cap_recheck_at_delivery_against_local_grant_store`.
- `Engine::sync_replica_cap_recheck_calls()` — observability for the per-row cap-recheck counter.
- `AtriumHandle` exposes: zone registration, Loro `with_zone` access, `merge_remote_change*`, peer-DID registration + resolution, local-device-DID + attestation + keypair binding, acceptor injection, `sync_subgraph` / `accept_sync_subgraph`, `leave` / `rejoin` / `close` (Phase-3 G16-B-G; idempotent — `leave()` on already-inactive is a no-op, `rejoin()` on already-active is a no-op; new ErrorCode `E_ATRIUM_INACTIVE` fires for sync-touching surfaces during the quiesced window; `is_active()` at `engine_sync.rs:1535` gates operator-UI affordances).
...
The engine itself does NOT carry `Engine::install_plugin` / `Engine::uninstall_plugin` — the plugin lifecycle proper (manifest review, install-time consent, plugin-DID minting, install record signing) lives in `benten-platform-foundation::plugin_lifecycle` and `benten-platform-foundation::manifest_store::install_plugin` per CLAUDE.md baked-in #18. The engine contributes three load-bearing seams that the platform-foundation crate composes against:

- `Engine::set_manifest_envelope_rechecker(rechecker: Arc<dyn ManifestEnvelopeRechecker>)` at `engine.rs:1761` — the post-build wiring point so `apply_atrium_merge`'s per-row recheck loop consults the platform-foundation rechecker (defense-in-depth at the T8 plugin-manifest-substitution attack class). Default is `NoopManifestEnvelopeRechecker` returning `NotApplicable` for every row.
- `Engine::read_node_as(principal, cid)` — the read pathway threaded with the active plugin-DID as principal during plugin-attributed reads (Class B β; see §4c).
- `EngineCapsHandle::delegate_capability(...)` — the runtime UCAN delegation seam used inside the plugin manifest envelope policy (Phase-4-Foundation cap-r1-9 ordering corollary).

### 4o. Top-level re-exports

`Engine`, `EngineGeneric`, `EngineBuilder`, `EngineError`, `EngineTransaction`, `EngineCapsHandle`, `EngineViewsHandle` (Wave-E HELD #1190 facet-handle split part 1), `SharesPolicyResolver` + `DelegationResolution` + `NoopSharesPolicyResolver` (Wave-E HELD #1197 Layer-3 manifest-`shares` enforcement port), `CapProof`, `Outcome` + `Trace` + `TraceStep` + `OutcomeExt`, `AnchorHandle`, `HandlerPredecessors`, `NestedTx`, `ReadViewOptions`, `RegisterReplaceOutcome`, `TerminalError`, `UserViewInputPattern`, `UserViewSpec` + `UserViewSpecBuilder`, `ViewCreateOptions`, `BudgetExhaustedView`, `DiagnosticInfo`, `SubgraphSpec` + `SubgraphSpecBuilder`, `PrimitiveSpec`, `WriteSpec`, `IntoCallInput`, `IntoSubgraphSpec`, `IterateBody`, `GrantSubject`, `RevokeScope`, `RevokeSubject`, `ChangeProbe`, `EmitBroadcast` + `EmitEvent` + `EmitSubscription`, `OnChangeCallback`, `SubscribeCursor`, `Subscription`, `StreamCursor`, `StreamHandle`, `STREAM_GRANT_CEILING_CHUNK_COUNT`, `STREAM_GRANT_CEILING_WALLCLOCK_MS`, `CHANGE_STREAM_MAX_BUFFERED`, `ResumePayload`, `SuspensionOutcome`, `RedbSuspensionStore`, `WaitTtlGcStats`, `ModuleManifest` + `ModuleManifestEntry` + `ManifestError` + `ManifestSignature` + `ManifestSummary` + `MigrationStep`, `SYSTEM_ZONE_PREFIXES`, `EngineConfig` + `EngineConfigError` + `SandboxSection`, `AstCacheStats`, `NOAUTH_STARTUP_LOG`, `SandboxNodeDescription`, `SANDBOX_UNAVAILABLE_ON_WASM_TEXT`. Plus `benten_errors::ErrorCode`, `benten_eval::PrimitiveKind` + `TypedCallOp` + `TYPED_CALL_PREFIX` + `Chunk` + `ChunkSink` + `CapSnapshot` + `InMemorySuspensionStore` + `SuspensionKey` + `SuspensionStore` + `SuspensionStoreError` + `WaitMetadata`.
...
- **`change_broadcast_prefilter.rs`** — Fwd-2 #1038 prefilter scaling: `prefiltered` vs `unfiltered_baseline` arms at N = {16, 64, 256, 1024}. Informational; the visible gap at N=1024 is the O(1)-vs-O(N) evidence (semantic invariant is unit-test-pinned).
...
- Identity concerns stay in `benten-id`. The engine consumes `Keypair` + `PublicKey` + `Signature` + `Ucan` + `Did` + `DeviceAttestation`/`CapabilityEnvelope` (the kept pure primitives — COLLAPSE P3 deleted `Acceptor`) and dispatches typed-CALL ops through them.
...
- The `crate::engine` module's resolved-alias `impl Engine` block (the `#[cfg(not(feature = "browser-backend"))] impl Engine` after line ~2080) is large and growing — it carries `register_module_bytes`, `register_subgraph` (both arms), `register_crud`, `call` + variants, `trace`, `handler_to_mermaid`, `handler_predecessors`, `set_manifest_envelope_rechecker`, and all of the dispatch-flow helpers. Several recent additions take the form `#[cfg(not(target_arch = "wasm32"))] pub async fn ...` (Atrium surfaces inside the same `impl Engine` block). Phase-3-backlog §1.2-followup is the named destination for "impl-block cascade" (lift methods to `impl<B: GraphBackend>` where possible) — many of these methods could live on the generic block. The v1-assessment-window per CLAUDE.md baked-in #15 names this cleanup as deferred to Phase-4-Meta.
- `engine.rs` itself at ~4556 LOC is past the comfortable readable threshold. The R6 Wave-2 split already moved diagnostics + caps + views + crud + modules + transaction + snapshot out; `engine.rs` retains the dispatch core + the registration paths + the apply-atrium-merge orchestrator + the `EngineGeneric` struct + the resolved-alias `impl Engine` block + the new `manifest_envelope_rechecker` slot wiring. The most natural next split would peel `register_subgraph*` (with the canary clippy `too_many_lines` allow attributes already in place) into its own `engine_register.rs`, but this is hygiene, not correctness.
...
**Architectural correction post-Phase-4-Foundation R5:** the original D-4F-2 framing ("materializer + plugin manifest live as sub-modules of `benten-engine`") was REVISED at the start of Phase-4-Foundation. Both materializer + plugin manifest + admin UI v0 + schema compiler + registry now live in a separate workspace crate `benten-platform-foundation` (under `crates/benten-platform-foundation/`). This preserves the crate-thinness arch contract (`benten-engine` stays the orchestrator; platform-shippable surfaces compose against it from the outside) and matches the dependency-direction baked-in #19 ("engine-level extensions are Rust crates compile-time linked").

### 8a. Materializer (G23-B) — landed in `benten-platform-foundation::materializer`

The schema-driven rendering materializer lives at `crates/benten-platform-foundation/src/materializer.rs`. It composes the engine's existing `IvmViewReadGate` + `cap_recheck::CapRecheckFn` + `UserViewSpec` surfaces — no new engine-side public surface. Registration goes through the engine's existing `register_user_view`.

### 8b. Schema-driven rendering compiler — landed in `benten-platform-foundation::schema_compiler`

The compiler proper lives at `crates/benten-platform-foundation/src/schema_compiler/`. It emits `SubgraphSpec` shapes the engine consumes via the existing `register_subgraph` path — same shape as STREAM / SUBSCRIBE / WAIT landed in Phase-2b.

### 8c. Admin UI v0 — landed in `benten-platform-foundation::admin_ui_v0`

The admin UI sources live at `crates/benten-platform-foundation/src/admin_ui_v0/`. It depends only on `Engine`'s existing public surface — every operator-observability accessor (`metrics_snapshot`, `audit_sequence`, `change_event_count`, `capability_writes_committed`, `ivm_subscriber_count`, `diagnose_read`, `sync_replica_cap_recheck_calls`, `handler_version_chain`, `walk_versions`, `device_cid`, `effective_actor_cid`, `active_module_capabilities`, `is_module_installed`, `thin_client_metrics`) is admin-UI-facing. Admin UI reads thread through `Engine::read_node_as` exclusively (the `admin_ui_v0_source_never_calls_engine_read_node_only_engine_read_node_as` grep-assert pin).

### 8d. Plugin lifecycle — landed in `benten-platform-foundation::plugin_lifecycle` + `::manifest_store`

Per CLAUDE.md baked-in #18, plugins are content-addressed shareable subgraphs. The plugin manifest schema lives at `crates/benten-platform-foundation/src/plugin_manifest.rs`; the install/uninstall lifecycle lives at `plugin_lifecycle.rs` + `manifest_store.rs`. The engine contributes three seams (per §4n above):

1. `Engine::set_manifest_envelope_rechecker(...)` — defense-in-depth at the sync-merge boundary against T8 manifest-substitution attacks
2. `Engine::read_node_as(plugin_did, cid)` — Class B β read pathway for plugin-attributed reads
3. `EngineCapsHandle::delegate_capability(...)` — UCAN delegation envelope wiring

Plugin install flow at HEAD (G24-D + G24-D-FP-1/2/3 + R4b-FP-1 wave): caller-mint-first contract — the platform-foundation `install_plugin` accepts a caller-minted plugin-DID + manifest content-CID + signed install record + envelope-recheck chain. The R6-FP-A wave added `InstallContext::expected_plugin_did` to defense the substitution attack at install time. Uninstall cascade revokes held caps (`plugin_uninstall_revokes_held_caps.rs` test pin) + clears private-namespace data (`plugin_uninstall_clears_private_namespace_data.rs`) + revokes downstream-delegated caps (`plugin_uninstall_cascade_revokes_delegated_caps.rs`). Composition-cycle detection runs at install time on a structural `Subgraph` representation of the plugin composition graph (per `plugin_meta_composition_cycle_rejected.rs`) — no new `PrimitiveKind` variant minted (CLAUDE.md #1 12-irreducible preserved).
...
- **Cycle detection across handler-CID call edges.** Currently the evaluator runs over a DAG (Inv-2 forbids cycles + Inv-7 forbids self-loops). User-registered subgraphs walk the static `SubgraphSpec` at register time and the invariant battery rejects cycles. But at materialization time + during `register_subgraph_replace` hot-reload, no cycle detection runs over the broader graph of handler-CID → handler-CID call edges (the `call_handler` cross-handler dispatch). Phase-3-backlog has not surfaced a finding here; it's worth flagging that handler-call graphs could form cycles that would only fail at iteration-budget exhaustion. (Note: plugin meta-composition DOES run cycle detection at install-time in `benten-platform-foundation` per `plugin_meta_composition_cycle_rejected.rs`; that's a separate graph from the handler-call graph.)
- **Generic-cascade lift (`Engine = EngineGeneric<RedbBackend>` vs full generic).** Phase-3-backlog §1.2-followup names "impl-block cascade" as the carried-forward work. At HEAD, the resolved-alias `impl Engine` block carries ~all of the dispatch core; the `impl<B: GraphBackend> EngineGeneric<B>` block carries constructors + a small number of cross-module accessors + the WAIT TTL GC entry points. The cleanest cascade requires the umbrella `GraphBackend` trait to surface `register_module_bytes` / `get_by_label` / `get_by_property` / closure-based `transaction(|tx| ...)` paths uniformly — currently those are inherent to `RedbBackend`. The v1-assessment-window per CLAUDE.md baked-in #15 includes "engine impl-block generic-cascade lift" as a named Phase-4-Meta pre-tag cleanup.
- **`Engine::get_node` visibility tightening (`pub` → `pub(crate)`).** Per `docs/future/phase-4-backlog.md §4.43` — the original Class B β framing intended engine-internal un-attributed reads to be `pub(crate)`. At HEAD the surface is `pub`, exposing the un-attributed read at the napi binding layer. Tightening is deferred to Phase-4-Meta as a v1-API-stabilization decision; external callers who need un-attributed reads have access at HEAD but per discipline should use `read_node_as` with the appropriate principal.
- **Materializer + subgraph-cache key.** The current `SubgraphCache` keys on `(handler_id, op, subgraph_cid)`. A materializer subgraph that walks per-row would benefit from a cache keyed on something finer (likely `(view_id, row_label, row_cid)`); the existing key shape may or may not generalize as `benten-platform-foundation::materializer` matures.
- **The `ast_cache` invalidation contract under future `register_subgraph_replace` semantics.** Currently the cache invalidates the OLD `handler_cid`'s entries on replace + repopulates the NEW. If Phase-4-Meta ships per-plugin manifest-bound subgraph variants (a plugin author overrides a handler at install-time), the invalidation contract grows a new axis. Open question.
- **EMIT-Named subgraph dispatch.** The engine-side `emit_with_handler(Named(handler_id))` records the routing decision but does NOT invoke the named subgraph; only the eval-side `benten_eval::primitives::emit::execute` Named arm invokes via `host.call_handler`. The G14-D wave-5a comment names this as a wave-paired (pim-4 §3.10) intentional asymmetry shipping at G16-D — there may still be a small gap on the engine side worth confirming post-Phase-3-close.
