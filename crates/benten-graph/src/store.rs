...
// Phase-4-Meta-Core G-CORE-1 (#989): per-DID storage-partition prefix.
//
// When a `WriteContext` carries `namespace_did = Some(did)`, every storage
// key generated by that write is wrapped in a `d:<cid_bytes>:` envelope:
//   `d:<did_bytes>:n:<cid>`
//   `d:<did_bytes>:e:<cid>`
//   `d:<did_bytes>:es:<src_cid><edge_cid>`
//   `d:<did_bytes>:et:<tgt_cid><edge_cid>`
//   `d:<did_bytes>:l:<label_bytes>:<cid>`  (per-DID label index)
//
// The `d:` byte (`0x64`) sorts BEFORE the legacy single-letter prefixes
// (`e`=0x65, `n`=0x6e, `s`=0x73) so the per-DID partition prefix is
// strictly disjoint from the un-namespaced keyspace — an unscoped scan
// over the legacy `n:` prefix cannot reach a `d:<did>:n:<cid>` key, and
// vice versa. The `__benten_schema_version` envelope at
// `SCHEMA_VERSION_KEY` (`0x5F` / `_`) sorts before `d:` so the schema
// envelope is still excluded from the full-store empty-prefix scan
// (see `RedbBackend::scan`).
//
// **CID-bytes-of-the-DID derivation:** the partition key is
// `Cid::as_bytes()` of the `namespace_did` — a fixed 36-byte BLAKE3
// CIDv1 frame. This is a stable function of the DID's content-addressed
// identity; no encoding choice is being made in this wave (the byte
// rendering of the per-DID prefix is a frozen-at-G-CORE-9 surface per
// the §3 G-CORE-1 design sub-decision).
//
// **Canonical-bytes neutrality (P-III):** these prefixes affect ONLY the
// storage-key shape. No `Node::cid()` / `Edge::cid()` / `Subgraph::cid()`
// / `to_canonical_bytes()` call site changes — the canary wave is
// public-SHAPE additive; the #843 golden CID
// `bafyr4icl4umfqvsu7awtnvg2iwt3bxebuywb5tp7wkejvufgp2xstgao5m` at
// `crates/benten-core/tests/with_node_renamed_inv14_bypass_pin.rs:70`
// stays UNCHANGED.
pub(crate) const PARTITION_PREFIX: &[u8] = b"d:";
pub(crate) const PARTITION_SEPARATOR: &[u8] = b":";
// Per-DID label index. Lives inside the per-DID partition envelope so the
// label range scan over `d:<did>:l:` keys is naturally confined to that
// DID's namespace. The `l:` sub-prefix doesn't collide with the global
// `LABEL_INDEX_TABLE` multimap (which holds the un-namespaced label index
// in a SEPARATE redb table, not in the `NODES_TABLE` keyspace).
pub(crate) const LABEL_SUBPREFIX: &[u8] = b"l:";

/// Build a per-DID partition prefix `d:<did_bytes>:`. Used as the upper-
/// envelope for every namespaced storage key + as the iterate prefix for
/// per-DID `ScopedView` operations.
pub(crate) fn partition_prefix(namespace_did: &Cid) -> Vec<u8> {
    let did_bytes = namespace_did.as_bytes();
    let mut k =
        Vec::with_capacity(PARTITION_PREFIX.len() + did_bytes.len() + PARTITION_SEPARATOR.len());
    k.extend_from_slice(PARTITION_PREFIX);
    k.extend_from_slice(did_bytes);
    k.extend_from_slice(PARTITION_SEPARATOR);
    k
}

/// `d:<did>:n:<cid>` — namespaced node key.
pub(crate) fn namespaced_node_key(namespace_did: &Cid, cid: &Cid) -> Vec<u8> {
...
/// `d:<did>:es:<src><edge>` — namespaced edge-source-index key.
pub(crate) fn namespaced_edge_src_index_key(
    namespace_did: &Cid,
    source: &Cid,
    edge: &Cid,
) -> Vec<u8> {
...
/// `d:<did>:es:<src>` — namespaced edge-source-index range prefix.
pub(crate) fn namespaced_edge_src_index_prefix(namespace_did: &Cid, source: &Cid) -> Vec<u8> {
    let mut k = partition_prefix(namespace_did);
    k.extend_from_slice(EDGE_SRC_PREFIX);
    k.extend_from_slice(source.as_bytes());
    k
...
/// `d:<did>:et:<tgt><edge>` — namespaced edge-target-index key.
pub(crate) fn namespaced_edge_tgt_index_key(
    namespace_did: &Cid,
    target: &Cid,
    edge: &Cid,
) -> Vec<u8> {
...
/// `d:<did>:l:<label_bytes>:<cid>` — namespaced label-index key.
///
/// The per-DID label index lives in the `NODES_TABLE` keyspace (alongside
/// the namespaced node bodies) rather than in the multimap
/// `LABEL_INDEX_TABLE` so the per-DID partition prefix can confine it.
/// Multi-label nodes write one entry per label.
pub(crate) fn namespaced_label_index_key(namespace_did: &Cid, label: &str, cid: &Cid) -> Vec<u8> {
    let mut k = partition_prefix(namespace_did);
    k.extend_from_slice(LABEL_SUBPREFIX);
...
/// `d:<did>:l:<label_bytes>:` — namespaced label-index range prefix for a
/// single label. Caller scans this prefix and extracts the trailing CID
/// suffix from each hit.
pub(crate) fn namespaced_label_index_prefix(namespace_did: &Cid, label: &str) -> Vec<u8> {
    let mut k = partition_prefix(namespace_did);
    k.extend_from_slice(LABEL_SUBPREFIX);
    k.extend_from_slice(label.as_bytes());
    k.extend_from_slice(PARTITION_SEPARATOR);
...
/// `d:<did>:n:` — namespaced node-keyspace iterate prefix. Used by
/// `ScopedView::iter_node_cids` to enumerate every node CID under a
/// single partition.
pub(crate) fn namespaced_node_prefix(namespace_did: &Cid) -> Vec<u8> {
    let mut k = partition_prefix(namespace_did);
    k.extend_from_slice(NODE_PREFIX);
    k
}
