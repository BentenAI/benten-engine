...
//!
//! Attack class: operator misconfigures `LegacyUcanStubBackend` in Phase 1 believing
...
//!
//! TDD contract: FAIL at R3 — `LegacyUcanStubBackend`, `CapError::NotImplemented`, the
...
/// evaluator with LegacyUcanStubBackend attempts a WRITE, the resulting error MUST
...
fn ucan_stub_error_routes_to_ON_ERROR_not_ON_DENIED() {
    ...
    let engine = Engine::builder()
        .capability_policy(Box::new(LegacyUcanStubBackend::new()))
    ...
    assert_eq!(
        taken_edge, "ON_ERROR",
        "LegacyUcanStubBackend::NotImplemented must route via ON_ERROR — routing it via \
